...
% Add this macro definition in your preamble (after \begin{document} or before if you prefer)
\newcommand{\drawpipeline}[1]{%
    % Parameter #1: stage to highlight (1=Predict/Fetch, 2=Decode, 3=Alloc, 4=Schedule/EX, 5=Retire)
    \begin{tikzpicture}[
        stage/.style={rectangle, draw=black, thick, minimum width=2.2cm, minimum height=0.9cm, font=\footnotesize},
        queue/.style={rectangle, draw=black, fill=green!60, minimum width=0.15cm, minimum height=0.7cm},
        highlight/.style={fill=yellow!60},
        scale=0.75, transform shape
    ]
        
        % Stages and queues
        \ifnum#1=1
            \node[stage, highlight] (fetch) {Predict/Fetch};
        \else
        ...
        \ifnum#1=2
            \node[stage, highlight, right=0.1cm of iq4] (decode) {Decode};
        \else
        ...
        \ifnum#1=3
            \node[stage, highlight, right=0.1cm of uopq4, minimum width=1.5cm] (alloc) {Alloc};
        \else
        ...
        % Schedule and EX as two connected boxes
        \ifnum#1=4
            \node[stage, highlight, right=0.1cm of rs4, minimum width=1.8cm] (schedule) {Schedule};
            \node[stage, highlight, right=0 of schedule, minimum width=0.8cm] (ex) {EX};
        \else
        ...
        \ifnum#1=5
            \node[stage, highlight, right=0.1cm of rob5] (retire) {Retire};
        \else
...
\begin{frame}{Pipeline: Predict/Fetch}
    \centering
    \drawpipeline{1} % Highlight stage 1 (Predict/Fetch)
