...
% Updated macro with more flexibility
\newcommand{\drawpipelineCustom}[5]{%
    ...
    \begin{tikzpicture}[
        ...
        \node[queue, right=0 of rob4] (rob5) {};
        
        % Retire
        \ifnum#5=1
            \node[stageHighlight, right=0.1cm of rob5] (retire) {Retire};
        \else
            \ifnum#5=2
                \node[stageGray, right=0.1cm of rob5] (retire) {Retire};
            \else
                \node[stage, right=0.1cm of rob5] (retire) {Retire};
            \fi
        \fi
        ...
        \node[font=\scriptsize] at ($(rob1.south)!0.5!(rob5.south)+(0,-0.15)$) {ROB};
    \end{tikzpicture}
}
...
\begin{frame}{Flush at Retire: Misp. Jump Retires}
    ...
    \begin{tikzpicture}[overlay, remember picture]
        % Define coordinates based on the nodes from drawpipelineCustom
        \coordinate (flush-start) at ($(retire.north)$);
        \coordinate (flush-corner) at ($(fetch.north)+(0,0.8)$);
        
        % Draw the flush arrow from retire to fetch
        \draw[->, red, ultra thick] (retire.north) -- ++(0,0.8) -| (fetch.north);
        
        % Add text along the path
        \node[font=\scriptsize] (redirect_fetch) at ($(fetch.north)+(-1.3,0.7)$) {Redirect Fetch};
        \node[font=\scriptsize] at ([yshift=-0.4cm]redirect_fetch) {+ update BPU};
        
        % Clear label for the backend
        \node[red, font=\bfseries] at ($(schedule.north)+(0,0.5)$) {Clear};
    \end{tikzpicture}
...
\begin{frame}{Jump Misprediction -- Flush at Execute}
    \begin{itemize}
        ...
        \item Un-stall Renamer and allow instructions from correct path to rename/allocate
    \end{itemize}
\end{frame}
...
\begin{frame}{Flush at EXE : Jump Gets to EXE}
    ...
    \begin{tikzpicture}[overlay, remember picture]
        \draw[->, red, ultra thick] ($(current page.north)+(-1,-1.5)$) -- ($(current page.north)+(-1,-2.8)$);
        \node[red, font=\small] at ($(current page.north)+(-4,-1.8)$) {Redirect Fetch};
        \node[red, font=\small] at ($(current page.north)+(-4,-2.1)$) {+ update BPU};
        \draw[->, red, thick] ($(current page.north)+(-3.5,-2.3)$) -- ($(current page.north)+(-5.5,-2.8)$);
        \node[red, font=\large\bfseries] at ($(current page.north)+(0,-2)$) {Flush};
    \end{tikzpicture}
...
\begin{frame}{Flush at EXE : Misp. Jump Retires}
    \centering
    \drawpipelineCustom{0}{0}{2}{2}{2}
    
    % Add Clear arrow
    \begin{tikzpicture}[overlay, remember picture]
        \draw[->, red, ultra thick] ($(current page.north)+(0,-1.5)$) -- ($(current page.north)+(0,-2.8)$);
        \node[red, font=\large\bfseries] at ($(current page.north)+(4,-2)$) {Clear};
    \end{tikzpicture}
...
\begin{frame}{Periodic Checkpoints}
    ...
    \begin{columns}[T]
        \begin{column}{0.9\textwidth}
            ...
            \vspace{0.3cm}
            % Placeholder for the checkpoint illustration
...
\begin{frame}{Checkpoints: Jump Gets to EXE}
    ...
    \begin{tikzpicture}[overlay, remember picture]
        \node[yellow!80!black, fill=yellow!20, font=\small] at ($(current page.north)+(-4,-1.5)$) {BPU Update};
        \draw[->, red, ultra thick] ($(current page.north)+(2,-1.5)$) -- ($(current page.north)+(2,-2.8)$);
        \node[red, font=\large\bfseries] at ($(current page.north)+(3.5,-2)$) {Clear};
    \end{tikzpicture}
...
\begin{frame}{Checkpoints: Jump Gets to EXE}
    \centering
    \drawpipelineCustom{2}{2}{0}{0}{0}
    
    % Add BPU Update and Clear labels
    \begin{tikzpicture}[overlay, remember picture]
        \node[yellow!80!black, fill=yellow!20, font=\small] at ($(current page.north)+(-4,-1.5)$) {BPU Update};
        \draw[->, red, thick] ($(current page.north)+(-4,-1.8)$) -- ($(current page.north)+(-5.5,-2.8)$);
        \draw[->, red, ultra thick] ($(current page.north)+(0,-1.5)$) -- ($(current page.north)+(0,-2.8)$);
        \node[red, font=\large\bfseries] at ($(current page.north)+(1.5,-2)$) {Clear};
    \end{tikzpicture}
