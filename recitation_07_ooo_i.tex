...
% Macro for ROB diagram - now takes 5 arguments for 5 rows
\newcommand{\drawROB}[5]{%
    \begin{tikzpicture}[scale=0.8]
        \matrix (rob) [matrix of nodes,
            nodes={draw, minimum width=2cm, minimum height=0.45cm, anchor=center, font=\scriptsize},
        ...
        \node[below=0.1cm of rob-4-2, font=\scriptsize] {$\vdots$};
    \end{tikzpicture}
}
...
\begin{frame}{ROB (Re-Order Buffer)}
    ...
    \vspace{0.5cm}
    \centering
    \drawROB{R2/R3}{R4+RB0}{R3+R3}{R3+RB2}{}
\end{frame}

\begin{frame}{ROB Example}
    \begin{columns}[T]
        \column{0.25\textwidth}
        {\ttfamily\small
        DIV R1, R2, R3\only<2->{ \checkmark}\\
        ADD R2, R4, R1\only<3->{ \checkmark}\\
        ADD R2, R3, R3\only<4->{ \checkmark}\\
        ADD R4, R3, R2\only<5->{ \checkmark}
        }

        \vspace{0.5cm}

        \only<5->{\textcolor{correctgreen}{\small No more False Dependencies! (but RAW still exists)}}
        ...
        \centering
        \drawROB{\only<2->{R2/R3}}{\only<3->{R4+RB0}}{\only<4->{R3+R3}}{\only<5->{R3+RB2}}{}
    \end{columns}
\end{frame}
...
\section{Q1: OOO Execution Walkthrough}
