...
\section{Q3: OOO Execution Trace}

\begin{frame}{Q3: OOO Execution Structures}
    ...
    \begin{itemize}
        ...
        \item \textbf{ROB (Re-Order Buffer):} Circular buffer maintaining program order
...
\begin{frame}{Structure Interaction}
    \centering
    \begin{tikzpicture}[font=\tiny]
        % IDQ at top
        \node[draw, fill=lightblue, minimum width=2cm, minimum height=0.8cm] (idq) at (0,5) {\textbf{IDQ}};

        % RAT in middle
        \node[draw, fill=green!20, minimum width=7cm, minimum height=2.2cm] (rat) at (0,2.8) {
            ...
            \begin{tabular}{|l|c|l|}
            \hline
            \textbf{Dest (Arch)} & \textbf{\#phys reg} & \textbf{Source} \\
            \hline
            EAX & xxx & 0 (RRF) \\
            EBX & 19 & 1 (ROB) \\
            ECX & 23 & 1 (ROB) \\
            ...
            \vspace{0.1cm}
            \tiny
            0: Read from RRF, 1: Read from ROB
            \end{minipage}
        };

        % ROB on left bottom
        \node[draw, fill=cyan!20, minimum width=3.5cm, minimum height=3cm, anchor=north] (rob) at (-4,1) {
            \begin{minipage}{3.3cm}
            \centering
            \textbf{ROB}

            \scriptsize Dest. (Phys Reg)

            \vspace{0.1cm}
            \tiny
            \begin{tabular}{|c|c|c|c|c|}
            \hline
            \textbf{\#} & \textbf{V} & \textbf{V} & \textbf{Data} & \textbf{DST} \\
            \hline
            19 & V & V & 12 & EBX \\
            23 & V & V & 33 & ECX \\
            37 & V & I & xxx & EAX \\
            ...
            \vspace{0.1cm}
            \tiny
            (4 write ports, 1 entry per uop)
            \end{minipage}
        };

        % RS on right bottom
        \node[draw, fill=orange!20, minimum width=3.2cm, minimum height=3cm, anchor=north] (rs) at (3.8,1) {
            \begin{minipage}{3cm}
            \centering
            \textbf{RS}
            ...
            \hline
             & \textbf{v} & \textbf{src1} & \textbf{v} & \textbf{src2} & \textbf{Pdst} \\
            \hline
            add & 1 & 97H & 1 & 12H & 37 \\
            ...
            \tiny
            Arch reg: from RRF\\
            Phys reg + valid: from ROB\\
            Phys reg + not valid: wait
            \end{minipage}
        };

        % RRF below ROB
        \node[draw, fill=blue!15, minimum width=2cm, minimum height=0.7cm, anchor=north] (rrf) at (-4,-2.3) {
        ...
        % Arrows
        \draw[->, thick] (idq) -- (rat);
        \draw[->, thick] (rat.south) -- ++(-2,-0.3) -- (rob.north);
        \draw[->, thick] (rat.south) -- ++(2,-0.3) -- (rs.north);

    \end{tikzpicture}

    \vspace{0.2cm}
    \tiny
    Instructions flow: IDQ $\rightarrow$ RAT (decode/rename) $\rightarrow$ ROB \& RS (track \& wait). Source \& dest values updated after EXE.
\end{frame}
