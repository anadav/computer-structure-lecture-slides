...
\begin{frame}{Register Renaming - Implementation}
    ...
    \begin{enumerate}
        \item \textbf{Architectural → Physical:} (e.g., R5→RB2)
        \begin{itemize}
        ...
        \item \textbf{Physical → Architectural:} (e.g., RB2→R5)
        \begin{itemize}
...
\begin{frame}{Q2a: Identify Dependencies}
    \begin{columns}[T]
        ...
        \only<2->{
            ...
            \vspace{0.3cm}
            \textcolor{correctgreen}{\small Note: RAW dependencies (e.g., (1)→(2) on R1) are true dependencies}
        }
    \end{columns}
\end{frame}
...
\begin{frame}{Q3: How Does OOO Execution Work Cycle-by-Cycle?}
    \textbf{Question:} Trace the following program through complete out-of-order execution, showing all structures at each cycle
...
\begin{frame}{Summary: Out-of-Order Execution}
    ...
    \begin{itemize}
        \item \textbf{RAT:} Arch→Phys register mappings (updated at decode)
        \item \textbf{ROB:} Phys→Arch mappings, preserves program order (for retirement)
