...
\begin{frame}{Cycle 3: Execute}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=south west, text width=5cm, font=\small] at (current page.south west) {
            \begin{block}{What's Happening}
                RB0 starts executing (no change from previous). Address R4+50 computed instantly (ADD takes 0 cycles). LD not yet dispatched from MOB.
            \end{block}
        };
    \end{tikzpicture}
\end{frame}
...
\begin{frame}{Cycle 5: Decode}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=south west, text width=5cm, font=\small] at (current page.south west) {
            \begin{block}{What's Happening}
                SUB decoded (RB4). RB1 value written to ROB. RB3 now ready, dispatched to execute [1]. RB0 at [2]. RB1 ready to retire but waits for RB0.
            \end{block}
        };
    \end{tikzpicture}
\end{frame}
