...
\pgfkeys{
    ...
    inst2/.initial={},
    inst3/.initial={},
    % Instruction background colors for code box (use white for no highlight)
    inst1color/.initial=white,
    inst2color/.initial=white,
    inst3color/.initial=white
}

% TikZ styles for pipeline diagram
\tikzset{
    pipeline stage/.style={draw, fill=lightblue, minimum width=1.2cm, minimum height=0.8cm},
    ...
    \begin{tikzpicture}[scale=0.8, transform shape]
        ...
        \ifx\instOne\empty
            ...
            \ifx\instlist\empty\else
                \node[code box, above=0.5cm of WB] {\instlist};
            \fi
        \else
            % Use individual instructions with background highlighting
            \node[code box, above=0.5cm of WB, align=left] {%
                \colorbox{\pgfkeysvalueof{/pipeline/inst1color}}{\instOne}%
                \ifx\instTwo\empty\else\\[-0.5ex]\colorbox{\pgfkeysvalueof{/pipeline/inst2color}}{\instTwo}\fi%
                \ifx\instThree\empty\else\\[-0.5ex]\colorbox{\pgfkeysvalueof{/pipeline/inst3color}}{\instThree}\fi%
            };
        \fi
...
\begin{frame}{Execution of Instructions with Variable Execution Time}
    ...
    \vspace{0.5cm}
    \centering
    \drawPipeline[showcallouts=true]
\end{frame}
...
\begin{frame}{How Many Cycles Does In-Order Execution Take?}
    \centering
    \drawPipeline[
        spaced=true,
        ...
        inst1color=highlightyellow,
        inst2color=highlightorange,
        inst3color=cyan!40
    ]
\end{frame}
...
\begin{frame}{Out-of-Order Execution: Cycle 7}
    ...
    R4 completes! (3 cycles earlier than in-order)
\end{frame}
