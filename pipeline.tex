...
\newcommand{\drawPipelineDiagram}[1][0,0]{%
    ...
    \begin{tikzpicture}[shift={(#1)},
        ...
        % Fetch Stage
        % PC MUX using circuitikz muxdemux
        \node[muxdemux, muxdemux def={Lh=3, NL=1, Rh=3, NR=2, NB=1, w=1.2}, 
              scale=0.8, rotate=90, fill=blue!20] (pcmux) at ($(origin) + (0.3,4.5)$) {};
        \node[font=\tiny, scale=0.5] at (pcmux.center) {MUX};
        
        % Define PC MUX coordinates for compatibility
        \coordinate (pcmux_in1) at (pcmux.rpin 1);
        \coordinate (pcmux_in2) at (pcmux.rpin 2);
        \coordinate (pcmux_out) at (pcmux.lpin 1);
        \coordinate (pcmux_selectorup) at (pcmux.bpin 1);
        ...
        % PC adder using circuitikz
        \node[adder, scale=0.6, fill=blue!15] (fe_adder) at ($(PC.center) + (0.7,1.8)$) {};
        \node[font=\tiny] at (fe_adder.center) {+};
        \coordinate (fe_adder_input1) at (fe_adder.in 1);
        \coordinate (fe_adder_input2) at (fe_adder.in 2);
        \coordinate (fe_adder_output) at (fe_adder.out);
        ...
        % Execute Stage
        % Execute adder using circuitikz
        \node[adder, scale=0.6, fill=blue!15] (ex_adder) at ($(L2.east)!0.5!(L3.west) + (0,1.56)$) {};
        \node[font=\tiny] at (ex_adder.center) {+};
        \coordinate (ex_adder_input1) at (ex_adder.in 1);
        \coordinate (ex_adder_input2) at (ex_adder.in 2);
        \coordinate (ex_adder_output) at (ex_adder.out);
        
        % Execute stage ALU source MUX using circuitikz muxdemux
        \node[muxdemux, muxdemux def={Lh=3, NL=1, Rh=3, NR=2, w=1.2}, 
              scale=0.8, rotate=270, fill=green!20] (exemux) at ($(L2.east)!0.5!(L3.west) + (0,-0.32)$) {};
        \node[font=\tiny, scale=0.5] at (exemux.center) {MUX};
        \coordinate (exemux_in1) at (exemux.rpin 1);
        \coordinate (exemux_in2) at (exemux.rpin 2);
        \coordinate (exemux_out) at (exemux.lpin 1);
        
        % ALU - keeping the custom trapezoid shape as circuitikz doesn't have a direct ALU shape
        ...
        % Write Back Stage MUX using circuitikz muxdemux
        \node[muxdemux, muxdemux def={Lh=3, NL=1, Rh=3, NR=2, w=1.2}, 
              scale=0.8, rotate=270, fill=green!20] (wbmux) at ($(L4.east) + (0.8,-0.21)$) {};
        \node[font=\tiny, scale=0.5] at (wbmux.center) {MUX};
        \coordinate (wbmux_in1) at (wbmux.rpin 1);
        \coordinate (wbmux_in2) at (wbmux.rpin 2);
        \coordinate (wbmux_out) at (wbmux.lpin 1);
