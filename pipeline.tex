...
\begin{frame}{Control Hazard on Branches}
    ...
    \begin{tikzpicture}[
        ...
        % Fetch Stage
        % MUX before PC
        \node[mux, rotate=90] (pcmux) at (-0.2,4) {};
        \node[component, fill=blue!20, minimum width=0.6cm, minimum height=0.6cm] (PC) at (0.5,2) {\small PC};
        \node[component, fill=purple!20, minimum width=0.6cm, minimum height=2cm] (ICache) at (2,2) {
            \begin{tabular}{c}
                \footnotesize inst.\\[-2pt]\footnotesize cache
            \end{tabular}
        };
        
        % PC adder (centered around 0)
        \begin{scope}[shift={(0,3.5)}, scale=0.8]
            ...
            \node at (0.15, 0.4) {\tiny +};
        \end{scope}
        \node[left, font=\tiny] (four) at (-0.1,3.5) {4};
        
        % Decode Stage
        \node[component, fill=pink!30, minimum width=1.2cm, minimum height=2.2cm] (RF) at (4.5,2.3) {};
        \node[align=center] at (4.5,3) {\footnotesize register\\[-2pt]\footnotesize file};
        % Register file labels - positioned closer to edges
        \node[font=\tiny, anchor=east] at ([xshift=-2mm]RF.west) {
            \begin{tabular}{r}
                src1\\[-1pt]
                src2\\[2pt]
                dst
        ...
        \node[font=\tiny, anchor=west] at ([xshift=2mm]RF.east) {
            \begin{tabular}{l}
                src1\\[-1pt]
                data\\[-1pt]
                src2\\[-1pt]
        ...
        % Sign extension
        \node[component, fill=green!20, ellipse, minimum width=0.8cm, minimum height=0.6cm, align=center] (SignExt) at (4.5,0) {
            \tiny sign\\[-3pt]\tiny ext.
        };
        
        % Instruction label (rotated)
        \node[font=\tiny, rotate=90] at (3.15,2) {instruction};
        
        % Control signals label (no decode oval)
        \node[font=\tiny] at (4.5,4.3) {control signals};
        \draw[orange, dashed, ->] (4.5,4.1) -- ++(2.5,0);
        ...
        % Memory Stage
        \node[component, fill=brown!20, minimum width=1.2cm, minimum height=2cm, align=center] (DCache) at (11,2) {
            \footnotesize data\\[-2pt]
            \footnotesize cache\\[3pt]
            \tiny address\\[8pt]
            \tiny data
        };
        ...
        \draw[->] (PC) -- (ICache);
        \draw[->] (ICache) -- (L1);
        \draw[->] (L1) -- ++(0.3,0) |- (RF.180);
        \draw[->] (L1) -- ++(0.3,0) |- (SignExt.west);
        
        % From RF to L2
        \draw[->] (RF.0) -- ++(0.2,0) |- ([yshift=5mm]L2.west);
        \draw[->] (RF.0) -- ++(0.2,0) |- ([yshift=-5mm]L2.west);
        ...
        % Write back path
        \draw[red, ->] (wbmux.east) -- ++(0.3,0) -- ++(0,-2.5) -| ([yshift=-6mm]RF.180);
        ...
        %\draw[->, blue] (PC.west) -- ++(0,0.5) -| (-0.15,3.5);
        % \draw[->, blue] (0.24,3.5) -- ++(0.5,0) -- ++(0,-1.8) -| (pcmux.south);
