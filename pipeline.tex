...
\node[icache, align=center](ic) at (3.6,-1.5) {Inst.\\ Cache};
...
% -------------------------- Decode -----------------------------------
\node[regfile, align=center, anchor=west] (rf) at (\pA+1.2,-1.3) {%
\textbf{Register}\\ \textbf{File}\\[6pt]
\begin{tabular}{@{}l@{}}
src1\\
src2\\
dst
...
\node[sign, align=center] (sext) at (10.5,-6.5) {Sign\\ Ext.};
...
% --------------------------- Memory ----------------------------------
\node[dcache, align=center, anchor=west] (dc) at (\pC+1.0,-1.3) {%
...
\begin{frame}{Control Hazard on Branches}
    ...
    \begin{tikzpicture}[
        ...
        mux/.style={trapezium, trapezium angle=70, draw, thick, minimum height=0.8cm, minimum width=0.3cm},
        ...
        \node[component, fill=purple!20, minimum width=1.5cm, minimum height=2cm] (ICache) at (2,2) {
            \begin{tabular}{c}
                Inst.\\Cache
            \end{tabular}
        };
        ...
        % PC adder
        \node[circle, draw, inner sep=2pt, fill=white] (adder) at (0.5,3.5) {\small +};
        \node[above, font=\tiny] at (adder.north) {4};
        \draw[->, blue] (PC.north) -- ++(0,0.5) -| (adder.west);
        \draw[->, blue] (adder.east) -- ++(0.8,0) -- ++(0,-1) -- ++(0.2,0);
        
        % Decode Stage
        \node[component, fill=pink!30, minimum width=1.8cm, minimum height=2.2cm] (RF) at (4.5,2.3) {
            \begin{tabular}{c}
                Register\\File
            \end{tabular}
        };
        ...
        % Register file ports
        \node[font=\tiny, anchor=east] at (RF.west) {
            \begin{tabular}{r}
                src1\\src2\\[3pt]dst
            \end{tabular}
        };
        \node[font=\tiny, anchor=west] at (RF.east) {
            \begin{tabular}{l}
                src1\\data\\src2\\data
            \end{tabular}
        };
        ...
        % Decode control
        \node[font=\tiny] at (3.5,4.5) {Instruction};
        \node[component, fill=orange!20, ellipse, minimum size=0.8cm] (decode) at (4.5,4) {\tiny decode};
        
        % Execute Stage  
        \node[mux, fill=green!20, rotate=270] (mux1) at (7,2.5) {};
        \node[mux, fill=green!20, rotate=270] (mux2) at (7,1.5) {};
        \node[component, fill=green!30, minimum width=1.5cm, minimum height=2cm] (ALU) at (8,2) {ALU};
        
        % Memory Stage
        \node[component, fill=brown!20, minimum width=1.8cm, minimum height=2cm] (DCache) at (11,2) {
            \begin{tabular}{c}
                Data\\Cache
            \end{tabular}
        };
        \node[font=\tiny, above] at (DCache.north) {address};
        \node[font=\tiny, below] at (DCache.south) {data};
        
        % Write Back Stage
        \node[mux, fill=green!20, rotate=270] (wbmux) at (13.5,2) {};
        ...
        \draw[->] (L1.north) -- ++(0.3,0) |- (decode);
        
        \draw[->] (RF.0) -- ++(0.2,0) |- (mux1.south);
        \draw[->] (RF.0) -- ++(0.2,0) |- (mux2.south);
        \draw[->] (SignExt.east) -| (mux2.north);
        
        \draw[->] (mux1) -- ++(0.3,0) |- (ALU.160);
        \draw[->] (mux2) -- ++(0.3,0) |- (ALU.200);
        \draw[->] (ALU) -- (L3);
        \draw[->] (L3) -- (DCache);
        \draw[->] (DCache) -- (L4);
        \draw[->] (L4) -- (wbmux);
        ...
        % Control signals
        \draw[orange, ->] (decode) -- ++(1.5,0) node[font=\tiny, above] {Control signals};
        
        % Branch paths
        \draw[orange!70, thick, ->] (L3.north) -- ++(0,2) -| node[near start, above, font=\tiny] {} (PC.north);
        \draw[blue!70, thick, ->] (L2.north) -- ++(0,1.5) -| (PC.north);
        
        % Code on the left
        \node[anchor=east, font=\footnotesize\ttfamily] at (-1,1.5) {
            \begin{tabular}{rl}
                0 & or\\
                4 & jcc 50H\\
                8 & and\\
