...
\begin{frame}{Using The BTB}
    ...
    \begin{tikzpicture}[scale=0.8, node distance=0.5cm, remember picture,
        ...
        \coordinate (id_exe_y) at ($(predinst.south) + (0, -0.3)$);
        \coordinate (exe_mem_y) at ([yshift=-0.3cm]branch.south);

        % Find leftmost element
        \coordinate (leftmost) at (pred.west);

        % Position labels and lines relative to flowchart
        ...
        \draw[->] (if_label.north) -- (if_label.north |- if_id_y);
        \draw[->] (if_label.south) -- (if_label.south |- id_exe_y);

        % ID label and arrows (positioned at midpoint between id_exe_y and exe_mem_y)
        \node[stage_label] (id_label) at (label_x |- id_mid_y) {ID};
        \draw[->] (id_label.north) -- (id_label.north |- id_exe_y);
        \draw[->] (id_label.south) -- (id_label.south |- exe_mem_y);

        % EXE label and arrow (positioned below exe_mem_y)
        \node[stage_label] (exe_label) at (label_x |- exe_mid_y) {EXE};
        \draw[->] (exe_label.north) -- (exe_label.north |- exe_mem_y);
...
\begin{frame}{Using The BTB (cont.)}
    ...
    \begin{tikzpicture}[scale=0.8, node distance=0.5cm, remember picture,
        ...
        \coordinate (exe_mem_y) at ([yshift=-0.3cm]branch.south);
        \coordinate (mem_wb_y) at ([yshift=-0.3cm]update.south);
        \coordinate (wb_end_y) at ([yshift=-0.3cm]correct.south);
        \coordinate (end_y) at ([yshift=-0.3cm]correctinst.south);

        % Find leftmost element
        \coordinate (leftmost) at (calc.west);
        ...
        \node[stage_label] (exe_label) at (label_x |- exe_mid_y) {EXE};
        \draw[->] (exe_label.north) -- (exe_label.north |- exe_mem_y);
        \draw[->] (exe_label.south) -- (exe_label.south |- mem_wb_y);
        ...
        \draw[->] (mem_label.north) -- (mem_label.north |- mem_wb_y);
        \draw[->] (mem_label.south) -- (mem_label.south |- wb_end_y);

        % WB label and arrows (positioned at midpoint between wb_end_y and end_y)
        \node[stage_label] (wb_label) at (label_x |- wb_mid_y) {WB};
        \draw[->] (wb_label.north) -- (wb_label.north |- wb_end_y);
        \draw[->] (wb_label.south) -- (wb_label.south |- end_y);
