...
% Usage: \placeMux[rotation]{x,y}{prefix}{fill color}[selector up/down offsets]
% rotation: angle in degrees (default 0)
% x,y: position
% prefix: name prefix for coordinates
% fill color: color for the mux (e.g., blue!20)
% Special handling: if prefix is "wb", use different port configuration for 270 rotation
\newcommand{\placeMux}[5][0]{%
    ...
    % Define input/output coordinates based on rotation
    \ifnum#1=90
        % Rotation 90: narrow side (west) is output, wide side (north) has inputs
        \coordinate (#3mux_in1) at ([xshift=-2mm]#3mux.north);
        \coordinate (#3mux_in2) at ([xshift=2mm]#3mux.north);
        \coordinate (#3mux_in3) at (#3mux.south);
    ...
    \else\ifnum#1=270
        % Rotation 270: different configs for different muxes
        \def\tmppref{#3}%
        \def\tmpwb{wb}%
        \ifx\tmppref\tmpwb
            % WB mux: inputs on north/south, output on east
            \coordinate (#3mux_in1) at (#3mux.north);
            \coordinate (#3mux_in2) at (#3mux.south);
            \coordinate (#3mux_out) at (#3mux.east);
        \else
            % EXE mux: inputs on south (wide side), output on north
            \coordinate (#3mux_in1) at ([yshift=4]#3mux.south);
            \coordinate (#3mux_in2) at ([yshift=-4]#3mux.south);
    ...
    \else
        % Default (0 degrees): narrow side is on left (output), wide side on right (inputs)
        \coordinate (#3mux_in1) at ([yshift=2mm]#3mux.east);
        \coordinate (#3mux_in2) at ([yshift=-2mm]#3mux.east);
        \coordinate (#3mux_out) at (#3mux.west);
        % Default selector positions
    ...
    % Apply custom selector offsets if provided
    \ifx\relax#5\relax\else
        \coordinate (#3mux_selectorup) at ([xshift=#5{1},yshift=#5{2}]#3mux.center);
        \coordinate (#3mux_selectordown) at ([xshift=#5{3},yshift=#5{4}]#3mux.center);
    \fi
}
...
\begin{frame}{Control Hazard on Branches}
    ...
    \begin{tikzpicture}[
        ...
        % Fetch Stage
        % PC MUX (rotate=90: narrow side is west, wide side is east)
        ...
        \coordinate (rf_selector) at ([xshift=-8mm, yshift=-10mm]RF.center);
        ...
        % Execute stage ALU source MUX (rotate=270: narrow side is east, wide side is west)
        \placeMux[270]{7,1.5}{exe}{green!20}{}
        ...
        % Write Back Stage MUX (rotate=270: narrow side is east, wide side is west)
