...
\begin{frame}{Control Hazard on Branches}
    ...
    \begin{tikzpicture}[
        ...
        mux/.style={trapezium, trapezium left angle=70, trapezium right angle=70, 
        ...
        % Main frame
        \draw[blue!50, very thick, rounded corners] (-0.5,-1) rectangle (14,5);
        ...
        % Fetch Stage
        % PC MUX with proper input/output nodes (rotate=90: output west/narrow, inputs east/wide)
        \node[mux, rotate=90, fill=blue!20] (pcmux) at (0.3,4.5) {};
        \node[circle, fill=black, inner sep=0.5pt] (pcmux_in1) at ([yshift=2mm]pcmux.east) {};
        \node[circle, fill=black, inner sep=0.5pt] (pcmux_in2) at (pcmux.east) {};
        \node[circle, fill=black, inner sep=0.5pt] (pcmux_in3) at ([yshift=-2mm]pcmux.east) {};
        \node[circle, fill=black, inner sep=0.5pt] (pcmux_out) at (pcmux.west) {};
        ...
        \node[align=center] at (2,2.6) {\footnotesize inst.\\[-4pt]\footnotesize cache};
 
        
        % PC adder - moved right
        \begin{scope}[shift={(1.2,3.8)}, scale=0.8]
            \coordinate (fe_adder) at (0,0);
            ...
            \coordinate (fe_adder_input1) at (0, 0.3);
            \coordinate (fe_adder_input2) at (0, -0.3);
            \coordinate (fe_adder_output) at (0.3, 0);
        \end{scope}
        \node[left, font=\tiny] (four) at (1.1,4.04) {4};
        
        % Decode Stage
        \node[component, fill=pink!30, minimum width=1.4cm, minimum height=2.2cm] (RF) at (4.5,2.3) {};
        \node[align=center] at (4.5,3) {\footnotesize register\\[-2pt]\footnotesize file};
        
        % Register file input ports (left side, inside the box)
        \node[port] (rf_src1) at ([xshift=-5mm, yshift=2mm]RF.center) {~src1};
        \node[port] (rf_src2) at ([xshift=-5mm, yshift=-5mm]RF.center) {~src2};
        \node[port] (rf_dst) at ([xshift=-4mm, yshift=-10mm]RF.center) {dst};
        
        % Register file output ports (right side, aligned to right edge)
        \node[port, anchor=east] (rf_src1_data) at ([xshift=7mm, yshift=2mm]RF.center) {src1};
        \node[port, anchor=east] (rf_data1) at ([xshift=7mm, yshift=-1mm]RF.center) {data};
        \node[port, anchor=east] (rf_src2_data) at ([xshift=7mm, yshift=-5mm]RF.center) {src2};
        \node[port, anchor=east] (rf_data2) at ([xshift=7mm, yshift=-8mm]RF.center) {data};
        
        % Sign extension - moved slightly right
        \node[component, fill=green!20, ellipse, minimum width=0.4cm, minimum height=0.3cm, align=center] (SignExt) at (4.9,0.4) {
            \scriptsize sign\\[-7pt]\scriptsize ext.
        };
        
        % Execute Stage
        % Execute adder
        \begin{scope}[shift={(7,3)}, scale=0.8]
            \coordinate (ex_adder) at (0,0);
            % Trapezoid coordinates
            \coordinate (toptopleft) at (0, 0.4);
            \coordinate (topbottomleft) at (0, 0.15);
            \coordinate (left) at (0.1, 0);
            ...
            \coordinate (ex_adder_input1) at (0, 0.3);
            \coordinate (ex_adder_input2) at (0, -0.3);
            \coordinate (ex_adder_output) at (0.3, 0);
        \end{scope}
        
        % Execute stage ALU source MUX (rotate=270: output east/narrow, inputs west/wide)
        \node[mux, rotate=270] (exemux) at (7,1.5) {};
        \node[circle, fill=black, inner sep=0.5pt] (exemux_in1) at ([yshift=2mm]exemux.west) {};
        \node[circle, fill=black, inner sep=0.5pt] (exemux_in2) at ([yshift=-2mm]exemux.west) {};
        \node[circle, fill=black, inner sep=0.5pt] (exemux_out) at (exemux.east) {};
        ...
        \node[align=center] at (10.8,2.6) {\footnotesize data\\[-4pt]\footnotesize cache};

        % Data Cache ports (aligned to left side of box)
        \node[port, anchor=west] (dc_addr) at ([xshift=-6.6mm, yshift=0mm]DCache.center) {address};
        \node[port, anchor=west] (dc_data) at ([xshift=-6.6mm, yshift=-7mm]DCache.center) {data};

        % DCache output port
        \node[port, anchor=east] (dc_data_out) at ([xshift=6.5mm, yshift=0mm]DCache.center){};
        
        % Write Back Stage MUX (rotate=270: output east/narrow, inputs west/wide)
        \node[mux, rotate=270] (wbmux) at (13.5,2) {};
        \node[circle, fill=black, inner sep=0.5pt] (wbmux_in1) at ([yshift=2mm]wbmux.west) {};
        \node[circle, fill=black, inner sep=0.5pt] (wbmux_in2) at ([yshift=-2mm]wbmux.west) {};
        \node[circle, fill=black, inner sep=0.5pt] (wbmux_out) at (wbmux.east) {};
        
        % Main data path connections
        \draw[->] (four) -- (fe_adder_input1);
        \draw[->] (pcmux_out) -- ++(-0.1,0) -- ++(0,-2.5) -- (PC.north);
        \draw[->] (PC.east) -- (ICache);
        ...
        \draw[->] (ICache -| L1.east) -- ++(0.3,0) node[yshift=5.5mm,rotate=90,above] {\tiny{}instruction}; 


        % From Register File outputs to L2
        \draw[->] (rf_src1_data.east) -- (rf_src1_data.east -| L2.west);
        \draw[->] (rf_data2.east) -- (rf_data2.east -| L2.west);
        \draw[->] (SignExt.east) -- (SignExt.east -| L2.west);
        
        % From L2 to Execute components
        \draw[->] (rf_src1_data.east -| L2.east) -- ++(0.2,0) |- (alu_input1);
        \draw[->] (rf_data2.east -| L2.east) -- (exemux_in1);
        \draw[->] (SignExt.east -| L2.east) -- (exemux_in2);
        ...
        \draw[->] (rf_data2.east -| L3.east) -- (dc_data.west);
        ...
        \draw[green, thick, ->] (wbmux_out) -- ++(0.3,0) -- ++(0,-2.9) -| (rf_dst.south);
        
        % PC MUX inputs: branch target from L3, branch from L2, and PC+4
        \draw[orange!70, thick, ->] (ex_adder_output -| L3.east) -- ++(0.2,0) -- ++(0,1.7) -| (pcmux_in1);
        \draw[blue!70, thick, ->] (L2.north) -- ++(0,1.5) -| (pcmux_in2);
        \draw[blue, thick, ->] (fe_adder_output) -- (fe_adder_output |- pcmux_in3) -- (pcmux_in3);
...
\begin{frame}{Using The BTB}
    ...
    \begin{tikzpicture}[scale=0.8, node distance=0.5cm]
        ...
        \draw[->] (hit) -| node[right] {no} ([xshift=65mm]inc);
