...
\begin{frame}{Control Hazard on Branches}
    ...
    \begin{tikzpicture}[
        ...
        % Fetch Stage
        % MUX before PC (blue for PC-related components) - moved up and right
        \node[mux, rotate=90, fill=blue!20] (pcmux) at (0.3,4.5) {};
        \coordinate (pcmux_input1) at (0.4,4.8);
        \coordinate (pcmux_input2) at (0.4,4.2);
        \coordinate (pcmux_output) at (0.2,4.5);
        ...
        \end{scope}
        \node[left, font=\tiny] (four) at (0.2,3.5) {4};
        ...
        \node[mux, rotate=270] (mux2) at (7,1.5) {};
        
        % ALU - with rotated text
        \begin{scope}[shift={(8,1.5)}, scale=0.8]
        ...
        \draw[->] (pcmux_output) -- ++(-0.4,0) |- (PC.west);
        \draw[->] (PC.east) -- (ICache);
        ...
        \draw[->] (rf_src1_data.east) -- (rf_src1_data -| L2.west);
        \draw[->] (rf_data2.east) -- (rf_data2 -| L2.west);
        \draw[->] (SignExt.east) -- (SignExt -| L2.west);
        
        % From L2 to Execute components
        \draw[->] ([yshift=5mm]L2.east) -- ++(0.2,0) |- (alu_input1);
        \draw[->] ([yshift=-5mm]L2.east) -| (mux2.south);
        \draw[->] ([yshift=-10mm]L2.east) -| (mux2.north);
        %\draw[->] ([yshift=-10mm]L2.east) -- ++(0.3,0) |- (7,3);
        \draw[->] (SignExt.east -| L2.east) -- ++(0.3,0) |- (ex_adder_input2);
        
        % MUX to ALU
        \draw[->] (mux2) -- ++(0.3,0) |- (alu_input2);
        ...
        \draw[->] (L3.east |- dc_addr) -- (dc_addr.west);
        \draw[->] (L3.east |- dc_data) -- (dc_data.west);
        \draw[->] (dc_data_out) -- (L4.west |- dc_data_out);
        
        % L4 to WB mux - two inputs (ALU result and memory data)
        \draw[->] ([yshift=2mm]L4.east) -| (wbmux.north);
        \draw[->] ([yshift=-2mm]L4.east) -| (wbmux.south);
        
        % Write back path - single output from mux
        \draw[green, ->] (wbmux) -- ++(0.3,0) -- ++(0,-2.9) -| ([xshift=-2mm]rf_dst.south);
        \draw[black, ->] ([yshift=-28mm]L4) -| ++(0.3,0) -- ++(0,-0) -| (rf_dst.south);
        
        % Branch paths - two inputs to PC mux
        \draw[orange!70, thick, ->] ([yshift=10mm]L3.north) -- ++(0,1) -| ([xshift=1mm]pcmux.north);
        \draw[blue!70, thick, ->] (L2.north) -- ++(0,1.5) -| ([xshift=-1mm]pcmux.north);
        
        % PC increment feedback - third input (making it clear there are inputs)
        %\draw[->, blue] (PC.north) -- ++(0,1.5) -| (0.15,3.5);
        \draw[->, blue] (0.54,3.5) -- ++(0.3,0) -- ++(0,-1.3) -| (pcmux.south);
