...
\begin{frame}[fragile]{Software Scheduling to Avoid Load Hazards}
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \coordinate (base) at ($(current page.north) + (0, -2.5)$);

        % Example C code (left column)
        \node[anchor=north west, align=left] (example) at ($(base) + (-5.5, 0)$) {
        ...
        % Slow code column
        \node[anchor=north west, align=left] (slow_title) at ($(base) + (-1.5, 0)$) {\textbf{Slow code}};
        ...
        % Fast code column
        \node[anchor=north west, align=left] (fast_title) at ($(base) + (2.5, 0)$) {\textbf{Fast code}};
