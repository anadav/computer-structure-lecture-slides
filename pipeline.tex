...
\begin{frame}{The Store Instruction}
    \centering
    \Large
    \textbf{sw} \textcolor{blue}{rt, rs, imm16}
    ...
    \begin{tikzpicture}
        ...
        \node at (3,0.5) {rs};
        \node at (5,0.5) {rt};
        \node at (8,0.5) {immediate};

        \node[above] at (1,1) {\small 31};
        \node[above] at (2,1) {\small 26};
        \node[above] at (4,1) {\small 21};
        \node[above] at (6,1) {\small 16};
        \node[above] at (10,1) {\small 0};

        \node[below] at (1,0) {\small 6 bits};
        \node[below] at (3,0) {\small 5 bits};
        \node[below] at (5,0) {\small 5 bits};
        \node[below] at (8,0) {\small 16 bits};
    \end{tikzpicture}

    \vspace{0.5cm}
    \begin{itemize}
        \item \textcolor{red}{mem[PC]} \hfill Fetch the instruction from memory
        \item \textcolor{red}{Addr $\leftarrow$ R[rs] + SignExt(imm16)}

        \hfill Calculate the memory address
        \item \textcolor{red}{Mem[Addr] $\leftarrow$ R[rt]} \hfill Store the register into memory
        \item \textcolor{red}{PC $\leftarrow$ PC + 4} \hfill Calculate the next instruction's address
    \end{itemize}

    \vspace{0.5cm}
    \textcolor{red}{Mem[Rs + SignExt[imm16]] $\leftarrow$ Rt} \hfill Example: \textcolor{blue}{sw rt, rs, imm16}
\end{frame}
...
\begin{frame}{Delayed Branch Performance}
    \begin{itemize}
        \item \textbf{Filling 1 delay slot is easy, 2 is hard, 3 is harder}
        \item \textbf{Assuming we can effectively fill d\% of the delayed slots}
        ...
        \vspace{0.5cm}
        \normalsize
        \item \textbf{For example, for d=0.5, we get $\text{CPI}_{\text{new}} = 1.3$}
        \item \textbf{Mixing architecture with micro-arch}
        \begin{itemize}
            \item New generations requires more delay slots
            \item Cause computability issues between generations
        \end{itemize}
    \end{itemize}
\end{frame}
