...
\begin{frame}{Control Hazard on Branches}
    ...
    \begin{tikzpicture}[
        ...
        \node[mux, rotate=90, fill=blue!20] (pcmux) at (0.2,4.5) {};
        \node[component, fill=blue!20, minimum width=0.6cm, minimum height=0.6cm] (PC) at (0.5,2) {\small PC};
        \node[component, fill=purple!20, minimum width=0.5cm, minimum height=2cm] (ICache) at (2,2) {
            \begin{tabular}{c}
                \footnotesize inst.\\[-2pt]\footnotesize cache
            \end{tabular}
        };
        
        % PC adder - moved right
        \begin{scope}[shift={(0.3,3.5)}, scale=0.8]
            \coordinate (adder) at (0,0);
            % Trapezoid coordinates
            \coordinate (toptopleft) at (0, 0.8);
            \coordinate (topbottomleft) at (0, 0.55);
            \coordinate (left) at (0.1, 0.4);
            \coordinate (bottomtopleft) at (0, 0.25);
            \coordinate (bottombottomleft) at (0, 0);
            \coordinate (topright) at (0.3, 0.6);
        ...
        \node[component, fill=green!20, ellipse, minimum width=0.7cm, minimum height=0.6cm, align=center] (SignExt) at (4.9,0) {
            \footnotesize sign\\[-3pt]\footnotesize ext.
        };
        
        % Instruction label (rotated)
        \node[font=\tiny, rotate=90] at (3.15,2) {instruction};
        
        % Execute Stage
        % Execute adder
        \begin{scope}[shift={(7,3)}, scale=0.8]
            \coordinate (adder2) at (0,0);
            % Trapezoid coordinates
            \coordinate (toptopleft) at (0, 0.8);
            \coordinate (topbottomleft) at (0, 0.55);
            \coordinate (left) at (0.1, 0.4);
        ...
        \begin{scope}[shift={(8,1.5)}, scale=0.8]
            ...
            % Trapezoid coordinates
            \coordinate (toptopleft) at (0, 1.6);
            \coordinate (topbottomleft) at (0, 1.1);
            \coordinate (left) at (0.2, 0.8);
            \coordinate (bottomtopleft) at (0, 0.5);
            \coordinate (bottombottomleft) at (0, 0);
            \coordinate (topright) at (0.6, 1.2);
            ...
            \node[rotate=90, font=\tiny] at (0.3, 0.8) {ALU};
        \end{scope}
        
        % Memory Stage
        \node[component, fill=brown!20, minimum width=1.4cm, minimum height=2cm] (DCache) at (11,2) {};
        \node[align=center] at (11,2.5) {\footnotesize data\\[-2pt]\footnotesize cache};
        
        % Data Cache ports (aligned to left side of box)
        \node[port, anchor=west] (dc_addr) at ([xshift=-7mm, yshift=0mm]DCache.center) {address};
        \node[port, anchor=west] (dc_data) at ([xshift=-7mm, yshift=-7mm]DCache.center) {data};
        ...
        \draw[->] (rf_src1_data.east) -- ([xshift=0mm, yshift=5mm]L2.west);
        \draw[->] (rf_data2.east) -- ([xshift=0mm, yshift=-5mm]L2.west);
        \draw[->] (SignExt.east) -| ([yshift=-10mm]L2.west);
        
        % From L2 to Execute components
        \draw[->] ([yshift=5mm]L2.east) -- ++(0.2,0) |- (8,2.1);
        \draw[->] ([yshift=-5mm]L2.east) -| (mux2.south);
        \draw[->] ([yshift=-10mm]L2.east) -| (mux2.north);
        \draw[->] ([yshift=-10mm]L2.east) -- ++(0.3,0) |- (7,3);
        \draw[->] ([yshift=5mm]L2.east) -- ++(0.3,0) |- (7,3.4);
        
        % MUX to ALU
        \draw[->] (mux2) -- ++(0.3,0) |- (8,0.9);
        
        % ALU and adder to L3
        \draw[->] (8.48,1.5) -- (L3);
        \draw[->] (7.24,3) -| ([yshift=10mm]L3.west);
        
        % L3 to Memory (address)
        \draw[->] (L3.east) -- ++(0.2,0) |- (dc_addr.west);
        
        % L3 to Memory (data)
        \draw[->] ([yshift=-7mm]L3.east) -- (dc_data.west);
        ...
        % Branch paths
        \draw[orange!70, thick, ->] ([yshift=10mm]L3.north) -- ++(0,1) -| (pcmux.north);
        \draw[blue!70, thick, ->] (L2.north) -- ++(0,1.5) -| ([xshift=-2mm]pcmux.north);
        
        % PC increment feedback
