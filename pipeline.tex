...
\begin{frame}{Control Hazard on Branches}
    ...
    \begin{tikzpicture}[
        ...
        % Decode Stage
        \node[component, fill=pink!30, minimum width=1.2cm, minimum height=1.8cm] (RF) at (4.5,2.3) {};
        ...
        \node[port] (rf_src1) at ([xshift=-4mm, yshift=1mm]RF.center) {~src1};
        \node[port] (rf_src2) at ([xshift=-4mm, yshift=-3mm]RF.center) {~src2};
        \node[port] (rf_dst) at ([xshift=-3.5mm, yshift=-7mm]RF.center) {dst};
        
        % Register file selector coordinate (slightly left of dst)
        \coordinate (rf_selector) at ([xshift=-7mm, yshift=-7mm]RF.center);
        
        % Register file output ports (right side, aligned to right edge)
        \node[port, anchor=east] (rf_src1_data) at ([xshift=6mm, yshift=0.9mm]RF.center) {src1};
        \node[port, anchor=east] (rf_data1) at ([xshift=6mm, yshift=-0.6mm]RF.center) {data};
        \node[port, anchor=east] (rf_src2_data) at ([xshift=6mm, yshift=-3mm]RF.center) {src2};
        \node[port, anchor=east] (rf_data2) at ([xshift=6mm, yshift=-4.5mm]RF.center) {data};
        
        % Sign extension - moved slightly right
        \node[component, fill=green!20, ellipse, minimum width=0.4cm, minimum height=0.3cm, align=center] (SignExt) at (5.1,0.7) {
        ...
        \node[port, anchor=west] (dc_addr) at ([xshift=-6.2mm, yshift=0mm]DCache.center) {address};
        \node[port, anchor=west] (dc_data) at ([xshift=-6.2mm, yshift=-7mm]DCache.center) {data};
