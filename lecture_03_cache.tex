...
\begin{tikzpicture}[
  % Define colors (include draw for outline)
  lightblue/.style={draw, fill=blue!25, minimum width=0.9cm, minimum height=0.22cm},
  lightgreen/.style={draw, fill=green!25, minimum width=0.9cm, minimum height=0.22cm},
  lightyellow/.style={draw, fill=yellow!25, minimum width=0.9cm, minimum height=0.22cm},
  lightpurple/.style={draw, fill=purple!25, minimum width=0.9cm, minimum height=0.22cm},
  whiteblock/.style={draw, fill=white, minimum width=0.9cm, minimum height=0.22cm},
  % Styles
  membox/.style={draw, minimum width=0.9cm, minimum height=0.22cm, inner sep=1pt},
  cachebox/.style={draw, minimum width=0.7cm, minimum height=0.22cm, inner sep=1pt},
  ...
  % 64B width indicator
  \node[anchor=north, font=\tiny] at (mem-14-2.south) {$\leftarrow$ 64B $\rightarrow$};
  ...
  % Cache title
  \node[anchor=south, font=\footnotesize\bfseries] at (cache-1-2.north) {cache};
...
\centering
\begin{tikzpicture}[
    validbox/.style={draw, thick, minimum width=0.3cm, minimum height=0.32cm, fill=red!90, font=\tiny, inner sep=0pt, text=white},
    tagbox/.style={draw, thick, minimum width=1.2cm, minimum height=0.32cm, fill=green!20},
    databox/.style={draw, thick, minimum width=2.8cm, minimum height=0.32cm, fill=white},
    offsetbox/.style={draw, thick, minimum width=0.5cm, minimum height=0.32cm, fill=orange!20},
    label/.style={font=\tiny},
...
% Labels for columns
\node[label, below=0.3cm of tag_field.south, xshift=-8mm] (tag_array_label) {Tag Array};
\node[label, below=0.3cm of offset_field.south, xshift=1.6cm] (data_array_label) {Data Array};
...
\foreach \i in {0,...,7} {
    ...
    % Data array
    \node[databox, right=1cm of and_gate_\i.out] (data\i) {};
}
...
\foreach \i in {0,...,7} {
    \draw[->, >=stealth, thick] (tag\i.east) -- (comp\i.lpin 2);
    \draw[->, >=stealth, thick] (comp\i.rpin 1) -- (and_gate_\i.in 2);
    \draw[->, >=stealth, thick] (and_gate_\i.out) -- (data\i.west);
    \draw[->, >=stealth, thick, color=red!90] (valid\i.east) |- (and_gate_\i.in 1);
}

% OR gate for hit/miss
\node[or port, number inputs=8, anchor=north, external pins width=0, fill=cyan!20,
      xscale=0.6, yscale=0.25, rotate=-90, below right=0.4cm and 0.5cm of and_gate_7.south, anchor=out] (or_gate) {};

% Connect comparators to OR gate
\foreach \i in {0,...,7} {
    \draw[->, >=stealth, thick] (and_gate_\i.out) -- (and_gate_\i.out -| or_gate.bin \the\numexpr\i+1\relax) node[data_connector] {} -- (or_gate.bin \the\numexpr\i+1\relax);
}
...
\begin{tikzpicture}[scale=0.75, transform shape,
    tagbox/.style={rectangle, draw, fill=green!30, minimum width=1.8cm, minimum height=0.5cm, font=\footnotesize},
    setbox/.style={rectangle, draw, fill=cyan!30, minimum width=1.2cm, minimum height=0.5cm, font=\footnotesize},
    offsetbox/.style={rectangle, draw, fill=orange!30, minimum width=1.2cm, minimum height=0.5cm, font=\footnotesize},
    cellstyle/.style={rectangle, draw, minimum width=1.2cm, minimum height=0.3cm, font=\tiny},
...
% Bit labels
\node[above=1pt of tag.north west, anchor=south west, font=\tiny] {39};
\node[above=1pt of tag.north east, anchor=south east, font=\tiny] {14};
\node[above=1pt of set.north east, anchor=south east, font=\tiny] {6};
\node[above=1pt of offset.north east, anchor=south east, font=\tiny] {0};

% Tag array position
\coordinate (tagarray_start) at ($(set.south) + (-0.5,-2.2)$);

% Tag array (256 sets shown as 4 visible rows)
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[tagcell, anchor=north] at (tagarray_start) (tagcell_\i) {\scriptsize Tag};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
        \node[tagcell, below=0cm of tagcell_\prev.south, anchor=north] (tagcell_\i) {};
    \fi
}

% Data array (next to tag array)
\foreach \i in {0,...,3} {
    \node[datacell, right=0.6cm of tagcell_\i] (datacell_\i) {\scriptsize Data};
}
\node[offsetcell, anchor=east] at ([xshift=-1mm]datacell_1.east) (thedata) {};

% Decoder
\node[decoder, anchor=rpin 1] at ($(tagcell_0.west) + (-0.5,0)$) (decoder) {};

% Comparator
\node[muxdemux, muxdemux def={Lh=0.6, Rh=0.3, NL=2, NB=1, w=0.5},
      external pins width=0, fill=green!30, rotate=270] at ([yshift=-1.2cm]tagcell_3.south) (comp) {};
\node[font=\fontsize{4}{4}\selectfont, rotate=90] at (comp.center) {=};

% Labels
\node[left=2mm of decoder.lpin 1, font=\scriptsize] {Set\#};
\node[above=1mm of tagcell_0.north west, anchor=south west, font=\scriptsize] {Tag Array};
\node[above=1mm of datacell_0.north west, anchor=south west, font=\scriptsize] {Data Array};
\node[below=2mm of comp.rpin 1, font=\scriptsize] {Hit/Miss};
\node[below=2mm of datacell_3.south, font=\scriptsize] {Data Out};
\node[right=1mm of tagcell_3.east, font=\tiny, text=gray] {$2^8$=256 sets};

% Connections
% Set to decoder
\draw[arrow] (set.south) -- ++(0,-0.5) -| (decoder.lpin 1);

% Tag to comparator
\draw (tag.south) -- ++(0,-0.8) coordinate (tagjunc);
\node[junction] at (tagjunc) {};
\draw[arrow] (tagjunc) -| (comp.lpin 1);
...
\foreach \i in {0,...,3} {
    \draw[arrow] (decoder.rpin 1 |- tagcell_\i.west) -- (tagcell_\i.west);
}

% Tag to data (dashed)
\foreach \i in {0,...,3} {
    \draw[dasharrow] (tagcell_\i.east) -- (datacell_\i.west);
}

% Selected line highlight
\draw[arrow, thick, orange] (tagcell_1.east) -- (datacell_1.west);

% Tag array to comparator
\draw[arrow] (tagcell_1.south) -- ++(0,-0.3) -| (comp.lpin 2);

% Comparator output
\draw[arrow] (comp.rpin 1) -- ++(0,-0.3);

% Data out
\draw[arrow] (datacell_1.south) -- ++(0,-0.5);
