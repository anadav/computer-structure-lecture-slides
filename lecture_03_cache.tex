...
\small
\begin{itemize}\small
  \item L2 to L1 bus may be narrower than cache line (e.g., 16B bus, 64B line $\Rightarrow$ 4 cycles)
  \item Fill buffer accumulates full line, then writes to L1; searched in parallel with cache
  \item Fetch critical chunk (containing missed address) first to reduce latency
  \item Streaming loads: serve data directly from fill buffer without L1 insertion to avoid pollution
\end{itemize}
\end{frame}
