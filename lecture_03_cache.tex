...
\begin{itemize}
  \item CPU performance improved much faster than DRAM latency/bandwidth
  \item Performance gap grew $\sim$50\%/year historically
...
\begin{tikzpicture}[scale=0.65]
  % Axes
  \draw[->, thick] (0,0) -- (6.5,0) node[below] {\footnotesize Time};
  \draw[->, thick] (0,0) -- (0,4.5) node[left, align=center] {\footnotesize Performance};

  % Y-axis labels (log scale)
  \foreach \y/\label in {0.5/1, 1.5/10, 2.5/100, 3.5/1000} {
    \draw (-0.1,\y) -- (0.1,\y);
    \node[left, font=\tiny] at (-0.1,\y) {\label};
  }

  % X-axis labels
  \foreach \x/\label in {0.5/1980, 2/1990, 3.5/2000, 5/2010, 6/2020} {
    \node[below, font=\tiny] at (\x,-0.1) {\label};
  }

  % CPU line (steep growth)
  \draw[thick, blue, line width=1.5pt] (0.5,0.5) -- (6,4);
  \node[blue, right, font=\scriptsize] at (5.8,4) {CPU};

  % DRAM line (slow growth)
  \draw[thick, red, line width=1.5pt] (0.5,0.5) -- (6,1.5);
  \node[red, right, font=\scriptsize] at (5.8,1.5) {DRAM};

  % Gap annotation
  \draw[<->, thick, green!60!black] (5,2.9) -- (5,1.35);
  \node[green!60!black, right, font=\tiny, align=left] at (5.1,2.1) {Gap};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
