...
\begin{tikzpicture}[scale=0.85, transform shape]
  ...
  % ==================== First scenario - Different sets (Direct Map) ====================
  \begin{scope}[shift={(0,0)}]
    % Title
    \node[anchor=south, align=center, font=\small] at (0.9, \cacheheight+1.1) {Different sets, so\\both can reside\\in the cache};

    % Direct Map label
    \node[font=\small, anchor=south] at (0.9, \cacheheight+0.15) {Direct Map};

    % Cache rectangle
    \draw[thick] (0, 0) rectangle (1.8, \cacheheight);

    % Draw rows with set numbers inside
    \foreach \i in {0, 1, ..., 8} {
      \pgfmathsetmacro{\y}{\i * \rowh}
      \draw (0, \y) -- (1.8, \y);
      \node[font=\scriptsize] at (0.9, \y + \rowh/2) {\i};
    }

    % Highlight different sets (green) - sets 2 and 6
    \fill[lightgreen] (0, 2*\rowh) rectangle (1.8, 3*\rowh);
    \node[font=\scriptsize] at (0.9, 2*\rowh + \rowh/2) {2};
    \fill[lightgreen] (0, 6*\rowh) rectangle (1.8, 7*\rowh);
    \node[font=\scriptsize] at (0.9, 6*\rowh + \rowh/2) {6};

    % Cache Size labels
    \draw[<->, thick] (-0.25, 0) -- (-0.25, \cacheheight/3);
    \node[anchor=east, font=\tiny, align=center] at (-0.3, \cacheheight/6) {Cache\\Size};
    \draw[<->, thick] (-0.25, \cacheheight/3) -- (-0.25, 2*\cacheheight/3);
    \node[anchor=east, font=\tiny, align=center] at (-0.3, \cacheheight/2) {Cache\\Size};
    \draw[<->, thick] (-0.25, 2*\cacheheight/3) -- (-0.25, \cacheheight);
    \node[anchor=east, font=\tiny, align=center] at (-0.3, 5*\cacheheight/6) {Cache\\Size};

    % Dots below
    \node at (0.9, -0.3) {$\vdots$};
  \end{scope}

  % ==================== Second scenario - Same set, 2-way OK ====================
  \begin{scope}[shift={(3.0,0)}]
    % Title
    \node[anchor=south, align=center, font=\small] at (1.15, \cacheheight+1.1) {Same set, still\\both can reside\\in the cache};

    % 2-way S.A. label
    \node[font=\small, anchor=south] at (1.15, \cacheheight+0.15) {2-way S.A.};

    % Cache rectangle with vertical divider
    \draw[thick] (0, 0) rectangle (2.3, \cacheheight);
    \draw[thick] (1.15, 0) -- (1.15, \cacheheight);

    % Draw rows with set numbers inside (in left way)
    \foreach \i in {0, 1, ..., 8} {
      \pgfmathsetmacro{\y}{\i * \rowh}
      \draw (0, \y) -- (2.3, \y);
      \node[font=\scriptsize] at (0.575, \y + \rowh/2) {\i};
      \node[font=\scriptsize] at (1.725, \y + \rowh/2) {\i};
    }

    % Highlight same set in both ways (green) - set 5
    \fill[lightgreen] (0, 5*\rowh) rectangle (1.15, 6*\rowh);
    \node[font=\scriptsize] at (0.575, 5*\rowh + \rowh/2) {5};
    \fill[lightgreen] (1.15, 5*\rowh) rectangle (2.3, 6*\rowh);
    \node[font=\scriptsize] at (1.725, 5*\rowh + \rowh/2) {5};

    % Way Size labels (all 6)
    \foreach \i in {0, 1, ..., 5} {
      \pgfmathsetmacro{\ybot}{\i * \cacheheight/6}
      \pgfmathsetmacro{\ytop}{(\i+1) * \cacheheight/6}
      \pgfmathsetmacro{\ymid}{(\ybot + \ytop) / 2}
      \draw[<->, thick] (-0.25, \ybot) -- (-0.25, \ytop);
      \node[anchor=east, font=\tiny, align=center] at (-0.3, \ymid) {Way\\Size};
    }

    % Dots below
    \node at (1.15, -0.3) {$\vdots$};
  \end{scope}

  % ==================== Third scenario - Same set, Direct Map conflict ====================
  \begin{scope}[shift={(6.4,0)}]
    % Title
    \node[anchor=south, align=center, font=\small] at (0.9, \cacheheight+1.1) {Same set, both\\\underline{cannot} reside in\\the cache};
    ...
    % Highlight conflict (red) - set 5
    \fill[lightred] (0, 5*\rowh) rectangle (1.8, 6*\rowh);
    \node[font=\scriptsize] at (0.9, 5*\rowh + \rowh/2) {5};
  ...
  % ==================== Fourth scenario - Same set, 2-way OK ====================
...
\small
\begin{itemize}
  \item Memory blocks map to cache sets: block address mod \#sets $\Rightarrow$ set number
  \item Blocks at addresses 0, N, 2N, ... all map to \textbf{same set} (where N = cache size)
  \item \textbf{Direct-mapped}: conflict when two blocks map to same set
  \item \textbf{2-way S.A.}: two blocks can coexist in same set (one per way)
\end{itemize}
...
\textbf{Key idea:}
\begin{itemize}
  \item Accessed way gets highest counter value ($k-1$)
  \item Other ways with higher counter are decremented
