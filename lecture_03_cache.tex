...
\begin{enumerate}[itemsep=0pt, topsep=2pt, leftmargin=1.5em]
  \footnotesize
  \item On P0: S1 $\rightarrow$ S2 (stores in program order)
  \item On P1: L1 $\rightarrow$ L2 (loads in program order)
  \item If L1 reads r1=1 (from S2), then S2 $\rightarrow$ L1
  \item Chain: S1 $\rightarrow$ S2 $\rightarrow$ L1 $\rightarrow$ L2 forces L2 to see X=1, so r2=1
\end{enumerate}
\end{frame}
