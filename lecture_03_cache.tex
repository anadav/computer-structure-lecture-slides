...
\begin{tikzpicture}
  ...
  \node[font=\bfseries] (p0) at (0, 0) {Processor 0};
  \node[hbinstr] (s1) at (-0.8, -0.8) {Store [\textcolor{varX}{X}] <- 1};
  \node[font=\scriptsize, gray] at (2.5, -0.8) {S1};
  \node[hbinstr] (s2) at (-0.8, -2.0) {Store [\textcolor{varY}{Y}] <- 1};
  \node[font=\scriptsize, gray] at (2.5, -2.0) {S2};
...
\begin{enumerate}[itemsep=0pt, topsep=2pt, leftmargin=1.5em]
  ...
  \item If L1 reads S2's value (r1=1), then S2 $\rightarrow$ L1
  \item Transitivity: S1 $\rightarrow$ S2 $\rightarrow$ L1 $\rightarrow$ L2, so L2 must see S1 $\Rightarrow$ r2=1
