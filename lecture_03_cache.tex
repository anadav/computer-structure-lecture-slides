...
\centering
\begin{tikzpicture}[
    cache/.style={draw, thick, minimum width=2cm, minimum height=2cm, align=center, font=\bfseries},
    vcache/.style={draw, thick, minimum width=1.2cm, minimum height=0.5cm, anchor=center},
    ]
    ...
    % Label for victim cache
    \node[above=2mm of victim, font=\small\bfseries, align=center] {Victim\\cache};

    % L2 cache (right)
    \node[cache, fill=green!30, right=0.8cm of victim] (l2) {L2\\cache};

    % Arrows
    \draw[->, very thick] (l1.east) -- node[above, font=\small] {(1)} (victim.west);
    \draw[->, very thick] (victim.east) -- node[above, font=\small] {(2)} (l2.west);
    \draw[->, very thick] (victim.south) .. controls +(0,-0.5) and +(0,-0.5) .. node[below, font=\small] {(3)} (l1.south);
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
