...
\usetikzlibrary{circuits.logic.US, tikzmark, shapes.callouts}
...
\tikzset{
  hbinstr/.style={font=\ttfamily\small, anchor=west},
...
\begin{tikzpicture}
  \matrix[matrix of nodes,
    ...
    column 1/.style={nodes={font=\bfseries, anchor=east, minimum width=1.5cm, align=right}},
    column 2/.style={nodes={anchor=east, minimum width=2cm}},
    column 3/.style={nodes={minimum width=4cm}},
    column 4/.style={nodes={anchor=west, minimum width=2cm}},
  ] (m) {
    Speed: \& \textcolor{green!70!black}{Fastest} \& \phantom{spacer} \& \textcolor{red!80!black}{Slowest} \\[0.3em]
    Size: \& \textcolor{red!80!black}{Smallest} \& \phantom{spacer} \& \textcolor{green!70!black}{Biggest} \\[0.3em]
    Cost: \& \textcolor{red!80!black}{Highest} \& \phantom{spacer} \& \textcolor{green!70!black}{Lowest} \\[0.3em]
    Power: \& \textcolor{red!80!black}{Highest} \& \phantom{spacer} \& \textcolor{green!70!black}{Lowest} \\
  };
  % Draw arrows individually for direction control
  \draw[<-, thick, gray] (m-1-2.east) ++(0.2,0) -- ($(m-1-4.west)+(-0.2,0)$); % Speed: Slowest -> Fastest
  \draw[->, thick, gray] (m-2-2.east) ++(0.2,0) -- ($(m-2-4.west)+(-0.2,0)$); % Size: Smallest -> Biggest
  \draw[->, thick, gray] (m-3-2.east) ++(0.2,0) -- ($(m-3-4.west)+(-0.2,0)$); % Cost: Highest -> Lowest
  \draw[->, thick, gray] (m-4-2.east) ++(0.2,0) -- ($(m-4-4.west)+(-0.2,0)$); % Power: Highest -> Lowest
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}[fragile]{Direct-Mapped Cache -- Example}
% Use same colors as the DM illustration
...
\begin{tikzpicture}[scale=0.95, transform shape,
    % Address box styles
    addrbox/.style={rectangle, draw, minimum height=0.5cm, font=\footnotesize, text depth=0.1em},
    tagbox/.style={addrbox, fill=green!30, minimum width=1.6cm},
    setbox/.style={addrbox, fill=cyan!30, minimum width=1.2cm},
    offsetbox/.style={addrbox, fill=orange!30, minimum width=1cm},
    % Cell styles for matrices
    cellstyle/.style={rectangle, draw, minimum width=0.9cm, minimum height=0.3cm,
                     text depth=0.1cm, text height=0.2cm, font=\tiny, inner sep=2pt},
    tagcell/.style={cellstyle, fill=green!30},
    datacell/.style={cellstyle, minimum width=1.2cm},
    offsetcell/.style={cellstyle, fill=orange!30, minimum width=2mm},
    decoder/.style={muxdemux, muxdemux def={Lh=1.5, Rh=3.2, NB=0, NL=1, NR=4, w=0.5, square pins=0},
                   external pins width=0, fill=cyan!20},
    arrow/.style={->, >=stealth, thick},
    dasharrow/.style={dashed, ->, >=stealth},
    junction/.style={circle, fill=black, inner sep=0pt, minimum size=3pt}
...
% Bit labels - adjacent to boxes
\node[anchor=south west, font=\tiny] at (tag.north west) {31};
\node[anchor=south east, font=\tiny] at (tag.north east) {14};
\node[anchor=south west, font=\tiny] at (set.north west) {13};
\node[anchor=south east, font=\tiny] at (set.north east) {5};
\node[anchor=south west, font=\tiny] at (offset.north west) {4};
\node[anchor=south east, font=\tiny] at (offset.north east) {0};
...
\matrix[matrix of nodes,
        column sep=0pt, row sep=0pt,
        ampersand replacement=\&,
        anchor=north] at ([xshift=-0.3cm, yshift=-1.5cm]set.south) (tagmat) {
  |[tagcell]| {\scriptsize Tag} \\
  |[tagcell, very thick, draw=orange]| \phantom{Tag} \\
  |[tagcell]| \phantom{Tag} \\
  |[tagcell]| \phantom{Tag} \\
...
\matrix[matrix of nodes,
        column sep=0pt, row sep=0pt,
        ampersand replacement=\&,
        anchor=north west] at ([xshift=5mm]tagmat.north east) (datamat) {
  |[datacell]| {\scriptsize Data} \\
  |[datacell, very thick, draw=orange]| \phantom{Data} \\
  |[datacell]| \phantom{Data} \\
  |[datacell]| \phantom{Data} \\
...
% Decoder
\node[decoder, anchor=rpin 1] at ([xshift=-4mm]tagmat-1-1.west) (decoder) {};

% Comparator
\node[muxdemux, muxdemux def={Lh=0.6, Rh=0.3, NL=2, NB=1, w=0.5},
      external pins width=0, fill=green!30, rotate=270] at ([xshift=2mm, yshift=-5mm]tagmat.south east) (comp) {};
\node[font=\fontsize{4}{4}\selectfont, rotate=90] at (comp.center) {=};

% Labels
\node[above=-1mm of tagmat.north, anchor=south, font=\scriptsize] {Tag Array};
\node[above=-1mm of datamat.north, anchor=south, font=\scriptsize] {Data Array};
\node[below=2mm of comp.rpin 1, font=\scriptsize] (hitmiss) {Hit/Miss};
\node[font=\scriptsize] at (hitmiss -| datamat) (dataout) {Data Out};
\node[right=1mm of datamat-4-1.east, font=\tiny, text=gray] {512 sets};

% Connections
% Set to decoder
\draw[arrow] (set.south) -- ++(0,-0.6) -| ([xshift=-3mm]decoder.lpin 1) -- (decoder.lpin 1);

% Tag to comparator
\draw[arrow] (tag.south) -- ++(0,-4mm) -| ([xshift=-5mm]decoder.lpin 1 |- tagmat-4-1.south) -- ++(0,-3mm) -| (comp.lpin 2);

% Offset to data
\draw[arrow] (offset.south) -- ++(0,-0.7) -| (thedata.north);
...
% Tag array to comparator (from south of matrix)
\draw[arrow] (tagmat-2-1.east) -| (comp.lpin 1);
...
\textbf{\# Sets:} & $\frac{\text{\#lines}}{\text{\#ways}} = 512$ sets
\end{tabular}

\vspace{0.2cm}
\begin{tabular}{ll}
\textcolor{offsetcolor}{\textbf{Offset bits:}} & $\log_2(\text{line size}) = 5$ bits\\
\textcolor{setcolor}{\textbf{Set bits:}} & $\log_2(\text{\#sets}) = 9$ bits\\
\textcolor{tagcolor}{\textbf{Tag bits:}} & $32 - (5+9) = 18$ bits\\
\end{tabular}

\vspace{0.3cm}
\textbf{Example:} Address \texttt{0x12345678}\\[2pt]

% Binary bits with braces pointing to hex values
% 18-bit tag, 9-bit set, 5-bit offset splits nibbles 5 and 7
\begin{tikzpicture}
  % Place nibbles individually for precise brace positioning
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at (0,0) (n1) {0001};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at ([xshift=2pt]n1.east) (n2) {0010};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at ([xshift=2pt]n2.east) (n3) {0011};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at ([xshift=2pt]n3.east) (n4) {0100};
  % Nibble 5 splits: 01 (tag) + 01 (set)
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at ([xshift=2pt]n4.east) (n5a) {01};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=setcolor] at (n5a.east) (n5b) {01};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=setcolor] at ([xshift=2pt]n5b.east) (n6) {0110};
  % Nibble 7 splits: 011 (set) + 1 (offset)
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=setcolor] at ([xshift=2pt]n6.east) (n7a) {011};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=offsetcolor] at (n7a.east) (n7b) {1};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=offsetcolor] at ([xshift=2pt]n7b.east) (n8) {1000};

  % Braces below pointing to hex values
  \draw[decorate, decoration={brace, amplitude=2pt, mirror, raise=1pt}, tagcolor]
      (n1.south west) -- (n5a.south east)
      node[midway, below=4pt, font=\scriptsize, text=black, align=center] {\texttt{0x048D1}\\[-2pt]{\tiny Tag (18b)}};
  \draw[decorate, decoration={brace, amplitude=2pt, mirror, raise=1pt}, setcolor]
      (n5b.south west) -- (n7a.south east)
      node[midway, below=4pt, font=\scriptsize, text=black, align=center] {\texttt{0x0B3}\\[-2pt]{\tiny Set (9b)}};
  \draw[decorate, decoration={brace, amplitude=2pt, mirror, raise=1pt}, offsetcolor]
      (n7b.south west) -- (n8.south east)
      node[midway, below=4pt, font=\scriptsize, text=black, align=center] {\texttt{0x18}\\[-2pt]{\tiny Offset (5b)}};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
...
\node[anchor=south east] at (set.north east) {\footnotesize 5};
\node[anchor=south west] at (offset.north west) {\footnotesize 4};
...
\node[anchor=south east, font=\footnotesize] at (set.north east) {5};
\node[anchor=south west, font=\footnotesize] at (offset.north west) {4};
...
% Binary bits with braces pointing to hex values
% 19-bit tag, 8-bit set, 5-bit offset splits nibbles 5 and 7
\begin{tikzpicture}
  % Place nibbles individually for precise brace positioning
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at (0,0) (n1) {0001};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at ([xshift=2pt]n1.east) (n2) {0010};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at ([xshift=2pt]n2.east) (n3) {0011};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at ([xshift=2pt]n3.east) (n4) {0100};
  % Nibble 5 splits: 010 (tag) + 1 (set)
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=tagcolor] at ([xshift=2pt]n4.east) (n5a) {010};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=setcolor] at (n5a.east) (n5b) {1};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=setcolor] at ([xshift=2pt]n5b.east) (n6) {0110};
  % Nibble 7 splits: 011 (set) + 1 (offset)
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=setcolor] at ([xshift=2pt]n6.east) (n7a) {011};
  \node[font=\ttfamily\footnotesize, inner sep=0, anchor=west, text=offsetcolor] at (n7a.east) (n7b) {1};
  ...
  \draw[decorate, decoration={brace, amplitude=2pt, mirror, raise=1pt}, tagcolor]
      (n1.south west) -- (n5a.south east)
      node[midway, below=4pt, font=\scriptsize, text=black, align=center] {\texttt{0x091A2}\\[-2pt]{\tiny Tag (19b)}};
  \draw[decorate, decoration={brace, amplitude=2pt, mirror, raise=1pt}, setcolor]
      (n5b.south west) -- (n7a.south east)
      node[midway, below=4pt, font=\scriptsize, text=black, align=center] {\texttt{0xB3}\\[-2pt]{\tiny Set (8b)}};
  \draw[decorate, decoration={brace, amplitude=2pt, mirror, raise=1pt}, offsetcolor]
      (n7b.south west) -- (n8.south east)
      node[midway, below=4pt, font=\scriptsize, text=black, align=center] {\texttt{0x18}\\[-2pt]{\tiny Offset (5b)}};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.9, transform shape]
  ...
  \def\rowh{0.32}
  \def\colw{1.6}
  \def\nrows{12}    % total rows to show
  \def\dmsets{6}    % sets for direct map
  \def\sasets{3}    % sets per way for 2-way
  \def\waysize{3}   % way size
  \def\cachesize{6} % cache size (2 ways)

  % Callout: explain what the figure shows (pointing to east of right 2-way S.A.)
  \node[rectangle callout, callout absolute pointer={(10.6+\colw, \nrows*\rowh/2)},
        draw, fill=yellow!20, font=\scriptsize, align=left, text width=2.2cm, anchor=west]
        at (10.6+\colw+0.5, \nrows*\rowh/2) {Each column shows how memory blocks map to cache sets (number = set \#)};

  % ==================== LEFT GROUP: Different sets - both OK ====================

  % --- Direct Map (different sets) ---
  \begin{scope}[shift={(0,0)}]
    \node[anchor=south, align=center, font=\small] at (\colw/2, \nrows*\rowh+0.9) {Different sets,\\both can reside\\in the cache};
    \node[font=\small, anchor=south] at (\colw/2, \nrows*\rowh+0.1) {Direct Map};

    \draw[thick] (0, 0) rectangle (\colw, \nrows*\rowh);

    % Draw rows with set numbers (0-5 repeating)
    \foreach \i in {0, 1, ..., 11} {
      \pgfmathsetmacro{\y}{\i * \rowh}
      \pgfmathtruncatemacro{\setnum}{mod(\i, \dmsets)}
      \draw (0, \y) -- (\colw, \y);
      \node[font=\scriptsize] at (\colw/2, \y + \rowh/2) {\setnum};
    }

    % Highlight different sets (green) - row 1 (set 1) and row 5 (set 5)
    ...
    \node[font=\scriptsize] at (\colw/2, 5*\rowh + \rowh/2) {5};

    % Cache Size label (6 rows)
    \draw[<->, thick] (-0.2, 0) -- (-0.2, \cachesize*\rowh);
    \node[anchor=east, font=\tiny, align=center] at (-0.25, 3*\rowh) {Cache\\Size};
    \draw[<->, thick] (-0.2, \cachesize*\rowh) -- (-0.2, \nrows*\rowh);
  ...
  \begin{scope}[shift={(3.2,0)}]
    \node[anchor=south, align=center, font=\small] at (\colw/2, \nrows*\rowh+0.9) {Different sets,\\both can reside\\in the cache};
    \node[font=\small, anchor=south] at (\colw/2, \nrows*\rowh+0.1) {2-way S.A.};

    \draw[thick] (0, 0) rectangle (\colw, \nrows*\rowh);

    % Draw rows with set numbers (0-2 repeating)
    \foreach \i in {0, 1, ..., 11} {
      \pgfmathsetmacro{\y}{\i * \rowh}
      \pgfmathtruncatemacro{\setnum}{mod(\i, \sasets)}
      \draw (0, \y) -- (\colw, \y);
      \node[font=\scriptsize] at (\colw/2, \y + \rowh/2) {\setnum};
    }

    % Highlight same rows as direct map (green) - row 1 (set 1) and row 5 (set 2)
    \fill[lightgreen] (0, 1*\rowh) rectangle (\colw, 2*\rowh);
    \node[font=\scriptsize] at (\colw/2, 1*\rowh + \rowh/2) {1};
    \fill[lightgreen] (0, 5*\rowh) rectangle (\colw, 6*\rowh);
    \node[font=\scriptsize] at (\colw/2, 5*\rowh + \rowh/2) {2};

    % Way Size labels (inner) - no Cache Size for 2-way
    \foreach \w in {0, 1, 2, 3} {
      \pgfmathsetmacro{\ybot}{\w * \waysize * \rowh}
      \pgfmathsetmacro{\ytop}{(\w + 1) * \waysize * \rowh}
  ...
  \begin{scope}[shift={(7.4,0)}]
    \node[anchor=south, align=center, font=\small] at (\colw/2, \nrows*\rowh+0.9) {Same set,\\\textcolor{red}{\underline{conflict}}\\$\Rightarrow$ eviction};
    \node[font=\small, anchor=south] at (\colw/2, \nrows*\rowh+0.1) {Direct Map};
    ...
    % Highlight conflict (red) - row 2 (set 2) and row 8 (set 2)
    \fill[lightred] (0, 2*\rowh) rectangle (\colw, 3*\rowh);
    \node[font=\scriptsize] at (\colw/2, 2*\rowh + \rowh/2) {2};
    \fill[lightred] (0, 8*\rowh) rectangle (\colw, 9*\rowh);
    \node[font=\scriptsize] at (\colw/2, 8*\rowh + \rowh/2) {2};
  ...
  \begin{scope}[shift={(10.6,0)}]
    \node[anchor=south, align=center, font=\small] at (\colw/2, \nrows*\rowh+0.9) {Same set,\\different ways\\$\Rightarrow$ both fit};
    \node[font=\small, anchor=south] at (\colw/2, \nrows*\rowh+0.1) {2-way S.A.};
    ...
    % Highlight same rows as direct map but green - row 2 (set 2) and row 8 (set 2)
    \fill[lightgreen] (0, 2*\rowh) rectangle (\colw, 3*\rowh);
    \node[font=\scriptsize] at (\colw/2, 2*\rowh + \rowh/2) {2};
    \fill[lightgreen] (0, 8*\rowh) rectangle (\colw, 9*\rowh);
    \node[font=\scriptsize] at (\colw/2, 8*\rowh + \rowh/2) {2};
...
\small
\begin{itemize}
  \item \textbf{Direct Map}: Two accesses map to set 2 $\Rightarrow$ conflict, one must be evicted
  \item \textbf{2-way S.A.}: Two accesses map to set 2, but each set has 2 ways $\Rightarrow$ both fit
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item More correlative to performance than miss rate
    \item Accounts for memory access frequency
  \end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item[\footnotesize$\rightarrow$] \footnotesize Prefetch
  \end{itemize}
  \item \textbf{Capacity}: working set $>$ cache size
  \begin{itemize}
    \item[\footnotesize$\rightarrow$] \footnotesize Larger cache, stream buffers
  \end{itemize}
  \item \textbf{Conflict}: set mapping causes evictions
  \begin{itemize}
    \item[\footnotesize$\rightarrow$] \footnotesize Higher associativity, victim cache
  \end{itemize}
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
% Define colors matching reference image (6 colors)
\definecolor{conflictgreen}{RGB}{0,160,0}
\definecolor{conflict2blue}{RGB}{0,180,220}
\definecolor{conflict3magenta}{RGB}{220,0,180}
\definecolor{conflict4yellow}{RGB}{255,255,0}
\definecolor{capacityyellow}{RGB}{255,200,0}
\definecolor{compulsoryred}{RGB}{220,0,0}

% Data: curves from highest (1-way) to lowest (8-way)
\addplot[name path=oneway, draw=black, thick] coordinates {
    (16,0.135) (32,0.105) (64,0.078) (128,0.055) (256,0.038) (512,0.026) (1024,0.018) (2048,0.012)
};
\addplot[name path=twoway, draw=black, thick] coordinates {
    (16,0.105) (32,0.080) (64,0.058) (128,0.042) (256,0.030) (512,0.022) (1024,0.016) (2048,0.011)
};
\addplot[name path=fourway, draw=black, thick] coordinates {
    (16,0.082) (32,0.062) (64,0.046) (128,0.034) (256,0.025) (512,0.019) (1024,0.014) (2048,0.010)
};
\addplot[name path=eightway, draw=black, thick] coordinates {
    (16,0.068) (32,0.052) (64,0.040) (128,0.030) (256,0.022) (512,0.017) (1024,0.013) (2048,0.0095)
};
% Fully associative (capacity + compulsory only, no conflict)
\addplot[name path=fullassoc, draw=black] coordinates {
    (16,0.058) (32,0.044) (64,0.034) (128,0.025) (256,0.018) (512,0.014) (1024,0.011) (2048,0.008)
};
% Compulsory baseline (constant)
\addplot[name path=compulsory, draw=black] coordinates {
    (16,0.005) (2048,0.005)
};
\addplot[name path=bottom, draw=none] coordinates {
    (16,0) (2048,0)
};

% Fill areas (bottom to top)
\addplot[compulsoryred] fill between[of=bottom and compulsory];
\addplot[capacityyellow] fill between[of=compulsory and fullassoc];
\addplot[conflict4yellow] fill between[of=fullassoc and eightway];
\addplot[conflict3magenta] fill between[of=eightway and fourway];
\addplot[conflict2blue] fill between[of=fourway and twoway];
\addplot[conflictgreen] fill between[of=twoway and oneway];

% Label style with tight inner sep
\tikzset{lbl/.style={font=\scriptsize, inner sep=1pt}}
\tikzset{misslbl/.style={font=\small\bfseries, inner sep=1pt}}

% Associativity labels in white area, arrows pointing left to curves
\node[lbl, anchor=west] (lbl1) at (axis cs:32,0.128) {1-way};
\draw[->, thick] (lbl1.west) -- (axis cs:18,0.12);
\node[lbl, anchor=west] (lbl2) at (axis cs:64,0.092) {2-way};
\draw[->, thick] (lbl2.west) -- (axis cs:26,0.08);
\node[lbl, anchor=west] (lbl4) at (axis cs:120,0.07) {4-way};
\draw[->, thick] (lbl4.west) -- (axis cs:34,0.058);
\node[lbl, anchor=west] (lbl8) at (axis cs:160,0.058) {8-way};
\draw[->, thick] (lbl8.west) -- (axis cs:65,0.038);

% Miss type labels (Conflict in upper right, no arrow)
\node[misslbl, rotate=-32] at (axis cs:350,0.095) {Conflict};
\node[misslbl, anchor=west] (caplab) at (axis cs:380,0.06) {Capacity};
\draw[->, thick] (caplab.west) -- (axis cs:180,0.015);
\node[misslbl] (complab) at (axis cs:800,0.04) {Compulsory};
\draw[->, thick] (complab.south) -- (axis cs:650,0.003);
\end{axis}
\end{tikzpicture}
\end{column}
\end{columns}
...
\definecolor{invalidcolor}{RGB}{180, 180, 180}
\definecolor{sharedcolor}{RGB}{100, 180, 100}
\definecolor{exclusivecolor}{RGB}{100, 150, 220}
\definecolor{modifiedcolor}{RGB}{220, 100, 100}

\begin{frame}{MESI Protocol: States}
\begin{itemize}
  \item \textbf{Coherent if:} (1) a read returns the last written value; (2) all processors see stores to the same address in the same order
\end{itemize}
\vspace{0.2cm}
\begin{center}
\begin{tabular}{lccc}
...
\begin{columns}[T]
\begin{column}{0.52\textwidth}
\textbf{A memory system is \emph{coherent} if:}
\begin{itemize}
  \item If P1 writes to address X,\\
...
% Callout on left
\begin{tikzpicture}
  \node[rectangle callout, callout absolute pointer={(3.5, 0.3)}, draw, fill=yellow!20,
        font=\small, align=left, text width=4.5cm]
...
\begin{tikzpicture}[scale=0.7, every node/.style={font=\scriptsize}]
  % L2 Cache
  \node[draw, thick, fill=brown!20, minimum width=4.5cm, minimum height=0.9cm, align=center] (l2) at (1.5,-2.5) {L2 cache (shared)};

  % Processor 1
  \node[draw, thick, fill=green!20, minimum width=2.2cm, minimum height=1.8cm, align=center, anchor=south west] (p1) at ([yshift=15]l2.north west) {};
  \node[above] at (p1.north) {\textbf{Processor 1}};
  \node[draw, thick, fill=lightgray!20, minimum width=1.9cm, minimum height=0.9cm, align=center] (l1p1) at (p1.center) {L1 cache};
  \node[draw, thick, fill=red!20, rounded corners=2pt, font=\scriptsize] at ([yshift=-2mm]l1p1.south) {Write X};

  % Processor 2
  \node[draw, thick, fill=green!20, minimum width=2.2cm, minimum height=1.8cm, align=center, anchor=south east] (p2) at ([yshift=15]l2.north east) {};
  \node[above] at (p2.north) {\textbf{Processor 2}};
  \node[draw, thick, fill=lightgray!20, minimum width=1.9cm, minimum height=0.9cm, align=center] (l1p2) at (p2.center) {L1 cache};
  \node[draw, thick, fill=blue!20, rounded corners=2pt, font=\scriptsize] at ([yshift=-2mm]l1p2.south) {Read X};

  % Memory
  \node[draw, thick, fill=blue!20, minimum width=3.8cm, minimum height=0.8cm, align=center, anchor=north] (mem) at ([yshift=-15]l2.south) {Memory};

  % Connections
  \draw[<->, thick] (p1.south) -- (p1.south |- l2.north);
  \draw[<->, thick] (p2.south) -- (p2.south |- l2.north);
  \draw[<->, thick] (l2.south) -- (mem.north);
\end{tikzpicture}
\end{column}
\end{columns}
...
\begin{frame}{Multi-processor System: Write Serialization}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{A memory system is \emph{coherent} if:}
\begin{itemize}
  \item If P1 writes to address X, and later on P2 reads X,
        and there are no other writes to X in between
        $\Rightarrow$ P2's read returns the value written by P1's write
  \vspace{0.2cm}
  \item Writes to the \underline{same} location are serialized:
...
\begin{tikzpicture}[scale=0.7, every node/.style={font=\scriptsize}]
  ...
  % Processor 1
  \node[draw, thick, fill=green!20, minimum width=2.2cm, minimum height=1.9cm, align=center, anchor=south west] (p1) at ([yshift=15]l2.north west) {};
  \node[above] at (p1.north) {\textbf{Processor 1}};
  \node[draw, thick, fill=lightgray!20, minimum width=1.9cm, minimum height=1.2cm, text depth=0.55cm] (l1p1) at (p1.center) {L1 cache};
  \node[draw, fill=white, font=\scriptsize\ttfamily, anchor=south] (x1) at ([yshift=1mm]l1p1.south) {X=0};

  % Processor 2
  \node[draw, thick, fill=green!20, minimum width=2.2cm, minimum height=1.9cm, align=center, anchor=south east] (p2) at ([yshift=15]l2.north east) {};
  \node[above] at (p2.north) {\textbf{Processor 2}};
  \node[draw, thick, fill=lightgray!20, minimum width=1.9cm, minimum height=1.2cm, text depth=0.55cm] (l1p2) at (p2.center) {L1 cache};
  \node[draw, fill=white, font=\scriptsize\ttfamily, anchor=south] (x2) at ([yshift=1mm]l1p2.south) {X=0};
  ...
  % Callouts pointing to X values
  \node[rectangle callout, callout absolute pointer={(x1.west)}, draw, fill=yellow!20,
        font=\scriptsize, align=center, text width=1.4cm, anchor=east]
        at (p1.west) {P1 \textcolor{red!70!black}{writes}\\X$\leftarrow$0};
  \node[rectangle callout, callout absolute pointer={(x2.east)}, draw, fill=yellow!20,
        font=\scriptsize, align=center, text width=1.4cm, anchor=west]
        at ([yshift=-1cm]p2.east) {P2 \textcolor{blue!70!black}{sees}\\X=0};
\end{tikzpicture}%
}%
% Slide 2: X=1
\only<2>{%
\begin{tikzpicture}[scale=0.7, every node/.style={font=\scriptsize}]
  ...
  \node[above] at (p1.north) {\textbf{Processor 1}};
  \node[draw, thick, fill=lightgray!20, minimum width=1.9cm, minimum height=1.2cm, text depth=0.55cm] (l1p1) at (p1.center) {L1 cache};
  \node[draw, fill=white, font=\scriptsize\ttfamily, anchor=south] (x1) at ([yshift=1mm]l1p1.south) {X=1};

  % Processor 2
  \node[draw, thick, fill=green!20, minimum width=2.2cm, minimum height=1.9cm, align=center, anchor=south east] (p2) at ([yshift=15]l2.north east) {};
  \node[above] at (p2.north) {\textbf{Processor 2}};
  \node[draw, thick, fill=lightgray!20, minimum width=1.9cm, minimum height=1.2cm, text depth=0.55cm] (l1p2) at (p2.center) {L1 cache};
  \node[draw, fill=white, font=\scriptsize\ttfamily, anchor=south] (x2) at ([yshift=1mm]l1p2.south) {X=1};
  ...
  \node[rectangle callout, callout absolute pointer={(x1.west)}, draw, fill=yellow!20,
        font=\scriptsize, align=center, text width=1.4cm, anchor=east]
        at ([yshift=-1cm]p1.west) {P1 \textcolor{blue!70!black}{sees}\\X=1};
  \node[rectangle callout, callout absolute pointer={(x2.east)}, draw, fill=yellow!20,
        font=\scriptsize, align=center, text width=1.4cm, anchor=west]
        at (p2.east) {P2 \textcolor{red!70!black}{writes}\\X$\leftarrow$1};
\end{tikzpicture}%
}%
\end{column}
\end{columns}
...
\begin{center}
\begin{tikzpicture}
  \node[draw, fill=red!15, rounded corners, font=\small, text width=14cm, align=center] {
    \textbf{Coherence guarantee:} All processors see writes in the same order.\\[2pt]
    P1 saw X: 0$\rightarrow$1. ~~P2 seeing 1$\rightarrow$0 would violate coherence.
  };
\end{tikzpicture}
\end{center}
}
...
\newcommand{\mpexample}[8]{% {p1-cache}{p1-state}{p2-cache}{p2-state}{l2-content}{l2-bits}{memory}{description}
  \centering
  \begin{tikzpicture}[scale=0.7, every node/.style={font=\scriptsize}]
    % L2 Cache with white rectangle for content
    \node[draw, thick, fill=brown!20, minimum width=4.5cm, minimum height=1.2cm, text depth=0.5cm] (l2) at (1.5,-2.5) {L2 cache (shared)};
    \ifx&#5&\else
      \node[draw, fill=white, font=\scriptsize\ttfamily, anchor=south] at ([yshift=1mm]l2.south) {#5};
    \fi
    ...
    % L1 Caches with white rectangles for content
    \node[draw, thick, fill=lightgray!20, minimum width=1.9cm, minimum height=1.1cm, text depth=0.45cm] (l1p1) at (p1.center) {L1 cache};
    \ifx&#1&\else
      \node[draw, fill=white, font=\scriptsize\ttfamily, anchor=south] at ([yshift=1mm]l1p1.south) {#1};
    \fi
    \node[draw, thick, fill=lightgray!20, minimum width=1.9cm, minimum height=1.1cm, text depth=0.45cm] (l1p2) at (p2.center) {L1 cache};
    \ifx&#3&\else
      \node[draw, fill=white, font=\scriptsize\ttfamily, anchor=south] at ([yshift=1mm]l1p2.south) {#3};
    \fi
    ...
    % Memory with white rectangle for content
    \node[draw, thick, fill=blue!20, minimum width=3.8cm, minimum height=1.1cm, text depth=0.45cm, anchor=north] (mem) at ([yshift=-20]l2.south) {Memory};
    \ifx&#7&\else
      \node[draw, fill=white, font=\scriptsize\ttfamily, anchor=south] at ([yshift=1mm]mem.south) {#7};
    \fi
...
\begin{frame}{Cache Coherence Summary}
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=0.8, every node/.style={font=\small}]
      ...
      \draw[->, thick] (I) -- node[left] {\tiny Read} (E);
      \draw[->, thick] (I) -- node[above] {\tiny Read} (S);
      \draw[->, thick] (E) -- node[below] {\tiny Write} (M);
      \draw[->, thick] ([xshift=-4mm]S) -- node[left] {\tiny Write} ([xshift=-4mm]M);
      \draw[->, thick, bend left=20] ([xshift=4mm]M) -- node[right] {\tiny Snoop} ([xshift=4mm]S);
...
\begin{frame}{x86 Ordering: Loads and Stores Keep Their Own Order}
Loads are seen in program order, and stores are seen in program order.
...
\begin{tikzpicture}
  % Processor 0
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.1cm, row sep=0.2cm,
    column 1/.style={font=\scriptsize, gray, anchor=east},
    column 2/.style={font=\ttfamily\footnotesize, anchor=west}
  ] (p0) at (0, 0) {
    |[font=\bfseries\footnotesize]| \phantom{S1} \& |[font=\bfseries\footnotesize]| Processor 0 \\
    S1 \& |[alias=s1]| Store [\textcolor{varX}{X}] <- 1 \\
    \rule{0pt}{0.4cm} \& \\
    S2 \& |[alias=s2]| Store [\textcolor{varY}{Y}] <- 1 \\
  };
  ...
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.1cm, row sep=0.2cm,
    ...
    |[font=\bfseries\footnotesize]| \phantom{L1} \& |[font=\bfseries\footnotesize]| Processor 1 \\
    |[alias=l1lbl]| L1 \& |[alias=l1]| r1 <- Load [\textcolor{varY}{Y}] \\
    \rule{0pt}{0.4cm} \& \\
    L2 \& |[alias=l2]| r2 <- Load [\textcolor{varX}{X}] \\
  };

  % Arrow 1: S1 -> S2 (program order)
  \draw[hbprogorder] (s1.south) -- (s2.north)
    node[hblabel, midway, left=3pt, draw, thin] {1};

  % Arrow 2: L1 -> L2 (program order)
  \draw[hbprogorder] (l1.south) -- (l2.north)
    node[hblabel, midway, left=3pt, draw, thin] {2};

  % Arrow 3: S2 -> L1 (cross-processor, Y variable)
  \draw[hbcross, varY] (s2.east) to[out=15, in=165]
    node[hblabel, above=1pt, pos=0.5, draw, thin] {3} (l1lbl.west);
\end{tikzpicture}
\end{center}

\vspace{-0.1cm}
{\footnotesize\textbf{Happens-before analysis:}}
\begin{enumerate}
  \footnotesize
  \item S1 $\rightarrow$ S2 --- program order (stores)
  \item L1 $\rightarrow$ L2 --- program order (loads)
  \item S2 $\rightarrow$ L1 --- L1 reads Y=1 from S2
  \item S1 $\rightarrow$ S2 $\rightarrow$ L1 $\rightarrow$ L2 $\Rightarrow$ L2 sees X=1, so r2=1
\end{enumerate}
\end{frame}

\begin{frame}{x86 Ordering: Stores Not Reordered with Older Loads}
...
\begin{tikzpicture}
  ...
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.1cm, row sep=0.2cm,
    ...
    |[font=\bfseries\footnotesize]| \phantom{L1} \& |[font=\bfseries\footnotesize]| Processor 0 \\
    L1 \& |[alias=l1]| r1 <- Load [\textcolor{varX}{X}] \\
    \rule{0pt}{0.4cm} \& \\
    S1 \& |[alias=s1]| Store [\textcolor{varY}{Y}] <- 1 \\
  };
  ...
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.1cm, row sep=0.2cm,
    ...
    |[font=\bfseries\footnotesize]| \phantom{L2} \& |[font=\bfseries\footnotesize]| Processor 1 \\
    |[alias=l2lbl]| L2 \& |[alias=l2]| r2 <- Load [\textcolor{varY}{Y}] \\
    \rule{0pt}{0.4cm} \& \\
    |[alias=s2lbl]| S2 \& |[alias=s2]| Store [\textcolor{varX}{X}] <- 1 \\
  };

  % Arrow 1: L1 -> S1 (program order)
  \draw[hbprogorder] (l1.south) -- (s1.north)
    node[hblabel, midway, left=3pt, draw, thin] {1};

  % Arrow 2: L2 -> S2 (program order)
  \draw[hbprogorder] (l2.south) -- (s2.north)
    node[hblabel, midway, left=3pt, draw, thin] {2};

  % Arrow 3: S2 -> L1 (cross-processor, X variable)
  \draw[hbcross, varX] (s2lbl.west) to[out=170, in=-10]
    node[hblabel, below=1pt, pos=0.5, draw, thin] {3} (l1.east);

  % Arrow 4: S1 -> L2 (cross-processor, Y variable)
  \draw[hbcross, varY] (s1.east) to[out=10, in=190]
    node[hblabel, above=1pt, pos=0.5, draw, thin] {4} (l2lbl.west);
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
  \footnotesize
  \item L1 $\rightarrow$ S1 --- program order (store after load)
  \item L2 $\rightarrow$ S2 --- program order (store after load)
  \item S2 $\rightarrow$ L1 --- L1 reads X=1 from S2 (if r1=1)
  \item S1 $\rightarrow$ L2 --- L2 reads Y=1 from S1 (if r2=1)
  \item Cycle: S2 $\rightarrow$ L1 $\rightarrow$ S1 $\rightarrow$ L2 $\rightarrow$ S2 --- happens-before cannot cycle!
\end{enumerate}
\end{frame}

\begin{frame}{x86 Ordering: Loads May Reorder with \emph{Different} Older Stores}
Loads may reorder with older stores to \emph{different} locations (but not the same).
...
\begin{tikzpicture}
  ...
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.1cm, row sep=0.2cm,
    ...
    S1 \& |[alias=s1]| Store [\textcolor{varX}{X}] <- 1 \\
    \rule{0pt}{0.4cm} \& \\
    L1 \& |[alias=l1]| r1 <- Load [\textcolor{varY}{Y}] \\
  };
  ...
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.1cm, row sep=0.2cm,
    ...
    |[font=\bfseries\footnotesize]| \phantom{S2} \& |[font=\bfseries\footnotesize]| Processor 1 \\
    S2 \& |[alias=s2]| Store [\textcolor{varY}{Y}] <- 1 \\
    \rule{0pt}{0.4cm} \& \\
    L2 \& |[alias=l2]| r2 <- Load [\textcolor{varX}{X}] \\
  };

  % NO program order arrows - different locations means no ordering
  \draw[gray!50, dashed, thick] (s1.south) -- (l1.north)
    node[midway, left=3pt, font=\scriptsize, gray] {indep.};

  \draw[gray!50, dashed, thick] (s2.south) -- (l2.north)
    node[midway, left=3pt, font=\scriptsize, gray] {indep.};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
  \footnotesize
  \item S1 accesses \textcolor{varX}{X}, L1 accesses \textcolor{varY}{Y} --- different locations, no ordering!
  \item S2 accesses \textcolor{varY}{Y}, L2 accesses \textcolor{varX}{X} --- different locations, no ordering!
  \item Without ordering, loads may execute before stores
  \item Possible: L1, L2 execute first (see 0), then S1, S2 --- allowed
\end{enumerate}
\end{frame}

\begin{frame}{x86 Ordering: Causality and Transitive Visibility}
Causality: if a processor observes an effect, all ``downstream'' effects must also be visible.
...
\begin{tikzpicture}
  ...
  % Processor 1 (2 instructions)
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.1cm, row sep=0.2cm,
    column 1/.style={font=\scriptsize, gray, anchor=east},
    column 2/.style={font=\ttfamily\footnotesize, anchor=west}
  ] (p1) at (5, 0) {
    |[font=\bfseries\footnotesize]| \phantom{L1} \& |[font=\bfseries\footnotesize]| Processor 1 \\
    |[alias=l1lbl]| L1 \& |[alias=l1]| r1 <- Load [\textcolor{varX}{X}] \\
    \rule{0pt}{0.4cm} \& \\
    S2 \& |[alias=s2]| Store [\textcolor{varY}{Y}] <- 1 \\
  };

  % Processor 2 (2 instructions)
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.1cm, row sep=0.2cm,
    column 1/.style={font=\scriptsize, gray, anchor=east},
    column 2/.style={font=\ttfamily\footnotesize, anchor=west}
  ] (p2) at (10, 0) {
    |[font=\bfseries\footnotesize]| \phantom{L2} \& |[font=\bfseries\footnotesize]| Processor 2 \\
    |[alias=l2lbl]| L2 \& |[alias=l2]| r2 <- Load [\textcolor{varY}{Y}] \\
    \rule{0pt}{0.4cm} \& \\
    L3 \& |[alias=l3]| r3 <- Load [\textcolor{varX}{X}] \\
  };

  % Arrow 1: S1 -> L1 (cross-processor, X variable)
  \draw[hbcross, varX] (s1.east) to[out=0, in=180]
    node[hblabel, above=0pt, pos=0.5, draw, thin] {1} (l1lbl.west);

  % Arrow 2: L1 -> S2 (program order on P1)
  \draw[hbprogorder] (l1.south) -- (s2.north)
    node[hblabel, midway, left=3pt, draw, thin] {2};

  % Arrow 3: S2 -> L2 (cross-processor, Y variable)
  \draw[hbcross, varY] (s2.east) to[out=0, in=180]
    node[hblabel, above=2pt, pos=0.5, draw, thin] {3} (l2lbl.west);

  % Arrow 4: L2 -> L3 (program order on P2)
  \draw[hbprogorder] (l2.south) -- (l3.north)
    node[hblabel, midway, left=3pt, draw, thin] {4};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
  \footnotesize
  \item S1 $\rightarrow$ L1 --- L1 reads X=1 from S1 (r1=1)
  \item L1 $\rightarrow$ S2 --- program order on P1
  \item S2 $\rightarrow$ L2 --- L2 reads Y=1 from S2 (r2=1)
  \item L2 $\rightarrow$ L3 --- program order on P2 (loads not reordered)
  \item Chain: S1 $\rightarrow$ L1 $\rightarrow$ S2 $\rightarrow$ L2 $\rightarrow$ L3 $\Rightarrow$ L3 must see X=1, so r3=1
\end{enumerate}
\end{frame}
