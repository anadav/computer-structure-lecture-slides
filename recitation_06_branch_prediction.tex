...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[
      box/.style={draw, minimum height=1.2cm, minimum width=3.5cm, fill=green!20, thick},
      arraybox/.style={draw, minimum height=2cm, minimum width=1cm, fill=white, thick},
      elementbox/.style={draw, minimum height=0.4cm, minimum width=0.8cm, fill=gray!30, thick},
      arrow/.style={-latex, thick},
      data_connector/.style={circle, fill, inner sep=1.2pt}
    ]
      % Branch IP input
      \node[font=\normalfont, text=blue] (branch_ip) at (0,0) {\textbf{Branch IP}};

      % Connector for Branch IP
      \node[data_connector, right=1cm of branch_ip.east] (conn) {};

      % Bimodal or Local box
      \node[box, align=center, right=1.5cm of conn, anchor=west] (bimodal) {Bimodal\\or Local};

      % Global box
      \node[box, below=0.5cm of bimodal] (global) {Global};

      % Chooser array (vertical rectangle)
      \node[arraybox, below=0.5cm of global] (chooser) {};
      \node[above=0.05cm of chooser.north, font=\small, text=magenta] {\textbf{Chooser array}};
      \node[below=0.05cm of chooser.south, font=\tiny, text=magenta, align=center] {\textbf{(2-bit sat. counters)}};

      % Highlighted element in the array
      \node[elementbox] at (chooser.center) {};

      % MUX using circuitikz muxdemux
      \node[muxdemux, muxdemux def={Lh=4, Rh=2, NL=2, NB=1, w=2},
            external pins width=0, fill=yellow!20, anchor=lpin 2] (mux) at ([xshift=1.5cm]global.east) {};

      % Label for MUX
      \node at (mux.center) [font=\footnotesize] {MUX};

      % Prediction output
      \node[right=1.5cm of mux.rpin 1, text=blue] (prediction) {Prediction};

      % Arrows - Branch IP to all three components
      \draw[thick] (branch_ip) -- (conn);
      \draw[arrow] (conn) |- (bimodal.west);
      \node[data_connector] (conn2) at (conn |- global.west) {};
      \draw[arrow] (conn2) -- (global.west);
      \draw[arrow] (conn) |- (chooser.west);

      % Arrows - from predictors to MUX
      \draw[arrow] (bimodal.east) -- ++(0.5, 0) |- (mux.lpin 1);
      \draw[arrow] (global.east) -- ++(0.5, 0) |- (mux.lpin 2);

      % Arrow - Chooser to MUX control (bottom pin)
      \draw[arrow] (chooser.east) -| (mux.bpin 1);

      % Arrow - MUX to Prediction
      \draw[arrow] (mux.rpin 1) -- (prediction);

      % Add labels for the chooser update rules
      \node[below=0.8cm of chooser.south, yshift=-0.3cm, align=left, font=\footnotesize, text=blue] {
        \textbf{+1} if Bimodal / Local correct and Global wrong\\
        \textbf{-1} if Bimodal / Local wrong and Global correct
      };
    \end{tikzpicture}
  \end{center}
