...
% Macro for labeled state table (normal)
\newcommand{\stateTableNormal}[2]{%
  % #1: label (e.g., "State IP1")
...
% Macro for labeled state table (faded)
\newcommand{\stateTableFaded}[2]{%
  % #1: label (e.g., "State IP1")
  % #2: state data (same format as \stateTable)
  \begin{tabular}{|c|c|}
    \toprule
    \multicolumn{2}{|c|}{\cellcolor{gray!30}#1} \\
    \midrule
    \stateRow{0}{#2} \\
    \stateRow{1}{#2} \\
    \stateRow{2}{#2} \\
...
\newcommand{\renderStateTables}{%
  ...
  \else
    ...
    \expandafter\ifnum\activeip=1\relax
      \let\macroOne\stateTableNormal
      \let\macroTwo\stateTableFaded
      \let\macroThree\stateTableFaded
    \else\expandafter\ifnum\activeip=2\relax
      \let\macroOne\stateTableFaded
      \let\macroTwo\stateTableNormal
      \let\macroThree\stateTableFaded
    \else\expandafter\ifnum\activeip=3\relax
      \let\macroOne\stateTableFaded
      \let\macroTwo\stateTableFaded
      \let\macroThree\stateTableNormal
    \else
      % Default: all normal
      \let\macroOne\stateTableNormal
      \let\macroTwo\stateTableNormal
...
\localFrameKV{
    ...
    states3={},
    active={1},
    description={Step 1: Prediction at IP1}
  }
