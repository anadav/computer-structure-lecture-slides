...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[
      box/.style={draw=black!80, line width=1pt, minimum height=1.2cm, minimum width=3cm, fill=cyan!15, rounded corners=2pt},
      arrow/.style={-latex, thick, draw=black!70},
      data_connector/.style={circle, fill=black, inner sep=1.5pt}
    ]
      % Branch IP input
      \node[font=\large, text=blue!80!black] (branch_ip) at (0,0) {Branch IP};

      % Connector for Branch IP
      \node[data_connector, right=0.7cm of branch_ip.east] (conn) {};

      % Bimodal or Local box
      \node[box, align=center, right=1cm of conn, anchor=west] (bimodal) {Bimodal\\or Local};

      % Global box
      \node[box, below=0.5cm of bimodal] (global) {Global};

      % Chooser array - drawn with loop and dashed lines
      \coordinate (array_start) at ([yshift=-0.7cm]global.south);

      % Draw array box outline
      \draw[draw=black!80, line width=1pt, fill=cyan!8]
        ([xshift=-0.5cm]array_start) rectangle ++(1.0cm, -2.5cm)
        node[pos=0.5] (chooser_center) {};

      % Draw dashed lines to show array elements
      \foreach \i in {1,2,3,4,5,6} {
        \draw[dashed, black!40] ([xshift=-0.5cm, yshift=-\i*0.4cm]array_start) -- ++(1.0cm, 0);
      }

      % Highlight one element in the middle
      \draw[draw=orange!70, line width=1.5pt, fill=orange!20]
        ([xshift=-0.45cm, yshift=-1.4cm]array_start) rectangle ++(0.9cm, -0.35cm);

      % Label for chooser array - BELOW the array
      \node[below=2.7cm of array_start, font=\small, text=black, align=center]
        {Chooser Array\\[-2pt]\scriptsize (2-bit sat. counters)};

      % Store chooser position for connections
      \coordinate (chooser_west) at ([xshift=-0.5cm, yshift=-1.57cm]array_start);
      \coordinate (chooser_east) at ([xshift=0.5cm, yshift=-1.57cm]array_start);
      ...
      % Label for MUX
      \node at (mux.center) [font=\normalsize] {MUX};

      % Prediction output
      \node[right=1cm of mux.rpin 1, font=\large, text=blue!80!black] (prediction) {Prediction};
  ...
  \begin{itemize}
    ...
    \item Counter update: increment (+1) if Bimodal/Local correct and Global wrong; decrement (-1) if Bimodal/Local wrong and Global correct
