...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[
      ...
      % Data connector circle to the right of Branch IP
      \node[data_connector, right=1cm of ip.east] (connector) {};

      % History cache - draw outer box
      \node[bigbox, right=2cm of ip, minimum width=2.5cm, minimum height=2.5cm] (hist_cache) {};
      ...
      \node[font=\scriptsize] at ([xshift=-0.85cm, yshift=-0.75cm]hist_cache.north) {tag};
      \node[font=\scriptsize] at ([xshift=0.35cm, yshift=-0.75cm]hist_cache.north) {history};

      % Reference point for history output
      \coordinate (history_out) at ([xshift=0.35cm, yshift=-0.75cm]hist_cache.south);
      ...
      % Arrows
      \draw[thick] (ip) -- (connector);
      \draw[arrow] (connector) |- (hist_cache.west);
      \draw[arrow] (history_out) |- node[pos=0.3, right] {h} (xor);
      % Route from connector around history cache to XOR
      \draw[thick] (connector) -- (lower_left) -- (lower_right);
