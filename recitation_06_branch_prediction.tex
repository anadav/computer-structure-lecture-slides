...
\begin{frame}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}[
      ...
      \node[muxdemux, muxdemux def={Lh=3, Rh=3, w=3, NL=0, NR=0},
            external pins width=0, fill=cyan!20] (imem) at (0,0) {};
      ...
      \node[muxdemux, muxdemux def={Lh=3, Rh=3, w=3, NL=4, NR=0},
      ...
      \node[muxdemux, muxdemux def={Lh=7.5, NL=2, Rh=3, NR=1, NB=2, NT=1, w=3, inset w=1.5, inset Lh=3, inset Rh=0, square pins=1},
            external pins width=0, scale=0.5, fill=cyan!20, right=0.5cm of L2] (alu) {};
      ...
      \node[muxdemux, muxdemux def={Lh=6, Rh=3, NL=2, NR=1, w=1.5},
            external pins width=0, scale=0.6, fill=cyan!20, right=0.5cm of L4] (wb_mux) {};
      ...
      \coordinate (if_pos) at ($(imem.north)!0.5!(L1.north)$);
      \coordinate (wb_pos) at ($(L4.north)!0.5!(wb_mux.north)$);
      ...
      % L1 to Register File
      \draw[arrow] (L1.east) -- (regfile.west);

      % Register File to L2
      \draw[arrow] (regfile.east) -- (L2.west);

      % L2 to ALU
      \draw[arrow] (L2.east) -- ++(0.3,0) |- (alu_in1);
      \draw[arrow] (L2.east) -- ++(0.3,0) |- (alu_in2);

      % ALU to L3
      \draw[arrow] (alu_out) -- (L3.west);

      % L3 to Data Memory
      \draw[arrow] (L3.east) -- (dmem.west);

      % Data Memory to L4
      \draw[arrow] (dmem.east) -- (L4.west);

      % L4 to WB Mux
      \draw[arrow] (L4.east) -- ++(0.2,0) |- (wbmux_in1);
      \draw[arrow] (L4.east) -- ++(0.2,0) |- (wbmux_in2);

      % WB Mux output with feedback to Register File
      \draw[arrow] (wbmux_out) -- ++(0.5,0) coordinate (wb_out_point)
