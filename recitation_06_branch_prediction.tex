...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[bigbox, minimum width=2.5cm, minimum height=2.5cm, right=1.5cm of ip] (hist_cache) {};
      ...
      % Define 2nd row coordinate for arrow target
      \coordinate (counter_row2) at ([yshift=-0.75cm]counter_array.north west);

      % Prediction output - using relative positioning
      \node[right=1.5cm of counter_array, font=\normalsize, text=blue!80!black] (prediction) {prediction};

      % Arrows
      \draw[arrow] (ip) -- (tag_entry);
      \draw[arrow] ([xshift=0.7cm]history_box) -- (counter_row2);
      \draw[arrow] ([xshift=0.7cm]counter_row2) -- (prediction);
