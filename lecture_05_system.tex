...
\begin{tikzpicture}[
    >=Stealth,
    thick,
    circuit/.style={fill=green!60!black, draw=green!40!black},
    substrate/.style={fill=cyan!60, draw=cyan!80!black},
    hbm/.style={fill=red!70!brown, draw=brown!50!black},
    base/.style={fill=basecolor, draw=cyan!80!black},
    emib/.style={fill=emibcolor, draw=blue!70!black},
    compute/.style={fill=blue!60!red!40, draw=blue!80!red!60},
    bump/.style={circle, fill=yellow, draw=black, minimum size=2mm},
    ball/.style={circle, fill=gray!60, draw=black!80, minimum size=3mm},
    label/.style={font=\tiny},
    small label/.style={font=\scriptsize}
  ]

% Define dimensions
\pgfmathsetmacro{\ballRadius}{0.15}  % 1.5mm in cm
\pgfmathsetmacro{\boardHeight}{0.7}

% Package Substrate (base layer)
\node[substrate, rectangle, minimum width=10cm, minimum height=1.3cm, text=white, font=\tiny]
  (substrate) at (5.5,0.85) {Package Substrate};

% Package Balls (positioned at bottom of substrate, using calc)
\foreach \x [count=\i from 0] in {0.8,1.3,...,10.2} {
  \node[ball, anchor=north] (ball\i) at ($(substrate.south west) + (\x-0.2,0)$) {};
}

% Circuit Board (positioned below balls, relative to ball1)
\node[circuit, rectangle, minimum width=13.5cm, minimum height=\boardHeight cm, text=white, font=\tiny]
  (board) at ($(ball1.south)+(3.45,-\boardHeight/2)$) {Circuit Board};
...
\node[base, rectangle, minimum width=5.7cm, minimum height=0.8cm, text=white, font=\tiny, anchor=south, align=left]
  (basedie) at ($(substrate.north west)+(2.85,0)$) {Base Die};

% HBM Stack (4 dies, positioned relative to substrate)
\foreach \y/\name [count=\j from 0] in {0.65/Die, 1.35/Die, 2.05/Die, 2.75/Die} {
  \node[hbm, rectangle, minimum width=2.7cm, minimum height=0.6cm, text=white, font=\tiny, anchor=south, align=left]
    (hbm\j) at ($(substrate.north west)+(-0.65,\y)$) {HBM DRAM \name};
  % TSVs (yellow bumps)
  \foreach \x in {2.4,2.6,2.8,3.0,3.2} {
    \fill[bump] (\x,{\y+0.65}) rectangle +(0.08,0.4);
  }
  % µBumps at bottom of each (relative to substrate)
  \foreach \x in {2.45,2.65,2.85,3.05,3.25} {
    \fill[ball, minimum size=1.5mm] (\x,{\y+0.6}) circle (0.75mm);
  }
}
...
% PHY blocks
\node[fill=emibcolor!70, draw=blue!70!black, rectangle, minimum width=0.7cm, minimum height=0.4cm, text=white, font=\tiny]
  (phy0) at (4.05,1.8) {PHY};
\node[fill=emibcolor!70, draw=blue!70!black, rectangle, minimum width=0.7cm, minimum height=0.4cm, text=white, font=\tiny]
  (phy1) at (4.85,1.8) {PHY};
...
\node[fill=basecolor!80, draw=cyan!80!black, rectangle, minimum width=3.4cm, minimum height=0.8cm, text=white, font=\tiny, anchor=south west]
  (baselogic) at ($(basedie.south east)+(0.4,0)$) {Base Logic Die};

% F2F µBumps (gray circles above Base Logic Die, using calc)
\foreach \x [count=\i from 0] in {0.3,0.6,...,4.2} {
  \node[circle, fill=gray!60, draw=black!80, inner sep=0pt, minimum size=0.5mm, anchor=south] (f2fbump\i) at ($(baselogic.north west) + (\x,0)$) {};
  \node[circle, fill=gray!60, draw=black!80, inner sep=0pt, minimum size=0.5mm, anchor=south] at ($(baselogic.north west) + (\x,0.06)$) {};
}

% Compute Dies (positioned above F2F µBumps)
\node[compute, rectangle, minimum width=3.0cm, minimum height=0.55cm, text=white, font=\tiny, anchor=south]
  (compute1) at (f2fbump7.north) {Compute/FPGA/Mem/RF Die};
\node[compute, rectangle, minimum width=1.7cm, minimum height=0.55cm, text=white, font=\tiny, anchor=east]
  (compute0) at ([xshift=-5mm]compute1.west) {Compute/FPGA/Mem/RF Die};

% Interconnect lines
\draw[gray!40] (5.0,2.2) -- (5.0,2.45) -- (8.5,2.45) -- (8.5,2.7);
\draw[gray!40] (6.0,2.2) -- (6.0,2.55) -- (9.0,2.55) -- (9.0,2.7);

% µBumps between EMIB and dies
\foreach \x in {4.1,4.4,4.7,5.0} {
  \fill[ball, minimum size=1mm] (\x,2.05) circle (0.5mm);
}

% Annotations
%\draw[->, thick] (-1.5,0.6) -- (0.4,0.9);
\node[label, anchor=east] (packagetrace) at (-1.5,0.6) {\shortstack{Standard\\Package Trace}};

%\draw[->, thick] (-1.2,1.3) -- (0.5,1.7);
\node[label, anchor=east] (bumps) at (-1.3,1.3) {Bumps};

%\draw[->, thick] ([xshift=-5mm]ball0) -- ([xshift=-\ballRadius cm]ball0);
\node[label, anchor=east, align=center] (packageballs) at ([xshift=-1.5cm]ball0) {Package\\Balls};

%\draw[->, thick] (packageballs.east) -- (ball0.west);
\node[label] (tsv1) at (2.7,4.7) {TSV};

%\draw[->, thick] (4.3,4.3) -- (3.3,3.7);
\node[label] (ubumps) at (4.5,4.5) {µBumps};

%\draw[->, thick] (5.0,3.8) -- (5.1,2.9);
\node[label] (emibannot) at (5.0,4.0) {EMIB};
...
\node[label] (tsv2) at (10.7,2.0) {TSV};
