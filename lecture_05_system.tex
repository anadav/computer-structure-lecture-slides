...
\begin{center}
\begin{tikzpicture}[scale=0.8,
    numanode/.style={draw, thick, minimum width=2.6cm, minimum height=2.8cm, rounded corners=2pt},
    coreblue/.style={draw, fill=blue!30, minimum width=0.35cm, minimum height=0.25cm},
    coregreen/.style={draw, fill=green!30, minimum width=0.35cm, minimum height=0.25cm},
    ramblock/.style={draw, fill=gray!30, minimum width=2.2cm, minimum height=0.8cm}
]
% NUMA Node 0
\node[numanode, fill=blue!10] (node0) at (0,0) {};
\node[anchor=north, font=\small\bfseries] at (node0.north) {NUMA Node 0};

% Cores for Node 0
\foreach \i in {-0.7,-0.35,0,0.35} {
    \node[coreblue] at (\i, 0.5) {};
    \node[coreblue] at (\i, 0.15) {};
}
\node[font=\tiny] at (0, -0.1) {8 cores};

% RAM for Node 0
\node[ramblock] (ram0) at (0, -0.85) {};
\node[font=\scriptsize] at (ram0.center) {32GB RAM};

% NUMA Node 1
\node[numanode, fill=green!10, right=0.8cm of node0] (node1) {};
\node[anchor=north, font=\small\bfseries] at (node1.north) {NUMA Node 1};

% Cores for Node 1
\foreach \i in {-0.7,-0.35,0,0.35} {
    \node[coregreen] at ([xshift=\i cm]node1.center |- 0,0.5) {};
    \node[coregreen] at ([xshift=\i cm]node1.center |- 0,0.15) {};
}
\node[font=\tiny] at (node1.center |- 0,-0.1) {8 cores};

% RAM for Node 1
\node[ramblock] (ram1) at (node1.center |- 0,-0.85) {};
\node[font=\scriptsize] at (ram1.center) {32GB RAM};

% Interconnect
\draw[<->, ultra thick, red] (node0.east) -- node[above, font=\small] {QPI/UPI} node[below, font=\tiny] {25.6 GB/s} (node1.west);

% Distance matrix label - positioned below nodes
\node[font=\small\bfseries, anchor=north] at ([yshift=-0.3cm]$(node0.south)!0.5!(node1.south)$) {NUMA Distance Matrix:};
\end{tikzpicture}

\vspace{-0.2cm}
\begin{tabular}{r|cc}
\scriptsize node & \scriptsize 0 & \scriptsize 1 \\
\hline
\scriptsize 0 & \scriptsize \textbf{10} & \scriptsize 21 \\
\scriptsize 1 & \scriptsize 21 & \scriptsize \textbf{10} \\
\end{tabular}
\end{center}
\end{columns}
\end{frame}
...
\node at (0, 1.8) {\textbf{Ring (Intel Sandy Bridge+)}};
\foreach \i in {0,1,2,3} {
    \node[cpu, fill=blue!30] (c\i) at ({90-\i*90}:1) {C\i};
}
\foreach \i in {0,1,2,3} {
    \pgfmathtruncatemacro{\j}{mod(\i+1,4)}
    \draw[->, thick, blue!70] (c\i) to[bend left=20] (c\j);
}
\node[mem] at (0,-2) {\tiny Mem};
\draw[<->, thick] (c2) -- (0,-1.6);
\node[text width=3cm, align=center] at (0,-2.9) {\tiny Bidirectional ring\\ Low latency\\ Limited scalability};
\end{scope}

% Mesh (Intel)
\begin{scope}[shift={(5,0)}]
\node at (0, 1.8) {\textbf{Mesh (Intel Skylake-SP)}};
\foreach \x in {0,1,2} {
    \foreach \y in {0,1} {
        \node[cpu, fill=green!30] (m\x\y) at (\x*0.7-0.7,\y*0.7-0.35) {};
    }
}
% Horizontal connections
\draw[<->, thick, green!60!black] (m00) -- (m10);
\draw[<->, thick, green!60!black] (m10) -- (m20);
\draw[<->, thick, green!60!black] (m01) -- (m11);
\draw[<->, thick, green!60!black] (m11) -- (m21);
% Vertical connections
\foreach \x in {0,1,2} {
    \draw[<->, thick, green!60!black] (m\x0) -- (m\x1);
}
\node[mem] at (0,-2) {\tiny Mem};
\draw[<->, thick] (m10) -- (0,-1.6);
\node[text width=3cm, align=center] at (0,-2.9) {\tiny 2D grid\\ Good scalability\\ Higher latency};
\end{scope}

% Crossbar (AMD)
\begin{scope}[shift={(10,0)}]
\node at (0, 1.8) {\textbf{Crossbar (AMD Zen)}};
\foreach \i in {0,1,2,3} {
    \node[cpu, fill=orange!30] (x\i) at ({45+\i*90}:1) {};
}
% Full crossbar connections
\foreach \i in {0,1,2} {
    \foreach \j in {1,2,3} {
        \ifnum\i<\j
            \draw[-, thick, orange!70] (x\i) -- (x\j);
        \fi
    }
}
\node[mem] at (0,-2) {\tiny IF};
\draw[<->, thick] (x2) -- (0,-1.6);
\node[text width=3cm, align=center] at (0,-2.9) {\tiny All-to-all within CCX\\ Lowest latency\\ Limited to small clusters};
\end{scope}
