...
% Package Balls (positioned first to use as reference)
\pgfmathsetmacro{\ballY}{0.2}

% Circuit Board (positioned relative to balls)
\fill[circuit] (-2,\ballY-0.7) rectangle node[midway, white, font=\tiny] {Circuit Board} (11.5,\ballY);
\foreach \x in {0.8,1.3,...,10.2} {
  \fill[ball, anchor=south] (\x,\ballY) circle (1.5mm);
}

% Package Substrate
\fill[substrate] (0.5,\ballY) rectangle node[midway, white, font=\tiny] {Package Substrate} (10.5,1.5);
...
% HBM Stack (4 dies)
\foreach \y/\name in {1.5/Die, 2.2/Die, 2.9/Die, 3.6/Die} {
  \fill[hbm] (0.8,\y) rectangle node[midway, white, font=\tiny] {HBM DRAM \name} (3.5,\y+0.6);
  % TSVs (yellow bumps)
...
\fill[base] (0.8,1.5) rectangle node[midway, white, font=\tiny] {Base Die} (6.5,2.3);

% EMIB section
\fill[emib] (3.8,1.5) rectangle node[midway, white, font=\tiny] {EMIB} (6.3,2.05);

% PHY blocks
\fill[emibcolor!70] (3.7,1.6) rectangle node[midway, white, font=\tiny] {PHY} (4.4,2.0);
\fill[emibcolor!70] (4.5,1.6) rectangle node[midway, white, font=\tiny] {PHY} (5.2,2.0);
...
% Base Logic Die
\fill[basecolor!80] (6.6,1.5) rectangle node[midway, white, font=\tiny] {Base Logic Die} (10.0,2.3);

% Compute Dies on right
\fill[compute] (4.8,2.3) rectangle node[midway, white, font=\tiny] {Compute/FPGA/Mem/RF Die} (6.5,2.85);
\fill[compute] (6.8,2.3) rectangle node[midway, white, font=\tiny] {Compute/FPGA/Mem/RF Die} (9.8,2.85);
...
\draw[->, thick] (-0.5,-0.3) -- (0.5,-0.1);
\node[label, anchor=east, align=center] at (-0.5,-0.5) {Package\\Balls};
