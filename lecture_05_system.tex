...
\node[base, minimum width=5.7cm, minimum height=0.8cm, anchor=south, text width=5.5cm]
  (basedie) at ($(pbump0.north)+(2.85,0)$) {Base Die};

% Base Logic Die (positioned relative to Base Die, to its right)
\node[base, fill=basecolor!80, minimum width=3.4cm, minimum height=0.8cm, anchor=west]
...
\foreach \y [count=\j from 0] in {0, 0.7, 1.4, 2.1} {
  ...
  \foreach \i in {0,1,2,3,4} {
    \node[ball, minimum size=1.5mm, anchor=north] (ubump\j-\i) at (tsv\j-\i.south |- hbm\j.south) {};
...
\node[compute, minimum width=1.7cm, minimum height=0.55cm, anchor=east]
