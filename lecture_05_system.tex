...
% Define positions
\pgfmathsetmacro{\substrateY}{0.2}
...
\node[substrate, rectangle, minimum width=10cm, minimum height=1.3cm, text=white, font=\tiny]
  (substrate) at (5.5,{\substrateY+0.65}) {Package Substrate};

% Package Balls (at bottom of substrate)
\foreach \x [count=\i from 0] in {0.8,1.3,...,10.2} {
  \node[ball] (ball\i) at (\x,\substrateY) {};
}

% Circuit Board (positioned below balls)
\node[circuit, rectangle, minimum width=13.5cm, minimum height=\boardHeight cm, text=white, font=\tiny]
  (board) at (4.75,{\substrateY-\ballRadius-\boardHeight/2}) {Circuit Board};
...
% HBM Stack (4 dies)
\foreach \y/\name [count=\j from 0] in {1.5/Die, 2.2/Die, 2.9/Die, 3.6/Die} {
  \node[hbm, rectangle, minimum width=2.7cm, minimum height=0.6cm, text=white, font=\tiny]
    (hbm\j) at (2.15,{\y+0.3}) {HBM DRAM \name};
  % TSVs (yellow bumps)
  \foreach \x in {2.4,2.6,2.8,3.0,3.2} {
    \fill[bump] (\x,\y) rectangle +(0.08,0.4);
  }
  % µBumps at bottom of each
  \foreach \x in {2.45,2.65,2.85,3.05,3.25} {
    \fill[ball, minimum size=1.5mm] (\x,\y-0.05) circle (0.75mm);
  }
}

% Base Die (wide section)
\node[base, rectangle, minimum width=5.7cm, minimum height=0.8cm, text=white, font=\tiny]
  (basedie) at (3.65,1.9) {Base Die};
...
% Base Logic Die
\node[fill=basecolor!80, draw=cyan!80!black, rectangle, minimum width=3.4cm, minimum height=0.8cm, text=white, font=\tiny]
  (baselogic) at (8.3,1.9) {Base Logic Die};

% Compute Dies on right
\node[compute, rectangle, minimum width=1.7cm, minimum height=0.55cm, text=white, font=\tiny]
  (compute0) at (5.65,2.575) {Compute/FPGA/Mem/RF Die};
\node[compute, rectangle, minimum width=3.0cm, minimum height=0.55cm, text=white, font=\tiny]
  (compute1) at (8.3,2.575) {Compute/FPGA/Mem/RF Die};

% F2F µBumps between dies and base
\foreach \x in {5.0,5.3,...,9.5} {
  \fill[bump, minimum size=1mm] (\x,2.28) circle (0.5mm);
  \fill[bump, minimum size=1mm] (\x,2.22) circle (0.5mm);
}
...
\node[label, anchor=east] at (-1.5,0.6) {\shortstack{Standard\\Package Trace}};

\draw[->, thick] (-1.2,1.3) -- (0.5,1.7);
\node[label, anchor=east] at (-1.3,1.3) {Bumps};

\draw[->, thick] ([xshift=-5mm]ball0) -- ([xshift=-\ballRadius cm]ball0);
\node[label, anchor=east, align=center] at ([xshift=-5mm]ball0) {Package\\Balls};

\draw[->, thick] (2.7,4.5) -- (2.8,3.7);
\node[label] at (2.7,4.7) {TSV};

\draw[->, thick] (4.3,4.3) -- (3.3,3.7);
\node[label] at (4.5,4.5) {µBumps};

\draw[->, thick] (5.0,3.8) -- (5.1,2.9);
\node[label] at (5.0,4.0) {EMIB};

\draw[->, thick] (3.0,5.2) -- (4.2,2.75);
\node[label, anchor=south] at (2.8,5.3) {Short Wires};

\draw[->, thick] (9.5,3.8) -- (8.0,2.9);
\node[label] at (10,3.8) {\shortstack{F2F\\µBumps}};

\draw[->, thick] (10.5,2.0) -- (9.5,2.45);
\node[label] at (10.7,2.0) {TSV};
