...
\begin{tikzpicture}[scale=0.8]
% Cross-section view
\node at (0,2.85) {\textbf{Package Cross-Section}};

% IHS
\draw[fill=gray!50] (-2,2) rectangle coordinate[pos=0] (ihs-bl) coordinate[pos=1] (ihs-tr) (2,2.6);
\node at (0,2.3) {\small IHS (Heat Spreader)};
...
\draw[ultra thick] (-3,-0.5) rectangle coordinate[pos=1] (socket-tr) (3,-0.2);
...
\coordinate (arrow-x) at ([xshift=7mm]ihs-tr);
\draw[<->, thick] (arrow-x |- ihs-tr) -- (arrow-x |- socket-tr) node[midway, right] {\small Package};
\end{tikzpicture}
\end{center}
