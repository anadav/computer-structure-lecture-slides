...
% Mesh line positions at tile edges (where routers are)
\def\tileHalfWidth{0.55}
\def\cacheCoreBoundary{-0.225}
\pgfmathsetmacro{\topRouterY}{\rowI - 0.35}

% Vertical mesh lines - from top row router to bottom row router
% Left 3 columns (A, B, C): lines on east edge
\foreach \col in {\colA, \colB, \colC} {
    \pgfmathsetmacro{\lineX}{\col + \tileHalfWidth}
    \pgfmathsetmacro{\bottomY}{\rowV + \cacheCoreBoundary}
    \draw[vmesh] (\lineX, \topRouterY) -- (\lineX, \bottomY);
}
% Right 3 columns (D, E, F): lines on west edge
...
% Horizontal mesh lines - only between leftmost and rightmost routers
\pgfmathsetmacro{\leftmostX}{\colA + \tileHalfWidth}
\pgfmathsetmacro{\rightmostX}{\colF - \tileHalfWidth}
% Top row horizontal line
\draw[hmesh] (\leftmostX, \topRouterY) -- (\rightmostX, \topRouterY);
% Cache/Core rows horizontal lines
\foreach \row in {\rowII, \rowIII, \rowIV, \rowV} {
    \pgfmathsetmacro{\lineY}{\row + \cacheCoreBoundary}
    \draw[hmesh] (\leftmostX, \lineY) -- (\rightmostX, \lineY);
}
...
% Cache/Core rows - anchored at NE corner for left cols, NW for right cols
% (between Cache bottom and Core top)
\foreach \col in {\colA, \colB, \colC} {
    \foreach \row in {\rowII, \rowIII, \rowIV, \rowV} {
        \pgfmathsetmacro{\rx}{\col + 0.55}
        \pgfmathsetmacro{\ry}{\row - 0.225}
        \node[router, anchor=north east] at (\rx, \ry) {};
    }
}
\foreach \col in {\colD, \colE, \colF} {
    \foreach \row in {\rowII, \rowIII, \rowIV, \rowV} {
        \pgfmathsetmacro{\rx}{\col - 0.55}
        \pgfmathsetmacro{\ry}{\row - 0.225}
        \node[router, anchor=north west] at (\rx, \ry) {};
    }
}
