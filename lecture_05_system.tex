...
% Y coordinates: top router center to bottom router center
\pgfmathsetmacro{\topCenterY}{\topRouterY + \routerHalf}
\pgfmathsetmacro{\bottomCenterY}{\rowV + \cacheCoreBoundary - \routerHalf}
...
\foreach \col in {\colA, \colB, \colC} {
    ...
    \draw[meshline] (\lineX - \lineGap, \topCenterY) -- (\lineX - \lineGap, \bottomCenterY);
    \draw[meshline] (\lineX + \lineGap, \topCenterY) -- (\lineX + \lineGap, \bottomCenterY);
}
...
\draw[meshline] (\leftmostX, \topLineY - \lineGap) -- (\rightmostX, \topLineY - \lineGap);
\draw[meshline] (\leftmostX, \topLineY + \lineGap) -- (\rightmostX, \topLineY + \lineGap);
% Cache/Core rows horizontal lines (router anchored at north, so center is down by routerHalf)
\foreach \row in {\rowII, \rowIII, \rowIV, \rowV} {
    \pgfmathsetmacro{\lineY}{\row + \cacheCoreBoundary - \routerHalf}
