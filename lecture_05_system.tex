...
\begin{circuitikz}[
  ...
  % Address decoder
  \node[box] (decoder) at (0,0) {Address\\Decoder};

  % Time delay generator
  \node[box, right=1.8cm of decoder] (timegen) {Time Delay\\Generator};
  \node[muxdemux, muxdemux def={Lh=4, Rh=4, w=1.5, NL=1, NB=1, NR=2},
        external pins width=0, below=1.5cm of timegen] (addrmux-sel) {};

  % Address mux (using circuitikz mux)
  \node[muxdemux, muxdemux def={Lh=4, Rh=2, w=1.5, NL=2, NB=1},
        external pins width=0, below=2.2cm of decoder, xshift=1cm] (addrmux) {};
  \node[below, font=\scriptsize] at (addrmux.south) {Addr Mux};

  % DRAM chip
  \node[box, fill=blue!10, right=3.5cm of addrmux] (dram) {DRAM};

  % Input coordinate
  \coordinate (input) at ([xshift=-2.5cm]decoder.west);

  % Address bus inputs
  \draw[->, thick] (input) -- node[above, font=\small] {A[20:23]} (decoder.west);
  \draw[->, thick] ([yshift=-0.8cm]input) -| node[above, font=\small, pos=0.3] {A[10:19]} ();
  \draw[->, thick] ([yshift=-1.6cm]input) -| node[above, font=\small, pos=0.3] {A[0:9]} (addrmux.blpin 2);

  % Decoder to Time delay gen (Chip select)
  \draw[->, thick] (decoder.east) -- node[above, font=\scriptsize] {Chip Select} (timegen.west);

  % Time delay gen to DRAM (RAS# and CAS#)
  \draw[->, thick] (timegen.east) -| ([xshift=0.5cm]timegen.east) |-
    node[near end, right, font=\scriptsize] {RAS\#} ([yshift=0.6cm]dram.west);
  \draw[->, thick] ([xshift=0.5cm]timegen.east) |-
    node[near end, right, font=\scriptsize] {CAS\#} ([yshift=-0.6cm]dram.west);

  % Time delay gen to Address mux select
  \draw[->, thick] (timegen.south) |- node[right, font=\scriptsize, pos=0.2] {Select} (mux-sel);

  % Address mux to DRAM (Memory address bus)
  \draw[->, thick] (mux-out) -- node[above, font=\scriptsize] {Memory Addr Bus} (dram.west);

  % R/W# signal
  \draw[->, thick] ([yshift=-3.2cm]input) -- node[above, font=\small] {R/W\#}
    ([yshift=-3.2cm]input -| dram.south);
  \draw[->, thick] ([yshift=-3.2cm]input -| dram.south) -- (dram.south);

  % Data output
  \draw[->, thick] (dram.east) -- node[above, font=\small] {D[0:7]} ([xshift=1.5cm]dram.east);
