...
% Slide: DRAM Scheduling Policies I
\begin{frame}{DRAM Scheduling Policies (I)}
\textbf{FCFS} (first come first served)
\begin{itemize}
\item Oldest request first
\end{itemize}

\vspace{0.5cm}
\textbf{FR-FCFS} (first ready, first come first served)
\begin{enumerate}
\item Row-hit first
\item Oldest first
\end{enumerate}

\textbf{Goal:} Maximize row buffer hit rate $\rightarrow$ \textcolor{blue}{maximize DRAM throughput}

\vspace{0.5cm}
\begin{itemize}
\item Actually, scheduling is done at the \textcolor{blue}{command level}
  \begin{itemize}
  \item Column commands (read/write) prioritized over row commands (activate/precharge)
  \item Within each group, older commands prioritized over younger ones
  \end{itemize}
\end{itemize}
\end{frame}

% Slide: DRAM Scheduling Policies II
\begin{frame}{DRAM Scheduling Policies (II)}
\textbf{A scheduling policy is a request prioritization order}

\vspace{0.5cm}
\textbf{Prioritization can be based on:}
\begin{itemize}
\item Request age
\item Row buffer hit/miss status
\item Request type (prefetch, read, write)
\item Requestor type (load miss or store miss)
\item Request criticality
  \begin{itemize}
  \item Oldest miss in the core?
  \item How many instructions in core are dependent on it?
  \item Will it stall the processor?
  \end{itemize}
\item Interference caused to other cores
\item ...
\end{itemize}
\end{frame}
...
\textbf{Open row}
\begin{itemize}
\item Keep the row open after an access
\item[+] Next access might need the same row $\rightarrow$ row hit
\item[--] Next access might need a different row $\rightarrow$ row conflict, wasted energy
\end{itemize}

\vspace{0.3cm}
\textbf{Closed row}
\begin{itemize}
\item Close the row after an access (if no other requests already in the request buffer need the same row)
\item[+] Next access might need a different row $\rightarrow$ avoid a row conflict
\item[--] Next access might need the same row $\rightarrow$ extra activate latency
\end{itemize}

\vspace{0.3cm}
\textbf{Adaptive policies}
\begin{itemize}
\item Predict whether or not the next access to the bank will be to the same row
\end{itemize}
\end{frame}
