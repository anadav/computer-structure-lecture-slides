...
\begin{tikztimingtable}[
  ...
  xscale=1.15,
  timing/rowdist=1.1cm,
  timing/name/.style={font=\small}
]
  RAS\# & H L 6L H 6L H 2L H \\
  CAS\# & 3H L H L H L H 7H L H L H L H 2H \\
  Addr  & U 2D{Row} D{Col} D{Col} D{Col} 5D{X} 2D{Row} D{Col} D{Col} D{Col} U \\
  Data  & 6U 2D{\textcolor{red}{Data}} 2D{Data} 2D{Data} 8U 2D{Data} 2D{Data} 2D{Data} \\
  \extracode
    % Timing annotations
    \begin{scope}[>=stealth]
      \draw[<->] (0.8,5.3) -- node[above,font=\scriptsize] {tRCD} (2.4,5.3);
      \draw[<->] (5.6,5.3) -- node[above,font=\scriptsize] {tRP} (10.4,5.3);
      \draw[<->] (2.4,-0.7) -- node[below,font=\scriptsize] {CL} (4.8,-0.7);
    \end{scope}
\end{tikztimingtable}
\end{center}
