...
\begin{tikzpicture}[scale=0.85, transform shape,
    tile/.style={draw, minimum width=1.1cm, minimum height=0.45cm, font=\tiny, inner sep=1pt, text depth=0.1ex},
    ...
    meshline/.style={line width=1pt},
    vmesh/.style={meshline, orange!70},
    hmesh/.style={meshline, cyan!50}
]
...
% Router offset from tile center (at NE corner of Core = SE corner of Cache)
\def\routerOffsetX{0.45}
\def\routerOffsetY{-0.225}
...
% Vertical mesh lines - on east side for left cols, west side for right cols
% Left 3 columns (A, B, C): lines on east side
\foreach \col in {\colA, \colB, \colC} {
    \pgfmathsetmacro{\lineX}{\col + \routerOffsetX}
    \draw[vmesh] (\lineX, 0.45) -- (\lineX, -4.7);
}
% Right 3 columns (D, E, F): lines on west side
\foreach \col in {\colD, \colE, \colF} {
    \pgfmathsetmacro{\lineX}{\col - \routerOffsetX}
    \draw[vmesh] (\lineX, 0.45) -- (\lineX, -4.7);
}

% Horizontal mesh lines (between cache and core)
\foreach \row in {\rowII, \rowIII, \rowIV, \rowV} {
    \pgfmathsetmacro{\lineY}{\row + \routerOffsetY}
    \draw[hmesh] (-0.6, \lineY) -- (6.35, \lineY);
}

% Routers at intersections - left cols on east, right cols on west
% Left 3 columns (A, B, C): router on east side (NE of core)
\foreach \col in {\colA, \colB, \colC} {
    \foreach \row in {\rowII, \rowIII, \rowIV, \rowV} {
        \pgfmathsetmacro{\rx}{\col + \routerOffsetX}
        \pgfmathsetmacro{\ry}{\row + \routerOffsetY}
        \node[router] at (\rx, \ry) {};
    }
}
% Right 3 columns (D, E, F): router on west side (NW of core)
\foreach \col in {\colD, \colE, \colF} {
    \foreach \row in {\rowII, \rowIII, \rowIV, \rowV} {
        \pgfmathsetmacro{\rx}{\col - \routerOffsetX}
        \pgfmathsetmacro{\ry}{\row + \routerOffsetY}
...
% Top/bottom arrows for UPI
\foreach \col in {\colA, \colB, \colC} {
    \pgfmathsetmacro{\arrowX}{\col + \routerOffsetX}
    \draw[->, thick] (\arrowX, 0.6) -- (\arrowX, 0.45);
    \draw[->, thick] (\arrowX, -4.7) -- (\arrowX, -4.85);
}
\foreach \col in {\colD, \colE, \colF} {
    \pgfmathsetmacro{\arrowX}{\col - \routerOffsetX}
    \draw[->, thick] (\arrowX, 0.6) -- (\arrowX, 0.45);
...
% Outer box
\draw[very thick, rounded corners=3pt] (-0.65, 0.5) rectangle (6.4, -4.75);
