...
\begin{tikzpicture}[scale=0.5,
    ...
    pics/numanode/.style={code={
        ...
        \draw[<->, thin] (-mem) -- (-bus);
        \node[scache, below=2.5mm of -bus, xshift=-4mm] (-c1) {\tiny c};
    ...
    pics/numanodeR/.style={code={
        ...
        \node[scache, below=2.5mm of -bus, xshift=-1mm] (-c2) {\tiny c};
        \node[below=2.5mm of -bus, xshift=2.5mm, font=\tiny] {..};
        \node[scache, below=2.5mm of -bus, xshift=6mm] (-cn) {\tiny c};
        \node[sproc, below=2.5mm of -c1] (-p1) {\tiny P};
        \node[sproc, below=2.5mm of -c2] (-p2) {\tiny P};
        \node[sproc, below=2.5mm of -cn] (-pn) {\tiny P};
        \draw[<->, thin] (-bus -| -io) -- (-io);
...
\begin{tikzpicture}[scale=0.7,
    ...
    mem/.style={comp, fill=blue!20},
    proc/.style={comp, fill=green!20},
    io/.style={comp, fill=orange!20}]

% Node 1 (anchor)
\node[proc] (p1) {P1};
\node[mem, below=2mm of p1] (m1) {Mem};
\node[io, below=2mm of m1] (io1) {I/O};

% Node 2
\node[proc, right=5mm of p1] (p2) {P2};
\node[mem, below=2mm of p2] (m2) {Mem};
\node[io, below=2mm of m2] (io2) {I/O};

% Node 3
\node[proc, right=5mm of p2] (p3) {P3};
\node[mem, below=2mm of p3] (m3) {Mem};
\node[io, below=2mm of m3] (io3) {I/O};

% Node 4
\node[proc, right=5mm of p3] (p4) {P4};
\node[mem, below=2mm of p4] (m4) {Mem};
\node[io, below=2mm of m4] (io4) {I/O};
...
% Network bus - below I/O nodes
\node[below=8mm of io2, xshift=5mm] (net-center) {};
\draw[ultra thick, red!60] ([xshift=-2.5cm]net-center.center) -- ([xshift=2.5cm]net-center.center);
\node[font=\scriptsize, below=1mm of net-center] {Network};

% Connections to network
\draw[->, thick] (io1) -- (io1 |- net-center.center);
\draw[->, thick] (io2) -- (io2 |- net-center.center);
\draw[->, thick] (io3) -- (io3 |- net-center.center);
\draw[->, thick] (io4) -- (io4 |- net-center.center);
