...
\begin{circuitikz}[
  ...
  % Address bus inputs
  \draw[->, thick] (input) -- ++(0.5,0) |- node[textnode, near end] {A[20:23]} (decoder.lpin 1);
  \draw[->, thick] ([xshift=5mm]input) |- node[textnode, near end] {A[10:19]} (addrmux.lpin 1);
  \draw[->, thick] ([xshift=5mm]input) |- node[textnode, near end] {A[0:9]} (addrmux.lpin 2);

  % Decoder to Time delay gen (Chip select)
  \draw[->, thick] (decoder.rpin 1) -- node[textnode] {Chip\\Select} (timegen.lpin 1);

  % Time delay gen to DRAM (RAS# and CAS#)
  \draw[->, thick] (timegen.rpin 1) -- node[textnode] {RAS\#}  ++(1.4, 0) |- 
    (dram.lpin 1);
  \draw[->, thick] (timegen.rpin 2) -- node[textnode] {CAS\#}  ++(1, 0) |-
    (dram.lpin 2);
