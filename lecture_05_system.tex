...
\begin{center}
\begin{tikzpicture}[
    tilebox/.style={draw, ultra thick, minimum width=2cm, minimum height=2.5cm},
    corebox/.style={draw, fill=blue!30, minimum width=0.65cm, minimum height=0.45cm, font=\tiny},
    cachebox/.style={draw, minimum width=1.5cm, minimum height=0.35cm, font=\tiny},
    minitile/.style={draw, thick, minimum width=1.1cm, minimum height=0.85cm}
]
% Single tile detail
\node[tilebox] (tile) at (0,0) {};
\node[font=\bfseries, anchor=south] at (tile.north) {Tile};
\node[corebox] (c1) at ([yshift=-0.4cm, xshift=-0.4cm]tile.north) {Core};
\node[corebox, right=0.1cm of c1] (c2) {Core};
\node[cachebox, fill=yellow!30, below=0.15cm of $(c1.south)!0.5!(c2.south)$] (l2) {L2 Cache};
\node[cachebox, fill=green!30, below=0.1cm of l2] (l3) {L3 Slice};
\node[cachebox, fill=red!20, below=0.1cm of l3, minimum height=0.2cm] (ring) {Ring Stop};
\node[font=\small, below=0.3cm of tile] {Single Tile};

% Multiple tiles - 2x2 grid
\node[minitile, right=1.8cm of tile, yshift=0.55cm] (t00) {};
\node[minitile, right=0.15cm of t00] (t10) {};
\node[minitile, below=0.15cm of t00] (t01) {};
\node[minitile, right=0.15cm of t01] (t11) {};

% Mini content
\node[draw, fill=blue!20, minimum width=0.4cm, minimum height=0.25cm] at ([yshift=0.15cm]t00.center) {};
\node[draw, fill=green!20, minimum width=0.9cm, minimum height=0.2cm] at ([yshift=-0.2cm]t00.center) {};
\node[draw, fill=blue!20, minimum width=0.4cm, minimum height=0.25cm] at ([yshift=0.15cm]t10.center) {};
\node[draw, fill=green!20, minimum width=0.9cm, minimum height=0.2cm] at ([yshift=-0.2cm]t10.center) {};
\node[draw, fill=blue!20, minimum width=0.4cm, minimum height=0.25cm] at ([yshift=0.15cm]t01.center) {};
\node[draw, fill=green!20, minimum width=0.9cm, minimum height=0.2cm] at ([yshift=-0.2cm]t01.center) {};
\node[draw, fill=blue!20, minimum width=0.4cm, minimum height=0.25cm] at ([yshift=0.15cm]t11.center) {};
\node[draw, fill=green!20, minimum width=0.9cm, minimum height=0.2cm] at ([yshift=-0.2cm]t11.center) {};

% Mesh lines
\draw[red, thick] ($(t00.south)!0.5!(t01.north)$) -- ($(t10.south)!0.5!(t11.north)$);
\draw[red, thick] ($(t00.east)!0.5!(t10.west)$) -- ($(t01.east)!0.5!(t11.west)$);
\node[font=\small, below=0.3cm of $(t01.south)!0.5!(t11.south)$] {4-Tile Mesh};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
    corebox/.style={draw, fill=blue!30, minimum width=0.3cm, minimum height=0.3cm, font=\tiny},
    cachebox/.style={draw, fill=green!30, minimum height=0.25cm, font=\tiny},
    iobox/.style={draw, minimum width=0.8cm, minimum height=0.25cm, text depth=0.1ex, font=\tiny},
    ccdbox/.style={draw, thick, fill=blue!10, minimum width=1.15cm, minimum height=1.1cm},
    iodbox/.style={draw, thick, fill=orange!10, minimum width=2.45cm, minimum height=0.85cm}
]
% Monolithic - build from content outward
\node[corebox] (mc1) at (0,0) {C};
\node[corebox, right=0.06cm of mc1] (mc2) {C};
\node[corebox, right=0.06cm of mc2] (mc3) {C};
\node[corebox, right=0.06cm of mc3] (mc4) {C};
\node[cachebox, minimum width=1.7cm, below=0.1cm of $(mc1.south)!0.5!(mc4.south)$] (ml3) {L3 Cache};
\node[iobox, fill=orange!30, below=0.08cm of ml3, xshift=-0.4cm] (mmc) {MC};
\node[iobox, fill=purple!30, right=0.06cm of mmc] (mio) {I/O};

% Monolithic box fitted around content
\node[draw, thick, fit=(mc1)(mc4)(ml3)(mmc)(mio), inner sep=0.12cm] (mono) {};
...
\node[ccdbox, right=1.3cm of mono, yshift=0.45cm] (ccd1) {};
\node[font=\tiny\bfseries, anchor=south] at (ccd1.north) {CCD};
\node[corebox, minimum width=0.25cm, minimum height=0.25cm] (cc1_1) at ([xshift=-0.25cm, yshift=0.2cm]ccd1.center) {C};
\node[corebox, minimum width=0.25cm, minimum height=0.25cm, right=0.04cm of cc1_1] (cc1_2) {C};
\node[corebox, minimum width=0.25cm, minimum height=0.25cm, below=0.04cm of cc1_1] (cc1_3) {C};
\node[corebox, minimum width=0.25cm, minimum height=0.25cm, right=0.04cm of cc1_3] (cc1_4) {C};
\node[cachebox, minimum width=0.9cm, below=0.06cm of $(cc1_3.south)!0.5!(cc1_4.south)$] (cl3_1) {L3};

% CCD 2
\node[ccdbox, right=0.15cm of ccd1] (ccd2) {};
\node[font=\tiny\bfseries, anchor=south] at (ccd2.north) {CCD};
\node[corebox, minimum width=0.25cm, minimum height=0.25cm] (cc2_1) at ([xshift=-0.25cm, yshift=0.2cm]ccd2.center) {C};
\node[corebox, minimum width=0.25cm, minimum height=0.25cm, right=0.04cm of cc2_1] (cc2_2) {C};
\node[corebox, minimum width=0.25cm, minimum height=0.25cm, below=0.04cm of cc2_1] (cc2_3) {C};
\node[corebox, minimum width=0.25cm, minimum height=0.25cm, right=0.04cm of cc2_3] (cc2_4) {C};
\node[cachebox, minimum width=0.9cm, below=0.06cm of $(cc2_3.south)!0.5!(cc2_4.south)$] (cl3_2) {L3};

% I/O Die
\node[iodbox, below=0.3cm of $(ccd1.south)!0.5!(ccd2.south)$] (iod) {};
\node[font=\tiny\bfseries, anchor=north] at ([yshift=-0.02cm]iod.north) {I/O Die};
\node[iobox, fill=orange!30, anchor=south, xshift=-0.45cm] at ([yshift=0.06cm]iod.south) (iomc) {MC};
\node[iobox, fill=purple!30, right=0.06cm of iomc] (iopcie) {PCIe};
...
% Section: Multi-Processor Systems
\begin{frame}
\sectionpage
\centering
{\Huge Multi-Processor Systems}
\end{frame}

% Slide: Multi-Processor Systems Introduction
\begin{frame}{Multi-Processor Systems}
\textbf{Multi-Processor Systems:}
\begin{itemize}
\item Use multiple CPUs to perform parallelizable complex tasks
\item Support multiple tasks and/or multiple users
\item System resources (memory, I/O, etc.) may be divided symmetrically or asymmetrically
\end{itemize}

\vspace{0.5cm}
\textbf{Symmetric Multiprocessing (SMP):}
\begin{itemize}
\item All CPUs are treated equally
\item Shared access to memory and I/O
\end{itemize}

\vspace{0.5cm}
\textbf{Asymmetric Multiprocessing (ASMP) Examples:}
\begin{itemize}
\item Non-Uniform Memory Access (NUMA)
\item Clustered multiprocessing
\end{itemize}
\end{frame}

% Slide: Multi-Processor Memory - UMA
\begin{frame}{Multi-Processor Memory: Shared Memory (UMA)}
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{Centralized Shared Memory (SMP)}
\begin{itemize}
\item Uniform Memory Access (UMA)
\item All processors access all memory locations at similar latency
\item Data sharing through memory reads/writes
\item Memory bandwidth becomes bottleneck
\item Not scalable to many processors
\end{itemize}

\column{0.45\textwidth}
\begin{center}
\begin{tikzpicture}[scale=0.7,
    box/.style={draw, minimum height=8mm, minimum width=12mm, font=\footnotesize},
    cache/.style={draw, minimum height=5mm, minimum width=10mm, font=\scriptsize, fill=yellow!20}]

% Memory at top
\node[box, minimum width=5cm, fill=blue!20] (mem) {Memory};

% System bus
\draw[ultra thick] (-2.2,-1) -- (2.2,-1);
\node[right, font=\scriptsize] at (2.2,-1) {System Bus};

% Processors with caches
\foreach \i/\x in {1/-1.5, 2/0, n/1.5} {
    \node[cache] (cache\i) at (\x,-1.8) {cache};
    \node[box, fill=green!20] (p\i) at (\x,-2.8) {P\i};
    \draw (cache\i) -- (\x,-1);
    \draw (cache\i) -- (p\i);
}
...
\begin{frame}{Multi-Processor Memory: Distributed Shared Memory (NUMA)}
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{Distributed Shared-Memory (DSM)}
\begin{itemize}
\item Also called NUMA (Non-Uniform Memory Access)
\item All processors can address all memory locations
\item Variable latency: local access faster than remote
\item Better scalability than UMA
\item Requires cache coherence protocol
\end{itemize}

\column{0.45\textwidth}
\begin{center}
\begin{tikzpicture}[scale=0.65,
    node/.style={draw, rounded corners, minimum height=6mm, minimum width=10mm, font=\scriptsize}]

% Create 4 nodes in a square
\foreach \i/\x/\y in {1/-1.2/1.2, 2/1.2/1.2, 3/-1.2/-1.2, 4/1.2/-1.2} {
    \node[node, fill=green!20] (p\i) at (\x,\y) {P\i};
    \node[node, fill=blue!20, below=1mm of p\i, minimum width=8mm] (m\i) {Mem};
    \node[node, fill=orange!20, below=1mm of m\i, minimum width=8mm] (io\i) {I/O};
}
...
\node[font=\scriptsize] at (0,0) {Interconnect};
\end{tikzpicture}
\end{center}
\end{columns}
\end{frame}
...
\begin{itemize}
\item Computer cluster: connected computers working together
\item Each processor can only directly address local memory
\item Communicates via sending/receiving messages
\item Extra burden on software for handling messages
\item Scales to many processors
\end{itemize}

\column{0.45\textwidth}
\begin{center}
\begin{tikzpicture}[scale=0.7,
    node/.style={draw, rounded corners, minimum height=5mm, minimum width=8mm, font=\scriptsize}]

% Create 4 nodes
\foreach \i/\x in {1/-1.5, 2/-0.5, 3/0.5, 4/1.5} {
    \node[node, fill=orange!20] (io\i) at (\x,0) {I/O};
    \node[node, fill=blue!20, above=1mm of io\i] (m\i) {Mem};
    \node[node, fill=green!20, above=1mm of m\i] (p\i) {P\i};
}

% Network at bottom
\draw[ultra thick] (-2,-0.8) -- (2,-0.8);
\node[font=\scriptsize] at (0,-1.2) {Network};
...
\footnotesize
\begin{tabular}{lll}
\textbf{UMA/SMP} & Simple programming & Limited scalability \\
\textbf{NUMA/DSM} & Shared address space & Variable latency \\
\textbf{Clusters} & High scalability & Complex programming \\
\end{tabular}
\end{block}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{Simple SMP on the same chip:}
\begin{itemize}
...
\column{0.45\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{figures/cmp_mesh.png}
\end{center}
\footnotesize
Ring or Mesh interconnect between cores, shared LLC, memory controller, and I/O
\end{columns}
\end{frame}

% Slide: Multi-Socket System
\begin{frame}{Multi-Socket System}
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{Architecture:}
\begin{itemize}
\item Each socket (chip) includes multiple cores (CMP)
\item Each socket has Inter-Processor Link (IPL) controllers
\item Sockets fully connected via high-speed point-to-point interconnect
\end{itemize}

\vspace{0.3cm}
\textbf{Coherency:}
\begin{itemize}
\item Coherent interconnect
\item Single shared memory address space
\item Directory-based home snoop coherency protocol
\end{itemize}

\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}[scale=0.6,
    socket/.style={draw, thick, rounded corners, minimum width=2.2cm, minimum height=1.5cm, fill=gray!10},
    ipl/.style={draw, fill=purple!30, minimum width=4mm, minimum height=4mm, font=\tiny},
    mem/.style={draw, fill=blue!20, minimum width=4mm, minimum height=6mm, font=\tiny, rotate=90}]

% Four sockets in 2x2
\foreach \i/\x/\y in {1/-1.5/1.5, 2/1.5/1.5, 3/-1.5/-1.5, 4/1.5/-1.5} {
    \node[socket] (s\i) at (\x,\y) {};
    \node[font=\scriptsize] at (\x,\y) {Socket \i};
}
...
% Memory below each socket
\foreach \i/\x/\y in {1/-1.5/0.4, 2/1.5/0.4, 3/-1.5/-2.6, 4/1.5/-2.6} {
    \node[draw, fill=blue!30, minimum width=1.5cm, minimum height=4mm, font=\tiny] at (\x,\y) {DDR};
}

\node[font=\tiny, red!70] at (0,0) {IPL};
\end{tikzpicture}
\end{center}
\end{columns}
\end{frame}
...
% Slide: Supercomputers
\begin{frame}{Supercomputers}
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{High Performance Computing (HPC) Clusters:}
\begin{itemize}
\item Group of servers connected with dedicated high-speed network
\item Top500 ranks the 500 most powerful non-distributed systems
\item Based on HPL (High-Performance LINPACK benchmark)
\end{itemize}

\vspace{0.3cm}
\textbf{Example: Trinity (Cray XC40)}\\
\footnotesize Los Alamos National Laboratory
\begin{itemize}
\item Cores: 979,968
\item Processor: Intel Xeon Phi 7250 68C 1.4GHz
\item Linpack: 14,137 TFlop/s
\item Theoretical Peak: 43,902 TFlop/s
\item Power: 3,843 kW
\end{itemize}

\column{0.45\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{figures/trinity_supercomputer.jpeg}
\end{center}

\vspace{0.2cm}
\footnotesize
\textbf{Note:} Modern top supercomputers exceed 1 ExaFlop/s (Frontier, Aurora)
\end{columns}
\end{frame}
