...
\node[box, fill=gray!20, minimum width=1.5cm] (header) at (0,0) {Header};
\node[box, fill=white, minimum width=3cm, anchor=west] (data) at (header.east) {Data};

% Transaction Layer label - right of Data
\node[layer, anchor=west] (translayer) at ([xshift=1.5cm]data.east) {\textbf{Transaction Layer}};

% Dashed line separator below
\draw[dashed, thick] ([yshift=-3mm,xshift=-1.5cm]header.south west) -- ([yshift=-3mm]translayer.east |- header.south);

% T-Layer Packet - below data with yshift=-6mm
\node[box, fill=blue!20, minimum width=2.5cm, anchor=north west] (tlpacket) at ([yshift=-6mm]data.south west) {T-Layer Packet};

% Packet Sequence Number - left of T-Layer Packet, wider and exactly 2 rows
\node[box, fill=green!20, minimum width=2.8cm, anchor=east] (pseq) at (tlpacket.west) {Packet Sequence\\Number};

% CRC - right of T-Layer Packet, adjacent
\node[box, fill=red!20, minimum width=1cm, anchor=west] (crc1) at (tlpacket.east) {CRC};

% Link Layer label - positioned at CRC -| translayer.west, anchored west
\node[layer, anchor=west] (linklayer) at (crc1.east -| translayer.west) {\textbf{Link Layer}};

% Dashed line separator below
\draw[dashed, thick, blue] ([yshift=-0.5cm]pseq.south west) -- ([yshift=-0.5cm]linklayer.south east);
\node[draw, circle, fill=teal!60, inner sep=2pt] at ([yshift=-0.5cm]linklayer.south east) {};

% L-Layer Packet - below T-Layer Packet with yshift=-6mm
\node[box, fill=green!20, minimum width=4.5cm, anchor=north west] (llpacket) at ([yshift=-6mm]tlpacket.south west) {L-Layer Packet};
...
\node[layer, anchor=west] (physlayer) at (frame2.east -| translayer.west) {\textbf{Physical Layer}};
...
\item Supports multiple generations (PCIe 3.0: 8 GT/s, PCIe 4.0: 16 GT/s, PCIe 5.0: 32 GT/s)
\end{itemize}
...
\textbf{PCIe supports four address spaces:}
