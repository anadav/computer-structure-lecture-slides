...
\textbf{Data path:}
\begin{itemize}
\item 64-bit wide data path
\item (72 bits with ECC/parity)
\end{itemize}

\vspace{0.2cm}
\textbf{Configuration:}
\begin{itemize}
\item \textbf{Single-sided:} 9 chips\\Each chip: 8-bit data bus
\item \textbf{Dual-sided:} 18 chips\\Each chip: 4-bit data bus
\end{itemize}

\vspace{0.2cm}
\textbf{Data bandwidth:}
\begin{itemize}
\item 64 bits on rising edge
\item 64 bits on falling edge
\item = 128 bits per clock cycle
\end{itemize}

\column{0.4\textwidth}
\begin{center}
\includegraphics[angle=90,width=0.8\textwidth,keepaspectratio]{figures/noun-ram-2198352.pdf}
\end{center}
\end{columns}

\vspace{0.2cm}
\textbf{Other pins:} Address (14), RAS, CAS, Chip Select (4), Power (17), Ground (18), Clock (4), etc.
\end{frame}
