...
\node[base, minimum width=5.7cm, minimum height=0.8cm, anchor=south west, text width=5.5cm]
  (basedie) at (pbump0.north -| substrate.west) {Base Die};

% Base Logic Die (positioned relative to Base Die, to its right)
\node[base, fill=basecolor!80, minimum width=3.8cm, minimum height=0.8cm, anchor=west]
  (baselogic) at ([xshift=4mm]basedie.east) {Base Logic Die};

% PHY blocks
\node[emib, fill=emibcolor!70, minimum width=0.7cm, minimum height=0.4cm, anchor=south east]
  (phy0) at ([xshift=-1mm, yshift=1mm]basedie.south east) {PHY};
\node[emib, fill=emibcolor!70, minimum width=0.7cm, minimum height=0.4cm, anchor=south west]
  (phy1) at ([xshift=1mm, yshift=1mm]baselogic.south west) {PHY};


% HBM Stack (4 dies, positioned relative to Base Die)
\foreach \y [count=\j from 0] in {0, 0.8, 1.6, 2.4} {
  ...
  \foreach \x [count=\i from 0] in {0.1,0.3,0.5,0.7,0.9} {
    \node[tsv, minimum width=0.08cm, minimum height=0.4cm, anchor=north] (tsv\j-\i) at ($(hbm\j.north east)+(-\x,0)$) {};
  }
  % µBumps at bottom of each (positioned south of TSVs)
  \foreach \i in {0,1,2,3,4} {
    \node[ball, minimum size=1mm, anchor=north] (ubump\j-\i) at (tsv\j-\i.south |- hbm\j.south) {};
...
\foreach \i in {0,1,2,3,4} {
  ...
  \node[ball, minimum size=0.8mm, anchor=north] (basebump\i) at ([xshift=-2mm * \i]phy0.east |- basedie.south) {};
  % Wiring between HBM bump and Base Die TSV
  \draw[very thin, white] (basetsv\i.south) -- ++(0,-1mm - \i * 0.5mm) -| (basebump\i.north);
}
...
\foreach \i in {0,1,...,10} {
  \node[graybump, minimum size=0.5mm] (f2fbump0-\i) at ([xshift=-1mm - \i * 3mm]basedie.north east) {};
  \node[graybump, minimum size=0.5mm] (f2fbump1-\i) at ([xshift=4mm + \i * 3mm]baselogic.north west) {};
  %\node[graybump, minimum size=0.5mm] at ($(baselogic.north west) + (\x,0.06)$) {};
}

% Compute Dies (positioned above F2F µBumps)
\node[compute, minimum width=3.0cm, minimum height=0.55cm, anchor=south]
  (compute1) at (f2fbump1-7.north -| baselogic) {Compute/FPGA/Mem Die};
\node[compute, minimum width=1.7cm, minimum height=0.55cm, anchor=south east]
  (compute0) at (f2fbump1-7.north -| basedie.east) {Compute/FPGA/Mem Die};
