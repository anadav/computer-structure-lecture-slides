...
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, calc, tikzmark, shapes.misc, matrix, fit, backgrounds, shapes.callouts}
...
\node[desc] (trans_desc) at ([xshift=0.4cm]trans.east) {Billions of switches (3-5nm)};
...
\node[example] (trans_ex) at ([xshift=-0.4cm]trans.west) {3nm = 100M+/mm²};
...
% Slide: Intra-Socket Interconnects
...
\vspace{0.2cm}
\textbf{Chiplet Advantages:}
\begin{itemize}
\item Mix process nodes
\item Reuse dies across products
\item Better manufacturing yields
\end{itemize}

\column{0.58\textwidth}
\begin{center}
\begin{tikzpicture}[scale=0.85,
    minicore/.style={draw, fill=blue!30, minimum width=0.28cm, minimum height=0.28cm, font=\tiny},
    minicache/.style={draw, fill=green!30, minimum width=0.7cm, minimum height=0.18cm, font=\tiny},
    ccxbox/.style={draw, dashed, inner sep=1pt},
    ccdbox/.style={draw, thick, fill=blue!10, inner sep=3pt},
    iobox/.style={draw, minimum width=0.8cm, minimum height=0.25cm, font=\tiny},
    iodbox/.style={draw, thick, fill=orange!10}
]
% CCD 1: CCX0 and CCX1 side by side
% CCX 0
\matrix[matrix of nodes, nodes={minicore}, row sep=1pt, column sep=1pt, ampersand replacement=\&] (ccx0) at (-2.85,0) {
    {} \& {} \\
    {} \& {} \\
};
\draw[blue] (ccx0-1-1) -- (ccx0-1-2);
\draw[blue] (ccx0-1-1) -- (ccx0-2-2);
\draw[blue] (ccx0-2-1) -- (ccx0-2-2);
\draw[blue] (ccx0-1-2) -- (ccx0-2-1);
\node[minicache, below=1pt of ccx0] (l3_0) {\tiny L3};
\node[ccxbox, fit=(ccx0)(l3_0)] (box0) {};
\node[font=\tiny, above=0pt of box0] (lbl0) {CCX0};

% CCX 1
\matrix[matrix of nodes, nodes={minicore}, row sep=1pt, column sep=1pt, ampersand replacement=\&] (ccx1) at (-1.35,0) {
    {} \& {} \\
    {} \& {} \\
};
\draw[blue] (ccx1-1-1) -- (ccx1-1-2);
\draw[blue] (ccx1-1-1) -- (ccx1-2-2);
\draw[blue] (ccx1-2-1) -- (ccx1-2-2);
\draw[blue] (ccx1-1-2) -- (ccx1-2-1);
\node[minicache, below=1pt of ccx1] (l3_1) {\tiny L3};
\node[ccxbox, fit=(ccx1)(l3_1)] (box1) {};
\node[font=\tiny, above=0pt of box1] (lbl1) {CCX1};

% IF between CCXs in CCD1
\draw[<->, red, thick] (box0.east) -- (box1.west);

% CCD 2: CCX2 and CCX3 side by side
% CCX 2
\matrix[matrix of nodes, nodes={minicore}, row sep=1pt, column sep=1pt, ampersand replacement=\&] (ccx2) at (1.35,0) {
    {} \& {} \\
    {} \& {} \\
};
\draw[blue] (ccx2-1-1) -- (ccx2-1-2);
\draw[blue] (ccx2-1-1) -- (ccx2-2-2);
\draw[blue] (ccx2-2-1) -- (ccx2-2-2);
\draw[blue] (ccx2-1-2) -- (ccx2-2-1);
\node[minicache, below=1pt of ccx2] (l3_2) {\tiny L3};
\node[ccxbox, fit=(ccx2)(l3_2)] (box2) {};
\node[font=\tiny, above=0pt of box2] (lbl2) {CCX2};

% CCX 3
\matrix[matrix of nodes, nodes={minicore}, row sep=1pt, column sep=1pt, ampersand replacement=\&] (ccx3) at (2.85,0) {
    {} \& {} \\
    {} \& {} \\
};
\draw[blue] (ccx3-1-1) -- (ccx3-1-2);
\draw[blue] (ccx3-1-1) -- (ccx3-2-2);
\draw[blue] (ccx3-2-1) -- (ccx3-2-2);
\draw[blue] (ccx3-1-2) -- (ccx3-2-1);
\node[minicache, below=1pt of ccx3] (l3_3) {\tiny L3};
\node[ccxbox, fit=(ccx3)(l3_3)] (box3) {};
\node[font=\tiny, above=0pt of box3] (lbl3) {CCX3};

% IF between CCXs in CCD2
\draw[<->, red, thick] (box2.east) -- (box3.west);

% CCD boxes on background layer
\begin{scope}[on background layer]
\node[ccdbox, fit=(box0)(box1)(lbl0)(lbl1), label={[font=\scriptsize\bfseries]above:CCD 1}] (ccd1) {};
\node[ccdbox, fit=(box2)(box3)(lbl2)(lbl3), label={[font=\scriptsize\bfseries]above:CCD 2}] (ccd2) {};
\end{scope}

% IF between CCDs
\draw[<->, red, ultra thick] (ccd1.east) -- node[above, font=\tiny] {IF} (ccd2.west);

% I/O Die
\node[iodbox, minimum width=5.5cm, minimum height=0.7cm, below=0.4cm of $(ccd1.south)!0.5!(ccd2.south)$] (iod) {};
\node[font=\scriptsize\bfseries] at (iod.center) {I/O Die};
\node[iobox, fill=orange!30, anchor=west] at ([xshift=1mm]iod.west) (iomc) {MC};
\node[iobox, fill=purple!30, anchor=east] at ([xshift=-1mm]iod.east) (iopcie) {PCIe};

% IF to I/O Die
\draw[<->, red, thick] (ccd1.south) -- (ccd1.south |- iod.north);
\draw[<->, red, thick] (ccd2.south) -- (ccd2.south |- iod.north);

\node[font=\small\bfseries, below=0.25cm of iod] (title) {AMD Ryzen (Chiplet)};
\node[font=\tiny, below=0.1cm of title, align=center] {CCX = Core Complex (4 cores + L3)\\CCD = Core Chiplet Die (2 CCX)\\IF = Infinity Fabric};
\end{tikzpicture}
\end{center}
\end{columns}
\end{frame}
...
\item Multi-socket systems use inter-socket interconnect (Intel UPI, AMD Infinity Fabric)
\end{itemize}
...
% Slide: Memory Architecture Spectrum
\begin{frame}{Memory Architecture Spectrum}
\begin{center}
\large
\begin{tabular}{lccc}
\toprule
...
\begin{itemize}
\item Local memory: $\sim$100 cycles
\item Remote memory: $\sim$150-200 cycles
\item Can be 50--100\% slower!
\item Interconnect bandwidth limited
\end{itemize}
...
\item Distance matrix exposed to software\\
      {\footnotesize (e.g., \texttt{numactl --hardware})}
\end{itemize}
...
% Slide 6: NUMA Software Implications
...
\begin{tikztimingtable}[
  ...
  \extracode
    ...
    \begin{scope}[>=stealth]
      ...
      % CL
      \draw[dashed] (4.75,-7.5) -- node[below,font=\scriptsize] {} (4.75,-4);
      \draw[dashed] (7,-7.5) -- node[below,font=\scriptsize] {} (7,-6.5);
      \draw[<->] (4.75,-7.5) -- node[below,font=\scriptsize] {CL} (7,-7.5);
...
% Slide: HBM Introduction
\begin{frame}{High Bandwidth Memory (HBM)}
\textbf{What is HBM?}
\begin{itemize}
\item \textbf{3D stacked} DRAM dies (4--8 layers)
\item Connected via \textbf{TSVs} (Through-Silicon Vias) -- vertical interconnects
\item Very wide bus (1024--4096 bits) vs DDR's 64 bits
\item Short signal paths $\rightarrow$ lower voltage/power
\end{itemize}
...
\item AI accelerators (TPUs, etc.)
\item High-performance computing
\end{itemize}

\vspace{0.5cm}
\begin{block}{Key Advantage}
Bandwidth: HBM3 delivers $\sim$800 GB/s per stack vs $\sim$50 GB/s for DDR5
\end{block}
\end{frame}
...
\begin{tikzpicture}[scale=0.7, every node/.style={scale=0.7},
    >=Stealth,
    thick,
    circuit/.style={rectangle, fill=green!60!black, draw=green!40!black, text=white, font=\small},
    substrate/.style={rectangle, fill=cyan!60, draw=cyan!80!black, text=white, font=\small},
    hbm/.style={rectangle, fill=red!70!brown, draw=brown!50!black, text=white, font=\scriptsize, align=left},
    base/.style={rectangle, fill=basecolor, draw=cyan!80!black, text=white, font=\small, align=left},
    ...
    compute/.style={rectangle, fill=blue!60!red!40, draw=blue!80!red!60, text=white, font=\scriptsize},
...
\node[substrate, minimum width=10cm, minimum height=1.3cm]
  (substrate) at (5.5,0.65) {Package Substrate};
...
% Base Die highlight on overlay 3
\only<3>{\draw[red, ultra thick, rounded corners=2pt] ([xshift=-1mm,yshift=-1mm]basedie.south west) rectangle ([xshift=1mm,yshift=1mm]basedie.north east);}

% Processor Die (positioned relative to Base Die, to its right)
\node[base, fill=purple!50, minimum width=5.8cm, minimum height=0.8cm, anchor=west]
  (baselogic) at ([xshift=4mm]basedie.east) {Processor Die};
...
% EMIB highlight on overlay 4
\only<4>{\draw[red, ultra thick, rounded corners=2pt] ([xshift=-1mm,yshift=-1mm]emib.south west) rectangle ([xshift=1mm,yshift=1mm]emib.north east);}
...
\foreach \j in {0,1,...,3} {
  \node[hbm, minimum width=2.7cm, minimum height=0.6cm, anchor=south west, text width=2.5cm]
    (hbm\j) at ([xshift=1mm, yshift=\j * 10mm + 1mm]basedie.north west) {HBM DRAM\\[2pt]Die};
...
% Annotations - Stage 1: Package Overview
\only<1>{
...
\textbf{Bandwidth} & 30-50 GB/s/channel & $\sim$800 GB/s/stack \\
...
\textbf{What is PCIe?}
\begin{itemize}
\item High-speed serial expansion bus for connecting hardware devices
\item Graphics cards, network cards, SSDs, sound cards, USB controllers, etc.
\item Devices are assigned addresses in processor's address space (\textbf{MMIO} -- Memory Mapped I/O)
\end{itemize}

\vspace{0.3cm}
\textbf{Layered Protocol Architecture:}
\begin{itemize}
\item \textbf{Software layers} generate read and write requests
\item \textbf{Hardware layers} process and transmit data reliably
\end{itemize}
...
\begin{frame}{PCIe Hardware Layers}
\begin{center}
...
\node[layer, right=of frame2] (physlayer) {\textbf{\textcolor{orange}{Physical}}};
\node[layer, anchor=east] (linklayer) at (crc1 -| physlayer.east) {\textbf{\textcolor{green!60!black}{Data Link}}};
\node[layer, anchor=east] (translayer) at (data -| physlayer.east) {\textbf{\textcolor{blue}{Transaction}}};
...
% Callouts above the figure - drawn last to be on top, each appears on single stage
\node<2>[rectangle callout, draw, fill=blue!10, callout absolute pointer={(translayer.north)},
      font=\scriptsize, align=center] at ([xshift=-2cm,yshift=1cm]translayer.north) {\textbf{Transaction}: splits\\data into packets};

\node<3>[rectangle callout, draw, fill=cyan!10, callout absolute pointer={(linklayer.north)},
      font=\scriptsize, align=center] at ([xshift=-2cm,yshift=2cm]linklayer.north) {\textbf{Data Link}: adds seq \#\\+ CRC for reliability};

\node<4>[rectangle callout, draw, fill=orange!10, callout absolute pointer={(physlayer.north)},
      font=\scriptsize, align=center] at ([xshift=-2cm,yshift=2cm]physlayer.north) {\textbf{Physical}: transmits\\bits over serial lanes};

\end{tikzpicture}
\end{center}
\end{frame}

% Slide: PCIe Key Concepts
\begin{frame}{PCIe: Key Concepts}
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{Device Enumeration}
\begin{itemize}
\item Firmware walks PCIe tree at boot
\item Assigns memory-mapped address ranges (BARs)
\item OS discovers devices via configuration space
\end{itemize}

\vspace{0.4cm}
\textbf{DMA (Direct Memory Access)}
\begin{itemize}
\item Device reads/writes system memory directly
\item CPU sets up transfer, device executes it
\item No CPU involvement during transfer
\end{itemize}

\column{0.5\textwidth}
\textbf{MSI (Message Signaled Interrupts)}
\begin{itemize}
\item Device signals interrupt by writing to special address
\item Interrupt controller delivers to target CPU
\end{itemize}
\end{columns}
\end{frame}
...
{\footnotesize\textcolor{gray}{\textit{Note: PCIe devices are auto-discovered; ACPI/Device Tree describes everything else.}}}
...
\begin{frame}{Summary}
\begin{itemize}
\item \textbf{Hierarchy:} Transistors $\rightarrow$ Core $\rightarrow$ Die $\rightarrow$ Package $\rightarrow$ System
\item \textbf{Multi-Processor:} UMA (simple) $\rightarrow$ NUMA (scalable) $\rightarrow$ Message Passing (clusters)
\item \textbf{DRAM:} Banks, rows, columns; DDR generations double bandwidth
\item \textbf{HBM:} 3D stacked DRAM with TSVs for GPUs/accelerators
\item \textbf{PCIe:} Layered protocol for device connectivity
\end{itemize}
\end{frame}
