...
\node[block, pattern=north east lines, pattern color=blue,
      above=0.5cm of arrow1] (writeblock) {};

\node[above=0.2cm of writeblock] (write) {Write};

% State 2: After Write
\foreach \i/\col in {1/blue!60, 2/blue!60, 3/green!60, 4/green!60} {
    \ifnum\i=1
        \node[block, pattern=north east lines, pattern color=\col, 
              right=0cm of memlbl.east, anchor=west, xshift={\i*\blocksize}] (s2_l1_\i) {};
    \else
        \node[block, fill=\col, right=0cm of memlbl.east,
              anchor=west, xshift={\i*\blocksize}] (s2_l1_\i) {};
    \fi
}
...
\foreach \i/\col in {1/blue!60, 2/blue!60, 3/green!60, 4/green!60, 5/yellow!60, 6/yellow!60} {
    \ifnum\i=1
        \node[block, pattern=north east lines, pattern color=\col,
              below=1.5cm of memlbl, anchor=north, xshift={\i*\blocksize}] (s2_mem_\i) {};
    \else
        \node[block, fill=\col, below=1.5cm of memlbl,
              anchor=north, xshift={\i*\blocksize}] (s2_mem_\i) {};
    \fi
}

% Arrow 2
\draw[arrow] ([xshift=1.5cm]s2_mem_6.east) -- ++(3,0) 
    node[midway, above, align=center] {Eviction to\\make room};

% State 3: Final
\node[state, right=12cm of l1lbl] (state3) {};
\foreach \i/\col in {1/yellow!60, 2/yellow!60, 3/green!60, 4/green!60} {
    \node[block, fill=\col, right=0cm of state3.east,
          anchor=west, xshift={\i*\blocksize}] (s3_l1_\i) {};
}

\foreach \i/\col in {1/blue!60, 2/blue!60, 3/green!60, 4/green!60, 5/yellow!60, 6/yellow!60} {
    \ifnum\i=1
        \node[block, pattern=north east lines, pattern color=\col,
              below=1.5cm of state3, anchor=north, xshift={\i*\blocksize}] (s3_mem_\i) {};
    \else
        \node[block, fill=\col, below=1.5cm of state3,
              anchor=north, xshift={\i*\blocksize}] (s3_mem_\i) {};
    \fi
}
