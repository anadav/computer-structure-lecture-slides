...
\node[label, font=\small] (addr_title) at (0, 4.5) {Address Fields};
\node[draw, thick, fill=green!20, minimum width=5.5cm, minimum height=0.7cm, font=\small] (tag_field) at (-0.5, 3.7) {Tag = Block\#};
...
\node[label, below=0.4cm of offset_field.south, xshift=1.5cm] (data_array_label) {Data array};
...
\foreach \i in {0,...,7} {
    ...
    % Data array
    \node[databox, right=0.8cm of comp\i] (data\i) {};
    \ifnum\i=0
        \node[label, font=\tiny] at (data\i.west) [anchor=west, xshift=1mm] {Line};
        % Orange box only on first line
        \node[offsetbox, anchor=west] at (data\i.east) (offset\i) {};
    \fi
}

% Mux below data array
\node[muxdemux, muxdemux def={Lh=2.5, Rh=1, NL=8, NR=1, w=1.2},
      external pins width=0, fill=cyan!20, below=0.6cm of data4.south] (mux) {};

% Tag input vertical line
\coordinate (tag_input_top) at ([xshift=-0.5cm]comp3.west);
\draw[->, >=stealth, thick] (tag_field.south) -- (tag_field.south |- tag_input_top) -| (tag_input_top);

% Connect tag input to all comparators
\foreach \i in {0,...,7} {
    \draw[->, >=stealth, thick] (tag_input_top) -- (tag_input_top -| comp\i.lpin 1) -- (comp\i.lpin 1);
}
...
\foreach \i in {0,...,7} {
    ...
    \draw[->, >=stealth, thick] (data\i.south) -- (data\i.south |- mux.blpin \pin) -- (mux.blpin \pin);
}

% OR gate for hit/miss
\node[or port, anchor=north, external pins width=0, fill=cyan!20,
      scale=0.4, rotate=-90, below=0.2cm of comp7.south] (or_gate) {};

% Connect comparators to OR gate
\coordinate (or_input) at ([yshift=0.3cm]or_gate.north);
\foreach \i in {0,...,7} {
    \draw[->, >=stealth, thick] (comp\i.bpin 1) -- (comp\i.bpin 1 |- or_input) -- (or_input);
}
\draw[->, >=stealth, thick] (or_input) -- (or_gate.bin 1);
...
% Data output
\node[label, below=0.2cm of mux.south, font=\bfseries] (data_out) {data};
\draw[->, >=stealth, thick] (mux.brpin 1) -- (data_out.north);

% Offset input to mux control
\coordinate (offset_input) at ([xshift=1cm]mux.east);
\draw[->, >=stealth, thick] (offset_field.south) |- (offset_input) |- (mux.north);

% Bit labels at bottom
\node[label, font=\tiny, below=0.2cm of tag7.south] {31};
\node[label, font=\tiny, below=0.2cm of data7.south east, xshift=3mm] {0};
