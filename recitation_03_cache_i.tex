...
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[tagcell, anchor=north] at (tag0_start) (tag0_\i) {\scriptsize Tag};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
        \node[tagcell, below=0cm of tag0_\prev.south, anchor=north] (tag0_\i) {};
    \fi
}
...
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[cellstyle, anchor=west] at (data0_start) (data0_\i) {\scriptsize Data};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
        \node[cellstyle, below=0cm of data0_\prev.south, anchor=north] (data0_\i) {};
    \fi
}
...
\node[font=\fontsize{6}{6}\selectfont, rotate=90] at (comp0.center) {=};
...
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[tagcell, anchor=north] at (tag1_start) (tag1_\i) {\scriptsize Tag};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
        \node[tagcell, below=0cm of tag1_\prev.south, anchor=north] (tag1_\i) {};
    \fi
}
...
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[cellstyle, anchor=west] at (data1_start) (data1_\i) {\scriptsize Data};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
        \node[cellstyle, below=0cm of data1_\prev.south, anchor=north] (data1_\i) {};
    \fi
}
...
\node[font=\fontsize{6}{6}\selectfont, rotate=90] at (comp1.center) {=};
...
\node[muxdemux, muxdemux def={Lh=0.5, Rh=1, NL=2, NB=1, w=0.8},
      external pins width=0, fill=gray!20, rotate=90] at ($(data0_3.south)!0.5!(data1_3.south) + (0,-2)$) (mux) {};

% Output labels
\node[below=0.8cm of orgate] (hitmiss) {Hit/Miss};
\node[below=0.5cm of mux.bpin 1] (dataout) {Data Out};
...
\draw[arrow] (comp1.rpin 1) |- (mux.north);

% MUX to data out
\draw[arrow] (mux.bpin 1) -- (dataout.north);
