...
\begin{frame}{Q: LRU Implementation -- How Many Bits?}
...
\begin{itemize}
    ...
    \item 2 bits per way $\times$ 4 ways = \textbf{8 bits}
\end{itemize}

\begin{center}
\begin{tikzpicture}[
    way/.style={minimum width=1cm, minimum height=0.7cm, font=\footnotesize\bfseries},
    ...
    rowlabel/.style={font=\footnotesize, minimum height=0.7cm, text depth=0.15cm},
...
\matrix[matrix of nodes,
    ...
    \only<2->{Hit way 1:} \& |[alt=<2->{way1}{way}]| \only<2->{01} \& |[alt=<2->{way0}{way}]| \only<2->{00} \& |[alt=<2->{way2}{way}]| \only<2->{10} \& |[alt=<2->{way3}{way}]| \only<2->{11} \\
    \only<3->{Hit way 2:} \& |[alt=<3->{way2}{way}]| \only<3->{10} \& |[alt=<3->{way1}{way}]| \only<3->{01} \& |[alt=<3->{way0}{way}]| \only<3->{00} \& |[alt=<3->{way3}{way}]| \only<3->{11} \\
    \only<4->{Hit way 3:} \& |[alt=<4->{way3}{way}]| \only<4->{11} \& |[alt=<4->{way2}{way}]| \only<4->{10} \& |[alt=<4->{way1}{way}]| \only<4->{01} \& |[alt=<4->{way0}{way}]| \only<4->{00} \\
};
% Headers
\node[header, text=blue!70, above=3mm of m-1-2] {MRU};
\node[header, text=blue!30!black, above=3mm of m-1-5] {LRU};

% On slide 5+: highlight that rightmost is evicted, leftmost not stored
\only<5->{
    \node[fit=(m-1-5)(m-4-5), draw=red, thick, rounded corners, inner sep=2pt, label={[font=\tiny, text=red]below:evicted}] {};
    \node[fit=(m-1-2)(m-4-2), draw=gray, dashed, rounded corners, inner sep=2pt, label={[font=\tiny, text=gray]below:not stored}] {};
    \fill[white, opacity=0.5] (m-1-2.north west) rectangle (m-4-2.south east);
}
\end{tikzpicture}
\end{center}

\only<5->{\textbf{Optimization:} Store only 3 ways $\rightarrow$ 4th is implicitly LRU $\rightarrow$ \textbf{6 bits}}
\end{frame}
