...
\tikzset{
    ...
    titlecell/.style={cellstyle, font=\footnotesize},
    decoder/.style={trapezium, trapezium angle=70, draw, fill=green!30, 
                   minimum width=0.8cm, minimum height=1cm, rotate=90},
...
\matrix[matrix of nodes, nodes={cellstyle}, 
        column sep=-\pgflinewidth, row sep=-\pgflinewidth, font=\scriptsize] 
...
% Way 1 decoder (demux)
\node[decoder] at (way1base) (decoder1) {};
\node[left=3mm of decoder1, font=\small] {Set\#};

% Way 1 Tag array matrix
\matrix[matrix of nodes, nodes={tagcell}, 
        column sep=-\pgflinewidth, row sep=-\pgflinewidth,
        right=8mm of decoder1.east, anchor=west] (tag1) {
    |[titlecell]| Tag \\
    \\
    $\vdots$ \\
    \\
...
% Decoder to tag arrays (dashed lines)
\foreach \i in {2,3,4} {
    \draw[dasharrow] (decoder0.east) -- ([yshift={-(\i-1)*0.35cm}]tag0.west);
    \draw[dasharrow] (decoder1.east) -- ([yshift={-(\i-1)*0.35cm}]tag1.west);
}
