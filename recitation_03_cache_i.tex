...
\textbf{Solution:}
\begin{itemize}
    \item Array size: 1024 ints × 4 bytes = 4096 bytes
    \item Block size: 16 bytes (4 ints per block)
    \item Total blocks needed: 4096/16 = 256 blocks
...
\begin{itemize}
    ...
    \item Miss rate = 256/1024 = 0.25 = 25\%
...
\textbf{Question:} Which locality principle is demonstrated?
...
\section{Types of Misses}
\begin{frame}{Three Types of Cache Misses - Detailed}
\begin{enumerate}
    \item \textbf{Compulsory (Cold) Misses}
    \begin{itemize}
        \item Block has never been in cache before
        \item Unavoidable on first access
        \item Cannot be reduced by cache design
    \end{itemize}
    
    \item \textbf{Conflict Misses}
    \begin{itemize}
        \item Block was in cache but evicted due to mapping conflicts
        \item Another block mapped to same set took its place
        \item Can be reduced by: increasing associativity, better mapping
    \end{itemize}
    
    \item \textbf{Capacity Misses}
    \begin{itemize}
        \item Block was evicted because cache is full
        \item More blocks accessed than cache can hold
        \item Would occur even in fully associative cache
        \item Can only be reduced by: increasing cache size
    \end{itemize}
\end{enumerate}
...
Set Associative & Good compromise & Moderate complexity \\
\midrule
Write Back & Less memory traffic & Needs dirty bit \\
Write Through & Simple, consistent & More memory traffic \\
\midrule
Large blocks & Better spatial locality & More transfer time \\
Small blocks & Less wasted transfer & Poor spatial locality \\
...
\begin{itemize}
    ...
    \item Different miss types require different optimizations:
    \begin{itemize}
        \item Compulsory: Prefetching
        \item Conflict: Increase associativity
        \item Capacity: Increase cache size
    \end{itemize}
    \item Real systems use multi-level cache hierarchies (L1, L2, L3)
\end{itemize}
\end{frame}
