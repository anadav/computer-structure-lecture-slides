...
\coordinate (way0base) at ($(set.south) + (-2,-2.5)$);
\coordinate (way1base) at ($(set.south) + (4,-2.5)$);

% Way 0 Tag array - using for loop (create first to align demux)
\coordinate (tag0_start) at ($(way0base) + (2.5,0.5)$);
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[tagcell, anchor=north] at (tag0_start) (tag0_\i) {\tiny Tag};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
...
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[cellstyle, anchor=west] at (data0_start) (data0_\i) {\tiny Data};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
...
\node[muxdemux, muxdemux def={Lh=0.3, Rh=0.6, NL=2, NB=1, w=0.5},
      external pins width=0, fill=green!30, rotate=90, below=0.8cm of tag0_3] (comp0) {};
...
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[tagcell, anchor=north] at (tag1_start) (tag1_\i) {\tiny Tag};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
...
\foreach \i in {0,...,3} {
    \ifnum\i=0
        \node[cellstyle, anchor=west] at (data1_start) (data1_\i) {\tiny Data};
    \else
        \pgfmathtruncatemacro{\prev}{\i-1}
...
\draw[arrow] (setjunction) -| (decoder0.lpin 1);
\draw[arrow] (setjunction) -| (decoder1.lpin 1);
...
\draw[arrow] (tag0_3.south) -- (comp0.lpin 2);
\draw[arrow] (tag1_3.south) -- (comp1.lpin 2);
