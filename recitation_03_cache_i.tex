...
\tikzset{
    % Style definitions for cache architecture
    ...
    orgatestyle/.style={or gate US, draw, logic gate inputs=nn, anchor=center,
                       minimum width=0.5cm, minimum height=0.5cm, rotate=270},
    % Junction dot style
...
\node[anchor=south] at (set.north) {\footnotesize 5};
...
\node[muxdemux, muxdemux def={Lh=1, Rh=4, NB=0, NL=1, NR=4, w=0.8, square pins=0},
      external pins width=0, fill=cyan!20, anchor=rpin 1] at ($(tag0_0.west) + (-0.5,0)$) (decoder0) {};
...
% Way 0 comparator - using muxdemux rotated 270 degrees
\node[muxdemux, muxdemux def={Lh=0.8, Rh=0.4, NL=2, NB=1, w=0.7},
      external pins width=0, fill=green!30, rotate=270] at ([xshift=5mm, yshift=-0.5cm] tag0_3.south east) (comp0) {};
\node[font=\fontsize{8}{8}\selectfont] at (comp0.center) {=};

% Way 0 label
\node[above=5mm of tag0_0.north] (way0label) {\textbf{Way 0}};
...
\node[muxdemux, muxdemux def={Lh=1, Rh=4, NB=0, NL=1, NR=4, w=0.8, square pins=0},
      external pins width=0, fill=cyan!20, anchor=rpin 1] at ($(tag1_0.west) + (-0.5,0)$) (decoder1) {};
...
% Way 1 comparator - using muxdemux rotated 270 degrees (matching Way 0)
\node[muxdemux, muxdemux def={Lh=0.8, Rh=0.4, NL=2, NB=1, w=0.7},
      external pins width=0, fill=green!30, rotate=270] at ([xshift=15mm, yshift=-3.5cm] tag1_3.south east) (comp1) {};
\node[font=\fontsize{8}{8}\selectfont] at (comp1.center) {=};

% Way 1 label
\node[above=5mm of tag1_0.north] (way1label) {\textbf{Way 1}};
...
\node[orgatestyle] at ($(comp0)!0.5!(comp1) + (0,-1.8)$) (orgate) {};

% MUX - using muxdemux, rotated 90 degrees, positioned relative to data arrays
\node[muxdemux, muxdemux def={Lh=2, Rh=1, NL=2, NB=2, w=1},
      external pins width=0, fill=gray!20, rotate=270, font=\scriptsize, anchor=lpin 1]
      at ([yshift=-1.2cm]data1_3.south) (mux) {\rotatebox{90}{MUX}};

% Output labels
\node[below=3mm of orgate.output] (hitmiss) {Hit/Miss};
...
\draw[arrow] (offsetjunction) -| (data0_0.north);
\draw[arrow] (offsetjunction) -| (data1_0.north);
...
\draw[arrow] (comp0.rpin 1) |- (mux.bpin 2);
\draw[arrow] (comp1.rpin 1) |- (mux.bpin 1);
...
\begin{frame}{Q2: Cache Directory Size}
...
\begin{frame}{Q2a: Number of Misses}
...
\begin{frame}{Q2b: Effect of Alignment}
...
\begin{frame}{Q2c: No Write Allocate}
...
\begin{frame}{Q2d: Locality Principles}
...
\section{Question 2: Cache Directory and Array Analysis}
...
\section{Question 3: Cache Access Pattern}
\begin{frame}{Q3: Cache Access Pattern}
\begin{columns}
\column{0.5\textwidth}
\textbf{Given:}
\begin{itemize}
...
\section{Review: LRU Implementation}
...
\node[muxdemux, muxdemux def={Lh=0.6, Rh=0.3, NL=2, NB=1, w=0.5},
      external pins width=0, fill=green!30, rotate=270] at ([xshift=1.5cm, yshift=-0.5cm] tag1_3.south west) (comp1) {};
...
% OR gate - positioned between comparators
\node[orgatestyle, rotate=180] at ($(comp0)!0.5!(comp1) + (0,-1.5)$) (orgate) {};
...
\node[muxdemux, muxdemux def={Lh=1, Rh=0.5, NL=2, NB=2, NR=1, w=0.8},
      external pins width=0, fill=gray!20, rotate=270,anchor=lpin 1] at ([yshift=-1cm]data1_3.south) (mux) {};
...
% Comparator outputs to OR gate
\draw[arrow] (comp0.rpin 1) |- ([yshift=3mm]orgate.input 1) -- (orgate.input 1);
\draw[arrow] (comp1.rpin 1) |- ([yshift=3mm]orgate.input 2) -- (orgate.input 2);

% OR gate to Hit/Miss
\draw[arrow] (orgate.output) -- (hitmiss.north);

% Comparator to MUX select (using comp1 output)
\draw[arrow] (comp0.rpin 1) |- (mux.bpin 1);
\draw[arrow] (comp1.rpin 1) |- (mux.bpin 2);
...
\section{Review: Advanced Cache Topics}
\begin{frame}{LRU Implementation for 4-Way Cache}
\textbf{Method 1: Full List (8 bits per set)}
\begin{itemize}
    \item Maintain linked list of 4 nodes (one per way)
...
\section{Question 4: Segment-Based Cache Design}
\begin{frame}{Q4: Segment-Based Cache Design}
\textbf{Problem Setup:}
\begin{itemize}
...
\begin{frame}{Q4c: Concrete Example}
