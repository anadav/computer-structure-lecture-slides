...
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, calc, tikzmark, shapes.misc, patterns, fit, circuits.logic.US, matrix, backgrounds, shapes.callouts, decorations.pathreplacing}

% Beamer overlay style for TikZ
\tikzset{
...
% Address breakdown macro with optional block number arrow
% Usage: \addressbreakdown{tag bits}{set bits}{offset bits}
%        \addressbreakdown[blocknumber]{tag bits}{set bits}{offset bits}
\NewDocumentCommand{\addressbreakdown}{O{} m m m}{%
    \begin{tikzpicture}[
    ...
    \pgfmathtruncatemacro{\totalbits}{#2 + #3 + #4}
    \pgfmathtruncatemacro{\tagstart}{\totalbits - 1}
    \pgfmathtruncatemacro{\tagend}{\totalbits - #2}
    \pgfmathtruncatemacro{\setstart}{\tagend - 1}
    \pgfmathtruncatemacro{\setend}{\tagend - #3}
    ...
    % Draw tag block
    \ifnum#2>0
        \pgfmathsetmacro{\tagwidth}{#2 * \bitwidth}
    ...
    \ifnum#3>0
        ...
        % Add bit numbers below - only one number if single bit
        \ifnum#3=1
            \node[bitnumber] at ($(set.south) + (0mm,-1mm)$) {\setstart};
        \else
    ...
    % Draw offset block
    \ifnum#4>0
        \pgfmathsetmacro{\offsetwidth}{#4 * \bitwidth}
    ...
    % Draw block number arrow if option is specified
    \IfStrEq{#1}{blocknumber}{%
        \ifnum#3>0
            % Arrow spans tag and set
            \draw[<->, thick] ([yshift=2mm]tag.north west) -- ([yshift=2mm]set.north east)
                node[midway, above, font=\scriptsize] {block number};
        \else
            % Arrow spans only tag (no set bits)
...
\subsection{Motivation \& Locality}
...
\subsection{Terminology \& Organizations}
...
\begin{center}
    \addressbreakdown[blocknumber]{27}{0}{5}
\end{center}
\end{tcolorbox}
...
\begin{frame}{Fully Associative -- Any-to-Any Mapping}
\centering
\begin{tikzpicture}[
    basecell/.style={draw, thick, minimum height=0.38cm, font=\footnotesize, text depth=0.08cm, inner sep=1pt},
    memblock/.style={basecell, minimum width=2.0cm, fill=white},
    memhighlight/.style={basecell, minimum width=2.0cm, fill=gray!20, font=\footnotesize\bfseries},
    tagcell/.style={basecell, minimum width=1.2cm, fill=green!30},
    datacell/.style={basecell, minimum width=1.5cm, fill=white},
    header/.style={font=\small\bfseries},
    dots/.style={font=\normalsize, minimum height=0.35cm}
]

% Memory blocks on the left
\matrix[matrix of nodes,
        ampersand replacement=\&,
        row sep=0mm,
        nodes={memblock},
        label={[header]above:Main Memory}
] (mem) {
    Block 1 \\
    Block 2 \\
    |[memblock]| {} \\
    |[memblock]| {} \\
    |[memhighlight]| Block m \\
    |[memblock]| {} \\
    |[memblock]| {} \\
    |[dots, fill=none, draw=none]| \vdots \\
    |[memblock]| {} \\
    |[memhighlight]| Block n \\
    |[memblock]| {} \\
};
...
\matrix[matrix of nodes,
        ...
        row sep=0mm,
        column sep=0mm,
        label={[header]above:Cache}
] (cache) {
    |[font=\scriptsize\bfseries, draw=none, fill=none]| Tag \& |[font=\scriptsize\bfseries, draw=none, fill=none]| Data \\
    |[tagcell]| \& |[datacell]| \\
    |[tagcell]| \& |[datacell]| \\
    |[tagcell]| \& |[datacell]| \\
    |[tagcell]| \& |[datacell]| \\
...
% Arrows from Block m (black) to ALL cache lines
\foreach \i in {2,...,9} {
    \draw[->, thick] (mem-5-1.east) -- (cache-\i-1.west);
}

% Arrows from Block n (red/magenta) to ALL cache lines
\foreach \i in {2,...,9} {
    \draw[->, thick, red!70!black] (mem-10-1.east) -- (cache-\i-1.west);
}

% Label explaining the mapping
\node[below=6mm of mem, font=\footnotesize, align=center, text width=3cm] {Any block can map\\to any cache line};

% Advantages/Disadvantages
\node[below=6mm of cache, font=\footnotesize, align=left] (pros) {
    \textcolor{green!50!black}{\textbf{+}} High hit rate\\
    \textcolor{red!70!black}{\textbf{--}} Expensive hardware
};
...
\begin{itemize}
    ...
    \item \textbf{Advantage:} Simple and cheap
    \item \textbf{Disadvantage:} Higher miss rate due to conflicts
\end{itemize}
...
\textbf{Address Fields:}\\
\begin{center}
    \addressbreakdown[blocknumber]{24}{3}{5}
\end{center}
\vspace{0.2cm}
\footnotesize
$3$ set bits $\Rightarrow 2^3 = 8$ sets\\
$5$ offset bits $\Rightarrow 2^5 = 32$ bytes/block
\end{tcolorbox}
\end{columns}
\end{frame}

\begin{frame}{Direct Mapping -- Fixed Location per Block}
\centering
\begin{tikzpicture}[
    basecell/.style={draw, thick, minimum height=0.38cm, font=\footnotesize, text depth=0.08cm, inner sep=1pt},
    memblock/.style={basecell, minimum width=3.8cm, fill=white},
    memhighlight00/.style={basecell, minimum width=3.8cm, fill=white, font=\footnotesize\bfseries},
    memhighlight10/.style={basecell, minimum width=3.8cm, fill=white, font=\footnotesize\bfseries},
    tagcell/.style={basecell, minimum width=1.2cm, fill=green!30},
    datacell/.style={basecell, minimum width=1.5cm, fill=white},
    setlabel/.style={font=\scriptsize\bfseries},
    header/.style={font=\small\bfseries},
    dots/.style={font=\normalsize, minimum height=0.35cm},
    cachesize/.style={font=\scriptsize\bfseries, align=center},
]
...
\matrix[matrix of nodes,
    ...
    |[memhighlight00]| block 0 (set = 00) \\
    |[memblock]| {} \\
    |[memhighlight10]| block 2 (set = 10) \\
    |[memblock]| {} \\
    |[memhighlight00]| block 4 (set = 00) \\
    |[memblock]| {} \\
    |[memhighlight10]| block 6 (set = 10) \\
    |[memblock]| {} \\
    |[dots, fill=none, draw=none]| \vdots \\
    |[memhighlight00]| block 4n (set = 00) \\
    |[memblock]| {} \\
    |[memhighlight10]| block 4n+2 (set = 10) \\
    |[memblock]| {} \\
};

% Cache Size brackets on the left
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick]
    ([xshift=-3mm]mem-1-1.north west) -- ([xshift=-3mm]mem-4-1.south west)
    node[midway, left=5pt, cachesize] {Cache\\Size};
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick]
    ([xshift=-3mm]mem-5-1.north west) -- ([xshift=-3mm]mem-8-1.south west)
    node[midway, left=5pt, cachesize] {Cache\\Size};
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick]
    ([xshift=-3mm]mem-10-1.north west) -- ([xshift=-3mm]mem-13-1.south west)
    node[midway, left=5pt, cachesize] {Cache\\Size};
...
\matrix[matrix of nodes,
        ...
        row sep=0mm,
        column sep=0mm,
        column 1/.style={nodes={setlabel, minimum width=0.8cm}},
        label={[header]above:Cache}
] (cache) {
    |[font=\scriptsize\bfseries, draw=none, fill=none]| Set\# \& |[font=\scriptsize\bfseries, draw=none, fill=none]| Tag \& |[font=\scriptsize\bfseries, draw=none, fill=none]| Data \\
    |[draw=none, fill=none]| \& |[tagcell]| \& |[datacell]| \\
    |[draw=none, fill=none]| \& |[tagcell]| \& |[datacell]| \\
    |[draw=none, fill=none]| \& |[tagcell]| \& |[datacell]| \\
    |[draw=none, fill=none]| \& |[tagcell]| \& |[datacell]| \\
};

% Arrows for set 00 (black) - blocks 0, 4, 4n all go to cache row 00
\draw[->, thick] (mem-1-1.east) -- (cache-2-2.west);
\draw[->, thick] (mem-5-1.east) -- (cache-2-2.west);
\draw[->, thick] (mem-10-1.east) -- (cache-2-2.west);

% Arrows for set 10 (red) - blocks 2, 6, 4n+2 all go to cache row 10
\draw[->, thick, red!70!black] (mem-3-1.east) -- (cache-4-2.west);
\draw[->, thick, red!70!black] (mem-7-1.east) -- (cache-4-2.west);
\draw[->, thick, red!70!black] (mem-12-1.east) -- (cache-4-2.west);

% Set# labels with semi-transparent background (drawn on top of arrows)
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache-2-1) {00};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache-3-1) {01};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache-4-1) {10};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache-5-1) {11};

% Label explaining the mapping
\node[below=-2mm of mem, font=\footnotesize, align=center, text width=3.5cm] {Each block maps to\\exactly one cache line};

% Advantages/Disadvantages
\node[below=6mm of cache, font=\footnotesize, align=left] (pros) {
    \textcolor{green!50!black}{\textbf{+}} Simple \& cheap\\
    \textcolor{red!70!black}{\textbf{--}} Conflicts $\to$ higher miss rate
};
...
\column{0.5\textwidth}
\begin{tcolorbox}[colback=gray!10]
\textbf{Address Fields (2-Way):}\\
\begin{center}
    \addressbreakdown[blocknumber]{25}{2}{5}
\end{center}
\vspace{0.2cm}
\footnotesize
$2$ set bits $\Rightarrow 2^2 = 4$ sets\\
$5$ offset bits $\Rightarrow 2^5 = 32$ bytes/block\\
Same cache size as direct mapped!
\end{tcolorbox}
\end{columns}
\end{frame}
...
\begin{frame}{2-Way Set Associative -- Mapping}
\centering
\begin{tikzpicture}[
    basecell/.style={draw, thick, minimum height=0.38cm, font=\footnotesize, text depth=0.08cm, inner sep=1pt},
    memblock/.style={basecell, minimum width=3.8cm, fill=white},
    memhighlight00/.style={basecell, minimum width=3.8cm, fill=white, font=\footnotesize\bfseries},
    memhighlight10/.style={basecell, minimum width=3.8cm, fill=white, font=\footnotesize\bfseries},
    tagcell/.style={basecell, minimum width=1.0cm, fill=green!30},
    datacell/.style={basecell, minimum width=1.2cm, fill=white},
    setlabel/.style={font=\scriptsize\bfseries},
    header/.style={font=\small\bfseries},
    wayheader/.style={font=\footnotesize\bfseries},
    dots/.style={font=\normalsize, minimum height=0.35cm},
    waysize/.style={font=\scriptsize\bfseries, align=center},
]
...
\matrix[matrix of nodes,
    ...
    |[memhighlight00]| 0 (set = 00) \\
    |[memblock]| {} \\
    |[memhighlight10]| 2 (set = 10) \\
    |[memblock]| {} \\
    |[memhighlight00]| 4 (set = 00) \\
    |[memblock]| {} \\
    |[memhighlight10]| 6 (set = 10) \\
    |[memblock]| {} \\
    |[dots, fill=none, draw=none]| \vdots \\
    |[memhighlight00]| 4n (set = 00) \\
    |[memblock]| {} \\
    |[memhighlight10]| 4n+2 (set = 10) \\
    |[memblock]| {} \\
};

% Way Size brackets on the left (spanning 4 rows each)
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick]
    ([xshift=-3mm]mem-1-1.north west) -- ([xshift=-3mm]mem-4-1.south west)
    node[midway, left=5pt, waysize] {Way\\Size};
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick]
    ([xshift=-3mm]mem-5-1.north west) -- ([xshift=-3mm]mem-8-1.south west)
    node[midway, left=5pt, waysize] {Way\\Size};
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick]
...
% WAY 0 cache
\node[wayheader, right=3.5cm of mem.north east, anchor=north] (way0title) {WAY 0};
\matrix[matrix of nodes,
        ampersand replacement=\&,
        below=1mm of way0title,
        row sep=0mm,
        column sep=0mm,
        column 1/.style={nodes={setlabel, minimum width=0.6cm}},
] (cache0) {
    |[font=\scriptsize\bfseries, draw=none, fill=none]| Set\# \& |[font=\scriptsize\bfseries, draw=none, fill=none]| Tag \& |[font=\scriptsize\bfseries, draw=none, fill=none]| Data \\
    |[draw=none, fill=none]| \& |[tagcell]| \& |[datacell]| \\
    |[draw=none, fill=none]| \& |[tagcell]| \& |[datacell]| \\
...
% WAY 1 cache
\node[wayheader, right=30mm of way0title] (way1title) {WAY 1};
\matrix[matrix of nodes,
        ampersand replacement=\&,
        below=1mm of way1title,
        row sep=0mm,
        column sep=0mm,
        column 1/.style={nodes={setlabel, minimum width=0.6cm}},
] (cache1) {
...
% Arrows for set 10 - each block can go to either Way 0 or Way 1
% Lines to Way 0 go straight, lines to Way 1 curve below both caches
% Block 2 (black)
\draw[->, thick] (mem-3-1.east) -- (cache0-4-2.west);
\draw[->, thick] (mem-3-1.east) to[out=-15, in=180] ([yshift=-5mm]cache0.south) to[out=0, in=-160] (cache1-4-2.west);
% Block 6 (red)
\draw[->, thick, red!70!black] (mem-7-1.east) -- (cache0-4-2.west);
\draw[->, thick, red!70!black] (mem-7-1.east) to[out=-10, in=180] ([yshift=-8mm]cache0.south) to[out=0, in=-170] (cache1-4-2.west);
% Block 4n+2 (blue)
\draw[->, thick, blue!70] (mem-12-1.east) -- (cache0-4-2.west);
\draw[->, thick, blue!70] (mem-12-1.east) to[out=-5, in=180] ([yshift=-11mm]cache0.south) to[out=0, in=-175] (cache1-4-2.west);

% Set# labels with semi-transparent background (drawn on top of arrows)
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache0-2-1) {00};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache0-3-1) {01};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache0-4-1) {10};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache0-5-1) {11};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache1-2-1) {00};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache1-3-1) {01};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache1-4-1) {10};
\node[font=\scriptsize\bfseries, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (cache1-5-1) {11};

% Label explaining the mapping
\node[below=-2mm of mem, font=\footnotesize, align=center, text width=3.5cm] {Each block can map to\\one of K cache lines};

% Advantages/Disadvantages
\node[below=8mm of cache1, font=\footnotesize, align=left, anchor=north] (pros) {
    \textcolor{green!50!black}{\textbf{+}} Balanced tradeoff\\
    \textcolor{red!70!black}{\textbf{--}} Needs replacement policy
};

\end{tikzpicture}
\end{frame}

\begin{frame}{Associativity Comparison (Same Cache Size)}
\vspace{-0.3cm}
\begin{center}
\textbf{8 blocks total, 4-byte blocks (2 offset bits)}
\end{center}
\vspace{-0.1cm}
...
\centering
\textbf{1-way (Direct Mapped)}\\
$8 \times 1 = 8$ blocks\\[1mm]
\scalebox{0.8}{\addressbreakdown[blocknumber]{24}{3}{2}}\\[0mm]
{\footnotesize 3 set bits $\Rightarrow$ 8 sets}

\vspace{3mm}
\textbf{4-way Set Associative}\\
$2 \times 4 = 8$ blocks\\[1mm]
\scalebox{0.8}{\addressbreakdown[blocknumber]{26}{1}{2}}\\[0mm]
{\footnotesize 1 set bit $\Rightarrow$ 2 sets}

\column{0.48\textwidth}
\centering
\textbf{2-way Set Associative}\\
$4 \times 2 = 8$ blocks\\[1mm]
\scalebox{0.8}{\addressbreakdown[blocknumber]{25}{2}{2}}\\[0mm]
{\footnotesize 2 set bits $\Rightarrow$ 4 sets}

\vspace{3mm}
\textbf{8-way (Fully Associative)}\\
$1 \times 8 = 8$ blocks\\[1mm]
\scalebox{0.8}{\addressbreakdown[blocknumber]{27}{0}{2}}\\[0mm]
{\footnotesize 0 set bits $\Rightarrow$ 1 set}
\end{columns}

\vspace{2mm}
\begin{center}
\textcolor{blue!70!black}{\textbf{More ways $\Rightarrow$ fewer sets $\Rightarrow$ fewer set bits $\Rightarrow$ larger tags}}
\end{center}
\end{frame}

\subsection{Policies \& Miss Types}
...
\hline
\textbf{Addr} & \textcolor{blue}{\textbf{Tag}} \textcolor{green!70!black}{\textbf{Set}} \textcolor{orange}{\textbf{Off}} & \textbf{Block} & \textbf{Tag} & \textbf{Set} \\
\hline
5 & \textcolor{blue}{00}\textcolor{green!70!black}{001}\textcolor{orange}{01} & 1 & 0 & 1 \\
7 & \textcolor{blue}{00}\textcolor{green!70!black}{001}\textcolor{orange}{11} & 1 & 0 & 1 \\
1 & \textcolor{blue}{00}\textcolor{green!70!black}{000}\textcolor{orange}{01} & 0 & 0 & 0 \\
4 & \textcolor{blue}{00}\textcolor{green!70!black}{001}\textcolor{orange}{00} & 1 & 0 & 1 \\
36 & \textcolor{blue}{01}\textcolor{green!70!black}{001}\textcolor{orange}{00} & 9 & 1 & 1 \\
8 & \textcolor{blue}{00}\textcolor{green!70!black}{010}\textcolor{orange}{00} & 2 & 0 & 2 \\
6 & \textcolor{blue}{00}\textcolor{green!70!black}{001}\textcolor{orange}{10} & 1 & 0 & 1 \\
12 & \textcolor{blue}{00}\textcolor{green!70!black}{011}\textcolor{orange}{00} & 3 & 0 & 3 \\
68 & \textcolor{blue}{10}\textcolor{green!70!black}{001}\textcolor{orange}{00} & 17 & 2 & 1 \\
\hline
\end{tabular}
\end{column}
\end{columns}
...
% Savebox for Q2 code snippet
\newsavebox{\QTwoCode}
\begin{lrbox}{\QTwoCode}
...
% Saveboxes for Q2a address format animation
% bitwidth=1.6mm gives ~51mm total width, fits 40% column well
% endianness=big puts MSB (31) on left, LSB (0) on right
\newsavebox{\addrEmpty}
\begin{lrbox}{\addrEmpty}
\begin{bytefield}[bitwidth=1.6mm, bitheight=6mm]{32}
  \bitheader[endianness=big]{0,31} \\
  \bitbox{32}[bgcolor=gray!30]{}
\end{bytefield}
\end{lrbox}

\newsavebox{\addrOffset}
\begin{lrbox}{\addrOffset}
\begin{bytefield}[bitwidth=1.6mm, bitheight=6mm]{32}
  \bitheader[endianness=big]{0,3,31} \\
  \bitbox{28}[bgcolor=gray!30]{} & \bitbox{4}[bgcolor=yellow!40]{\tiny offset}
\end{bytefield}
\end{lrbox}

\newsavebox{\addrSetOffset}
\begin{lrbox}{\addrSetOffset}
\begin{bytefield}[bitwidth=1.6mm, bitheight=6mm]{32}
  \bitheader[endianness=big]{0,3,4,7,31} \\
  \bitbox{24}[bgcolor=gray!30]{} & \bitbox{4}[bgcolor=green!40]{\tiny set} & \bitbox{4}[bgcolor=yellow!40]{\tiny off}
\end{bytefield}
\end{lrbox}

\newsavebox{\addrFull}
\begin{lrbox}{\addrFull}
\begin{bytefield}[bitwidth=1.6mm, bitheight=6mm]{32}
  \bitheader[endianness=big]{0,3,4,7,8,31} \\
  \bitbox{24}[bgcolor=blue!40]{\tiny tag} & \bitbox{4}[bgcolor=green!40]{\tiny set} & \bitbox{4}[bgcolor=yellow!40]{\tiny off}
\end{bytefield}
\end{lrbox}
...
\uncover<2->{\textbf{Solution:}}
\begin{itemize}
    \item<2-> Cache size: 1KB = 1024 bytes
    \item<2-> Block size: 16 bytes $\rightarrow$ 4 offset bits
    \item<3-> Number of blocks: $\frac{1024}{16} = 64$ blocks
    \item<3-> 4-way associative $\rightarrow$ 16 sets $\rightarrow$ 4 set bits
    \item<4-> Tag bits: 32 - 4 (offset) - 4 (set) = 24 bits
    \item<5-> Per line: 24 (tag) + 1 (valid) = 25 bits
    \item<5-> No dirty bit (write-through)
    \item<5-> No LRU bits (random replacement)
\end{itemize}

\uncover<5->{\textbf{Total:} $25 \times 4 \times 16 = \mathbf{1600}$ \textbf{bits}}
...
\only<1>{\usebox{\addrEmpty}}%
\only<2>{\usebox{\addrOffset}}%
\only<3>{\usebox{\addrSetOffset}}%
\only<4->{\usebox{\addrFull}}
\end{columns}
\end{frame}

\begin{frame}{Q2b: Number of Misses}
\textbf{Question:} Maximum number of misses during execution? What type are they?
...
\textbf{Result:}
\begin{itemize}
    \item \textbf{256} misses total
    \item Miss rate = $\frac{256}{1024} = 0.25 = 25\%$
    \item All are \textbf{compulsory} misses
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Maximum misses: 257 \textbf{(one extra)}
\end{itemize}
...
\hline
A0 & \textcolor{red}{00} & 0x\textcolor{red}{0}0000000 - 0x\textcolor{red}{0}FFFFFFF \\
B0 & \textcolor{blue}{01} & 0x\textcolor{blue}{1}0000000 - 0x\textcolor{blue}{1}FFFFFFF \\
A1 & \textcolor{green!70!black}{10} & 0x\textcolor{green!70!black}{2}0000000 - 0x\textcolor{green!70!black}{2}FFFFFFF \\
B1 & \textcolor{violet}{11} & 0x\textcolor{violet}{3}0000000 - 0x\textcolor{violet}{3}FFFFFFF \\
\hline
\end{tabular}
\end{center}
\pause
...
\hline
\texttt{0x\textcolor{red}{0}5000010} & \textcolor{red}{A0} & 0 & 0-3 (A sets) \\
\texttt{0x\textcolor{blue}{1}2345678} & \textcolor{blue}{B0} & 1 & 4-7 (B sets) \\
\texttt{0x\textcolor{green!70!black}{2}5000000} & \textcolor{green!70!black}{A1} & 0 & 0-3 (A sets) \\
\texttt{0x\textcolor{violet}{3}ABCDEF0} & \textcolor{violet}{B1} & 1 & 4-7 (B sets) \\
\hline
\end{tabular}
\end{center}
...
\section{Question 4: LRU Implementation}
\begin{frame}{Q4a: LRU Implementation -- How Many Bits?}
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{Question:} Implement an LRU policy for a 4-way cache. How many bits per set?

\vspace{0.3cm}
\textbf{Method 1: Full List}
\begin{itemize}
    \item Maintain list of 4 ways in order of usage
    \item 2 bits per way $\times$ 4 ways = \textbf{8 bits} per set
\end{itemize}

\only<5->{\textbf{Observation:} LRU column is evicted on miss $\rightarrow$ can we optimize?}

\column{0.45\textwidth}
\centering
\begin{tikzpicture}[
    way/.style={minimum width=0.9cm, minimum height=0.65cm, font=\footnotesize\bfseries, text=white},
    way0/.style={way, fill=green!60!black},
    way1/.style={way, fill=blue!70},
    way2/.style={way, fill=red!50!black},
    way3/.style={way, fill=blue!30!black},
    rowlabel/.style={font=\footnotesize, minimum height=0.65cm, text depth=0.15cm},
...
\matrix[matrix of nodes,
        ...
        row sep=2mm,
        column sep=1mm,
        column 1/.style={nodes={rowlabel, anchor=east}},
] (m) {
    Initial state: \& |[way0]| 00 \& |[way1]| 01 \& |[way2]| 10 \& |[way3]| 11 \\
    Hit way 1: \& |[alt=<2->{way1}{way}]| \only<2->{01} \& |[alt=<2->{way0}{way}]| \only<2->{00} \& |[alt=<2->{way2}{way}]| \only<2->{10} \& |[alt=<2->{way3}{way}]| \only<2->{11} \\
    \only<2->{Hit way 2:} \& |[alt=<3->{way2}{way}]| \only<3->{10} \& |[alt=<3->{way1}{way}]| \only<3->{01} \& |[alt=<3->{way0}{way}]| \only<3->{00} \& |[alt=<3->{way3}{way}]| \only<3->{11} \\
    \only<3->{Hit way 3:} \& |[alt=<4->{way3}{way}]| \only<4->{11} \& |[alt=<4->{way2}{way}]| \only<4->{10} \& |[alt=<4->{way1}{way}]| \only<4->{01} \& |[alt=<4->{way0}{way}]| \only<4->{00} \\
};
% Headers with arrow
\node[header, above=3mm of m-1-2] (mruhead) {MRU};
\node[header, above=3mm of m-1-5] (lruhead) {LRU};
\draw[->, thick] (mruhead) -- (lruhead);

% On slide 5+: highlight that rightmost is evicted on miss
\only<5->{
    \node[fit=(m-1-5)(m-4-5), draw=red, thick, rounded corners, inner sep=2pt,
          label={[font=\tiny, text=red, align=center]below:evicted\\on miss}] {};
}
\end{tikzpicture}
\end{columns}
\end{frame}
\begin{frame}{Q4b: LRU Implementation -- Optimized Methods}
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{Method 2: Partial List}
\begin{itemize}
    \item Store only 3 most recent ways
    \item 4th way is implicitly LRU
    \item 2 bits $\times$ 3 = \textbf{6 bits} per set
\end{itemize}

\only<2->{
\vspace{0.5cm}
\textbf{Method 3: Optimal Encoding}
\begin{itemize}
    \item Number of possible orderings: $4! = 24$
    \item Need $\lceil \log_2(24) \rceil = \textbf{5 bits}$
...
\matrix[matrix of nodes,
    ...
    Initial state: \& |[way0]| 00 \& |[way1]| 01 \& |[way2]| 10 \& |[way3]| 11 \\
    Hit way 1: \& |[way1]| 01 \& |[way0]| 00 \& |[way2]| 10 \& |[way3]| 11 \\
    Hit way 2: \& |[way2]| 10 \& |[way1]| 01 \& |[way0]| 00 \& |[way3]| 11 \\
    Hit way 3: \& |[way3]| 11 \& |[way2]| 10 \& |[way1]| 01 \& |[way0]| 00 \\
};
% Headers with arrow
\node[header, above=3mm of m-1-2] (mruhead) {MRU};
\node[header, above=3mm of m-1-5] (lruhead) {LRU};
...
% Highlight: rightmost is evicted on miss
\node[fit=(m-1-5)(m-4-5), draw=red, thick, rounded corners, inner sep=2pt,
      label={[font=\tiny, text=red, align=center]below:evicted\\on miss}] (lrufit) {};
% Callout for "not stored" - below table, pointing to LRU column
\node[rectangle callout, callout absolute pointer={(lrufit.south west)},
      draw, fill=yellow!20, font=\scriptsize, align=center]
      at ([yshift=-12mm]m.south) {not stored};
\end{tikzpicture}
\end{columns}
\end{frame}
