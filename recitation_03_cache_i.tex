...
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, calc, tikzmark, shapes.misc, patterns, fit, circuits.logic.US, matrix}
...
\tikzset{
    ...
    muxshape/.style={trapezium, trapezium angle=70, draw, minimum width=1cm, minimum height=0.8cm},
...
% Comparators
\node[comparator, below=8mm of tag0] (comp0) {$=$};
\node[comparator, below=8mm of tag1] (comp1) {$=$};

% OR gate - using built-in anchors
\node[orgatestyle, below=1.5cm of $(comp0)!0.5!(comp1)$] (orgate) {};

% MUX - still custom but with defined anchors
\node[muxshape, below=2cm of data1] (mux) {MUX};
% Define MUX connection points as separate coordinates
...
\draw[arrow] (tagjunction) -| ([xshift=-0.3cm]comp0.north west);
\draw[arrow] (tagjunction) -| ([xshift=-0.3cm]comp1.north west);
...
\draw[arrow] (tag0.south) -- (comp0.north);
\draw[arrow] (tag1.south) -- (comp1.north);

% Data array to MUX connections
\draw[arrow] (data0.south) |- (muxin0);
\draw[arrow] (data1.south) |- (muxin1);

% Comparator outputs to OR gate - using built-in anchors
\draw[arrow] (comp0.south) |- (orgate.input 1);
\draw[arrow] (comp1.south) |- (orgate.input 2);

% OR gate to Hit/Miss - using built-in anchor
\draw[arrow] (orgate.output) -- (hitmiss.north);

% Comparator to MUX select
\draw[arrow] (comp1.south) -- ++(0,-0.3) -| (muxsel);
