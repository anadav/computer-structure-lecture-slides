...
\node[muxdemux, muxdemux def={NT=2, NB=1, NL=2, NR=2, Lh=0.9, Rh=1.8, w=0.5}, external pins width=0, anchor=brpin 1, rotate=90] 
    (tlbmux) at ([yshift=-8mm]tlbval-3-0.south east) {};
\node[font=\tiny, align=center] at (tlbmux) {Way MUX};

% Physical page# output - below mux
\node[addr, fill=orange!20, minimum width=1.2cm] (ppn) at ([yshift=-5mm]tlbmux.bbpin 1) {PPN};
...
\node[muxdemux, muxdemux def={NB=0, NT=1, NR=8, NL=1, Lh=1, Rh=3, w=0.5}, below=2mm of cachelabel, external pins width=0] 
    (cachedec) {};
...
\node[muxdemux, muxdemux def={NT=4, NB=1, NL=4, NR=4, Lh=1.2, Rh=2.4, w=1}, external pins width=0, anchor=brpin 1, rotate=90] 
    (cachemux) at ([yshift=-8mm, xshift=4mm]cdata-7-1.south east) {};
...
\node[addr, fill=yellow!30, minimum width=0.8cm] (dataout) at ([yshift=-5mm]cachemux.bbpin 1) {Data};
...
\foreach \way in {0,...,1} {
    ...
    \draw[arrow] (tlbcomp-\way.south) -- ++(0,-0.1) -| (tlbor.in \inputnum);
}

% TLB values to mux - connect to top pins of MUX
\foreach \way in {0,...,1} {
    \pgfmathtruncatemacro{\muxpin}{\way + 1}
    \draw[arrow, gray] (tlbval-3-\way.south) -- ++(0,-0.3) -| (tlbmux.brpin \muxpin);
}

% TLB mux output to physical page#
\draw[arrow] (tlbmux.bbpin 1) -- (ppn.north);
...
% Cache data to mux - connect to top pins of MUX
\foreach \way in {0,...,3} {
    \pgfmathtruncatemacro{\muxpin}{\way + 1}
    \draw[arrow, gray] (cdata-7-\way.south) -- ++(0,-0.3) -| (cachemux.btpin \muxpin);
}

% Mux output to data output
\draw[arrow] (cachemux.bbpin 1) -- (dataout.north);
\end{tikzpicture}
\end{columns}
\end{frame}
