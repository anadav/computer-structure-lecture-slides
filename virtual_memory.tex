...
\begin{tikzpicture}[scale=0.75]
    % Page table entries
    \node[font=\small] at (2.5,4.5) {Page Table (showing 20 PTEs)};
    
    % Draw individual PTEs
    \foreach \i in {0,...,3} {
        \pgfmathsetmacro\y{4 - \i*0.35}
        \draw[thick] (0,\y) rectangle (5,\y-0.3);
        \fill[gray!20] (0,\y) rectangle (5,\y-0.3);
        \node[font=\tiny] at (0.3,\y-0.15) {PTE\i};
        \node[font=\tiny] at (2.5,\y-0.15) {Regular 4KB page};
    }
    
    % Contiguous block (16 entries)
    \draw[thick, red, line width=1.5pt] (-0.1,2.5) rectangle (5.1,0.1);
    \foreach \i in {0,...,15} {
        \pgfmathsetmacro\idx{int(\i + 4)}
        \pgfmathsetmacro\y{2.4 - \i*0.15}
        ...
        \node[font=\tiny] at (0.3,\y-0.07) {PTE\idx};
        \pgfmathsetmacro\offset{int(\i * 4)}
        \node[font=\tiny] at (2.5,\y-0.07) {Offset +\offset KB};
    }
    
    % Label for contiguous block
    \node[right, font=\small, red] at (5.2,1.25) {64KB};
    \node[right, font=\tiny, red] at (5.2,0.95) {Contiguous};
    \node[right, font=\tiny, red] at (5.2,0.65) {Block};
    
    % Arrow showing coalescing
    \draw[->, thick, blue] (5.5,1.25) -- (6.5,1.25);
    ...
    % Annotation
    \node[below, font=\tiny, align=center] at (8.5,-0.7) {1 TLB entry\\covers 16 pages};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
