...
\begin{tikzpicture}[scale=0.68,
    ...
    tlbcell/.style={draw, minimum width=0.25cm, minimum height=0.2cm, inner sep=0pt, font=\tiny},
    cachecell/.style={draw, minimum width=0.25cm, minimum height=0.2cm, inner sep=0pt, font=\tiny},
...
% TLB Structure
\node[label, below left=8mm and 5mm of vset] (tlblabel) {TLB};
% TLB Set decoder - relative to label (rotated for horizontal output)
\node[muxdemux, muxdemux def={NB=0, NT=1, NR=4, NL=0, Lh=0.3, Rh=1.0, w=0.5}, below=2mm of tlblabel] 
    (tlbdec) {};
\node[font=\tiny] at (tlbdec) {\rotatebox{90}{Dec}};

% TLB Tag array (4 sets x 4 ways) - relative to decoder
\node[tlbcell, fill=green!15, right=3mm of tlbdec.north] (tlbtag-0-0) {};
\foreach \way in {1,...,3} {
...
\node[tlbcell, fill=purple!15, right=3mm of tlbtag-0-3] (tlbval-0-0) {};
\foreach \way in {1,...,3} {
...
\node[comp, anchor=north] at ([yshift=-3mm]tlbtag-3-0.south) (tlbcomp-0) {=};
\foreach \way in {1,...,3} {
    \node[comp, anchor=north] at ([yshift=-3mm]tlbtag-3-\way.south) (tlbcomp-\way) {=};
}

% OR gate for TLB hit/miss - rotated downward, lower position
\node[or port, scale=0.4, number inputs=4, rotate=-90, below=8mm of tlbcomp-1] (tlbor) {};

% TLB Way MUX - rotated with wide part up, wider to match table
\node[muxdemux, muxdemux def={NT=4, NB=1, NL=0, NR=0, Lh=0.3, Rh=0.3, w=1.2}, rotate=180, below=10mm of tlbval-3-2] 
    (tlbmux) {};
\node[font=\tiny, rotate=180] at (tlbmux) {MUX};

% Physical page# output - below mux
\node[addr, fill=orange!20, minimum width=1.2cm, below=5mm of tlbmux] (ppn) {PPN};

% Cache Structure - relative to page offset
\node[label, below right=8mm and 10mm of pset] (cachelabel) {Cache};
% Cache Set decoder - relative to label (rotated for horizontal output)
\node[muxdemux, muxdemux def={NB=0, NT=1, NR=4, NL=0, Lh=0.3, Rh=1.2, w=0.5}, below=2mm of cachelabel] 
    (cachedec) {};
\node[font=\tiny] at (cachedec) {Dec};

% Cache Tag array - relative to decoder
\node[cachecell, fill=purple!15, right=3mm of cachedec.north] (ctag-0-0) {};
\foreach \way in {1,...,3} {
...
\node[comp, anchor=north] at ([yshift=-3mm]ctag-3-0.south) (ccomp-0) {=};
\foreach \way in {1,...,3} {
    \node[comp, anchor=north] at ([yshift=-3mm]ctag-3-\way.south) (ccomp-\way) {=};
}

% OR gate for cache hit/miss - rotated downward, lower position
\node[or port, scale=0.4, number inputs=4, rotate=-90, below=8mm of ccomp-1] (cacheor) {};

% Cache Way MUX - rotated with wide part up, wider to match table
\node[muxdemux, muxdemux def={NT=4, NB=1, NL=0, NR=0, Lh=0.3, Rh=0.3, w=1.2}, rotate=180, below=10mm of cdata-3-2] 
    (cachemux) {};
\node[font=\tiny, rotate=180] at (cachemux) {MUX};

% Hit/Miss output - relative to OR gate
\node[font=\tiny, below=3mm of cacheor] {Hit/Miss};

% Data output - below mux
\node[addr, fill=yellow!30, minimum width=0.8cm, below=5mm of cachemux] (dataout) {Data};

% Add colored backgrounds using fit
% TLB background
\begin{scope}[on background layer]
    \node[bgbox, fill=blue!5, fit=(tlbdec)(tlbtag-0-0)(tlbval-3-3)(tlbcomp-0)(tlbcomp-3)(tlbor)(tlbmux)] (tlbbg) {};
\end{scope}

% Cache background
\begin{scope}[on background layer]
    \node[bgbox, fill=green!5, fit=(cachedec)(ctag-0-0)(cdata-3-3)(ccomp-0)(ccomp-3)(cacheor)(cachemux)] (cachebg) {};
\end{scope}

% Connections
% VPN set to TLB decoder (from west/left)
\draw[arrow] (vset.south) -- ++(0,-0.3) -| (tlbdec.left);

% Tag from address to all TLB comparators (from top with slight offset)
\foreach \way in {0,...,3} {
    \draw[arrow, blue] (vtag.south) -- ++(0,-0.8) -| ([xshift={-1mm + 0.3*\way}]tlbcomp-\way.north);
}

% Page offset set to Cache (parallel - highlighted in orange)
\draw[arrow, orange, thick] (pset.south) -- ++(0,-0.3) -| (cachedec.top);

% Selected TLB row output to comparators (from top with slight offset)
\foreach \way in {0,...,3} {
    \draw[arrow] (tlbtag-1-\way.south) -- ([xshift=1mm]tlbcomp-\way.north);
}

% Comparator outputs to TLB mux (top inputs after rotation) and OR gate
\foreach \way in {0,...,3} {
    \pgfmathtruncatemacro{\muxpin}{\way + 1}
    \draw[arrow] (tlbcomp-\way.south) |- (tlbmux.tpin \muxpin);
    % Connect to OR gate for hit detection  
    \pgfmathtruncatemacro{\inputnum}{\way + 1}
    \draw (tlbcomp-\way.south) -- ++(0,-0.1) -| (tlbor.in \inputnum);
}

% TLB values to mux - each way to its corresponding pin
\foreach \way in {0,...,3} {
    \pgfmathtruncatemacro{\muxpin}{\way + 1}
    \pgfmathsetmacro{\xpos}{-0.3 + 0.2*\way}
    \draw[arrow, gray] (tlbval-1-\way.south) -- ++(0,-0.3) -| ([xshift=\xpos cm]tlbmux.north);
}

% TLB mux output (from bottom after rotation) to physical page#
\draw[arrow] (tlbmux.bpin 1) -- (ppn.north);

% Physical page# to all cache comparators (from top with slight offset)
\foreach \way in {0,...,3} {
    \draw[arrow, blue, thick] (ppn.east) -- ++(2.5,0) |- ++(0,1.5) -| ([xshift={-1mm}]ccomp-\way.north);
}

% Selected cache row to comparators (from top with slight offset)
\foreach \way in {0,...,3} {
    \draw[arrow] (ctag-1-\way.south) -- ([xshift=1mm]ccomp-\way.north);
}

% Cache comparator outputs to mux (top inputs after rotation) and OR gate
\foreach \way in {0,...,3} {
    \pgfmathtruncatemacro{\muxpin}{\way + 1}
    \draw[arrow] (ccomp-\way.south) |- (cachemux.tpin \muxpin);
    % Connect to OR gate for hit detection
    \pgfmathtruncatemacro{\inputnum}{\way + 1}
    \draw (ccomp-\way.south) -- ++(0,-0.1) -| (cacheor.in \inputnum);
}

% Cache data to mux - each way to its corresponding pin
\foreach \way in {0,...,3} {
    \pgfmathtruncatemacro{\muxpin}{\way + 1}
    \pgfmathsetmacro{\xpos}{-0.3 + 0.2*\way}
    \draw[arrow, gray] (cdata-1-\way.south) -- ++(0,-0.3) -| ([xshift=\xpos cm]cachemux.north);
}

% Mux output (from bottom after rotation) to data output
\draw[arrow] (cachemux.bpin 1) -- (dataout.north);
\end{tikzpicture}
\end{columns}
\end{frame}
