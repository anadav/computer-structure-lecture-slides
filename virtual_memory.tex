...
\node[or port, scale=0.4, number inputs=2, rotate=-90, below=8mm of tlbcomp-0, external pins width=0] (tlbor) {};

% TLB Way MUX - positioned to match width of value table (2 ways)
\node[muxdemux, muxdemux def={NT=2, NB=1, NL=2, NR=2, Lh=0.9, Rh=1.8, w=0.5}, external pins width=0, rotate=90] 
...
\node[muxdemux, muxdemux def={NT=4, NB=1, NL=4, NR=4, Lh=1.2, Rh=2.4, w=1}, external pins width=0, rotate=90] 
...
\foreach \way in {0,...,3} {
    \pgfmathtruncatemacro{\muxpin}{\way + 1}
    \draw[arrow, gray] (cdata-7-\way.south) -- ++(0,-0.3) -| (cachemux.brpin \muxpin);
}
