...
\node[addr, fill=blue!20, minimum width=3cm] (vpn) at (0,5) {Virtual page number};
\node[addr, fill=yellow!20, minimum width=1.5cm, right=0pt of vpn] (poffset) {Page offset};

% Tag and Set below VPN
\node[addr, fill=green!20, minimum width=2cm] at ([yshift=-0.5cm]vpn.west) (vtag) {Tag};
...
\node[addr, fill=cyan!10, minimum width=0.75cm] at ([yshift=-0.5cm]poffset.west |- vtag) (pset) {set};
\node[addr, fill=gray!10, minimum width=0.75cm, right=0pt of pset] (pdisp) {disp};

% TLB Structure
\node[label] at (-1, 3.2) {TLB};
% TLB Set decoder - smaller on left
\node[muxdemux, muxdemux def={Lh=0.8cm, Rh=0.3cm, w=0.5cm}, rotate=-90] 
    (tlbdec) at (-1.3, 2) {};
\node[font=\tiny, rotate=-90] at (tlbdec) {Dec};

% TLB Tag array (4 sets x 4 ways)
\foreach \set in {0,...,3} {
    \foreach \way in {0,...,3} {
        \node[tlbcell, fill=green!15] (tlbtag-\set-\way) at (0.3*\way, 2.5-0.3*\set) {};
    }
}
...
\foreach \set in {0,...,3} {
    \foreach \way in {0,...,3} {
        \node[tlbcell, fill=purple!15] (tlbval-\set-\way) at (1.5+0.3*\way, 2.5-0.3*\set) {};
    }
}

% TLB comparators below tags
\foreach \way in {0,...,3} {
    \node[comp] (tlbcomp-\way) at (0.3*\way, 1) {=};
}

% OR gate for TLB hit/miss
\node[or port, scale=0.4] (tlbor) at (0.45, 0.5) {};

% TLB Way MUX
\node[muxdemux, muxdemux def={Lh=0.8cm, Rh=0.3cm, w=0.8cm}] 
    (tlbmux) at (2, 0.3) {};
\node[font=\tiny] at (tlbmux) {MUX};

% Physical page# output
\node[addr, fill=orange!20, minimum width=1.2cm] (ppn) at (2, -0.5) {PPN};

% Cache Structure
\node[label] at (5.5, 3.2) {Cache};
% Cache Set decoder - wider on right for set outputs
\node[muxdemux, muxdemux def={Lh=0.5cm, Rh=1.2cm, w=0.6cm}, rotate=-90] 
    (cachedec) at (4, 2) {};
\node[font=\tiny, rotate=-90] at (cachedec) {Dec};

% Cache Tag array (larger - 8 ways shown as 2 blocks)
\foreach \set in {0,...,3} {
    \foreach \way in {0,...,3} {
        \node[cachecell, fill=purple!15] (ctag-\set-\way) at (4.8+0.25*\way, 2.5-0.25*\set) {};
    }
}
...
\foreach \set in {0,...,3} {
    \foreach \way in {0,...,3} {
        \node[cachecell, fill=yellow!15] (cdata-\set-\way) at (6.2+0.25*\way, 2.5-0.25*\set) {};
    }
}

% Cache comparators below tags - only 4 for the 4 ways we show
\foreach \way in {0,...,3} {
    \node[comp] (ccomp-\way) at (4.8+0.25*\way, 1) {=};
}

% OR gate for cache hit/miss
\node[or port, scale=0.4] (cacheor) at (5.3, 0.5) {};
...
% Hit/Miss output
\node[font=\tiny] at (6.5, -0.2) {Hit/Miss};

% Data output
\node[addr, fill=yellow!30, minimum width=0.8cm] (dataout) at (6.5, -0.7) {Data};
