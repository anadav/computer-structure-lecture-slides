...
\textbf{TLB Entry with PCID:}
\begin{center}
\begin{tikzpicture}[scale=0.62,
    comp/.style={trapezium, draw, thick, trapezium left angle=70, trapezium right angle=110, minimum height=0.35cm, minimum width=0.5cm},
    and/.style={semicircle, draw, thick, minimum width=0.5cm, rotate=270}]
    
    % Virtual address
    \draw[thick] (0,4.8) rectangle (6,5.3);
    \fill[orange!20] (0,4.8) rectangle (3.5,5.3);
    \fill[yellow!20] (3.5,4.8) rectangle (4.8,5.3);
    \fill[gray!20] (4.8,4.8) rectangle (6,5.3);
    
    \node[font=\tiny] at (1.75,5.05) {VPN (Tag bits)};
    \node[font=\tiny] at (4.15,5.05) {Set};
    \node[font=\tiny] at (5.4,5.05) {Offset};
    
    % Arrow to TLB for set selection
    \draw[->, thick] (4.15,4.8) -- (4.15,4.2);
    \node[right, font=\tiny] at (4.3,4.5) {Index};
    
    % TLB array (4-way set associative)
    \node[font=\small] at (3,4) {Selected TLB Set};
    \foreach \way in {0,...,3} {
        \pgfmathsetmacro\y{3.5 - \way*0.5}
        \draw[thick] (0,\y) rectangle (6,\y-0.4);
        
        % PCID field
        \fill[blue!20] (0,\y) rectangle (0.8,\y-0.4);
        % TAG field  
        \fill[orange!20] (0.8,\y) rectangle (3.5,\y-0.4);
        % PFN field
        \fill[green!20] (3.5,\y) rectangle (5.5,\y-0.4);
        % Flags
        \fill[red!20] (5.5,\y) rectangle (6,\y-0.4);
        
        \node[font=\tiny] at (0.4,\y-0.2) {PCID};
        \node[font=\tiny] at (2.15,\y-0.2) {TAG};
        \node[font=\tiny] at (4.5,\y-0.2) {PFN};
        \node[font=\tiny] at (5.75,\y-0.2) {V};
        
        \node[right, font=\tiny] at (6.1,\y-0.2) {Way \way};
    }
    
    % CR3 PCID
    \draw[thick] (-1.5,0.5) rectangle (-0.5,0.9);
    \fill[blue!20] (-1.5,0.5) rectangle (-0.5,0.9);
    \node[font=\tiny] at (-1,0.7) {PCID};
    \node[below, font=\tiny, align=center] at (-1,0.5) {from\\CR3};
    
    % Comparators for each way
    \foreach \way in {0,...,3} {
        \pgfmathsetmacro\y{3.3 - \way*0.5}
        
        % PCID comparator
        \node[comp, fill=white] (pcidcomp\way) at (0.4,\y-0.8) {};
        \node[font=\tiny] at (0.4,\y-0.8) {=};
        
        % TAG comparator  
        \node[comp, fill=white] (tagcomp\way) at (2.15,\y-0.8) {};
        \node[font=\tiny] at (2.15,\y-0.8) {=};
        
        % AND gate
        \node[and, fill=yellow!20] (and\way) at (3.5,\y-0.8) {};
        \node[font=\tiny] at (3.5,\y-0.8) {\&};
        
        % Connect TLB PCID to comparator
        \draw[->] (0.4,\y-0.4) -- (pcidcomp\way.north);
        
        % Connect CR3 PCID to comparator
        \draw[-] (-0.5,0.7) -- (-0.3,0.7) -- (-0.3,\y-0.8) -- (pcidcomp\way.west);
        
        % Connect TLB TAG to comparator
        \draw[->] (2.15,\y-0.4) -- (tagcomp\way.north);
        
        % Connect VA TAG to comparator
        \draw[-] (1.75,4.8) -- (1.75,4.6-\way*0.1) -- (1.9,4.6-\way*0.1) -- (1.9,\y-0.8) -- (tagcomp\way.west);
        
        % Connect comparators to AND gate
        \draw[->] (pcidcomp\way.east) -- (and\way.west);
        \draw[->] (tagcomp\way.east) -- (and\way.210);
        
        % Hit signal
        \draw[->] (and\way.east) -- ++(0.5,0) node[right, font=\tiny] {Hit\way};
    }
    
    % Priority encoder / MUX indication
    \draw[thick, dashed] (4.5,1.5) rectangle (5.8,0.2);
    \node[font=\tiny, align=center] at (5.15,0.85) {Priority\\Encoder};
    
    % Connect hits to priority encoder
    \foreach \way in {0,...,3} {
        \pgfmathsetmacro\y{3.3 - \way*0.5}
        \draw[-] (4.5,\y-0.8) -- (4.8,\y-0.8) -- (4.8,1.2-\way*0.2) -- (4.5,1.2-\way*0.2);
    }
    
    % Final output
    \draw[->] (5.8,0.85) -- ++(0.5,0) node[right, font=\tiny, align=left] {TLB\\Hit};
    
    % Labels
    \node[font=\tiny, below] at (3,-1.5) {Match: PCID from CR3 = PCID in entry};
    \node[font=\tiny, below] at (3,-1.8) {AND: TAG from VA = TAG in entry};
\end{tikzpicture}
\end{center}

\textbf{Key points:}
\begin{itemize}
\item Each way has dual comparators
\item PCID compared against CR3[11:0]
\item TAG compared against VA bits
\item Both must match (AND) for hit
\end{itemize}
\end{column}
\end{columns}
\end{frame}
