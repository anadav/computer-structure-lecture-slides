...
% Set# and disp below page offset - as a breakdown of page offset
\node[addr, fill=cyan!10, minimum width=0.75cm, below=3mm of poffset.west, anchor=west] (pset) {set};
...
% TLB Set decoder - relative to label
\node[muxdemux, muxdemux def={NB=0, NL=1, NR=4, Lh=0.6, Rh=1.0, w=0.5}, rotate=-90, below=2mm of tlblabel] 
    (tlbdec) {};
\node[font=\tiny, rotate=-90] at (tlbdec) {Dec};
...
% TLB comparators below tags - relative positioning
\node[comp, below=5mm of tlbtag-3-0] (tlbcomp-0) {=};
\foreach \way in {1,...,3} {
    \node[comp, below=5mm of tlbtag-3-\way] (tlbcomp-\way) {=};
}

% OR gate for TLB hit/miss - relative to comparators
\node[or port, scale=0.4, number inputs=4, below right=3mm and 2mm of tlbcomp-1] (tlbor) {};

% TLB Way MUX - relative to value array
\node[muxdemux, muxdemux def={NB=1, NL=4, NR=0, Lh=1.0, Rh=0.3, w=0.6}, below=8mm of tlbval-3-2] 
    (tlbmux) {};
\node[font=\tiny] at (tlbmux) {MUX};
...
% Cache Set decoder - relative to label
\node[muxdemux, muxdemux def={NB=0, NL=1, NR=4, Lh=0.4, Rh=1.2, w=0.5}, rotate=-90, below=2mm of cachelabel] 
    (cachedec) {};
\node[font=\tiny, rotate=-90] at (cachedec) {Dec};
...
% Cache comparators below tags - relative positioning
\node[comp, below=5mm of ctag-3-0] (ccomp-0) {=};
\foreach \way in {1,...,3} {
    \node[comp, below=5mm of ctag-3-\way] (ccomp-\way) {=};
}

% OR gate for cache hit/miss - relative to comparators
\node[or port, scale=0.4, number inputs=4, below right=3mm and 2mm of ccomp-1] (cacheor) {};

% Cache Way MUX - relative to data array
\node[muxdemux, muxdemux def={NB=1, NL=4, NR=0, Lh=1.0, Rh=0.3, w=0.6}, below=8mm of cdata-3-2] 
    (cachemux) {};
\node[font=\tiny] at (cachemux) {MUX};
...
% Connections
% VPN to TLB (set)
\draw[arrow] (vset.south) -- ++(0,-0.3) -| (tlbdec.north);

% Tag to TLB comparators
\draw[arrow, blue] (vtag.south) -- ++(0,-1.5) -| ($(tlbcomp-0.west) + (-0.2,0)$);

% Page offset set to Cache (parallel - highlighted in orange)
\draw[arrow, orange, thick] (pset.south) -- ++(0,-0.3) -| (cachedec.north);

% Selected TLB row output to comparators
\foreach \way in {0,...,3} {
    \draw[arrow] (tlbtag-1-\way.south) -- (tlbcomp-\way.north);
}

% Comparator outputs to TLB mux and OR gate
\foreach \way in {0,...,3} {
    \draw[arrow] (tlbcomp-\way.south) |- ([yshift={0.1cm-0.05cm*\way}]tlbmux.west);
...
% TLB values to mux
\foreach \way in {0,...,3} {
    \draw[arrow, gray] (tlbval-1-\way.south) |- ([yshift={-0.1cm-0.05cm*\way}]tlbmux.west);
}

% TLB mux to physical page#
\draw[arrow] (tlbmux.south) -- (ppn.north);

% Physical page# to cache comparators
\draw[arrow, blue, thick] (ppn.east) -| ++(2,0) |- ($(ccomp-0.west) + (-0.2,0)$);

% Selected cache row to comparators
\foreach \way in {0,...,3} {
    \draw[arrow] (ctag-1-\way.south) -- (ccomp-\way.north);
}

% Cache comparators to mux and OR gate
\foreach \way in {0,...,3} {
    \draw[arrow] (ccomp-\way.south) |- ([yshift={0.15cm-0.04cm*\way}]cachemux.west);
    % Connect to OR gate for hit detection
    \pgfmathtruncatemacro{\inputnum}{\way + 1}
...
% Cache data to mux
\foreach \way in {0,...,3} {
    \draw[arrow, gray] (cdata-1-\way.south) |- ([yshift={-0.05cm-0.03cm*\way}]cachemux.west);
}

% Mux to data output
\draw[arrow] (cachemux.south) -- (dataout.north);
\end{tikzpicture}
\end{columns}
\end{frame}
