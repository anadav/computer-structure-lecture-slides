...
\node[or port, scale=0.4, number inputs=4, rotate=-90, external pins width=0] (cacheor) at ([yshift=-10mm]cache-or-x) {};
...
\node[label] (tlblabel) at (tlb-label-x |- tlbdec.north) [above=1mm] {TLB};

\coordinate (cache-label-x) at ($(cachedec.west)!0.5!(cdata-0-3.east)$);
\node[label] (cachelabel) at (cache-label-x |- cachedec.north) [above=1mm] {Cache};
...
\draw[arrow] (vset.south) -- ++(0,-0.5) -|  ([xshift=-5mm]tlbdec.blpin 1) -- (tlbdec.blpin 1);

% Tag from address to all TLB comparators (from top with slight offset)
\foreach \way in {0,...,1} {
    \draw[arrow, blue] (vtag.south) 
        |- ([xshift=5mm, yshift=-3mm] vtag.south west)
