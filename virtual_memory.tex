...
\node[muxdemux, muxdemux def={NT=2, NB=1, NL=0, NR=0, Lh=0.5, Rh=0.5, w=1.2}] 
    (tlbmux) at ([yshift=-12mm, xshift=2mm]tlbval-3-0.south east) {};
...
\node[muxdemux, muxdemux def={NT=4, NB=1, NL=0, NR=0, Lh=0.5, Rh=0.5, w=2.4}] 
    (cachemux) at ([yshift=-12mm, xshift=6mm]cdata-7-1.south east) {};
...
\foreach \way in {0,...,1} {
    ...
    \draw[arrow, gray] (tlbval-3-\way.south) -- ++(0,-0.3) -| (tlbmux.tpin \muxpin);
}
...
\foreach \way in {0,...,3} {
    \pgfmathtruncatemacro{\muxpin}{\way + 1}
    \draw[arrow, gray] (cdata-7-\way.south) -- ++(0,-0.3) -| (cachemux.tpin \muxpin);
}
