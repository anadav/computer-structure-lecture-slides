...
\begin{tikzpicture}[scale=0.72,
    ...
    data/.style={rectangle, draw, thick, minimum width=1.1cm, minimum height=1.8cm, fill=white},
    arrow/.style={->, thick},
    ...
    % Field labels above
    \node[above, font=\tiny] at (0.5,8.3) {63};
    \node[above, font=\tiny] at (1.6,8.3) {47 39 38};
    \node[above, font=\tiny] at (4.1,8.3) {30 29};
    \node[above, font=\tiny] at (5.5,8.3) {21 20};
    \node[above, font=\tiny] at (6.9,8.3) {12 11};
    \node[above, font=\tiny] at (10.5,8.3) {0};
    ...
    % Page table hierarchy - staggered heights
    % PML4 Table
    \node[table, minimum height=1.6cm] (pml4) at (2,5.2) {};
    \node[font=\tiny] at (2,5.4) {PML4};
    \node[font=\tiny] at (2,5.1) {Table};
    \node[entry] (pml4en) at (2,4.5) {};
    \node[font=\tiny] at (pml4en) {PML4 en};
    
    % PDP Table
    \node[table, minimum height=1.6cm] (pdp) at (4,4.7) {};
    \node[font=\tiny] at (4,5) {Page};
    \node[font=\tiny] at (4,4.7) {Directory};
    \node[font=\tiny] at (4,4.4) {Pointer};
    \node[entry] (pdpen) at (4,3.8) {};
    \node[font=\tiny] at (pdpen) {PDP en};
    
    % Page Directory
    \node[table, minimum height=1.6cm] (dir) at (6,4.2) {};
    \node[font=\tiny] at (6,4.5) {Page};
    \node[font=\tiny] at (6,4.2) {Directory};
    \node[entry] (pde) at (6,3.4) {};
    \node[font=\tiny] at (pde) {PDE};
    
    % Page Table - with text on two lines
    \node[table, minimum height=1.6cm] (pt) at (8,3.7) {};
    \node[font=\tiny] at (8,4) {Page};
    \node[font=\tiny] at (8,3.7) {Table};
    \node[entry] (pte) at (8,3) {};
    \node[font=\tiny] at (pte) {PTE};
    
    % Data Page with data in rectangle
    \node[data] (datapage) at (10.2,3.5) {};
    \node[font=\tiny] at (10.2,4) {4KByte};
    \node[font=\tiny] at (10.2,3.6) {Page};
    % Draw rectangle around "data"
    \draw[thick] (9.8,3.1) rectangle (10.6,2.7);
    \node[font=\tiny] at (10.2,2.9) {data};
    
    % CR3
    \node[rectangle, draw, thick, minimum width=1.2cm, minimum height=0.4cm] (cr3) at (0.5,2) {\tiny CR3 (PDPTR)};
    
    % Arrows from address fields to table entries (to west side) with "/" on line
    % PML4 arrow with "9" near origin and "/" on line
    \draw[arrow] (2.7,7.5) -- (2.7,6.5) -- (0.8,6.5) -- (0.8,4.5) -- (pml4en.west);
    \node[font=\tiny] at (3,7.3) {9};
    \draw[thick] (2.65,7.2) -- (2.75,7.15);
    
    % PDP arrow with "9" near origin and "/" on line
    \draw[arrow] (4.1,7.5) -- (4.1,6) -- (2.8,6) -- (2.8,3.8) -- (pdpen.west);
    \node[font=\tiny] at (4.4,7.3) {9};
    \draw[thick] (4.05,7.2) -- (4.15,7.15);
    
    % DIR arrow with "9" near origin and "/" on line
    \draw[arrow] (5.5,7.5) -- (5.5,5.5) -- (4.8,5.5) -- (4.8,3.4) -- (pde.west);
    \node[font=\tiny] at (5.8,7.3) {9};
    \draw[thick] (5.45,7.2) -- (5.55,7.15);
    
    % TABLE arrow with "9" near origin and "/" on line
    \draw[arrow] (6.9,7.5) -- (6.9,5) -- (6.8,5) -- (6.8,3) -- (pte.west);
    \node[font=\tiny] at (7.2,7.3) {9};
    \draw[thick] (6.85,7.2) -- (6.95,7.15);
    
    % OFFSET arrow to data with "12" near origin and "/" on line
    \draw[arrow] (9.3,7.5) -- (9.3,6.5) -- (10.2,6.5) -- (10.2,4.5);
    ...
    \draw[dashedarrow] (cr3.east) -- ($(cr3.east) + (0.3,0)$) -- ($(cr3.east) + (0.3,2.2)$) -- ($(pml4.south) + (0,-0.2)$) -- (pml4.south);
    \node[font=\tiny] at ($(cr3.east) + (0.7,0.3)$) {M-12 (4KB aligned)};
    
    % PML4 en to PDP Table south - dashed with proper right angles
    \draw[dashedarrow] (pml4en.east) -- ($(pml4en.east) + (0.3,0)$) -- ($(pml4en.east) + (0.3,-1.2)$) |- ($(pdp.south) + (0,-1.2)$) -| (pdp.south);
    \draw[thick] ($(pml4en.east) + (0.25,0.05)$) -- ($(pml4en.east) + (0.35,-0.05)$);
    \node[font=\tiny] at ($(pml4en.east) + (0.5,-0.2)$) {M-12};
    
    % PDP en to Page Directory south - dashed with proper right angles
    \draw[dashedarrow] (pdpen.east) -- ($(pdpen.east) + (0.3,0)$) -- ($(pdpen.east) + (0.3,-1)$) |- ($(dir.south) + (0,-1)$) -| (dir.south);
    \draw[thick] ($(pdpen.east) + (0.25,0.05)$) -- ($(pdpen.east) + (0.35,-0.05)$);
    \node[font=\tiny] at ($(pdpen.east) + (0.5,-0.2)$) {M-12};
    
    % PDE to Page Table south - dashed with proper right angles
    \draw[dashedarrow] (pde.east) -- ($(pde.east) + (0.3,0)$) -- ($(pde.east) + (0.3,-0.8)$) |- ($(pt.south) + (0,-0.8)$) -| (pt.south);
    \draw[thick] ($(pde.east) + (0.25,0.05)$) -- ($(pde.east) + (0.35,-0.05)$);
    \node[font=\tiny] at ($(pde.east) + (0.5,-0.2)$) {M-12};
    
    % PTE to data - dashed with proper right angles
    \draw[dashedarrow] (pte.east) -- ($(pte.east) + (0.6,0)$) -- ($(pte.east) + (0.6,0.5)$) -- ($(datapage.west) + (-0.3,0.5)$) -- ($(datapage.west) + (-0.3,-0.5)$) -- ($(datapage.west) + (0,-0.5)$);
    \draw[thick] ($(pte.east) + (0.55,0.05)$) -- ($(pte.east) + (0.65,-0.05)$);
