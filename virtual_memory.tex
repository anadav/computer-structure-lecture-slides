...
\begin{itemize}
\item \textbf{Problem:} TLB flush on every context switch is expensive
    \begin{itemize}
    \item Lose all cached translations → cold TLB after switch
    \item High miss rate until TLB warms up again
    \end{itemize}
\item \textbf{Solution: Address Space ID (ARM) / Process Context ID (x86)}
    \begin{itemize}
    \item Tag TLB entries with process identifier (12-bit PCID in x86)
    \item Multiple processes' translations can coexist in TLB
    \item On context switch: just change current PCID, no flush needed
    \end{itemize}
\item \textbf{Software impact:}
    \begin{itemize}
    \item OS must manage PCID allocation (limited to 4096 IDs)
    \item Significant performance gain for frequent context switches
    \item Critical for mitigating Meltdown patches overhead
    \item Example: Linux KPTI uses separate PCIDs for user/kernel
    \end{itemize}
\end{itemize}
\end{frame}
