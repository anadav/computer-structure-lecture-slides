...
% Temporary simplified version - full diagram causing compilation issues
\begin{center}
\textbf{PMH Page Walk Process}

VA[47:12] $\rightarrow$ TLB check $\rightarrow$ STLB check

If miss: Walk through 4 levels:
\begin{enumerate}
\item CR3 $\rightarrow$ PML4 Cache $\rightarrow$ L1/L2/L3/Memory $\rightarrow$ PML4 entry
\item PML4 entry $\rightarrow$ PDP Cache $\rightarrow$ L1/L2/L3/Memory $\rightarrow$ PDP entry
\item PDP entry $\rightarrow$ PDE Cache $\rightarrow$ L1/L2/L3/Memory $\rightarrow$ PDE entry
\item PDE entry $\rightarrow$ PT Cache $\rightarrow$ L1/L2/L3/Memory $\rightarrow$ PTE
\end{enumerate}

Each level checks specialized cache first, then L1\$, L2\$, L3\$, then Memory
\end{center}
\end{frame}

% The complex TikZ diagram was causing compilation issues - removed entirely
\iffalse
\begin{tikzpicture}[
    scale=0.55,
    transform shape,
    node distance=3mm,
    decision/.style={diamond, draw, fill=blue!10, text width=1cm, align=center, inner sep=1pt, minimum height=0.5cm, aspect=3, font=\tiny},
    process/.style={rectangle, draw, fill=green!10, text width=2.5cm, align=center, minimum height=0.5cm, inner sep=2pt, font=\tiny},
    data/.style={ellipse, draw, fill=yellow!10, text width=1.8cm, align=center, minimum height=0.4cm, inner sep=2pt, font=\tiny},
    arrow/.style={->, >=stealth, thick},
    every node/.style={font=\tiny}
]

% Top level - VA and TLBs
\node[data, at=(0,1)] (va) {VA[47:12]};
\node[decision, below=5mm of va] (tlb) {TLB hit};
\node[decision, below=5mm of tlb] (stlb) {STLB hit};

% Result node
\node[data, at=(10,-1)] (pte) {PTE};

% Top row - base addresses
\node[data, at=(-2,0), text width=1.2cm] (cr3) {CR3 PML4 base};
\node[data, at=(2,0), text width=1.2cm] (pml4e) {PML4 entry};
\node[data, at=(6,0), text width=1.2cm] (pdpe) {PDP entry};
\node[data, at=(10,0), text width=1.2cm] (pdee) {PDE entry};

% PML4 hierarchy
\node[decision, below=8mm of cr3] (pml4cache) {PML4\$ hit};
\node[process, below=4mm of pml4cache, text width=2.8cm] (pml4load) {inject load to PML4 base + VA[47:39]×8};
\node[decision, below=3mm of pml4load] (pml4l1) {L1\$ hit};
\node[decision, below=3mm of pml4l1] (pml4l2) {L2\$ hit};
\node[decision, below=3mm of pml4l2] (pml4l3) {L3\$ hit};
\node[process, below=3mm of pml4l3] (pml4mem) {Memory};

% PDP hierarchy
\node[decision, below=8mm of pml4e] (pdpcache) {PDP\$ hit};
\node[process, below=4mm of pdpcache, text width=2.8cm] (pdpload) {inject load to PDP base + VA[38:30]×8};
\node[decision, below=3mm of pdpload] (pdpl1) {L1\$ hit};
\node[decision, below=3mm of pdpl1] (pdpl2) {L2\$ hit};
\node[decision, below=3mm of pdpl2] (pdpl3) {L3\$ hit};
\node[process, below=3mm of pdpl3] (pdpmem) {Memory};

% PDE hierarchy
\node[decision, below=8mm of pdpe] (pdecache) {PDE\$ hit};
\node[process, below=4mm of pdecache, text width=2.8cm] (pdeload) {inject load to PDE base + VA[29:21]×8};
\node[decision, below=3mm of pdeload] (pdel1) {L1\$ hit};
\node[decision, below=3mm of pdel1] (pdel2) {L2\$ hit};
\node[decision, below=3mm of pdel2] (pdel3) {L3\$ hit};
\node[process, below=3mm of pdel3] (pdemem) {Memory};

% PT hierarchy
\node[decision, below=8mm of pdee] (ptcache) {PT\$ hit};
\node[process, below=4mm of ptcache, text width=2.8cm] (ptload) {inject load to PT base + VA[20:12]×8};
\node[decision, below=3mm of ptload] (ptl1) {L1\$ hit};
\node[decision, below=3mm of ptl1] (ptl2) {L2\$ hit};
\node[decision, below=3mm of ptl2] (ptl3) {L3\$ hit};
\node[process, below=3mm of ptl3] (ptmem) {Memory};

% Main flow arrows
\draw[arrow] (tlb) -- node[right, font=\tiny] {No} (stlb);
\draw[arrow] (tlb.east) -- node[above, font=\tiny] {Yes} ++(0.5,0) |- (pte);
\draw[arrow] (stlb.east) -- node[above, font=\tiny] {Yes} ++(0.3,0) |- (pte);
\draw[arrow] (stlb.south) -- node[right, font=\tiny] {No} ++(0,-0.3) -| (pml4cache);

% Connections between hierarchy levels
\draw[arrow] (cr3) -- (pml4cache);
\draw[arrow] (pml4e) -- (pdpcache);
\draw[arrow] (pdpe) -- (pdecache);
\draw[arrow] (pdee) -- (ptcache);

% PML4 cache hierarchy arrows
\draw[arrow] (pml4cache) -- node[left, font=\tiny] {No} (pml4load);
\draw[arrow] (pml4load) -- (pml4l1);
\draw[arrow] (pml4l1) -- node[right, font=\tiny] {No} (pml4l2);
\draw[arrow] (pml4l2) -- node[right, font=\tiny] {No} (pml4l3);
\draw[arrow] (pml4l3) -- node[right, font=\tiny] {No} (pml4mem);
\draw[arrow] (pml4cache.east) -- node[above, font=\tiny] {Yes} ++(0.3,0) |- (pml4e);
\draw[arrow] (pml4l1.east) -- ++(0.2,0) |- (pml4e);
\draw[arrow] (pml4l2.east) -- ++(0.2,0) |- (pml4e);
\draw[arrow] (pml4l3.east) -- ++(0.2,0) |- (pml4e);
\draw[arrow] (pml4mem.east) -- ++(0.2,0) |- (pml4e);

% PDP cache hierarchy arrows
\draw[arrow] (pdpcache) -- node[left, font=\tiny] {No} (pdpload);
\draw[arrow] (pdpload) -- (pdpl1);
\draw[arrow] (pdpl1) -- node[right, font=\tiny] {No} (pdpl2);
\draw[arrow] (pdpl2) -- node[right, font=\tiny] {No} (pdpl3);
\draw[arrow] (pdpl3) -- node[right, font=\tiny] {No} (pdpmem);
\draw[arrow] (pdpcache.east) -- node[above, font=\tiny] {Yes} ++(0.3,0) |- (pdpe);
\draw[arrow] (pdpl1.east) -- ++(0.2,0) |- (pdpe);
\draw[arrow] (pdpl2.east) -- ++(0.2,0) |- (pdpe);
\draw[arrow] (pdpl3.east) -- ++(0.2,0) |- (pdpe);
\draw[arrow] (pdpmem.east) -- ++(0.2,0) |- (pdpe);

% PDE cache hierarchy arrows
\draw[arrow] (pdecache) -- node[left, font=\tiny] {No} (pdeload);
\draw[arrow] (pdeload) -- (pdel1);
\draw[arrow] (pdel1) -- node[right, font=\tiny] {No} (pdel2);
\draw[arrow] (pdel2) -- node[right, font=\tiny] {No} (pdel3);
\draw[arrow] (pdel3) -- node[right, font=\tiny] {No} (pdemem);
\draw[arrow] (pdecache.east) -- node[above, font=\tiny] {Yes} ++(0.3,0) |- (pdee);
\draw[arrow] (pdel1.east) -- ++(0.2,0) |- (pdee);
\draw[arrow] (pdel2.east) -- ++(0.2,0) |- (pdee);
\draw[arrow] (pdel3.east) -- ++(0.2,0) |- (pdee);
\draw[arrow] (pdemem.east) -- ++(0.2,0) |- (pdee);

% PT cache hierarchy arrows
\draw[arrow] (ptcache) -- node[left, font=\tiny] {No} (ptload);
\draw[arrow] (ptload) -- (ptl1);
\draw[arrow] (ptl1) -- node[right, font=\tiny] {No} (ptl2);
\draw[arrow] (ptl2) -- node[right, font=\tiny] {No} (ptl3);
\draw[arrow] (ptl3) -- node[right, font=\tiny] {No} (ptmem);
\draw[arrow] (ptcache.east) -- node[above, font=\tiny] {Yes} ++(0.3,0) -- (pte);
\draw[arrow] (ptl1.east) -- ++(0.2,0) |- (pte);
\draw[arrow] (ptl2.east) -- ++(0.2,0) |- (pte);
\draw[arrow] (ptl3.east) -- ++(0.2,0) |- (pte);
\draw[arrow] (ptmem.east) -- ++(0.2,0) |- (pte);
...
\begin{tikzpicture}[scale=0.7]
    ...
    \node[below, font=\tiny] at (5.4,-0.8) {12-bit PCID};
    \draw[->] (5.4,0) -- (5.4,-0.3);
\end{tikzpicture}
\end{center}
...
\textbf{TLB Lookup with PCID:}
\begin{center}
\begin{tikzpicture}[scale=0.68]
    % Virtual address at top
    \draw[thick] (0,5.5) rectangle (5.5,6);
    \fill[orange!20] (0,5.5) rectangle (3.2,6);
    \fill[yellow!20] (3.2,5.5) rectangle (4.3,6);
    \fill[gray!20] (4.3,5.5) rectangle (5.5,6);
    
    \node[font=\footnotesize] at (1.6,5.75) {VPN (Tag bits)};
    \node[font=\footnotesize] at (3.75,5.75) {Set};
    \node[font=\footnotesize] at (4.9,5.75) {Offset};
    
    \node[above, font=\small] at (2.75,6) {Virtual Address};
    
    % Arrow for set indexing
    \draw[->, thick] (3.75,5.5) -- (3.75,4.8) node[midway, right, font=\tiny] {Index};
    
    % TLB Set label
    \node[font=\small] at (2.75,4.5) {Selected TLB Set};
    
    % TLB entries (4 ways)
    \foreach \way in {0,...,3} {
        \pgfmathsetmacro\y{4 - \way*0.55}
        % Entry box
        \draw[thick] (0,\y) rectangle (5.5,\y-0.45);
        
        % PCID field
        \fill[blue!20] (0,\y) rectangle (0.7,\y-0.45);
        \node[font=\tiny] at (0.35,\y-0.225) {PCID};
        
        % TAG field
        \fill[orange!20] (0.7,\y) rectangle (3.2,\y-0.45);
        \node[font=\tiny] at (1.95,\y-0.225) {TAG};
        
        % PFN field
        \fill[green!20] (3.2,\y) rectangle (5,\y-0.45);
        \node[font=\tiny] at (4.1,\y-0.225) {PFN};
        
        % Valid bit
        \fill[red!20] (5,\y) rectangle (5.5,\y-0.45);
        \node[font=\tiny] at (5.25,\y-0.225) {V};
        
        % Way label
        \node[right, font=\tiny] at (5.7,\y-0.225) {Way \way};
    }
    
    % CR3 PCID source
    \draw[thick] (-2,1.5) rectangle (-1,2);
    \fill[blue!20] (-2,1.5) rectangle (-1,2);
    \node[font=\tiny] at (-1.5,1.75) {PCID};
    \node[below, font=\tiny] at (-1.5,1.5) {from CR3};
    
    % Comparison logic for each way - cleaner layout
    \foreach \way in {0,...,3} {
        \pgfmathsetmacro\y{3.775 - \way*0.55}
        \pgfmathsetmacro\compy{0.8 - \way*0.2}
        
        % Comparator boxes
        % PCID comparator
        \draw[thick] (0.1,\compy) rectangle (0.6,\compy-0.15);
        \node[font=\tiny] at (0.35,\compy-0.075) {=};
        
        % TAG comparator
        \draw[thick] (1.7,\compy) rectangle (2.2,\compy-0.15);
        \node[font=\tiny] at (1.95,\compy-0.075) {=};
        
        % AND gate (rectangular style)
        \draw[thick] (3.2,\compy) rectangle (3.7,\compy-0.15);
        \node[font=\tiny] at (3.45,\compy-0.075) {\&};
        
        % Connections from TLB entries to comparators
        \draw[-] (0.35,\y-0.45) -- (0.35,\compy);
        \draw[-] (1.95,\y-0.45) -- (1.95,\compy);
        
        % Connection from CR3 PCID (clean routing)
        \draw[-] (-1,1.75) -- (-0.7,1.75) -- (-0.7,\compy-0.075) -- (0.1,\compy-0.075);
        
        % Connection from VA TAG (clean routing)
        \draw[-] (1.6,5.5) -- (1.6,5.3-\way*0.05) -- (1.5,5.3-\way*0.05) -- (1.5,\compy-0.075) -- (1.7,\compy-0.075);
        
        % Connect comparators to AND
        \draw[-] (0.6,\compy-0.075) -- (3.2,\compy-0.075);
        \draw[-] (2.2,\compy-0.075) -- (3.2,\compy-0.075);
        
        % Valid bit check
        \draw[-] (5.25,\y-0.45) -- (5.25,\y-0.55) -- (3.45,\y-0.55) -- (3.45,\compy-0.15);
        
        % Hit output
        \draw[->] (3.7,\compy-0.075) -- (4.2,\compy-0.075);
        \node[right, font=\tiny] at (4.2,\compy-0.075) {Hit\way};
    }
    
    % Priority encoder
    \draw[thick, dashed] (4.7,0.5) rectangle (5.5,-0.1);
    \node[font=\tiny, align=center] at (5.1,0.2) {Priority\\Encoder};
    
    % Hit signals to encoder
    \foreach \way in {0,...,3} {
        \pgfmathsetmacro\compy{0.725 - \way*0.2}
        \draw[-] (4.5,\compy) -- (4.7,\compy);
    }
    
    % Final TLB Hit
    \draw[->] (5.5,0.2) -- (6,0.2) node[right, font=\tiny] {TLB Hit};
    
    % Bottom caption
    \node[font=\tiny, below] at (2.75,-0.5) {Match: PCID from CR3 = PCID in entry};
    \node[font=\tiny, below] at (2.75,-0.75) {AND: TAG from VA = TAG in entry};
    \node[font=\tiny, below] at (2.75,-1) {AND: Valid bit = 1};
\end{tikzpicture}
\end{center}
