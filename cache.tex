...
\begin{tikzpicture}[
    state/.style={circle, draw, minimum size=2cm, thick},
...
\node[state] (shared) at (5,3) {Shared};
\node[state] (exclusive) at (10,0) {Exclusive};
\node[state] (modified) at (5,-3) {Modified};

% Self-loops
\path[arrow] 
    (invalid) edge[loop left] node[text width=2cm, align=center] {any\\snoop} (invalid)
    (shared) edge[loop above] node {snoop read} (shared)
    (shared) edge[loop right] node {read hit} (shared)
    (exclusive) edge[loop right] node {read hit} (exclusive)
    (modified) edge[loop below] node[text width=2.5cm, align=center] {read/write\\hit} (modified);

% Transitions between states
\path[arrow]
    % From Invalid
    (invalid) edge[bend left=20, cyan!70!black] node[above, sloped, text width=3cm, align=center] {read miss \&\\somewhere else} (shared)
    (invalid) edge[bend right=20, blue!70!black] node[below, sloped, text width=2.5cm, align=center] {read miss,\\only here} (exclusive)
    (invalid) edge[red!70!black] node[below] {write} (modified)
    
    % From Shared
    (shared) edge[bend left=20, red!70!black] node[below, sloped] {snoop write} (invalid)
    (shared) edge[blue!70!black] node[right] {write} (modified)
    
    % From Exclusive
    (exclusive) edge[bend left=20, cyan!70!black] node[above, sloped] {snoop read} (shared)
    (exclusive) edge[black] node[right, text width=2.5cm, align=center] {write hit\\(write to\\memory)} (modified)
    
    % From Modified
    (modified) edge[bend left=20, black] node[below, sloped, text width=3cm, align=center] {write hit\\(setup for late write)} (exclusive)
    (modified) edge[red!70!black] node[left] {snoop write} (invalid)
    (modified) edge[cyan!70!black] node[left] {snoop read} (shared);
