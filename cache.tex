...
\path[arrow] 
    (invalid) edge[loop left] node[font=\tiny] {any snoop} (invalid)
    (shared) edge[loop above] node[font=\tiny, text width=1.5cm, align=center] {read hit,\\snoop read} (shared)
    (exclusive) edge[loop right] node[font=\tiny] {read hit} (exclusive)
    (modified) edge[loop below] node[font=\tiny, text width=1.5cm, align=center] {read/write\\hit} (modified);

% Transitions between states - all unidirectional
\path[arrow]
    % From Invalid
    (invalid) edge[bend left=20, cyan] node[font=\tiny, above, sloped, text width=2cm, align=center] {read miss \&\\others have it} (shared)
    (invalid) edge[bend right=20, blue] node[font=\tiny, below, sloped, text width=1.8cm, align=center] {read miss,\\exclusive} (exclusive)
    (invalid) edge[red] node[font=\tiny, below] {write miss} (modified)
    
    % From Shared
    (shared) edge[bend right=20, red] node[font=\tiny, above left, sloped] {snoop write} (invalid)
    (shared) edge[blue] node[font=\tiny, right] {write hit} (modified)
    
    % From Exclusive  
    (exclusive) edge[bend right=20, cyan] node[font=\tiny, below, sloped] {snoop read} (shared)
    (exclusive) edge[black] node[font=\tiny, right, text width=1.5cm, align=center] {write hit} (modified)
    
    % From Modified
    (modified) edge[bend right=20, black] node[font=\tiny, above, sloped, text width=2cm, align=center] {snoop read\\write back} (shared)
