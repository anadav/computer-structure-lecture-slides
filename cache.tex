...
\begin{itemize}
  ...
  \begin{itemize}
    \item 1 bit per set to mark the latest way accessed in the set
    \item Evict the way which is not pointed by the bit
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item When replacement is needed: evict way with counter = 0
  \end{itemize}
...
%\textbf{Before Fusion:}
\begin{itemize}
  \item[\textcolor{green}{$\diamond$}] First loop: hit 7/8 of iterations
...
%\textbf{After Fusion:}
\begin{itemize}
  \item[\textcolor{green}{$\diamond$}] First line: hit 7/8 of iterations
  \item[\textcolor{green}{$\diamond$}] Second line: hit all
\end{itemize}

\vspace{0.2cm}
\footnotesize
\textit{Note: Only load accesses are taken into account for hit rate calculation}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
  \item \textbf{2-dimension array in memory:}
\end{itemize}
...
\vspace{0.3cm}
\begin{block}{Original program: access are 100 bytes apart}
\small
...
\vspace{0.2cm}
\begin{block}{Reversing the loops order: access are adjacent}
\small
