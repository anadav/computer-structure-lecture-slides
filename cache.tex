...
% Self-loops with smaller text
\path[arrow] 
    (invalid) edge[loop left] node[font=\tiny, text width=1.5cm, align=center] {any\\snoop} (invalid)
    (shared) edge[loop above] node[font=\tiny] {snoop read} (shared)
    (shared) edge[loop right] node[font=\tiny] {read hit} (shared)
    (exclusive) edge[loop right] node[font=\tiny] {read hit} (exclusive)
    (modified) edge[loop below] node[font=\tiny, text width=1.8cm, align=center] {read/write\\hit} (modified);

% Transitions between states with smaller text
\path[arrow]
    % From Invalid
    (invalid) edge[bend left=15, cyan] node[font=\tiny, above, sloped, text width=2.2cm, align=center] {read miss \&\\somewhere else} (shared)
    (invalid) edge[bend right=15, blue] node[font=\tiny, below, sloped, text width=1.8cm, align=center] {read miss,\\only here} (exclusive)
    (invalid) edge[red] node[font=\tiny, below] {write} (modified)
    
    % From Shared
    (shared) edge[bend left=15, red] node[font=\tiny, below, sloped] {snoop write} (invalid)
    (shared) edge[blue] node[font=\tiny, right] {write} (modified)
    
    % From Exclusive
    (exclusive) edge[bend left=15, cyan] node[font=\tiny, above, sloped] {snoop read} (shared)
    (exclusive) edge[black] node[font=\tiny, right, text width=1.8cm, align=center] {write hit\\(write to\\memory)} (modified)
    
    % From Modified
    (modified) edge[bend left=15, black] node[font=\tiny, below, sloped, text width=2.2cm, align=center] {write hit\\(setup for late write)} (exclusive)
    (modified) edge[red] node[font=\tiny, left] {snoop write} (invalid)
    (modified) edge[cyan] node[font=\tiny, left] {snoop read} (shared);
