...
\begin{tikzpicture}[
    ...
    \draw[haltarrow, bend left=15] (mt) to node[labelstyle, below right, pos=0.5] {Thread 0\\executes HALT} (st1);

    % Blue arrows - Interrupt (labeled)
    \draw[intarrow, bend left=15] (lp) to node[labelstyle, left, blue, pos=0.5] {Interrupt} (st0);
    \draw[intarrow, bend right=15] (lp) to node[labelstyle, right, blue, pos=0.5] {Interrupt} (st1);
    \draw[intarrow, bend left=15] (st0) to node[labelstyle, left, blue, pos=0.5] {Interrupt} (mt);
    \draw[intarrow, bend left=15] (st1) to node[labelstyle, right, blue, pos=0.5] {Interrupt} (mt);
\end{tikzpicture}
\end{columns}
\end{frame}
