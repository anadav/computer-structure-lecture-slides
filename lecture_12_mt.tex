...
\begin{tikzpicture}[
    ...
    latchcell/.style={draw, minimum width=0.55cm, minimum height=0.55cm, font=\tiny, inner sep=1pt},
    latch t0/.style={latchcell, fill=red!25},
    latch t1/.style={latchcell, fill=yellow!40},
    rscell/.style={draw, minimum width=0.5cm, minimum height=0.14cm, inner sep=0pt},
    rs t0/.style={rscell, fill=red!25},
    rs t1/.style={rscell, fill=yellow!40},
    ...
    \matrix[matrix of nodes, ampersand replacement=\&, row sep=-\pgflinewidth, column sep=0pt,
            nodes={latchcell}] (instptr) {
        |[latch t0, rotate=90]| Inst Ptr \\
        |[latch t1, rotate=90]| Inst Ptr \\
    };
    \draw[decorate, decoration={zigzag, segment length=2mm, amplitude=0.5mm}]
        (instptr-1-1.south west) -- (instptr-1-1.south east);
    ...
    \matrix[matrix of nodes, ampersand replacement=\&, row sep=-\pgflinewidth, column sep=0pt,
            right=0.3cm of fetch, nodes={latchcell}] (instq) {
        |[latch t0, rotate=90]| Inst Q \\
        |[latch t1, rotate=90]| Inst Q \\
    };
    \draw[decorate, decoration={zigzag, segment length=2mm, amplitude=0.5mm}]
        (instq-1-1.south west) -- (instq-1-1.south east);
    ...
    \matrix[matrix of nodes, ampersand replacement=\&, row sep=-\pgflinewidth, column sep=0pt,
            right=0.3cm of decode, nodes={latchcell}] (uopq) {
        |[latch t0, rotate=90]| $\mu$opQ \\
        |[latch t1, rotate=90]| $\mu$opQ \\
    };
    \draw[decorate, decoration={zigzag, segment length=2mm, amplitude=0.5mm}]
        (uopq-1-1.south west) -- (uopq-1-1.south east);

    % Alloc/Rename stage
    \node[pipestage, right=0.3cm of uopq, fill=yellow!40] (alloc) {Alloc\\rename};

    % RS (shared with interleaved entries, single column)
    \matrix[matrix of nodes, ampersand replacement=\&, right=0.3cm of alloc, row sep=-\pgflinewidth, column sep=0pt,
            nodes={rscell}] (rs) {
        |[rs t0]| ~ \\
        |[rs t1]| ~ \\
        |[rs t0]| ~ \\
    ...
    \node[font=\tiny, above=0.01cm of rs] {RS};
    ...
    \matrix[matrix of nodes, ampersand replacement=\&, row sep=-\pgflinewidth, column sep=0pt,
            right=0.3cm of sched, nodes={latchcell}] (rob) {
        |[latch t0, rotate=90]| ROB \\
        |[latch t1, rotate=90]| ROB \\
    };
    \draw[decorate, decoration={zigzag, segment length=2mm, amplitude=0.5mm}]
        (rob-1-1.south west) -- (rob-1-1.south east);
    ...
    % Bottom row - Register boxes with legend
    \node[font=\scriptsize, below=0.6cm of sched.south, anchor=north east, xshift=-0.3cm] (legendlabel) {Legend:};
    \node[bottombox, fill=red!25, right=0.15cm of legendlabel.east, anchor=west] (archstate0) {Arch\\State};
    \node[bottombox, fill=blue!15, right=0.1cm of archstate0] (physreg) {Physical\\Registers};
