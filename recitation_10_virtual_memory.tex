...
\begin{tikzpicture}[
  ...
  \matrix[matrix of nodes,
    ...
    |[fill=correctgreen!30, minimum width=1.2cm]| V \&
...
\matrix[matrix of nodes,
        ...
        column sep=-\pgflinewidth,
        ampersand replacement=\&] (vaddr) {
  |[fill=vpncolor, minimum width=2.2cm]| VPN \&
  |[fill=offsetcolor, minimum width=4cm]| Page Offset (q bits) \\
};
\node[lbl, left=3mm of vaddr] {Virtual:};

% Physical Address (cache indexing) - aligned so set+disp is under offset
\matrix[matrix of nodes,
        nodes={addrfield},
        column sep=-\pgflinewidth,
        below=1cm of vaddr,
        ampersand replacement=\&] (cache) {
  |[fill=pfncolor!50, minimum width=2.2cm]| tag \&
  |[fill=memorycolor, minimum width=2cm]| set (c bits) \&
  |[fill=highlightorange, minimum width=2cm]| disp (b bits) \\
};
\node[lbl, left=3mm of cache] {Physical:};

% Brace showing set+disp contained in offset
\draw[decorate, decoration={brace, amplitude=4pt, mirror}, thick, correctgreen]
  (cache-1-2.south west) -- (cache-1-3.south east)
  node[midway, below=2pt, font=\scriptsize, correctgreen] {b + c $\leq$ q};

% Arrow from offset to set+disp region
\draw[<-, thick, correctgreen]
  ([yshift=-1mm]vaddr-1-2.south) -- ([yshift=1mm]cache-1-2.north -| vaddr-1-2.south)
  node[midway, right, font=\scriptsize] {contained in offset};
