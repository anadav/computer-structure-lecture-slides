...
% Semantic colors for address fields (for consistency)
\colorlet{vpncolor}{lightblue}
\colorlet{pfncolor}{correctgreen!30}
\colorlet{offsetcolor}{highlightyellow}
\colorlet{validcolor}{cyan!30}
\colorlet{modifiedcolor}{highlightyellow}
\colorlet{protcolor}{lightblue}
\colorlet{tlbcolor}{correctgreen!30}
\colorlet{ptcolor}{lightblue!30}
\colorlet{ptbrcolor}{highlightorange}
...
\begin{center}
\begin{tikzpicture}[
  ptefield/.style={draw, minimum height=10mm, inner sep=3mm, font=\small\bfseries},
]
  \matrix[matrix of nodes,
          nodes={ptefield},
          column sep=-\pgflinewidth,
          ampersand replacement=\&] (pte) {
    |[fill=validcolor, minimum width=1.2cm]| V \&
    |[fill=highlightyellow, minimum width=1.2cm]| M \&
    |[fill=pfncolor, minimum width=4cm]| PFN \\
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.85,
  ...
  \matrix[matrix of nodes,
    ...
    |[fill=vpncolor, minimum width=2cm]| VPN \&
    |[fill=offsetcolor, minimum width=1.5cm]| offset \\
  };
  \node[lbl, above=0mm of vaddr] {Virtual Address};

  % Page Table - positioned relative to VPN field
  \node[draw, fill=ptcolor, minimum width=2.2cm, minimum height=1.6cm,
        below=0.8cm of vaddr-1-1, align=center] (pt) {Page\\Table};

  % PTBR - positioned relative to page table
  \node[draw, fill=ptbrcolor, inner sep=3mm,
  ...
  \matrix[matrix of nodes,
    ...
    |[fill=pfncolor, minimum width=2cm]| PFN \&
    |[fill=offsetcolor, minimum width=1.5cm]| offset \\
  };
...
\section{Question 1: Virtual Memory}
...
\pause
\textbf{Step 1:} Determine field sizes
\begin{itemize}
  \item PFN is \textcolor{correctgreen}{23 bits} (from PTE structure)
  \item Physical address (32 bits) $=$ PFN $+$ offset $\Rightarrow$ offset $= 32 - 23 = $ \textcolor{orange}{9 bits}
  \item VPN $=$ [virt addr] $-$ [offset] $= 30 - 9 = $ \textcolor{blue}{21 bits}
\end{itemize}

\pause
\textbf{Step 2:} Parse virtual address 0x0A100100

\begin{center}
\begin{bytefield}[bitwidth=1.1em, endianness=big, boxformatting={\centering\scriptsize}]{30}
  \bitheader{0,8,9,29} \\
  \bitbox{21}[bgcolor=vpncolor]{0x050800}
  \bitbox{9}[bgcolor=offsetcolor]{0x100} \\[-.5ex]
  \bitbox[]{21}{\tiny VPN}
  \bitbox[]{9}{\tiny offset}
\end{bytefield}
\end{center}
\vspace{-3mm}
\pause
\textbf{Step 3:} Calculate PTE address
\begin{itemize}
  \item PTE addr $=$ PTBR $+$ VPN $\times$ 4 $=$ 0x1000 $+$ 0x050800 $\times$ 4 $=$ \textbf{0x00201200}
\end{itemize}
\end{frame}

\begin{frame}{Question 1: Address Translation (cont.)}
\textbf{Step 4:} Read and parse PTE at address 0x00201200

Content: 0xC0 01 00 94 (little-endian) $\rightarrow$ 0x940100C0

\begin{center}
\begin{bytefield}[bitwidth=0.9em, endianness=big, boxformatting={\centering\scriptsize}]{32}
  \bitheader{0,22,23,25,26,27,30,31} \\
  \bitbox{1}[bgcolor=validcolor]{1}
  \bitbox{4}[bgcolor=lightblue]{0010}
  \bitbox{1}[bgcolor=highlightyellow]{0}
  \bitbox{3}[bgcolor=gray!20]{000}
  \bitbox{23}[bgcolor=pfncolor]{0x0001C0} \\[-.5ex]
  \bitbox[]{1}{\tiny V}
  \bitbox[]{4}{\tiny Prot}
  \bitbox[]{1}{\tiny M}
  \bitbox[]{3}{\tiny --}
...
\pause
\textbf{Step 5:} Construct physical address

\begin{center}
\begin{bytefield}[bitwidth=0.9em, endianness=big, boxformatting={\centering\scriptsize}]{32}
  \bitheader{0,8,9,31} \\
  \bitbox{23}[bgcolor=pfncolor]{0x0001C0}
  \bitbox{9}[bgcolor=offsetcolor]{0x100} \\[-.5ex]
  \bitbox[]{23}{\tiny PFN}
  \bitbox[]{9}{\tiny offset}
\end{bytefield}
...
Physical address $=$ (PFN $\ll$ 9) $|$ offset $=$ (0x0001C0 $\ll$ 9) $|$ 0x100 $=$ \textbf{0x000380A0}
...
\column{0.4\textwidth}
\begin{tikzpicture}[scale=0.8,
  addrfield/.style={draw, minimum height=7mm, inner sep=2mm, font=\scriptsize},
  lbl/.style={font=\tiny\bfseries},
  arrow/.style={->, thick, >=stealth},
]
  % Virtual Address - wider fields
  \matrix[matrix of nodes,
          nodes={addrfield},
          column sep=-\pgflinewidth,
          ampersand replacement=\&] (vaddr) {
    |[fill=vpncolor, minimum width=2cm]| VPN \&
    |[fill=offsetcolor, minimum width=1.4cm]| Offset \\
  };
  \node[lbl, above=0mm of vaddr] {Virtual Address};

  % TLB and Page Table - stacked below VPN
  \node[draw, fill=tlbcolor, minimum width=1.8cm, minimum height=8mm,
        below=0.6cm of vaddr-1-1] (tlb) {TLB};
  \node[draw, fill=ptcolor, minimum width=1.8cm, minimum height=8mm,
        below=2mm of tlb, align=center, font=\scriptsize] (pt) {Page Table};

  % Physical Address - aligned with virtual address
  \matrix[matrix of nodes,
          nodes={addrfield},
          column sep=-\pgflinewidth,
          anchor=east,
          ampersand replacement=\&] (paddr) at (vaddr.east |- pt.south) [yshift=-0.6cm] {
    |[fill=pfncolor, minimum width=2cm]| PFN \&
    |[fill=offsetcolor, minimum width=1.4cm]| Offset \\
  };
  \node[lbl, below=0mm of paddr] {Physical Address};
  ...
  % Arrows
  % VPN -> TLB (hit - fast path)
  \draw[arrow, correctgreen!70!black] (vaddr-1-1.south) -- (tlb.north)
    node[midway, left, font=\tiny] {hit};

  % TLB -> Page Table (miss)
  \draw[arrow, dashed, incorrectred] (tlb.south) -- (pt.north)
    node[midway, left, font=\tiny] {miss};

  % TLB/PT -> PFN
  \draw[arrow, correctgreen!70!black] (pt.south) -- (paddr-1-1.north)
    node[midway, left, font=\tiny] {PFN};

  % Offset passthrough
  \draw[arrow, dashed, orange!80!black] (vaddr-1-2.south) -- (paddr-1-2.north)
    node[midway, right, font=\tiny] {same};

\end{tikzpicture}
\end{columns}
\end{frame}

\section{Question 2: Cache Optimization}

\begin{frame}{Question 2a: Address Structure}
\textbf{Given:}
\begin{itemize}
...
\textbf{Question:} Describe the virtual and physical address structure.

\pause
\textbf{Solution:}
\begin{itemize}
\item Virtual address space is $2^n$ bytes $\rightarrow$ virtual address is $\log_2(2^n) = $ \textcolor{blue}{n bits}
\item Physical memory is $2^m$ bytes $\rightarrow$ physical address is $\log_2(2^m) = $ \textcolor{blue}{m bits}
\item Page size is $2^q$ bytes $\rightarrow$ offset field is $\log_2(2^q) = $ \textcolor{orange}{q bits}
\end{itemize}

\begin{center}
\begin{tikzpicture}[
  addrfield/.style={draw, minimum height=8mm, inner sep=2mm, font=\small},
  lbl/.style={font=\small},
]
  % Virtual Address
  \matrix[matrix of nodes, nodes={addrfield}, column sep=-\pgflinewidth,
          ampersand replacement=\&] (vaddr) {
    |[fill=vpncolor, minimum width=3.2cm]| VPN (n-q bits) \&
    |[fill=offsetcolor, minimum width=2.5cm]| Page Offset (q bits) \\
  };
  \node[lbl, right=3mm of vaddr] {Virtual Address};

  % Physical Address - anchor east to align Page Offset fields
  \matrix[matrix of nodes, nodes={addrfield}, column sep=-\pgflinewidth,
          anchor=north east, ampersand replacement=\&] (paddr) at ([yshift=-8mm]vaddr.south east) {
    |[fill=pfncolor, minimum width=2.5cm]| PFN (m-q bits) \&
    |[fill=offsetcolor, minimum width=2.5cm]| Page Offset (q bits) \\
  };
  \node[lbl, right=3mm of paddr] {Physical Address};
\end{tikzpicture}
\end{center}
\end{frame}

\begin{frame}{Question 2b: Cache and Virtual Memory Optimization}
...
\begin{center}
\begin{tikzpicture}[scale=0.85,
  addrfield/.style={draw, minimum height=6mm, text height=1.5ex, text depth=0.5ex, inner sep=2mm, font=\small},
  lbl/.style={font=\small},
]

% Virtual Address - Page Offset must be visually >= set + disp
\matrix[matrix of nodes,
        nodes={addrfield},
        column sep=-\pgflinewidth,
        ampersand replacement=\&] (vaddr) {
  |[fill=vpncolor, minimum width=2cm]| VPN \&
  |[fill=offsetcolor, minimum width=5cm]| Page Offset (q bits) \\
};
\node[lbl, left=3mm of vaddr] (vlbl) {Virtual:};

% Physical Address - positioned relative to vaddr.south east with anchor north east
\matrix[matrix of nodes,
        nodes={addrfield},
        column sep=-\pgflinewidth,
        anchor=north east,
        ampersand replacement=\&] (cache) at ([yshift=-8mm]vaddr.south east) {
  |[fill=pfncolor!50, minimum width=2cm]| tag \&
  |[fill=memorycolor, minimum width=2.2cm]| set (c bits) \&
  |[fill=highlightorange, minimum width=2.2cm]| disp (b bits) \\
};
\node[lbl, anchor=east] at (vlbl.east |- cache) {Physical:};

% Dashed lines from Page Offset boundaries down to show containment
\draw[dashed, gray] (vaddr-1-2.south west) -- (vaddr-1-2.south west |- cache-1-1.north);
\draw[dashed, gray] (vaddr-1-2.south east) -- (cache-1-3.north east);

% Brace showing set+disp contained in offset
\draw[decorate, decoration={brace, amplitude=3pt, mirror}, thick, correctgreen]
  (cache-1-2.south west) -- (cache-1-3.south east)
  node[midway, below=1pt, font=\scriptsize, correctgreen] {$q \geq b + c$};
