...
\usepackage{ragged2e}
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, calc, tikzmark, shapes.misc, shapes.multipart, fit, decorations.pathreplacing, matrix}
...
\author{Digital Computer Systems}
\date{234267}
...
\begin{center}
\begin{tikzpicture}[scale=0.9]
  \node[draw, rectangle split, rectangle split horizontal, rectangle split parts=2] (vaddr) {
    \nodepart{one} VPN
    \nodepart{two} Page Offset (q bits)
  };
  
  \node[draw, rectangle split, rectangle split horizontal, rectangle split parts=3] (cache) at (0,-2) {
    \nodepart{one} tag
    \nodepart{two} set (c bits)
    \nodepart{three} disp (b bits)
  };
  
  \draw[<->, thick, correctgreen] (vaddr.two south) -- (cache.three north) node[midway, right] {contained in offset};
\end{tikzpicture}
\end{center}
