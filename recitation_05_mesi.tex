...
%==========================================
\begin{frame}{Memory Models and MESI}
\begin{block}{Connection to MESI}
Memory models define when stores become \textbf{globally visible} to other processors.
MESI protocol is the \emph{mechanism} that makes stores visible --- but \textbf{when} they're drained from the store buffer is a memory model decision.
\end{block}
...
Modern processors reorder instructions and buffer stores for performance.
The memory model tells you which orderings you can rely on.
\end{alertblock}
\end{frame}
...
%==========================================
\begin{frame}{Memory Model Exercise -- Setup}
\textbf{Consider a simple memory model with these rules:}
\begin{enumerate}
\item Stores to the \textbf{same} location are totally ordered
\item A load returns the value of the most recent store (in happens-before order) to that location
\item Within a processor, \textbf{all operations to the same location} are ordered by program order
\item Operations to \textbf{different locations} may be freely reordered
\end{enumerate}

\vspace{1em}
\begin{center}
\textbf{Initial state:} \texttt{X = 0, Y = 0}
\vspace{0.5em}

\begin{tabular}{c|c}
\textbf{Processor 0} & \textbf{Processor 1} \\
\hline
\texttt{Store [X] $\leftarrow$ 1} & \texttt{Store [Y] $\leftarrow$ 1} \\
\texttt{r1 $\leftarrow$ Load [Y]} & \texttt{r2 $\leftarrow$ Load [X]} \\
\end{tabular}
\end{center}

\vspace{0.5em}
Is the outcome \texttt{r1 = 0, r2 = 0} possible? \textbf{Prove using happens-before analysis.}
\end{frame}

%==========================================
\begin{frame}{Memory Model Exercise -- Solution}
\textbf{Is \texttt{r1 = 0, r2 = 0} possible?}
...
\begin{itemize}
\item P0's store is to \texttt{X}, P0's load is from \texttt{Y} --- \textbf{different locations}
\item P1's store is to \texttt{Y}, P1's load is from \texttt{X} --- \textbf{different locations}
\item By rule 4, operations to different locations may be reordered
\end{itemize}

\pause
\vspace{0.5em}
\textbf{Valid execution order:}
\begin{enumerate}
\item P0 executes load from Y $\rightarrow$ reads 0 (r1 = 0)
\item P1 executes load from X $\rightarrow$ reads 0 (r2 = 0)
\item P0 executes store to X
\item P1 executes store to Y
\end{enumerate}

\pause
\vspace{0.5em}
\begin{tcolorbox}[colback=green!10, colframe=green!50!black, top=1mm, bottom=1mm]
\textbf{Key insight:} No happens-before chain connects P0's store to P1's load (or vice versa).
The loads ``raced'' ahead of the stores because there's no ordering constraint between operations to different locations.
\end{tcolorbox}
\end{frame}

%==========================================
\begin{frame}{Memory Model Exercise 2 -- Total Store Order}
\small
Stores to the same location have a total order visible to all processors.
...
\begin{tikzpicture}[scale=0.82]
  ...
  % Program order arrows (loads not reordered)
  \draw[hbprogorder] (p2-2-2.south) -- (p2-4-2.north)
    node[hblabel, midway, left=2pt, draw, thin] {\tiny 1};
  \draw[hbprogorder] (p3-2-2.south) -- (p3-4-2.north)
    node[hblabel, midway, left=2pt, draw, thin] {\tiny 2};

  \onslide<2->{
    % Cross-processor: S1 -> L1 (r1=1)
    \draw[hbcross, varX] (p0-2-2.east) to[out=10, in=160]
      node[hblabel, above=0pt, pos=0.5, draw, thin] {\tiny 3} (p2-2-1.west);
    % Cross-processor: S2 -> L2 (r2=2)
    \draw[hbcross, varX, densely dashed] (p1-2-2.east) to[out=-10, in=170]
      node[hblabel, below=0pt, pos=0.5, draw, thin] {\tiny 4} (p2-4-2.west);
  }
  \onslide<3->{
    % Cross-processor: S2 -> L3 (r3=2)
    \draw[hbcross, varX, densely dashed] (p1-2-2.east) to[out=10, in=160]
      node[hblabel, above=0pt, pos=0.6, draw, thin] {\tiny 5} (p3-2-2.west);
    % Cross-processor: S1 -> L4 (r4=1) - this creates the contradiction
    \draw[hbcross, varX, looseness=1.2] (p0-2-2.east) to[out=-20, in=200]
      node[hblabel, below=0pt, pos=0.5, draw, thin] {\tiny 6} (p3-4-1.west);
  }
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
  ...
  \onslide<2->{\item r1=1 $\Rightarrow$ S1 $\rightarrow$ L1; \quad r2=2 $\Rightarrow$ S2 $\rightarrow$ L2; \quad Chain: S1 $\rightarrow$ L1 $\rightarrow$ L2, so S1 before S2}
  \onslide<3->{\item r3=2 $\Rightarrow$ S2 $\rightarrow$ L3; \quad r4=1 $\Rightarrow$ S1 $\rightarrow$ L4; \quad Chain: S2 $\rightarrow$ L3 $\rightarrow$ L4, so S2 before S1}
  \onslide<4->{\item \textbf{Contradiction!} S1 before S2 AND S2 before S1 $\Rightarrow$ \textbf{Not allowed.}}
\end{enumerate}
\end{frame}

%==========================================
\begin{frame}{Memory Model Exercise 3 -- Three Processors (x86/TSO)}
\small
\textbf{Initial:} \texttt{X = Y = Z = 0}
\hfill Is \texttt{r1 = 1, r2 = 1, r3 = 0} possible?

\vspace{0.2em}
\begin{center}
\begin{tikzpicture}[scale=0.85]
  % Processor 0
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.05cm, row sep=0.1cm,
    column 1/.style={font=\scriptsize, gray, anchor=east},
    column 2/.style={font=\ttfamily\scriptsize, anchor=west}
  ] (p0) at (0, 0) {
    |[font=\bfseries\scriptsize]| \phantom{S0} \& |[font=\bfseries\scriptsize]| P0 \\
    S0 \& |[alias=s0]| St [\textcolor{varX}{X}] <- 1 \\
  };
  ...
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.05cm, row sep=0.1cm,
    ...
    |[alias=l1lbl]| L1 \& |[alias=l1]| r1 <- Ld [\textcolor{varX}{X}] \\
    \rule{0pt}{0.2cm} \& \\
    S1 \& |[alias=s1]| St [\textcolor{varY}{Y}] <- 1 \\
  };
  ...
  \matrix[matrix of nodes, ampersand replacement=\&, anchor=north, column sep=0.05cm, row sep=0.1cm,
    ...
    |[alias=l2lbl]| L2 \& |[alias=l2]| r2 <- Ld [\textcolor{varY}{Y}] \\
    \rule{0pt}{0.2cm} \& \\
    L3 \& |[alias=l3]| r3 <- Ld [\textcolor{varX}{X}] \\
  };

  \onslide<2->{
    % Program order arrows
    \draw[hbprogorder] (l1.south) -- (s1.north)
      node[hblabel, midway, left=1pt, draw, thin] {\tiny 1};
    \draw[hbprogorder] (l2.south) -- (l3.north)
      node[hblabel, midway, left=1pt, draw, thin] {\tiny 2};
  }
  \onslide<3->{
    % Cross-processor: S0 -> L1 (r1=1 means L1 saw S0)
    \draw[hbcross, varX] (s0.east) -- (l1.west)
      node[hblabel, above=0pt, pos=0.5, draw, thin] {\tiny 3};
    % Cross-processor: S1 -> L2 (r2=1 means L2 saw S1)
    \draw[hbcross, varY] (s1.east) -- (l2.west)
      node[hblabel, above=0pt, pos=0.5, draw, thin] {\tiny 4};
  }
\end{tikzpicture}
\end{center}

\vspace{-0.2cm}
\only<1>{\phantom{\textbf{Analysis:}}}%
\only<2>{\footnotesize\textbf{TSO:} L1$\rightarrow$S1 (load before store), L2$\rightarrow$L3 (loads ordered)}%
\only<3>{\footnotesize\textbf{Cross-proc:} r1=1 $\Rightarrow$ S0$\rightarrow$L1. \quad r2=1 $\Rightarrow$ S1$\rightarrow$L2.}%
\only<4->{\footnotesize\textbf{Chain:} S0 $\xrightarrow{3}$ L1 $\xrightarrow{1}$ S1 $\xrightarrow{4}$ L2 $\xrightarrow{2}$ L3. Thus S0 $\rightarrow$ L3, so L3 must see X=1 $\Rightarrow$ r3=1. \textbf{Not possible!}}
\end{frame}
