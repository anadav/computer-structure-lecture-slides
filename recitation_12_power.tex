...
\column{0.48\textwidth}
\textbf{Voltage-Frequency Points (GHz):}
\vspace{-2mm}
\begin{table}
\centering
\scriptsize
\begin{tabular}{|c|>{\columncolor{gray!10}}c|>{\columncolor{gray!25}}c|}
\hline
V (V) & Small & Large \\
\hline
0.6 & 1.0 & 0.7 \\
...
\begin{frame}{Q4a: Voltage-Frequency Analysis}
\textbf{Second, we compute the power for each voltage-frequency point:}
...
\begin{frame}{Q4a: Computing Power (Example)}
\begin{columns}[t]
\column{0.4\textwidth}
\textbf{From V-F table (first two rows):}
\vspace{1mm}

\scriptsize
\begin{tabular}{|c|>{\columncolor{gray!10}}c|>{\columncolor{gray!25}}c|}
\hline
\makecell{V (V)} & \makecell{Small (GHz)} & \makecell{Large (GHz)} \\
\hline
0.6 & 1.0 & 0.7 \\
0.65 & 1.25 & 1.0 \\
\hline
...
\textbf{Power equations:}
\vspace{-2mm}
$$P_{act} = C \cdot V^2 \cdot f \quad;\quad P_{tot} = P_{act} + \text{Leak}$$

\textbf{At V = 0.6V:}
\begin{itemize}\scriptsize
    \item Small: $P_{act} = 500 \cdot 0.6^2 \cdot 1.0 = 0.18$W $\Rightarrow P_{tot} = 0.68$W
    \item Large: $P_{act} = 1500 \cdot 0.6^2 \cdot 0.7 = 0.38$W $\Rightarrow P_{tot} = 1.38$W
\end{itemize}

\textbf{At V = 0.65V:}
\begin{itemize}\scriptsize
    \item Small: $P_{act} = 500 \cdot 0.65^2 \cdot 1.25 = 0.26$W $\Rightarrow P_{tot} = 0.76$W
    \item Large: $P_{act} = 1500 \cdot 0.65^2 \cdot 1.0 = 0.63$W $\Rightarrow P_{tot} = 1.63$W
\end{itemize}
\end{columns}

\pause
\vspace{2mm}
\alert{Continue for all voltage points to build the full power table...}
\end{frame}

\begin{frame}{Q4a: Power Table Analysis (4W Budget)}
\textbf{Find maximum frequency within 4W budget:}
\vspace{2mm}
\begin{columns}[t]
\column{0.57\textwidth}
\scriptsize
\begin{tabular}{|c|>{\columncolor{gray!10}}c|>{\columncolor{gray!25}}c|>{\columncolor{gray!10}}c|>{\columncolor{gray!25}}c|>{\columncolor{gray!10}}c|>{\columncolor{gray!25}}c|}
\hline
\makecell{V\\(V)} &
\makecell{Freq\\Small\\(GHz)} &
\makecell{Freq\\Large\\(GHz)} &
\makecell{$P_{act}$\\Small\\(W)} &
\makecell{$P_{act}$\\Large\\(W)} &
\makecell{$P_{tot}$\\Small\\(W)} &
\makecell{$P_{tot}$\\Large\\(W)} \\
\hline
0.6 & 1.0 & 0.7 & 0.18 & 0.38 & 0.68 & 1.38 \\
0.65 & 1.25 & 1.0 & 0.26 & 0.63 & 0.76 & 1.63 \\
0.7 & 1.5 & 1.35 & 0.37 & 0.99 & 0.87 & 1.99 \\
0.75 & 1.75 & 1.75 & 0.49 & 1.48 & 0.99 & 2.48 \\
0.8 & 2.0 & 2.25 & 0.64 & 2.16 & 1.14 & 3.16 \\
\only<2->{\cellcolor{cyan!35}}0.85 & 2.25 & \only<2->{\cellcolor{cyan!50}}2.5 & 0.81 & 2.71 & 1.31 & \only<2->{\cellcolor{green!40}}3.71 \\
0.9 & 2.5 & 3.0 & 1.01 & 3.65 & 1.51 & 4.65 \\
0.95 & 2.75 & 3.5 & 1.24 & 4.74 & 1.74 & 5.74 \\
\only<2->{\cellcolor{orange!40}}1.0 & \only<2->{\cellcolor{orange!60}}3.0 & 4.0 & 1.50 & 6.00 & \only<2->{\cellcolor{yellow!50}}2.00 & 7.00 \\
\hline
\end{tabular}

\column{0.43\textwidth}
\pause
\textbf{Within 4W budget:}

\vspace{2mm}
\textbf{Small core:} \\
All points fit! Max at \colorbox{orange!40}{V=1.0V}: \\
\colorbox{orange!60}{f=3.0 GHz}, \colorbox{yellow!50}{P=2.0W}

\vspace{3mm}
\textbf{Large core:} \\
Max at \colorbox{cyan!35}{V=0.85V}: \\
\colorbox{cyan!50}{f=2.5 GHz}, \colorbox{green!40}{P=3.71W}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[->,thick] (P0 |- P1) -- ([yshift=-3mm]P0);
