...
\begin{frame}{Q6: Power Table Analysis (2W per Core)}
...
0.7 & \only<2->{\cellcolor{cyan!40}}1.35 & 1.5 & 0.99 & 0.37 & \only<2->{\cellcolor{green!40}}1.99 & 0.87 \\
...
1 & 4 & \only<2->{\cellcolor{yellow!50}}3 & 6.00 & 1.50 & 7.00 & \only<2->{\cellcolor{orange!50}}2.00 \\
\hline
\end{tabular}
...
\only<1>{Finding frequencies that fit within the 2W power budget per core}

\only<2->{
\textbf{Large cores:} 1.35 GHz \\
\textbf{Small cores:} 3 GHz
}
\end{columns}
\end{frame}

\begin{frame}{Q6: Performance Calculation}
