...
% Title and field labels above VA
\node[anchor=south, font=\small\bfseries] at ($(vaA_vpn.north)!0.5!(vaA_offset.north)$) {Write Virtual Address A};
\node[mylabel, above=1mm of vaA_vpn] {VPN};
\node[mylabel, above=1mm of vaA_offset] {OFFSET};

% Markers for braces - positioned at bits 13 and 6
\coordinate (vaA_bit13) at ($(vaA_vpn.south east) + (-0.25cm, 0)$);
...
% PA for address A - positioned relative to set label using at with yshift
\node[addrbox, fill=green!20, minimum width=2cm, anchor=north east] (paA_pfn) at ([yshift=-0.5cm]vaA_set_label.south -| vaA_offset.east) {\texttt{0...0000}};
\node[addrbox, fill=cyan!20, minimum width=2.5cm, anchor=west] (paA_offset) at (paA_pfn.east) {\texttt{0000 0000 0000}};
\node[font=\footnotesize, left=2mm of paA_pfn] {PA:};
...
% VB for address B
\node[addrbox, fill=green!20, minimum width=2cm] (vbB_vpn) at (6.5, 2.5) {\texttt{0011...00}\textcolor{red}{\textbf{00}}};
...
% Title and field labels above VB
\node[anchor=south, font=\small\bfseries] at ($(vbB_vpn.north)!0.5!(vbB_offset.north)$) {Read Virtual Address B};
\node[mylabel, above=1mm of vbB_vpn] {VPN};
\node[mylabel, above=1mm of vbB_offset] {OFFSET};

% Markers for braces - positioned at bits 13 and 6
\coordinate (vbB_bit13) at ($(vbB_vpn.south east) + (-0.25cm, 0)$);
...
% PB for address B - positioned relative to set label using at with yshift
\node[addrbox, fill=green!20, minimum width=2cm, anchor=north east] (pbB_pfn) at ([yshift=-0.5cm]vbB_set_label.south -| vbB_offset.east) {\texttt{0...0011}};
\node[addrbox, fill=cyan!20, minimum width=2.5cm, anchor=west] (pbB_offset) at (pbB_pfn.east) {\texttt{0000 0000 0000}};
