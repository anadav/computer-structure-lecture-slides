...
\begin{frame}{Question 3 (cont.): Aliasing Example}
...
% Define Y level for address boxes at top
\def\addry{4.5}
...
\node[anchor=north west, font=\small\bfseries] at (0, 3.2) {Write Virtual Address A};

% VA for address A
\node[addrbox, fill=green!20, minimum width=2cm] (vaA_vpn) at (0, 2.5) {\texttt{0000...0000}};
...
% Arrow
\draw[myarrow] ($(vaA_vpn.east)!0.5!(vaA_offset.west)$) -- ++(0, -0.5) node[midway, right] {};

% PA for address A
\node[addrbox, fill=green!20, minimum width=2cm] (paA_pfn) at (0, 1.5) {\texttt{0...0000}};
...
% Braces and labels for A
\draw[decorate, decoration={brace, amplitude=5pt, mirror}] (vaA_offset.south west) -- (vaA_offset.south east) node[midway, below=3pt, font=\scriptsize, blue] {Set: [13:6] = 0};
...
\node[anchor=north west, font=\small\bfseries] at (6.5, 3.2) {Read Virtual Address B};

% VB for address B
\node[addrbox, fill=green!20, minimum width=2cm] (vbB_vpn) at (6.5, 2.5) {\texttt{0011...0000}};
...
% Arrow
\draw[myarrow] ($(vbB_vpn.east)!0.5!(vbB_offset.west)$) -- ++(0, -0.5) node[midway, right] {};

% PB for address B
\node[addrbox, fill=green!20, minimum width=2cm] (pbB_pfn) at (6.5, 1.5) {\texttt{0...0011}};
...
% Braces and labels for B
\draw[decorate, decoration={brace, amplitude=5pt, mirror}] (vbB_offset.south west) -- (vbB_offset.south east) node[midway, below=3pt, font=\scriptsize, blue] {Set: [13:6] = 0};
...
\vspace{0.3em}
\textbf{Problem:} Both addresses use the same set (0), but have different physical addresses (Tag 0 vs Tag 3). Without including PA bits [13:12] in the tag, we would incorrectly get a cache hit!
