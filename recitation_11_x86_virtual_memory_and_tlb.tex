...
\begin{tikzpicture}[
    ...
    comparator/.style={circle, draw, thick, minimum size=0.6cm, font=\small}
]
...
\node[bitlabel, anchor=south west] at (pfn.north west) {40};
\node[bitlabel, anchor=south east] at (pfn.north east) {13};
\node[bitlabel, anchor=south west, right=0mm of pfn.north east] {12};

% Cache ways - moved higher
\node[draw, thick, minimum width=1.5cm, minimum height=2.5cm, anchor=north west]
     (way0) at (5, 3.2) {};
\node[mylabel] at (way0.center) {Way 0};

\node[draw, thick, minimum width=1.5cm, minimum height=2.5cm, anchor=north west]
     (way1) at (7, 3.2) {};
\node[mylabel] at (way1.center) {Way 1};
...
% Way 0 comparator lower than way 1
\node[comparator] (comp0) at ([xshift=0.5cm, yshift=-0.6cm]way0.south east) {=};
\node[comparator] (comp1) at ([xshift=0.5cm, yshift=-0.3cm]way1.south east) {=};

% Arrows from PFN to comparators with horizontal connector
\draw[myarrow] (pfn.south) |- (comp0.west);
\draw[myarrow] (pfn.south) |- (comp1.west);

% PFN bits label on horizontal connector
\node[mylabel, below] at (pfn.south) {40:12};
