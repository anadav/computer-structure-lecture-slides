...
\begin{frame}{Question 3: What if we didn't use [13:12] for tag?}
...
% Set box - bits 13:6
\node[addrbox, fill=cyan!40, minimum width=1.2cm, anchor=south west] (set) at (2.5, \addry) {Set};
\node[bitlabel, anchor=north west] (bit13) at ([xshift=-1.5mm]set.south west) {13};
\node[bitlabel, right=-2.5mm of bit13] (bit12) {12};
\node[bitlabel, right=-2.5mm of bit12] (bit11) {11};
\node[bitlabel, anchor=north east] at (set.south east) {6};
...
\node[addrbox, fill=pink!30, minimum width=0.8cm, anchor=south west] (offset) at (3.7, \addry) {Offset};
\node[bitlabel, anchor=north west] at (offset.south west) {5};
\node[bitlabel] at (offset.south east) {0};
...
\draw[decorate, decoration={brace, amplitude=5pt, mirror}] (pbB_pfn.south west) -- (pbB_pfn.south east) node[midway, below=3pt, font=\scriptsize, green!50!black] {Tag: 3};
\node[font=\scriptsize, red, below=8mm of pbB_pfn] {(0 if we don't take [13:12])};
