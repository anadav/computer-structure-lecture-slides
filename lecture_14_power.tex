...
\begin{tikzpicture}[scale=0.7,
    ...
    % Section title at same x as "No Turbo", close to boosted cores
    \node[section title] at (no-turbo-title |- t-core-0.north) [above=1mm] {With Turbo};

    % TDP line higher (at 2cm from bottom instead of 1.8cm)
    \draw[tdp line] ([xshift=-2mm]t-core-0.south west) ++(0, 2cm) coordinate (tdp-start) --
    ...
    % Boost arrow from TDP line to top of boosted core (4mm right)
    \draw[->, green!60!black, very thick]
        ([xshift=4mm]tdp-start -| t-core-1.east) -- ([xshift=4mm]t-core-1.north east)
