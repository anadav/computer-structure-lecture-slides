...
\begin{column}{0.55\textwidth}
\begin{itemize}
    \item \textbf{C0: Active} -- CPU executing, P-states effective
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
...
\begin{column}{0.55\textwidth}
\begin{itemize}
    \item \textbf{C1: Halt} -- Stop pipeline \& most clocks
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\begin{tikzpicture}[scale=0.8]
...
\begin{column}{0.55\textwidth}
\begin{itemize}
    \item \textbf{C3: Deep Sleep} -- Stop all clocks, flush caches
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\begin{tikzpicture}[scale=0.8]
...
\begin{column}{0.55\textwidth}
\begin{itemize}
    \item \textbf{C6: Power Gate} -- Save state, power off, $P \approx 0$
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\begin{tikzpicture}[scale=0.8]
...
\begin{tikzpicture}[scale=0.7]
    % No Turbo
    \node[font=\small\bfseries] at (2.2, 3.2) {No Turbo};
    \foreach \i in {0,1,2,3} {
        \fill[corecolor] (\i*1.2, 0) rectangle (\i*1.2+1, 2.5);
    }
    \node[white, font=\scriptsize] at (0.5, 1.25) {0};
    \node[white, font=\scriptsize] at (1.7, 1.25) {1};
    \node[white, font=\scriptsize] at (2.9, 1.25) {2};
    \node[white, font=\scriptsize] at (4.1, 1.25) {3};
    \draw[dashed, red] (-0.2, 2.5) -- (5.2, 2.5);
    \node[right, font=\tiny, red] at (5.3, 2.5) {TDP};

    % Turbo - shifted down
    \node[font=\small\bfseries] at (2.2, -0.5) {With Turbo};
    % Cores 2,3 inactive (power gated)
    \fill[idlecolor] (2.4, -4) rectangle (3.4, -3.8);
    \fill[idlecolor] (3.6, -4) rectangle (4.6, -3.8);
    \node[font=\scriptsize, above] at (2.9, -3.8) {2};
    \node[font=\scriptsize, above] at (4.1, -3.8) {3};
    \node[font=\tiny, gray] at (3.5, -4.3) {(off)};

    % Cores 0,1 boosted
    \fill[corecolor] (0, -4) rectangle (1, -1);
    \fill[corecolor] (1.2, -4) rectangle (2.2, -1);
    \node[white, font=\scriptsize] at (0.5, -2.5) {0};
    \node[white, font=\scriptsize] at (1.7, -2.5) {1};

    \draw[dashed, red] (-0.2, -2) -- (5.2, -2);
    \node[right, font=\tiny, red] at (5.3, -2) {TDP};

    % Boost arrow and label - to the right
    \draw[->, green!60!black, very thick] (2.5, -2.2) -- (2.5, -1.2);
    \node[font=\scriptsize, green!60!black, right] at (2.6, -1.7) {Boost!};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
...
\begin{tabular}{lccc}
\toprule
& \textbf{Big Core} & \textbf{Small Core} & \textbf{Given} \\
\midrule
Area & 4mm$^2$ & 2mm$^2$ & Leakage: 0.25W/mm$^2$ \\
IPC & 3 & 2 & $C = \text{IPC} \times 750$pF \\
