...
\begin{tikzpicture}[
    ...
    \coordinate (col2) at (4, 0);
    \coordinate (col3) at (8, 0);
    ...
    % OR label
    \node[font=\scriptsize, red] at ($(clust-big-box.south)!0.5!(clust-lit-box.north)$) {OR};

    % Title - close to illustration
    \node[approach title, above=2mm of clust-big-box.north] {Clustered Switching};
    ...
    \draw[<->, thick] (inker-big1.south) -- (inker-lit1.north);
    ...
    \draw[<->, thick] (inker-big2.south) -- (inker-lit2.north);
    ...
    % Title - close to illustration (use fit node spanning both vcores)
    ...
    \node[approach title, above=2mm of glob-bigs.north] {Global Scheduling};

    % Description
    \node[desc] at ([yshift=-15mm]col3) {OS sees all cores. Direct task migration. Most flexible.};

    % ============ FLEXIBILITY SCALE ============
    \coordinate (scale-y) at (0, -3.4);
    \draw[->, thick] ([xshift=-1.5cm]scale-y) -- ([xshift=9.5cm]scale-y);
    \node[font=\small, anchor=east] at ([xshift=-1.6cm]scale-y) {Simple};
    \node[font=\small, anchor=west] at ([xshift=9.6cm]scale-y) {Flexible};
    ...
    \node[above=1mm, font=\tiny] at (col3 |- scale-y) {Global};
\end{tikzpicture}
\end{center}
\end{frame}
