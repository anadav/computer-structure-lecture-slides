...
\begin{tikzpicture}[
    y=0.78cm,
    bar youtube/.style={fill=blue!60, draw=blue!80},
    bar gaming/.style={fill=red!50, draw=red!70},
    label/.style={font=\scriptsize, anchor=east},
    spec/.style={font=\tiny, gray},
    value/.style={font=\tiny, anchor=west},
]
    % Scale: 615 max -> 5.8cm width
    \def\scale{0.00943}
    \def\barh{0.28}  % bar height
    \def\gap{0.08}   % gap between youtube and gaming bars

    % Data: {y-position}{laptop}{spec}{youtube}{gaming}
    \foreach \ypos/\laptop/\spec/\yt/\gm in {
        6/{ASUS TUF A16}/{Ryzen 7 7735HS}/615/67,
        5/{Razer Blade 14}/{Ryzen 9 · RTX 4070}/560/175,
        4/{ASUS Zephyrus G14}/{Ryzen 9 · RTX 4060}/516/175,
        3/{HP Victus 16}/{i7-13700H · RTX 4060}/479/118,
        2/{Razer Blade 16}/{i9-13950HX · RTX 4090}/424/152,
        1/{Alienware x16}/{i9-13900HK · RTX 4080}/368/98,
        0/{ASUS ROG G16}/{i7-13650HX · RTX 4060}/301/90%
    } {
        % Laptop name and spec
        \node[label] at (-0.2, \ypos+0.1) {\laptop};
        \node[spec, anchor=east] at (-0.2, \ypos-0.22) {\spec};

        % YouTube bar (top)
        \fill[bar youtube] (0, \ypos+\gap) rectangle (\yt*\scale, \ypos+\gap+\barh);
        \node[value, blue!80] at (\yt*\scale+0.1, \ypos+\gap+\barh/2) {\yt};

        % Gaming bar (bottom)
        \fill[bar gaming] (0, \ypos-\barh) rectangle (\gm*\scale, \ypos);
        \node[value, red!70] at (\gm*\scale+0.1, \ypos-\barh/2) {\gm};
    }

    % X-axis
    \draw[thick, ->] (0, -0.5) -- (6.5, -0.5);

    % X-axis ticks and labels
    \foreach \x in {0, 120, 240, 360, 480, 600} {
        \draw (\x*\scale, -0.4) -- (\x*\scale, -0.6);
        \node[font=\tiny, below] at (\x*\scale, -0.55) {\x};
    }
    \node[font=\scriptsize] at (3, -1.4) {Time [minutes]};

    % Y-axis line
    \draw[thick] (0, -0.5) -- (0, 6.5);

    % Legend (right side, stacked)
    \node[anchor=north west, font=\scriptsize] at (6.8, 6.3) {%
        \begin{tabular}{@{}l@{}}
        \tikz[baseline=-0.5ex]{\fill[bar youtube] (0,0) rectangle (0.35,0.2);} Video \\[1mm]
        \tikz[baseline=-0.5ex]{\fill[bar gaming] (0,0) rectangle (0.35,0.2);} Gaming
        \end{tabular}%
    };

    % Source (below x-axis, right side)
    \node[font=\tiny, gray, anchor=north east] at (8.5, -1.5) {Source: \href{https://jarrods.tech/these-gaming-laptops-have-the-best-battery-life-in-2023/}{jarrods.tech} (2023)};

    % Overlay callouts (each appears on one slide only)
    \only<2>{
        % Callout: 2x difference - arrow right of bars
        \draw[very thick, orange, <->] (6.85, 6.35) -- (6.85, -0.15);
        \node[rectangle callout, callout absolute pointer={(6.85, 3.2)},
              fill=orange!20, font=\scriptsize, align=center] at (8.3, 3.2)
            {$2\times$ range\\impacts\\usability!};
    }

    \only<3>{
        % Callout: ASUS TUF - dGPU usage drains battery fast (left of figure)
        \node[rectangle callout, callout absolute pointer={(-2.8, 6.2)},
              fill=orange!20, font=\scriptsize, align=left, text width=2.4cm,
              anchor=east] at (-3.2, 4.5)
            {Best for video, worst for gaming -- dGPU heavily utilized};
    }
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[
    % Styles
    cpu bar/.style={fill=blue!70, minimum height=7mm, anchor=west, inner sep=0},
    gfx bar/.style={fill=green!60!black, minimum height=7mm, anchor=west, inner sep=0},
    bar label/.style={white, font=\scriptsize},
    scenario label/.style={font=\scriptsize, anchor=east},
]
    % Total bar width
    \def\totalwidth{5.5cm}

    % Heavy CPU scenario (first row, used as reference)
    \node[cpu bar, minimum width=0.8*\totalwidth] (cpu1) {};
    \node[gfx bar, minimum width=0.2*\totalwidth] (gfx1) at (cpu1.east) {};
    \node[bar label] at (cpu1.center) {CPU};
    \node[bar label] at (gfx1.center) {GFX};
    \node[scenario label, left=1mm of cpu1.west] {Heavy CPU:};

    % Total Package Power label with arrows
    \node[above=6mm of cpu1.north, anchor=south, xshift=0.1*\totalwidth, font=\small] (total-label) {Total Package Power};
    \draw[->, thick] (total-label.west) -- (total-label.west -| cpu1.west);
    \draw[->, thick] (total-label.east) -- (total-label.east -| gfx1.east);

    % Heavy GFX scenario
    \node[cpu bar, minimum width=0.3*\totalwidth, below=3mm of cpu1.south west, anchor=north west] (cpu2) {};
    \node[gfx bar, minimum width=0.7*\totalwidth] (gfx2) at (cpu2.east) {};
    \node[bar label] at (cpu2.center) {CPU};
    \node[bar label] at (gfx2.center) {Graphics};
    \node[scenario label, left=1mm of cpu2.west] {Heavy GFX:};

    % Balanced scenario
    \node[cpu bar, minimum width=0.5*\totalwidth, below=3mm of cpu2.south west, anchor=north west] (cpu3) {};
    \node[gfx bar, minimum width=0.5*\totalwidth] (gfx3) at (cpu3.east) {};
    \node[bar label] at (cpu3.center) {CPU};
    \node[bar label] at (gfx3.center) {Graphics};
    \node[scenario label, left=1mm of cpu3.west] {Balanced:};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[scale=0.7,
    ...
    \matrix[matrix of nodes, ampersand replacement=\&,
             column sep=2mm,
             nodes={big core}] (big-cores) {
        |[name=big0]| {\shortstack{Big\\Core}} \& |[name=big1]| {\shortstack{Big\\Core}} \\
    };

    % Little cores matrix - positioned below big cores
    \matrix[matrix of nodes, ampersand replacement=\&,
             column sep=1.5mm,
             below=4mm of big-cores,
             nodes={little core}] (little-cores) {
        |[name=little0]| {\shortstack{Little\\Core}} \& |[name=little1]| {\shortstack{Little\\Core}} \& |[name=little2]| {\shortstack{Little\\Core}} \& |[name=little3]| {\shortstack{Little\\Core}} \\
    };

    % Labels
    \node[section label, above=2mm of big-cores.north] {High performance};
    \node[section label, below=2mm of little-cores.south] {Energy efficient};
...
\begin{tikzpicture}[
    ...
    % Big cores matrix
    \matrix[matrix of nodes, ampersand replacement=\&,
            column sep=1mm, anchor=center,
            nodes={big core}] (clust-big-mat) at ([yshift=10mm]col1) {
        |[name=clust-big1]| \& |[name=clust-big2]| \\
    };
    ...
    \matrix[matrix of nodes, ampersand replacement=\&,
            column sep=1mm, anchor=center,
            nodes={little core}] (clust-lit-mat) at ([yshift=-6mm]col1) {
        |[name=clust-lit1]| \& |[name=clust-lit2]| \\
    };
    ...
    \node[approach title, above=2mm of clust-big-mat.north] {Clustered Switching};
    \node[desc] at ([yshift=-15mm]col1) {OS sees one cluster. Switch entire cluster based on load.};

    % ============ IN-KERNEL SWITCHING ============
    % Virtual core 1 - big core (aligned with clustered big cores)
    \matrix[matrix of nodes, ampersand replacement=\&,
            anchor=center, nodes={big core}] (inker-big1-mat) at ([xshift=-9mm]col2 |- clust-big-mat) {
        |[name=inker-big1]| \\
    };
    % Virtual core 1 - little core (aligned with clustered little cores)
    \matrix[matrix of nodes, ampersand replacement=\&,
            anchor=center, nodes={little core}] (inker-lit1-mat) at ([xshift=-9mm]col2 |- clust-lit-mat) {
        |[name=inker-lit1]| \\
    };
    ...
    % Virtual core 2 - big core
    \matrix[matrix of nodes, ampersand replacement=\&,
            anchor=center, nodes={big core}] (inker-big2-mat) at ([xshift=9mm]col2 |- clust-big-mat) {
        |[name=inker-big2]| \\
    };
    % Virtual core 2 - little core
    \matrix[matrix of nodes, ampersand replacement=\&,
            anchor=center, nodes={little core}] (inker-lit2-mat) at ([xshift=9mm]col2 |- clust-lit-mat) {
        |[name=inker-lit2]| \\
    };
    ...
    % ============ GLOBAL SCHEDULING (HMP) ============
    % Little cores matrix first (to position big cores relative to it)
    \matrix[matrix of nodes, ampersand replacement=\&,
            column sep=1mm, anchor=center,
            nodes={little core}] (glob-lit-mat) at ([yshift=-6mm]col3) {
        |[name=glob-lit1]| \& |[name=glob-lit2]| \& |[name=glob-lit3]| \& |[name=glob-lit4]| \\
    };

    % Big cores matrix - centered above little cores, same y as clustered big cores
    \matrix[matrix of nodes, ampersand replacement=\&,
            column sep=1mm, anchor=center,
            nodes={big core}] (glob-big-mat) at (glob-lit-mat.center |- clust-big-mat.center) {
        |[name=glob-big1]| \& |[name=glob-big2]| \\
    };

    % Title and description
    \node[approach title, above=2mm of glob-big-mat.north] {HMP};
    ...
    % In-Kernel callout (overlay 3 only)
    \only<3>{
...
\begin{tikzpicture}[scale=0.55]
    ...
    \node[font=\tiny] at (5, 1.1) {Idle};

    % Budget buildup arrow
    \draw[->, green!60!black, thick] (4.7, 0.6) -- (5.3, 0.6);
    \node[font=\tiny, green!60!black] at (5, 0.3) {Budget builds};
\end{tikzpicture}
