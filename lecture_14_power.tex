...
\begin{tikzpicture}[scale=0.7,
    ...
    boosted core/.style={fill=corecolor, minimum width=1cm, minimum height=2.3cm, inner sep=0, anchor=south west},
    ...
    % Section title centered above all cores
    \node[section title, above=3mm of $(nt-core-1.north)!0.5!(nt-core-2.north)$] (no-turbo-title) {No Turbo};
    ...
    % === With Turbo Section ===
    % Boosted cores 0,1 (taller) - positioned below first section
    \node[boosted core, below=12mm of nt-core-0.south west, anchor=north west] (t-core-0) {};
    \node[boosted core, core gap] (t-core-1) at (t-core-0.south east) {};
    ...
    \node[font=\tiny, gray, below=2mm of $(t-core-2.south)!0.5!(t-core-3.south)$] {(off)};

    % Section title at same x as "No Turbo", above boosted cores
    \node[section title] at (no-turbo-title |- t-core-0.north) [above=3mm] {With Turbo};

    % TDP line at normal core height (1.8cm from bottom, boosted cores exceed it)
    \draw[tdp line] ([xshift=-2mm]t-core-0.south west) ++(0, 1.8cm) coordinate (tdp-start) --
        ([xshift=2mm]t-core-3.south east |- tdp-start) coordinate (tdp-end)
        node[tdp label, right] {TDP};

    % Boost arrow from TDP line to top of boosted core
    \draw[->, green!60!black, very thick]
        ([xshift=2mm]tdp-start -| t-core-1.east) -- ([xshift=2mm]t-core-1.north east)
