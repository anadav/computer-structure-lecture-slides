...
\node[muxdemux, muxdemux def={Rh=1, Lh=1, w=8, NT=1, NB=16, square pins=1},
      external pins width=0, fill=yellow!20, font=\scriptsize]
    (ireg) at ([yshift=-5mm]scheduler.south) {instruction register};

% Arrow from scheduler to instruction register
\draw[->, thick] (scheduler.south) -- (ireg.tpin 1);

% SIMD lanes - calculate dimensions for centering
\pgfmathsetmacro{\lanewidth}{1.1}      % Lane width in cm (1.8x of original 0.6cm)
\pgfmathsetmacro{\lanespacing}{1.26}   % Spacing between lane centers (1.8x of original 0.7cm)
\pgfmathsetmacro{\numlanesminus}{15}   % Number of lanes minus 1 (for spacing calculation)
\pgfmathsetmacro{\lanesoffset}{\numlanesminus*\lanespacing/2}  % Offset so lane centers are symmetric around ireg center

% SIMD lanes container - offset to center under instruction register
\coordinate (lanes-start) at ([xshift=-\lanesoffset cm, yshift=-0.5cm]ireg.south);

% SIMD lanes using foreach
\foreach \i in {0,...,15} {
    \coordinate (lane\i) at ([xshift=\i*\lanespacing cm]lanes-start);
...
%    \node[rotate=90, font=\fontsize{5}{6}\selectfont] at ([yshift=-2.5cm]lane\i-box.north) {Load/store unit};
    ...
    \node[muxdemux, muxdemux def={Lh=1, Rh=0.5, w=0.8, inset w=0.3, inset Lh=0.6, inset Rh=0, square pins=1, NL=2, NR=1},
        ...
        (lane\i-alu) at ([yshift=-0.6cm]lane\i-box.north) {};
    \node[box, anchor=north] at ([yshift=-0.1cm]lane\i-alu.east) (reg\i) {Regs\\1K$\times$32};
    \node[box, anchor=north] at ([yshift=-0.1cm]reg\i.south) (lsu\i) {load\\store\\unit};
    ...
    \draw[->, thin] (ireg.bpin \pinnum) -- ++(0, -0.2) -| (lane\i-box.north);
}

% Label for lanes - positioned to the right of all threads
\node[anchor=west, font=\small, align=center] at ([xshift=0.5cm]lane15-box.east)
    {(Thread Processors)\\SIMD Lanes};
