...
Given a system with L2 cache where the inclusion principle is maintained.

For each of the following cases, indicate whether L2 is useful or not:

\vspace{0.3cm}
\begin{columns}[T]
\begin{column}{0.35\textwidth}
\textbf{Case 1:} Both cache levels are the same size
\begin{itemize}
    \small
...
\vspace{0.5cm}
\onslide<2->{\textcolor{green!50!black}{\textbf{Useful}}

\small L2's higher associativity allows it to hold conflicting lines that L1 cannot.}
\end{column}
...
\begin{tikzpicture}[scale=0.65, node distance=0.4cm]
    ...
    % Draw horizontal lines for direct-mapped cells
    \draw (l1left.north west) -- (l1left.north east);
    \draw ([yshift=-0.5cm]l1left.north west) -- ([yshift=-0.5cm]l1left.north east);
    \draw ([yshift=-1cm]l1left.north west) -- ([yshift=-1cm]l1left.north east);
    \draw ([yshift=-1.5cm]l1left.north west) -- ([yshift=-1.5cm]l1left.north east);
    ...
    % Draw vertical lines for fully-associative cells
    \draw ([xshift=-0.8cm]l2left.north) -- ([xshift=-0.8cm]l2left.south);
    \draw ([xshift=-0.3cm]l2left.north) -- ([xshift=-0.3cm]l2left.south);
    \draw ([xshift=0.3cm]l2left.north) -- ([xshift=0.3cm]l2left.south);
    \draw ([xshift=0.8cm]l2left.north) -- ([xshift=0.8cm]l2left.south);
    \node[cacheA, minimum width=0.4cm, minimum height=0.4cm] at ([xshift=-0.55cm]l2left.center) {\tiny A};
    ...
    % Right side - After
    \node[above] at (4, 2) {\footnotesize\textbf{After}};
    
    % L1 cache - right side
    \node[draw, rectangle, fill=cyan!10, minimum width=0.7cm, minimum height=2cm, label=left:{\tiny L1:}] (l1right) at (4,0) {};
    % Draw horizontal lines
    \draw (l1right.north west) -- (l1right.north east);
    \draw ([yshift=-0.5cm]l1right.north west) -- ([yshift=-0.5cm]l1right.north east);
    \draw ([yshift=-1cm]l1right.north west) -- ([yshift=-1cm]l1right.north east);
    \draw ([yshift=-1.5cm]l1right.north west) -- ([yshift=-1.5cm]l1right.north east);
    ...
    % Draw vertical lines
    \draw ([xshift=-0.8cm]l2right.north) -- ([xshift=-0.8cm]l2right.south);
    \draw ([xshift=-0.3cm]l2right.north) -- ([xshift=-0.3cm]l2right.south);
    \draw ([xshift=0.3cm]l2right.north) -- ([xshift=0.3cm]l2right.south);
    \draw ([xshift=0.8cm]l2right.north) -- ([xshift=0.8cm]l2right.south);
    \node[cacheA, minimum width=0.4cm, minimum height=0.4cm] at ([xshift=-0.55cm]l2right.center) {\tiny A};
    \node[cacheB, minimum width=0.4cm, minimum height=0.4cm] at (l2right.center) {\tiny B};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item<2-> \textbf{Answer:} Useful. There is benefit in L2. Since access time is identical and it is larger, we prefer to read directly from L2, so L1 is not useful for system performance.
    \end{itemize}
    
    \item<3-> Access time to L2 and main memory is identical.
    \begin{itemize}
        \item<4-> \textbf{Answer:} Not useful. L2 is redundant because we can access main memory at the same speed without risk of miss.
    \end{itemize}
\end{enumerate}
\end{frame}
