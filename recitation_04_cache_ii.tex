...
\begin{tikzpicture}[scale=0.7, node distance=0.8cm]
    % L1 cache - left side
    \node[draw, rectangle, fill=cyan!20, minimum width=0.8cm, minimum height=3cm, label=above:{\small L1: direct mapped}] (l1left) {};
    \node[fill=yellow!40, minimum width=0.7cm, minimum height=0.5cm] at (l1left.center) {A};
    
    % L2 cache - left side (positioned below L1)
    \node[draw, rectangle, fill=cyan!30, minimum width=4cm, minimum height=0.8cm, below=1.5cm of l1left, label=left:{\small L2: fully associative}] (l2left) {};
    \node[fill=red!40, minimum width=0.7cm, minimum height=0.6cm] at ([xshift=-1cm]l2left.center) {A};
    
    % Arrow
    \draw[->, thick, blue!70, line width=2pt] ([xshift=0.8cm]l1left.east) -- ++(2,0);
    
    % L1 cache - right side
    \node[draw, rectangle, fill=cyan!20, minimum width=0.8cm, minimum height=3cm, right=4cm of l1left, label=above:{\small L1}] (l1right) {};
    \node[fill=yellow!40, minimum width=0.7cm, minimum height=0.5cm] at (l1right.center) {B};
    
    % L2 cache - right side (positioned below L1 right)
    \node[draw, rectangle, fill=cyan!30, minimum width=4cm, minimum height=0.8cm, below=1.5cm of l1right, label=left:{\small L2}] (l2right) {};
    \node[fill=red!40, minimum width=0.7cm, minimum height=0.6cm] at ([xshift=-0.8cm]l2right.center) {A};
    \node[fill=yellow!40, minimum width=0.7cm, minimum height=0.6cm] at ([xshift=0.8cm]l2right.center) {B};
\end{tikzpicture}
\end{center}

\onslide<2->{\textbf{Answer:} Useful! L2 can hold conflicting lines that L1 cannot due to higher associativity.}
\end{frame}
