...
To ensure the inclusion principle is maintained, every eviction of a cache line from a lower-level cache (L2 or L3) will trigger eviction of the same line from higher-level caches (closer to CPU).
...
\begin{itemize}
    ...
    \item On every access to line $n$, generate a prefetch request for line \textcolor{red!70!black}{\textbf{$n+8$}}
    \item \textbf{Why $n+8$?} Because accessing 4 elements per line takes $4 \times 1 = 4$ cycles, so we need to prefetch $\frac{32}{4} = 8$ lines ahead
