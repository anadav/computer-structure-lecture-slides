...
\begin{tikzpicture}[
    ...
    yellowbg/.style={fill=yellow!30, draw=black, rounded corners, inner sep=5pt},
...
\node[redbox, below=1.5cm of inv1] (snoopx1) {Snoop X};
...
\node[decision, below=of select3] (dirty3) {P dirty?};
%\node[bluebox, left=0.8cm of dirty3] (wrreq) {Yes: Wr Req};
\node[whitebox, below=0.8cm of dirty3] (inv3) {Invalidate P};
\node[whitebox, right=of inv3] (update) {Update P};

\node[bluered, right=of victimx] (updatelru) {Update LRU};
\node[bluered, above=of updatelru] (fillm3) {Fill M in P};
\node[bluered, below=of dirtyx] (fillm2) {Fill M \& Update LRU};
\node[bluered, below=1cm of snoopx1] (fillm1) {Fill M \& Update LRU};

\begin{scope}[on background layer]
\node[yellowbg, fit=(snoopp1)(dirty1)(inv1)] (bg1) {};
\node[yellowbg, fit=(snoopx1)(ldotsx1)] (bg1) {};
\node[yellowbg, fit=(snoopp2)(dirty2)(inv2)] (bg1) {};
...
\draw[thick, ->] (dirty1) -- (dirty2.west) node[above,midway] {\small Yes: Wr Req};
...
\draw[->, rounded corners]
  (dirty2.east)
  -| ($(dirty2)!0.5!(dirty3)$)                                  % over to midpoint x
  node[right] {\small Yes: Wr Req}  
  |- ($ (dirty3.north) + (0,0.5) $)          % up to 0.5 above snoopp1.north
  -- (dirty3.north);                         % straight down
...
\draw[->] (l3read) -- (select3);
\draw[->] (select3) edge[bend left=30, blue] node[above] {\small Snoop Request} (snoopp2.north);
\draw[->] (select3) -- (dirty3);
...
\draw[->, red, rounded corners]
  (updatelru.south)
  -- ++(0,-0.5)                               % down 0.5
  -| ($(updatelru)!0.5!(free)$)                                  % over to midpoint x
  |- ($ (free.north) + (0,0.5) $)          % up to 0.5 above snoopp1.north
