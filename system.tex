...
% Descriptions
\node[text width=4cm, right] at (3,0) {\small Billions of switches\\ (5-10nm process)};
\node[text width=4cm, right] at (3,1.2) {\small Execution unit + L1/L2\\ (1-2 threads)};
\node[text width=4cm, right] at (3,2.4) {\small Core(s) + L3 slice + interconnect\\ (modular unit)};
\node[text width=4cm, right] at (3,3.6) {\small Complete silicon chip\\ (monolithic or chiplet)};
\node[text width=4cm, right] at (3,4.8) {\small Die(s) + substrate + pins\\ (what goes in socket)};
\node[text width=4cm, right] at (3,6) {\small Multiple sockets + memory + I/O\\ (complete computer)};

% Examples on left
\node[text width=3cm, left] at (-3,0) {\small \textit{7nm = 10M+/mm²}};
\node[text width=3cm, left] at (-3,1.2) {\small \textit{Golden Cove, Zen 4}};
\node[text width=3cm, left] at (-3,2.4) {\small \textit{2-core tile}};
\node[text width=3cm, left] at (-3,3.6) {\small \textit{CCD, Alder Lake die}};
\node[text width=3cm, left] at (-3,4.8) {\small \textit{Xeon, EPYC, Core i9}};
\node[text width=3cm, left] at (-3,6) {\small \textit{2S server, workstation}};
