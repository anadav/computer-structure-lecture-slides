...
% Add components
\node[core] (c0) at (0,1) {\tiny C0};
\node[core] (c1) at (1,0) {\tiny C1};
\node[core] (c2) at (0,-1) {\tiny C2};
\node[core] (c3) at (-1,0) {\tiny C3};
\node[cache] at (0,-1.8) {\tiny L3 Slice};
\node[mc] at (0,-2.3) {\tiny MC};

% Annotations
\node[text width=5cm, align=left] at (0,-4.2) {
\footnotesize
• Bidirectional ring\\
• Each stop: core + L3 slice\\
...
\node[cache] at (0,-1.8) {\tiny L3 Distributed};
\node[mc] at (0,-2.3) {\tiny MC};
...
\node at (0, 1.5) {\footnotesize (AMD Zen CCX)};
% CCX box
\draw[dashed, thick] (-1.2,-1.2) rectangle (1.2,1.2);
% Cores in CCX
\foreach \i in {0,1,2,3} {
    \node[core] (z\i) at ({45+\i*90}:0.7) {\tiny};
}
% Full crossbar
\foreach \i in {0,1,2,3} {
    \foreach \j in {0,1,2,3} {
        \ifnum\i<\j
            \draw[gray!40] (z\i) -- (z\j);
        \fi
    }
}
\node[cache] at (0,-1.8) {\tiny L3 Shared};
\node at (0,0) {\tiny CCX};

% Annotations
\node[text width=5cm, align=left] at (0,-4.2) {
\footnotesize
• Full crossbar in CCX\\
• 4 cores per CCX\\
• Multiple CCX per die\\
• Lowest latency
};
\end{scope}
