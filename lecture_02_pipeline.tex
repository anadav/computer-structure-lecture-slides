...
\usepackage{listings}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, calc, tikzmark, shapes.misc, backgrounds, matrix}
\usetheme{Madrid}
\lstset{
    escapeinside={@}{@},
...
\begin{circuitikz}[scale=0.9, transform shape]
    ...
    % Bypass around DM
    \draw[line] (L3.east) -- ++(2mm, 0) coordinate (bypass_start) -| ([xshift=-1mm, yshift=-1mm]DM.south west) -| ([xshift=1mm]DM.east) -- (L4.west);
...
% Macro for drawing a single pipeline stage sequence using pgfkeys
\pgfkeys{
    /pipelinestage/.is family, /pipelinestage,
    default/.style = {
        prefix = stage,
        xpos = 0,
        ypos = 0,
    ...
    prefix/.estore in = \stageprefix,
    xpos/.estore in = \stagexpos,
    ypos/.estore in = \stageypos,
    im color/.estore in = \imcolor,
    reg1 color/.estore in = \regonecolor,
    dm color/.estore in = \dmcolor,
    reg2 color/.estore in = \regtwocolor
}

\newcommand{\drawpipelinestage}[1][]{%
    \pgfkeys{/pipelinestage, default, #1}%
    \begin{scope}[shift={(\stagexpos,\stageypos)}]
        \tikzstyle{stagebox} = [rectangle, draw, thick, minimum width=0.5cm, minimum height=0.5cm, inner sep=1pt, font=\scriptsize]

        % IM block
        \node[stagebox, fill=\imcolor] (\stageprefix-im) at (0,0) {IM};

        % First Reg block
        \node[stagebox, fill=\regonecolor] (\stageprefix-reg1) at (0.7,0) {Reg};

        % ALU - simple trapezoid like in Pipeline Architecture
        \begin{scope}[shift={(1.4,0)}]
            \coordinate (tl) at (-0.1, 0.25);
            \coordinate (bl) at (-0.1, -0.25);
            \coordinate (tr) at (0.1, 0.15);
            \coordinate (br) at (0.1, -0.15);

            \draw[thick, fill=white] (tl) -- (tr) -- (br) -- (bl) -- cycle;
            \coordinate (\stageprefix-alu) at (0,0);
        \end{scope}

        % DM block
        \node[stagebox, fill=\dmcolor] (\stageprefix-dm) at (2.1,0) {DM};

        % Second Reg block
        \node[stagebox, fill=\regtwocolor] (\stageprefix-reg2) at (2.8,0) {Reg};
    \end{scope}
}
...
\begin{frame}{Register File Split}
    ...
    \begin{tikzpicture}[scale=0.9, every node/.style={transform shape}]
        ...
        \draw[->, thick] (0, 3) -- (10, 3);
        \foreach \i in {1,...,9} {
            \node[above, font=\scriptsize] at (\i, 3) {\i};
        }

        % Program execution order label
        \node[anchor=north, align=center, font=\small] at (-2, 2.5) {Program\\execution\\order};

        % Instructions
        \node[anchor=east, font=\small\ttfamily] at (-0.3, 2) {lw R2, 30(R1)};
        \node[anchor=east, font=\small\ttfamily] at (-0.3, 1) {and R12, R2, R5};
        \node[anchor=east, font=\small\ttfamily] at (-0.3, 0) {or R13, R6, R2};
        \node[anchor=east, font=\small\ttfamily] at (-0.3, -1) {add R14, R2, R2};
        \node[anchor=east, font=\small\ttfamily] at (-0.3, -2) {sw R15, 100(R2)};

        % Draw pipeline stages using the macro in a loop
        % Each instruction starts 0.8 units to the right and 1 unit down from previous
        \foreach \i in {0,...,4} {
            \pgfmathsetmacro{\xoffset}{0.5 + \i * 0.8}
            \pgfmathsetmacro{\yoffset}{2 - \i}
            % Highlight reg1 for instruction 1 (and) to show read dependency
            \ifnum\i=1
                \drawpipelinestage[prefix=i\i, xpos=\xoffset, ypos=\yoffset,
                                  reg1 color=orange!40]
            \else
                \drawpipelinestage[prefix=i\i, xpos=\xoffset, ypos=\yoffset]
            \fi
        }
