...
\tikzset{
    ...
    bypass_ctrl/.style={draw=purple!70!black, line width=1.5pt, fill=purple!20,
                        rounded corners=4pt, minimum width=4cm, minimum height=1cm,
                        align=center,
                        font=\small\bfseries},
}
...
\node[bypass_ctrl, anchor=north] (BypassCtrl) at ([yshift=-8mm]IDEX.south -| BypassMuxA) {\\Bypass Control};

% Draw connections from src register numbers (from RegFile left pins)
\coordinate (src1_tap) at ([xshift=-10mm]BypassCtrl.north);
\coordinate (src2_tap) at ([xshift=-5mm]BypassCtrl.north);
\coordinate (src1_bend) at ([xshift=-5mm]RegFile.lpin 1);
\coordinate (src2_bend) at ([xshift=-8mm]RegFile.lpin 2);
\draw[->, purple, line width=1.5pt] (src1_bend) |- ([yshift=4mm]src1_tap) -- (src1_tap);
\draw[->, purple, line width=1.5pt] (src2_bend) |- ([yshift=2mm]src2_tap) -- (src2_tap);
% Tap marks on the input lines
\fill[purple] (src1_bend) circle (2pt);
\fill[purple] (src2_bend) circle (2pt);

% Draw connections from dst registers
\coordinate (dst1_tap) at ([xshift=5mm]BypassCtrl.north);
\coordinate (dst2_tap) at ([xshift=10mm]BypassCtrl.north);
\draw[->, purple!70!black, thick] (prod_r5.south) -- ++(0,-2mm) -| (dst1_tap);
\draw[->, purple!70!black, thick] (prod_r4.south) -- ++(0,-4mm) -| (dst2_tap);

% Labels for inputs (positioned at tap coordinates)
\node[font=\tiny, purple!70!black, rotate=90, anchor=south] at (src1_tap) {src1};
\node[font=\tiny, purple!70!black, rotate=90, anchor=south] at (src2_tap) {src2};
\node[font=\tiny, purple!70!black, rotate=90, anchor=south] at (dst1_tap) {dst$_{t-1}$};
\node[font=\tiny, purple!70!black, rotate=90, anchor=south] at (dst2_tap) {dst$_{t-2}$};

% Control signals to muxes (using bottom pins)
\draw[->, purple!70!black, thick, dashed] ([xshift=-3mm]BypassCtrl.north) -- (BypassMuxA.bpin 1);
\draw[->, purple!70!black, thick, dashed] ([xshift=3mm]BypassCtrl.north) -- (BypassMuxB.bpin 1);
