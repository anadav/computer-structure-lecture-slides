...
% === Bypass MUXes ===
% Style for 3-input bypass mux
\tikzset{
    bypass_mux/.style={muxdemux, muxdemux def={Lh=1.2, Rh=0.6, NL=3, NR=1, NT=1, w=0.6, square pins=1},
...
% Draw bypass arrows going INTO the muxes
\draw[->, green!50!black, line width=2pt] (prod_r4.north) to[out=120, in=-30] (BypassMuxA.south east);
\draw[->, green!50!black, line width=2pt] (prod_r5.north) to[out=130, in=-60] (BypassMuxB.south east);
...
\node[draw=green!50!black, line width=1.5pt, fill=green!10, rounded corners=6pt,
      ...
      at ([yshift=4mm]ExplainPos) {
    \textbf{Bypass paths:}\\
    R4 $\leftarrow$ MEM/WB latch (from \texttt{lw})\\
    R5 $\leftarrow$ EX/MEM latch (from \texttt{sub})
};
