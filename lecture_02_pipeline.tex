...
\ifdrawaluinputs
\draw[data_path] (IDEX.east |- ALUMux.lpin 1) -- (WriteDataJunction) -- (ALUMux.lpin 1);
\node[data_connector] at (WriteDataJunction) {};
\draw[data_path] (WriteDataJunction) -- (ALUMux.blpin 1);
\fi
...
\pgfkeys{
    ...
    % Bypass theme (for bypass hardware slides - wider ALU spacing for bypass muxes)
    bypass/.style={
        pipeline reg color=yellow!40,
        pc color=blue!30,
        regfile color=red!20,
    ...
    draw alu inputs/.is if=drawaluinputs,
}
\newif\ifshowdecode
...
\begin{frame}{Using Bypass to Solve RAW Dependency}
    \begin{itemize}
        ...
        \item Hardware forwards data from EX/MEM or MEM/WB pipeline registers
    \end{itemize}
    ...
    \begin{circuitikz}[scale=0.9, transform shape]
        % Timeline at top
        \node[anchor=south west, font=\small] at (-2.5, 2.7) {Time (clock cycles)};
        \draw[->, thick] (0, 2.5) -- (8, 2.5);
        \foreach \i in {1,...,7} {
            \node[above, font=\scriptsize] at (\i, 2.5) (i\i) {\i};
        }

        % Instructions with named nodes
        \node[anchor=east, font=\small\ttfamily] (sub_inst) at (-0.3, 1.8) {sub \textcolor{red}{R2}, R1, R3};
        \node[anchor=east, font=\small\ttfamily, anchor=north east] (and_inst) at ([yshift=-15mm]sub_inst.south east) {and R12, \textcolor{red}{R2}, R5};

        % Draw pipeline stages
        \drawpipelinearch[prefix=sub, at={(sub_inst -| i1)}, scale=0.58]
        \drawpipelinearch[prefix=and, at={(and_inst -| i2)}, scale=0.58]

        % Highlight sub's ALU (EXE stage) - where result is computed
        \node[muxdemux, muxdemux def={Lh=2, Rh=1.5, NL=2, NR=1, w=1.2, inset w=0.5, inset Lh=1, inset Rh=0},
              external pins width=0, fill=blue!40, draw=blue!60!black, line width=0.5pt,
              scale=0.58, anchor=center] at (sub-ALU.center) {};

        % Highlight and's ALU (EXE stage) - where data is needed
        \node[muxdemux, muxdemux def={Lh=2, Rh=1.5, NL=2, NR=1, w=1.2, inset w=0.5, inset Lh=1, inset Rh=0},
              external pins width=0, fill=green!40, draw=green!60!black, line width=0.5pt,
              scale=0.58, anchor=center] at (and-ALU.center) {};

        % Green forwarding arrow from sub's L3 (after EXE) to and's L2 (before EXE)
        \draw[->, ultra thick, green!60!black]
            (sub-L3.south) -- (and-L2.north)
            node[midway, right, font=\scriptsize, text=green!50!black] {forward};

        % Program execution order label
        \node[anchor=south east, align=center, font=\small] at (sub_inst.north west) (ordertext) {Program\\order};
        \draw[->, thick] (ordertext.south) -- (ordertext |- and_inst);
    \end{circuitikz}
\end{frame}
...
% Draw the core pipelined CPU diagram
\PipelinedCPUCore[bypass]

% Fade out the TargetAdder and its lines (not relevant for bypass)
% First cover original lines with white
\draw[white, line width=2pt] (execute_in_next_seq) |- (TargetAdder.lpin 1);
\draw[white, line width=2pt] ([xshift=-7mm]ALUMux.lpin 2) |- (TargetAdder.lpin 2);
\draw[white, line width=2pt] (TargetAdder.rpin 1) -- (TargetAdder.rpin 1 -| EXMEM.west);
% Cover the TargetAdder
\fill[white] ([xshift=-1mm,yshift=1mm]TargetAdder.north west) rectangle ([xshift=1mm,yshift=-1mm]TargetAdder.south east);
% Redraw faded and dotted
\begin{scope}[opacity=0.3]
    \node[pcpu_adder, anchor=center] at (TargetAdder.center) {+};
    \draw[data_path, densely dotted, thin] (execute_in_next_seq) |- (TargetAdder.lpin 1);
    \draw[data_path, densely dotted, thin] ([xshift=-7mm]ALUMux.lpin 2) |- (TargetAdder.lpin 2);
    \draw[data_path, densely dotted, thin] (TargetAdder.rpin 1) -- (TargetAdder.rpin 1 -| EXMEM.west);
\end{scope}
...
% Define coordinates for stacked labels (always, so anchoring works)
\coordinate (dst_stack_base) at (dst_idex_pos);
\coordinate (dst_stack_mid) at ([yshift=3mm]dst_stack_base);
\coordinate (dst_stack_top) at ([yshift=6mm]dst_stack_base);

% Program listing and annotations - only on last slide (7)
\onslide<7->{
\node[draw, thick, fill=white, rounded corners=3pt, inner sep=4pt,
      anchor=north west, font=\small\ttfamily, align=left]
      (ProgList) at ([xshift=-5mm,yshift=-25mm]PC.south west) {
...
\node[dst_value] (dst_problem) at (dst_stack_base) {R12};

% Source register labels stacked above dst
\node[bypass_value] (bad_src2) at (dst_stack_mid) {R4};
\node[bypass_value] (bad_src1) at (dst_stack_top) {R5};
...
\tikzset{
    bypass_mux/.style={muxdemux, muxdemux def={Lh=1.4, Rh=0.7, NL=3, NR=1, NB=1, w=0.6, square pins=1},
    ...
    bypass_callout/.style={rectangle callout, draw=blue!70!black, line width=1.5pt, fill=blue!10,
                           font=\small, inner sep=6pt, text width=6.5cm, align=left},
}

% Fixed position for callouts (top right of page)
\coordinate (callout_pos) at ([xshift=-2cm,yshift=1cm]DMem.north east);

% === Stage 1: Bypass MUXes ===
% Bypass mux for ALU input A (src1) - positioned left of ALU.lpin 1
\node[bypass_mux, anchor=rpin 1] (BypassMuxA) at ([xshift=-10mm]ALU.lpin 1) {};
\node[font=\tiny, green!40!black, above=0.5mm of BypassMuxA] {FwdA};

% Bypass mux for ALU input B (src2) - positioned left of ALUMux
\node[bypass_mux, anchor=rpin 1] (BypassMuxB) at ([xshift=-10mm]ALUMux.lpin 1) {};
\node[font=\tiny, green!40!black, above=0.5mm of BypassMuxB] {FwdB};

% Draw wiring through bypass muxes (original direct lines disabled via draw alu inputs=false)
% Src1 (to FwdA): from IDEX position -| to bypass mux lpin 1, then rpin 1 to ALU
\draw[thick, ->] (IDEX.east |- ALU.blpin 1) -| ([xshift=-5mm]BypassMuxA.lpin 1) -- (BypassMuxA.lpin 1);
\draw[data_path] (BypassMuxA.rpin 1) -- (ALU.blpin 1);
% Src2 (to FwdB): from IDEX position -| to bypass mux lpin 1, then rpin 1 to ALUMux
\draw[thick, ->] (IDEX.east |- ALUMux.lpin 1) -| ([xshift=-5mm]BypassMuxB.lpin 1) -- (BypassMuxB.lpin 1);
\draw[data_path] (BypassMuxB.rpin 1) -- (ALUMux.blpin 1);
...
% === Stage 2 also includes FwdA/FwdB control outputs ===
% Control signals to muxes (using top pins 3 and 4)
\onslide<2->{
\draw[bypass_ctrl_out] (BypassCtrl.tpin 3) |- ([yshift=-3mm]BypassMuxB.bpin 1) -- (BypassMuxB.bpin 1);
\draw[bypass_ctrl_out] (BypassCtrl.tpin 4) |- ([yshift=-5mm]BypassMuxA.bpin 1) -- (BypassMuxA.bpin 1);
}
...
Compares source registers with destination registers of previous instructions.\\
Outputs FwdA/FwdB: 2-bit signals selecting mux input
};

% === Stage 3: Control inputs (src1, src2) ===
...
\textbf{Source Register Inputs}\\[2pt]
src1, src2: source register indices from ID/EX latch
};

% === Stage 4: Control inputs (dst_{t-1}, dst_{t-2}) ===
...
% Callout for stage 4
\node<4>[bypass_callout, callout absolute pointer={(dst_exmem_tap)}, anchor=south west] at (callout_pos) {
\textbf{Destination Register Inputs}\\[2pt]
dst$_{t-1}$: destination register from EX/MEM (previous instruction)\\
dst$_{t-2}$: destination register from MEM/WB (2 instructions ago)
};
...
\onslide<5->{
\draw[bypass_data_exmem] (exmem_bypass_tap) |- (exmem_fwdB_junc) |- (BypassMuxB.lpin 2);
\node[bypass_data_exmem_tap] at (exmem_fwdA_junc) {};
\draw[bypass_data_exmem] (exmem_fwdA_junc) |- (BypassMuxA.lpin 2);
}
...
% Cycle indicator on title bar - only on last slide (7)
\only<7->{
\begin{tikzpicture}[remember picture, overlay]
    \node[draw=blue!60!black, line width=1.5pt, fill=blue!60, text=white,
          rounded corners=4pt, font=\normalsize\bfseries, inner sep=5pt, anchor=east]
...
%% Slide: Forwarding Logic
\begin{frame}[fragile]{Forwarding Logic}
\begin{columns}[T]
...
\small
\textbf{// For ALUSelA (src1):}\\[2pt]
\tikz[remember picture, baseline=(if_l3.base)]{\node[inner sep=0pt] (if_l3) {\texttt{\textcolor{orange!80!black}{if} (\tikz[remember picture, baseline=(l3_marker.base)]{\node[inner sep=0pt] (l3_marker) {L3};}.RegWrite \&\& \tikz[remember picture, baseline=(lw_check.base)]{\node[inner sep=0pt] (lw_check) {L3.op != lw};} \&\& L3.dst == L2.src1)}};}\\
\hspace*{2em}\texttt{ALUSelA = \textcolor{orange!80!black}{\textbf{1}}}\\
\texttt{\textcolor{cyan!70!black}{else if} (L4.RegWrite \&\& L4.dst == L2.src1)}\\
\hspace*{2em}\texttt{ALUSelA = \textcolor{cyan!70!black}{\textbf{2}}}\\
\texttt{else}\\
\hspace*{2em}\texttt{ALUSelA = \textbf{0}}
\end{minipage}%
}

\vspace{0.28cm}
\textit{\small Same logic applies to ALUSelB (for src2)}
\end{column}
\begin{column}{0.29\textwidth}
\centering
\begin{circuitikz}[scale=0.85, transform shape]
    % Define bypass mux style (similar to Bypass Hardware slide)
    \tikzset{
        bypass_mux_large/.style={muxdemux, muxdemux def={Lh=2.8, Rh=1.4, NL=3, NR=1, NB=1, w=1.2, square pins=1},
                           external pins width=0, fill=green!40, draw=green!50!black, line width=1.5pt},
        pin_num/.style={font=\scriptsize\bfseries, text=green!20!black},
        line_label/.style={font=\scriptsize, above, inner sep=1pt},
    }

    % Draw the mux
    \node[bypass_mux_large] (Mux) at (0,0) {};
    \node[font=\small\bfseries, green!40!black, above=2mm of Mux] {FwdA / FwdB};

    % Pin numbers at the border pins (inside mux edge)
    \node[pin_num, anchor=west] at (Mux.blpin 1) {0};
    \node[pin_num, anchor=west] at (Mux.blpin 2) {1};
    \node[pin_num, anchor=west] at (Mux.blpin 3) {2};

    % Input lines with labels ABOVE
    \draw[thick, ->] ([xshift=-22mm]Mux.lpin 1) -- (Mux.lpin 1)
        node[line_label, midway] {ID/EX (reg)};
    \draw[thick, ->, orange!80!black] ([xshift=-22mm]Mux.lpin 2) -- (Mux.lpin 2)
        node[line_label, midway, text=orange!80!black] {EX/MEM (L3)};
    \draw[thick, ->, cyan!70!black] ([xshift=-22mm]Mux.lpin 3) -- (Mux.lpin 3)
        node[line_label, midway, text=cyan!70!black] {MEM/WB (L4)};

    % Output line
    \draw[thick, ->] (Mux.rpin 1) -- ([xshift=12mm]Mux.rpin 1)
        node[line_label, midway] {to ALU};

    % Control input from bottom (shorter arrow)
    \draw[thick, ->, purple!70!black] ([yshift=-9mm]Mux.bpin 1) -- (Mux.bpin 1);
    \node[font=\scriptsize, purple!70!black, anchor=north] at ([yshift=-9mm]Mux.bpin 1) {ALUSel};
\end{circuitikz}
\end{column}
\end{columns}

% Callouts using overlay
\begin{tikzpicture}[remember picture, overlay]
    \tikzset{
        callout_box/.style={rectangle callout, draw=blue!70!black, line width=1pt, fill=blue!10,
                            font=\small, inner sep=5pt, align=left, rounded corners=2pt},
    }
    % Callout 1: Latch naming convention
    \only<2>{
        \node[callout_box, callout absolute pointer={(l3_marker.north)}, anchor=south west]
            at ([yshift=5mm]l3_marker.north west) {
            \textbf{Latch naming}\\
            L2 = ID/EX~~L3 = EX/MEM~~L4 = MEM/WB
        };
    }
    % Callout 2: L3 precedence explanation
    \only<3>{
        \node[callout_box, callout absolute pointer={(if_l3.east)}, anchor=west]
            at ([xshift=5mm, yshift=1.5cm]if_l3.east) {
            \textbf{L3 has priority}\\
            More recent instruction\\
            wins if both match
        };
    }
    % Callout 3: lw check explanation
    \only<4>{
        \node[callout_box, callout absolute pointer={(lw_check.north)}, anchor=south west]
            at ([xshift=1cm, yshift=5mm]lw_check.north west) {
            \textbf{Why exclude \texttt{lw}?}\\
            Load result not ready until MEM\\
            $\rightarrow$ \hyperlink{load-use-hazard}{Load-Use Hazard}
        };
    }
\end{tikzpicture}
\end{frame}
...
%% Slide: Can't Always Bypass (Load-Use Hazard)
\begin{frame}[label=load-use-hazard]{Can't Always Bypass: Load-Use Hazard}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Load result not available until \textbf{after} MEM stage $\Rightarrow$ can't forward to EXE
        \end{itemize}
        \item A hazard detection unit is needed to \textbf{stall} the pipeline
    \end{itemize}
    ...
    \begin{circuitikz}[scale=0.9, transform shape]
        ...
        % Instructions with named nodes
        \node[anchor=east, font=\small\ttfamily] (lw_inst) at (-0.3, 1.8) {lw \textcolor{red}{R2}, 30(R1)};
        \node[anchor=east, font=\small\ttfamily, anchor=north east] (and_inst) at ([yshift=-15mm]lw_inst.south east) {and R12, \textcolor{red}{R2}, R5};

        % Draw pipeline stages
        \drawpipelinearch[prefix=lw, at={(lw_inst -| i1)}, scale=0.58]
        \drawpipelinearch[prefix=and, at={(and_inst -| i2)}, scale=0.58]

        % Highlight lw's DM (MEM stage) - where load result becomes available
        \fill[blue!40] (lw-DM.north west) rectangle (lw-DM.south east);
        \node[font=\tiny, text=white] at (lw-DM.center) {DM};

        % Highlight and's ALU (EXE stage) - redraw with red fill
        \node[muxdemux, muxdemux def={Lh=2, Rh=1.5, NL=2, NR=1, w=1.2, inset w=0.5, inset Lh=1, inset Rh=0},
              external pins width=0, fill=red!40, draw=red!60!black, line width=0.5pt,
              scale=0.58, anchor=center] at (and-ALU.center) {};

        % "need data here" label - near and's L2
        \node[font=\scriptsize, text=red!70!black, align=right, anchor=south east]
            at ([xshift=-1mm, yshift=1mm]and-L2.north) {need data\\here};

        % Arrow from lw's L4 (after MEM) to and's L2 (before EX) - data not ready in time
        \draw[<-, ultra thick, red!70!black, dashed]
            (and-L2.north) -- (lw-L4.south)
            node[below right, font=\scriptsize, text=blue!70!black] {available here};

        % Program execution order label
        \node[anchor=south east, align=center, font=\small] at (lw_inst.north west) (ordertext) {Program\\order};
        \draw[->, thick] (ordertext.south) -- (ordertext |- and_inst);
    \end{circuitikz}
\end{frame}

%% Slide: Stall on Load-Use Hazard
\begin{frame}[fragile]{Stall on Load-Use Hazard}
\fcolorbox{gray!60}{gray!10}{%
\begin{minipage}{0.7\linewidth}
\small
\textbf{// Detect load-use hazard (stall required):}\\[2pt]
\texttt{\textcolor{red!70!black}{if} (L2.RegWrite \&\& L2.op == \textcolor{red!70!black}{lw} \&\&}\\
\hspace*{1.5em}\texttt{(L2.dst == L1.src1 || L2.dst == L1.src2))}\\
\hspace*{3em}\texttt{\textcolor{red!70!black}{\textbf{stall}}}
\end{minipage}%
}

\vspace{0.2cm}
\begin{itemize}
    \item De-assert write-enable on PC and IF/ID --- instruction stays in IF
    \item Issue a NOP (bubble) into the L2 latch (ID/EX)
\end{itemize}
...
\begin{circuitikz}[scale=0.9, transform shape]
    ...
    % Instructions with named nodes
    \node[anchor=east, font=\small\ttfamily] (lw_inst) at (-0.3, 1.8) {lw \textcolor{red}{R2}, 30(R1)};
    \node[anchor=east, font=\small\ttfamily, anchor=north east] (and_inst) at ([yshift=-8mm]lw_inst.south east) {and R12, \textcolor{red}{R2}, R5};

    % Draw pipeline stages - lw starts at cycle 1
    \drawpipelinearch[prefix=lw, at={(lw_inst -| i1)}, scale=0.58]

    % Bubble: and's first IM (cycle 2) - stalled, stays in IF
    \node[muxdemux, muxdemux def={Lh=2, Rh=2, w=1.5, NL=1, NR=1, square pins=1},
          external pins width=0, fill=yellow!60, draw=orange!70!black, line width=1pt,
          scale=0.58, anchor=center] (bubble_IM) at (and_inst -| i2) {IM};

    % and continues from cycle 3 (after stall)
    \drawpipelinearch[prefix=and, at={(and_inst -| i3)}, scale=0.58]

    % Highlight lw's DM (MEM stage) - where load result becomes available
    \fill[blue!40] (lw-DM.north west) rectangle (lw-DM.south east);
    \node[font=\tiny, text=white] at (lw-DM.center) {DM};

    % Highlight and's ALU (EXE stage) - green since forwarding now works
    \node[muxdemux, muxdemux def={Lh=2, Rh=1.5, NL=2, NR=1, w=1.2, inset w=0.5, inset Lh=1, inset Rh=0},
          external pins width=0, fill=green!40, draw=green!60!black, line width=0.5pt,
          scale=0.58, anchor=center] at (and-ALU.center) {};
    ...
    % Callout for bubble
    \node[rectangle callout, draw=orange!70!black, line width=1pt, fill=orange!10,
          font=\scriptsize, inner sep=4pt, align=left,
          callout absolute pointer={(bubble_IM.north)}, anchor=south]
          at ([yshift=1mm]bubble_IM.north) {
        \textbf{Bubble}
    };

    % Green arrow from lw's L4 to and's L2 - forwarding now works after stall
    \draw[->, ultra thick, green!60!black]
        (lw-L4.south) -- (and-L2.north)
        node[midway, right, font=\scriptsize, text=green!50!black] {forward};
\end{circuitikz}
\end{frame}
...
\begin{frame}{Data Hazards: When Do We Need Forwarding?}
    ...
    \begin{tikzpicture}[
        ...
        \node[dist, anchor=east] at ([xshift=-5mm]m-2-1.west |- m-2-1.south east) {Distance 1};
        \node[dist, anchor=east] at ([xshift=-5mm]m-4-1.west |- m-4-1.south east) {Distance 2};
        \node[dist, anchor=east] at ([xshift=-5mm]m-6-1.west |- m-6-1.south east) {Distance 3};
...
\node[draw, thick, fill=white, rounded corners=3pt, inner sep=4pt,
      anchor=north, font=\small\ttfamily, align=left]
      (ProgramListing) at ([yshift=-5mm, xshift=8mm]WB_label.south) {
    \begin{tabular}{rl}
    0: & or \textcolor{gray!60}{r6,r7} \\
    4: & beq 50 \\
    8: & and \textcolor{gray!60}{r8,r9} \\
    12: & sw \textcolor{gray!60}{r10,r11} \\
    16: & sub \textcolor{gray!60}{r1,r2} \\
    \end{tabular}
};
...
% === Slide 1: Empty pipeline, explain beq (PPT slide 30) ===
\only<1>{
    \node[explain_box] at (StageExplainPos) {
        \textbf{beq rs, rt, offset; ~if rs == rt then PC $\leftarrow$ PC + offset}\\[2mm]
        $\diamond$ To fetch the next instruction, we need to know \textbf{if the branch is taken}\\
        $\diamond$ If taken, we also need the \textbf{target address}\\
        $\diamond$ Otherwise, we might fetch the wrong instructions!
    };
}

% === Slide 2: PC=8, beq at IF/ID, or at ID/EX (PPT slide 31) ===
\only<2>{
    \node[ip_value] at ([yshift=-3mm]PC.south) {8};
    % Instructions
    \node[latch_inst] (beq2) at (instruction.south) {beq};
    ...
    % Explanation
    \node[explain_box] at (StageExplainPos) {
        $\diamond$ \texttt{beq} was fetched -- but we don't know yet it's a branch!\\
        $\diamond$ We must fetch \emph{something}, so we fetch the next sequential instruction
    };
}

% === Slide 3: IP=12, beq at ID/EX, and at IF/ID, or at EX/MEM (PPT slide 32) ===
\only<3>{
    \node[ip_value] at ([yshift=-3mm]PC.south) {12};
    % Instructions
    \node[latch_inst] at (instruction.south) {and};
    \node[latch_inst] (beq3) at (IDEX.center |- instruction.south) {beq};
    ...
    % Explanation
    \node[explain_box] at (StageExplainPos) {
        $\diamond$ We decoded \texttt{beq} -- now we know it's a branch\\
        $\diamond$ Target is being calculated, but we don't know the condition outcome yet
    };
}

% === Slide 4: PC=16, beq at EX/MEM (PPT slide 33) ===
\only<4>{
    ...
    \node[latch_inst] at (EXMEM.center |- instruction.south) {beq};
...
% === Slide 5: Flush - PC=50, beq at MEM/WB, flushed instructions (PPT slide 37) ===
\only<5>{
    ...
    % beq at MEM/WB (branch was resolved in previous cycle)
    \node[latch_inst] at (MEMWB.center |- instruction.south) {beq};
    ...
    % Explanation box
    \node[explain_box] at (StageExplainPos) {
        If the \underline{beq} is resolved as taken at EXE $\Rightarrow$ flush the pipeline:\\
...
\begin{frame}{Control Hazard}
    \begin{tcolorbox}[colback=blue!5, colframe=blue!60!black, title=Control Hazard, fonttitle=\bfseries]
        A \textbf{control hazard} occurs when the next instruction to fetch depends on the outcome of a \textbf{branch} or \textbf{jump} that hasn't been resolved yet.
        Solutions: \textbf{stall}, \textbf{predict not-taken}, or \textbf{dynamic prediction} (BTB).
    \end{tcolorbox}

    \vspace{0.5cm}
    \textbf{The problem:}
    \begin{itemize}
        \item Branch outcome known at EXE stage (cycle 4)
        \item But we need to fetch next instruction at cycle 2
        \item If we guess wrong $\rightarrow$ flush and restart
    \end{itemize}
\end{frame}
...
%% Slide: Branch Target Buffer (BTB)
...
%% Slide: Branch Misprediction
\begin{frame}{Branch Misprediction}
\begin{center}
\begin{tikzpicture}
...
\begin{tikzpicture}[remember picture, overlay]
    ...
    \only<2>{\node[title_cycle_box, anchor=east] at ([xshift=-5mm, yshift=-5mm]current page.north east) {Cycle 4};}
    \only<3>{\node[title_cycle_box, anchor=east] at ([xshift=-5mm, yshift=-5mm]current page.north east) {Cycle 5};}
    \only<4>{\node[title_cycle_box, anchor=east] at ([xshift=-5mm, yshift=-5mm]current page.north east) {Cycle 6};}
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Overlap execution of multiple instructions
        \item Increases throughput, not individual instruction latency
    \end{itemize}
    \vspace{0.3cm}
    \item \textbf{Pipeline Hazards}
    \begin{itemize}
        \item Structural: resource conflicts $\rightarrow$ duplicate hardware
        \item Data (RAW): forwarding/bypass, stall for load-use
        \item Control: branch prediction, BTB
    \end{itemize}
    \vspace{0.3cm}
    \item \textbf{Branch Handling}
    \begin{itemize}
        \item Predict not-taken: simple, 50\% accuracy
        \item Dynamic prediction (BTB): better accuracy, needs flush on mispredict
    \end{itemize}
\end{itemize}
\end{frame}
