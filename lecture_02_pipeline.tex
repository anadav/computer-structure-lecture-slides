...
\begin{tikzpicture}[scale=0.8]
    ...
    \node[above=1mm of btb-1-3.north, text depth=0ex, font=\scriptsize] {History};
...
\only<4>{
    \coordinate (CalloutPointer) at ([xshift=0.5cm]DirComp);
    \node[rectangle callout, draw=myorange!80!black, line width=1pt, fill=myorange!10,
          rounded corners=3pt, font=\scriptsize, align=left, inner sep=4pt,
          callout absolute pointer=(CalloutPointer), text width=3.5cm]
          at ([xshift=1cm, yshift=-3cm]VerifyBox.south east) {
        Verify direction\\
...
% === Latch value styles ===
% Consistent instruction style on latches
% latch_inst: instruction name on latch
% latch_val: numeric value on latch
% new_highlight: highlight for newly appearing elements

% === Animation: Instructions on latches + values + callouts ===
% Slide 1: or fetching, lookup BTB
\only<1>{
    % Instruction on IF/ID latch
    \node[inst_label] at (IFID.center) {or};
    % Stage explanation
    \node[anchor=north, font=\scriptsize, align=center, text width=12cm]
          at ([yshift=-3mm]BPU.south -| ALU) {
        \textbf{Fetch:} Looking up IP=4 in I\$ and BTB in parallel
    };
}
...
\only<2>{
    ...
    \node[inst_label] at (IFID.center) {jcc};
    % Latch values - predicted target and direction appear
    \node[draw, fill=green!30, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=15mm]IFID.center) {50};
    \node[draw, fill=orange!30, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=25mm]IFID.center) {taken};
    \node[draw, fill=white, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=5mm]IFID.center) {8};
    % Stage explanation
    \node[anchor=north, font=\scriptsize, align=center, text width=12cm]
          at ([yshift=-3mm]BPU.south -| ALU) {
        \textbf{Fetch:} BTB hit! Predicted target=50, direction=taken, next seq=8
    };
}
...
\only<3>{
    % Instructions on latches
    \node[inst_label] at (IFID.center) {sub};
    \node[inst_label] at (IDEX.center) {jcc};
    % Latch values propagating
    \node[draw, fill=green!30, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=15mm]IDEX.center) {50};
    \node[draw, fill=orange!30, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=25mm]IDEX.center) {taken};
    \node[draw, fill=white, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=5mm]IDEX.center) {8};
    % Stage explanation
    \node[anchor=north, font=\scriptsize, align=center, text width=12cm]
          at ([yshift=-3mm]BPU.south -| ALU) {
        \textbf{Decode:} jcc decoded, prediction values propagate through pipeline
    };
}
...
\only<4>{
    % Instructions on latches
    \node[inst_label] at (IFID.center) {mul};
    \node[inst_label] at (IDEX.center) {sub};
    \node[inst_label] at (EXMEM.center) {jcc};
    % Latch values - now at EX/MEM for verification
    \node[draw, fill=green!30, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=15mm]EXMEM.center) {50};
    \node[draw, fill=orange!30, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=25mm]EXMEM.center) {taken};
    \node[draw, fill=white, rounded corners=2pt, font=\scriptsize, inner sep=2pt]
          at ([yshift=5mm]EXMEM.center) {8};
    % Stage explanation
    \node[anchor=north, font=\scriptsize, align=center, text width=12cm]
          at ([yshift=-3mm]BPU.south -| ALU) {
        \textbf{Execute:} jcc executes, actual direction computed $\rightarrow$ verify prediction
    };
...
\only<5>{
    % Instructions on latches
    \node[inst_label] at (IFID.center) {add};
    \node[inst_label] at (IDEX.center) {mul};
    \node[inst_label] at (EXMEM.center) {sub};
    \node[inst_label] at (MEMWB.center) {jcc};
    % Verification OK
    \node[font=\scriptsize\bfseries, fill=green!40, rounded corners=3pt, inner sep=4pt, text=green!30!black]
          at (VerifyBox.center) {OK};
    % Stage explanation
    \node[anchor=north, font=\scriptsize, align=center, text width=12cm]
          at ([yshift=-3mm]BPU.south -| ALU) {
        \textbf{Memory:} Prediction verified correct! Pipeline continues normally
    };
}
...
% Code listing
\node[anchor=north west, font=\scriptsize\ttfamily, align=left] at ([xshift=5mm]WBMux.east |- IF_label.south) {
    0: or\\
    4: jcc 50\\
    8: and\\
    ...\\
...
% === Animated misprediction scenario with callouts ===
\only<1>{
    \node[inst_label, fill=green!40] at (IMem.center) {mul};
    \node[inst_label, fill=green!40] at (RegFile.center) {sub};
    \node[inst_label] at (ALU.center) {jcc};
    \node[inst_label] at (DMem.center) {or};
    % Callout: Wrong direction detected
    \node[rectangle callout, draw=red!80!black, line width=1pt, fill=red!10,
          rounded corners=3pt, font=\scriptsize, align=center, inner sep=4pt,
          callout absolute pointer={(DirComp.east)}, text width=2.5cm]
          at ([xshift=2cm, yshift=-1cm]DirComp.east) {
        Wrong direction\\flush
    };
}
\only<2>{
    \node[flush_label] at (IMem.center) {mul};
    \node[flush_label] at (RegFile.center) {sub};
    \node[inst_label] at (ALU.center) {jcc};
    \node[inst_label] at (DMem.center) {or};
    \node[font=\scriptsize, fill=red!60, text=white, rounded corners, inner sep=3pt] at (VerifyBox.center) {$\neq$};
    % Callout: Along with repair IP
    \node[rectangle callout, draw=red!80!black, line width=1pt, fill=red!10,
          rounded corners=3pt, font=\scriptsize, align=center, inner sep=4pt,
          callout absolute pointer={(RepairMux.south)}, text width=3cm]
          at ([yshift=-1.5cm]RepairMux.south) {
        Along with the repair IP
    };
}
\only<3>{
    \node[inst_label, fill=gray!30, opacity=0.5] at (IMem.center) {NOP};
    \node[inst_label, fill=gray!30, opacity=0.5] at (RegFile.center) {NOP};
    \node[inst_label] at (ALU.center) {jcc};
    \node[inst_label] at (DMem.center) {or};
    \node[font=\scriptsize, text=red!70!black, align=center] at ([yshift=-8mm]BPU.south) {Flush pipeline!\\Repair IP $\leftarrow$ 8 (fall-through)};
}
\only<4>{
    \node[inst_label, fill=blue!40] at (IMem.center) {and};
    \node[inst_label, fill=gray!30, opacity=0.5] at (RegFile.center) {--};
    \node[inst_label, fill=gray!30, opacity=0.5] at (ALU.center) {--};
    \node[inst_label] at (DMem.center) {jcc};
    \node[font=\scriptsize, text=blue!70!black, align=center] at ([yshift=-8mm]BPU.south) {Fetching from correct path\\3 cycles penalty};
}
