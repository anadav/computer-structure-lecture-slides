...
\texttt{~~~~(L2.dst == L1.src1 || L2.dst == L1.src2))}\\
\texttt{~~~~\textcolor{red!70!black}{\textbf{stall}}}

    \vspace{0.3cm}
    \begin{itemize}
        \item De-assert the enable to the L1 latch (IF/ID) and to the PC
        \begin{itemize}
            \item The dependent instruction stays another cycle in L1
        \end{itemize}
        \item Issue a NOP (bubble) into the L2 latch (ID/EX)
        \item Allow the load instruction to proceed through MEM
    \end{itemize}
