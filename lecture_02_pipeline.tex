...
\node[alu_style, anchor=blpin 1] (ALU) at ([xshift=4cm]RegFile.brpin 1) {};
...
\node[muxdemux, muxdemux def={Lh=4.1, NL=10, Rh=4.1, NR=10, w=2.8, square pins=1},
      external pins width=0, align=center, text depth=6.5ex, fill=yellow!20, anchor=lpin 5] (DMem) at ([xshift=2.5cm]ALU.brpin 4) {Data\\Cache};
...
\begin{scope}[on background layer]
    ...
    % IF stage shade
    \fill[blue!8] ([xshift=-8mm]PC.west |- stage_top) rectangle ([xshift=4mm]IMem.east |- stage_bottom);

    % ID stage shade
    \fill[green!8] ([xshift=4mm]IMem.east |- stage_top) rectangle ([xshift=8mm]RegFile.east |- stage_bottom);

    % EX stage shade
    \fill[blue!8] ([xshift=8mm]RegFile.east |- stage_top) rectangle ([xshift=5mm]ALU.east |- stage_bottom);

    % MEM stage shade
    \fill[green!8] ([xshift=5mm]ALU.east |- stage_top) rectangle ([xshift=5mm]DMem.east |- stage_bottom);

    % WB stage shade
    \fill[blue!8] ([xshift=5mm]DMem.east |- stage_top) rectangle ([xshift=12mm]WBMux.east |- stage_bottom);
\end{scope}
...
\draw[data_path, orange!80!black] (Decode.east) -- ++(15mm,0) node[control_label, above=-1pt, pos=0.5] {Control signals};
...
% RegFile to ALU
\draw[data_path] (RegFile.brpin 1) -- (ALU.blpin 1);
\coordinate (WriteDataJunction) at ([xshift=-3mm]ALUMux.lpin 1);
\draw[data_path] (RegFile.brpin 2) -- (WriteDataJunction) node[data_connector] {} -- (ALUMux.blpin 1);

% Write data to DMem
\draw[data_path] (WriteDataJunction) |- (DMem.lpin 10);

% Sign Extend to ALUMux and Target Adder
\draw[data_path] (IMem.brpin 2) |- (SignExt.west);
...
\draw[data_path] (TargetAdder.brpin 1) -- ++(5mm,0) |- (PCSrcMux.rpin 1);
...
\draw[data_path] (ALUResConn) |- ([yshift=-15mm]DMem.south) -| ([xshift=-5mm]WBMux.lpin 2) -- (WBMux.lpin 2);
