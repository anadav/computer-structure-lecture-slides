...
% === Bypass MUXes ===
% Style for 3-input bypass mux (with bottom pin for control)
\tikzset{
    ...
    % Bypass control: muxdemux with 4 top pins, 2 bottom pins, ~50% wider (w=6), ~100% taller (Lh=Rh=2)
    bypass_ctrl/.style={muxdemux, muxdemux def={Lh=2, Rh=2, NL=0, NR=0, NT=4, NB=2, w=6, square pins=1},
...
\node[bypass_ctrl, anchor=north] (BypassCtrl) at ([yshift=-8mm]IDEX.south -| BypassMuxA) {};
% Text in lower half
\node[font=\small\bfseries, purple!70!black] at ([yshift=-4mm]BypassCtrl.center) {Bypass Control};
% Pin labels inside the box (near top edge, rotated)
...
% Route: tap → west of IDEX → down to BypassCtrl
\draw[->, purple, line width=1.5pt] (src1_tap) |- (bad_src1.west) |- ([yshift=4mm]BypassCtrl.tpin 1) -- (BypassCtrl.tpin 1);
\draw[->, purple, line width=1.5pt] (src2_tap) |- (bad_src2.west) |- ([yshift=2mm]BypassCtrl.tpin 2) -- (BypassCtrl.tpin 2);
\draw[->, purple, line width=1.5pt] (bad_src1.east) -| (BypassCtrl.tpin 2);
\draw[->, purple, line width=1.5pt] (bad_src2.east) -| (BypassCtrl.tpin 1);
% Tap marks on the input lines
\fill[purple] (src1_tap) circle (2pt);
\fill[purple] (src2_tap) circle (2pt);

% Draw connections from dst register index lines (tap from propagation lines between latches)
\coordinate (dst_exmem_tap) at ([yshift=2mm]dst_exmem_in);
\coordinate (dst_memwb_tap) at ([yshift=4mm]dst_memwb_in);
\draw[->, purple, line width=1.5pt] (dst_exmem_tap) -- ++(0,-2mm) -| (BypassCtrl.tpin 3);
\draw[->, purple, line width=1.5pt] (dst_memwb_tap) -- ++(0,-4mm) -| (BypassCtrl.tpin 4);
% Tap marks on the dst lines
\fill[purple] (dst_exmem_tap) circle (2pt);
\fill[purple] (dst_memwb_tap) circle (2pt);

% Control signals to muxes (using bottom pins)
\draw[->, purple!70!black, thick, dashed] (BypassCtrl.bpin 1) -- (BypassMuxA.bpin 1);
\draw[->, purple!70!black, thick, dashed] (BypassCtrl.bpin 2) -- (BypassMuxB.bpin 1);
