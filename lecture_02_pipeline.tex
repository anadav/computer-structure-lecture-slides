...
%% Slide: Pipelined CPU (with label for againframe)
%% Overlay 1: plain CPU diagram; overlays 2-6: pipeline example cycles (shown via againframe)
\begin{frame}<1>[label=pipelined-cpu]{Pipelined CPU}
\begin{center}
\scalebox{0.58}{
\begin{circuitikz}[
    % Component styles
...
% === Styles for pipeline example (matching Control Hazard on Branches) ===
\tikzset{
    ...
    explain_box/.style={draw=green!50!black, line width=1.5pt, fill=green!10, rounded corners=6pt,
                        font=\small, inner sep=8pt, align=left},
}

% === Coordinates for instruction and data placement ===
% Instruction positions: shown at the latch OUTPUT (next stage's input)
% Fetch output: at instruction.south (on IF/ID latch)
% Other stages: at next latch using (latch |- Decode) pattern
...
% Execute output (at EXMEM latch)
\coordinate (alu_result_pos) at (EXMEM |- ALU);
% Memory output (at MEMWB latch)
\coordinate (mem_data_pos) at (MEMWB |- DMem.rpin 5);

% Dst register propagation coordinates (use same Y level as dst path)
% These align with where the dst arrow goes through each latch
...
% Explanation box position (1cm lower than before)
\coordinate (ExplainPos) at ([yshift=-40mm]IMem.south -| ALU);

% === Program listing at bottom left (shown on overlays 2+) ===
\only<2->{
...
\only<5>{
    ...
    \node[explain_box] at (ExplainPos) {
        \textbf{Cycle 4:} \texttt{lw} accesses D-Cache at address [R1]+9
    };
}

% === Cycle 5 (overlay 6): lw in WB, sub in Memory, and in Execute, or in Decode ===
\only<6>{
    \node[latch_inst] at (inst_decode_pos) {or};
    \node[latch_inst] at (inst_execute_pos) {and};
    \node[latch_inst] at (inst_memory_pos) {sub};
    \node[latch_inst] at (inst_wb_pos) {lw};
    % or decoding: read R6, R7
    \node[data_value] at (src1_data_pos) {[R6]};
    \node[data_value] at (src2_data_pos) {[R7]};
    \node[dst_value] at (dst_idex_pos) {R13};
    % and executing
    \node[data_value] at (alu_result_pos) {[R4]\&[R5]};
    \node[dst_value] at (dst_exmem_pos) {R12};
    % sub in memory (passes through)
    \node[dst_value] at (dst_memwb_pos) {R11};
    % lw writeback - show data being written
    \node[data_value] at ([xshift=8mm]WBMux.east) {M[..]};
    \node[explain_box] at (ExplainPos) {
        \textbf{Cycle 5:} \texttt{lw} writes M[[R1]+9] to R10. First instruction completes!
    };
}
...
%% Slide: Pipeline Example - using againframe to show cycles 1-5
\againframe<2>{pipelined-cpu}
\againframe<3>{pipelined-cpu}
\againframe<4>{pipelined-cpu}
\againframe<5>{pipelined-cpu}
\againframe<6>{pipelined-cpu}
