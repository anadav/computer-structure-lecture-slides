...
\texttt{~~~~ALUSelA = \textcolor{orange!80!black}{\textbf{1}}}\\
\texttt{\textcolor{cyan!70!black}{else if} (L4.RegWrite \&\& L4.dst == L2.src1)}\\
\texttt{~~~~ALUSelA = \textcolor{cyan!70!black}{\textbf{2}}}\\
\texttt{else}\\
\texttt{~~~~ALUSelA = \textbf{0}}
...
\begin{tikzpicture}[remember picture, overlay]
    ...
    \only<2>{
        \node[callout_box, callout absolute pointer={(l3_marker.south)}, anchor=north west]
            at ([yshift=-3mm]l3_marker.south west) {
            \textbf{Latch naming}\\
...
\begin{frame}[label=load-use-hazard]{Can't Always Bypass: Load-Use Hazard}
    ...
    \begin{circuitikz}[scale=0.9, transform shape]
        ...
        % "need data here" label - top left of ALU, two lines
        \node[font=\scriptsize, text=red!70!black, align=right, anchor=south east]
            at ([xshift=-1mm, yshift=1mm]and-ALU.north west) {need data\\here};

        % Arrow from lw's DM to and's ALU north west - showing data would need to go back in time
        \draw[<-, ultra thick, red!70!black, dashed]
            (and-ALU.north west) -- (lw-DM.south east)
...
\begin{tikzpicture}[remember picture, overlay]
    \node[rectangle callout, draw=orange!70!black, line width=1pt, fill=orange!10,
          ...
          at ([yshift=5mm]bubble_IM.north) {
        \textbf{Bubble:} and stays in IF\\
        while lw proceeds to MEM
    };
\end{tikzpicture}
\end{frame}
...
\node[draw, thick, fill=white, rounded corners=3pt, inner sep=4pt,
    ...
    12: & sw \textcolor{gray!60}{r10,r11}, xshift=8mm \\
