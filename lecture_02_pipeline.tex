...
\node[pcsrc_mux, anchor=south] (PCSrcMux) at ([yshift=13mm]IMem.north west) {};
...
\coordinate (calc_target_exmem) at (TargetAdder.brpin 1 -| EXMEM.west);
...
\draw[arrow, blue!70!black, line width=1.5pt] (calc_target_mem_out) -- ++(3mm,0) |- ([yshift=5mm]PCSrcMux.north) -| (PCSrcMux.rpin 1);

% === Zero (branch taken) signal to mux select ===
\coordinate (zero_mem_out) at (exmem_alu_zero -| EXMEM.east);
\draw[arrow, orange!70!black, line width=1.5pt] (zero_mem_out) -- ++(6mm,0) |- ([yshift=8mm]PCSrcMux.north) -- (PCSrcMux.tpin 1);

% === Flush signal from EX stage ===
\coordinate (flush_line_y) at ([yshift=20mm]EXMEM.north);
\draw[repair_path, line width=1.5pt] (EXMEM.north) -- (flush_line_y) node[pos=0.7, repair_label, right] {flush} -| (IDEX.north);
\node[data_connector, fill=red!70!black] at (flush_line_y -| IDEX.north) {};
\draw[repair_path, line width=1.5pt] (flush_line_y -| IDEX.north) -- ++(0,0) -| (IFID.north);
...
% === Instructions on latches (showing branch resolved, flush happening) ===
% Flushed instructions
\node[latch_inst_flush] at (Instruction.south) {\sout{sub}};
\node[latch_inst_flush] at (IDEX.center |- Instruction.south) {\sout{and}};
% jcc at EX/MEM (just resolved)
\node[latch_inst, draw=red!70!black, line width=2pt, fill=yellow!30] at (EXMEM.center |- Instruction.south) {jcc};
% or at MEM/WB
\node[latch_inst] at (MEMWB.center |- Instruction.south) {or};

% Latch values (next seq addresses)
\node[latch_nextseq] at (IDEX.center |- decode_out_next_seq) {16};
\node[latch_nextseq] at ([yshift=10mm]EXMEM.center |- execute_in_next_seq) {12};

% Program listing with border (same position as BTB slide)
\node[draw, thick, fill=white, rounded corners=3pt, inner sep=4pt,
    ...
    0: & or \\
    4: & jcc 50 \\
    8: & \textcolor{red}{\sout{and}} \\
    12: & \textcolor{red}{\sout{sub}} \\
    \multicolumn{2}{c}{...} \\
    50: & mul \\
    54: & add \\
    \end{tabular}
...
% Coordinate for stage explanation box at bottom (same as BTB slide)
\coordinate (StageExplainPos) at ([yshift=-15mm]IMem.south -| RegFile);
...
\node[draw=red!50!black, line width=1.5pt, fill=red!10, rounded corners=6pt,
    ...
    \textcolor{red!60!black}{\textbf{Control Hazard:}} Branch resolved at Execute $\Rightarrow$ flush IF/ID and ID/EX, set IP $\leftarrow$ target (50)
};

% Flush indicator at top
\node[draw=red!70!black, line width=2pt, fill=red!20, rounded corners=5pt,
      font=\normalsize\bfseries, inner sep=6pt, text=red!70!black]
      at ([yshift=5mm]flush_line_y -| IDEX) {FLUSH};
...
\only<4>{
    % Direction comparator callout (lower)
    \node[rectangle callout, draw=orange!70!black, line width=1pt, fill=orange!10,
          ...
          callout absolute pointer={(DirComp.east)}]
          at ([xshift=2cm, yshift=-3mm]DirComp.east) {
        Verify direction
    };
    % Target comparator callout (upper)
    \node[rectangle callout, draw=green!60!black, line width=1pt, fill=green!10,
          rounded corners=3pt, font=\scriptsize, inner sep=4pt,
          callout absolute pointer={(TargetComp.east)}]
          at ([xshift=2cm, yshift=3mm]TargetComp.east) {
        Verify target
    };
}
...
\only<4>{
    ...
    % Verification callouts - separate for each comparator
    % Direction comparator (lower)
    \node[rectangle callout, draw=orange!70!black, line width=1.5pt, fill=orange!10,
          rounded corners=4pt, font=\small, inner sep=5pt,
          callout absolute pointer={(DirComp.east)}]
          at ([xshift=2.5cm, yshift=-5mm]DirComp.east) {
        \textbf{Verify direction}
    };
    % Target comparator (upper)
    \node[rectangle callout, draw=green!60!black, line width=1.5pt, fill=green!10,
          rounded corners=4pt, font=\small, inner sep=5pt,
          callout absolute pointer={(TargetComp.east)}]
          at ([xshift=2.5cm, yshift=5mm]TargetComp.east) {
        \textbf{Verify target}
    };
...
\only<2>{
    ...
    \node[latch_inst_flush] at (Instruction.south) {mul};
    \node[latch_inst_flush] at (IDEX.center |- Instruction.south) {sub};
    ...
    \node[rectangle callout, draw=red!80!black, line width=1.5pt, fill=yellow!20,
          rounded corners=4pt, font=\small, align=center, inner sep=6pt,
          callout absolute pointer={(RepairMux.south)}, text width=3.5cm]
    ...
    \node[draw=red!50!black, line width=1.5pt, fill=red!8, rounded corners=6pt,
          font=\small, inner sep=8pt, anchor=west] at (StageExplainPos) {
        \textcolor{red!60!black}{\textbf{Flush:}} Issue flush, select repair IP = 8 (next sequential)
    };
}
