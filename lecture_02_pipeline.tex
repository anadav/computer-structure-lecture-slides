...
\begin{frame}{Multi-Cycle Control}
    ...
    \begin{tikzpicture}[scale=0.6, transform shape]
        ...
        \node[latch, anchor=west] (idex) at ([xshift=3cm]ifid.east) {};
        \node[below=1mm of idex, font=\scriptsize] {ID/EX};

        \node[latch, anchor=west] (exmem) at ([xshift=3cm]idex.east) {};
        \node[below=1mm of exmem, font=\scriptsize] {EX/MEM};

        \node[latch, anchor=west] (memwb) at ([xshift=3cm]exmem.east) {};
        \node[below=1mm of memwb, font=\scriptsize] {MEM/WB};

        % Control unit - positioned relative to IF/ID
        \node[draw, thick, ellipse, minimum width=1.5cm, minimum height=2cm, align=center, orange,
              anchor=south west] (control) at (ifid.north east) {Control};
        ...
        \draw[->, thick] (ifid.east) -- ++(0.3, 0) |- \node[above, font=\scriptsize] {Instruction} (control.west);

        % Control signals from Control unit to ID/EX - using |- with yshift
        \draw[->, thick, orange] (control.east) -| ([yshift=3mm]idex_wb.west);
        \draw[->, thick, orange] (control.east) -| ([yshift=0mm]idex_m.west);
        \draw[->, thick, orange] (control.east) -| ([yshift=-3mm]idex_ex.west);
        \draw[->, thick, orange] (control.east) -| ([yshift=-8mm]idex.west);
        \draw[->, thick, orange] (control.east) -| ([yshift=-12mm]idex.west);

        % WB signals propagating through stages
        \draw[->, thick, orange] (idex_wb.east) -- (exmem_wb.west);
        \draw[->, thick, orange] (exmem_wb.east) -- (memwb_wb.west);
        \draw[->, thick, orange] (memwb_wb.east) -- +(8mm,0) node[right, font=\tiny] {$\cdots$};

        % M signals propagating through stages
        \draw[->, thick, orange] (idex_m.east) -- (exmem_m.west);
        \draw[->, thick, orange] (exmem_m.east) -- +(8mm,0) node[right, font=\tiny] {$\cdots$};

        % EX signals
        \draw[->, thick, orange] (idex_ex.east) -- +(8mm,0) node[right, font=\tiny] {$\cdots$};
    \end{tikzpicture}
\end{frame}
