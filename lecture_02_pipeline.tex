...
% Labels for src1, src2 inputs (from ID/EX)
\node[font=\tiny, purple!70!black, rotate=90, anchor=south] at ([xshift=-10mm]BypassCtrl.north) {src1};
\node[font=\tiny, purple!70!black, rotate=90, anchor=south] at ([xshift=-5mm]BypassCtrl.north) {src2};

% Labels for dst inputs (from EX/MEM and MEM/WB)
\node[font=\tiny, purple!70!black, rotate=90, anchor=south] at ([xshift=5mm]BypassCtrl.north) {dst$_{t-1}$};
\node[font=\tiny, purple!70!black, rotate=90, anchor=south] at ([xshift=10mm]BypassCtrl.north) {dst$_{t-2}$};

% Draw connections from src registers (R4, R5 labels going down)
\coordinate (src1_tap) at ([xshift=-10mm]BypassCtrl.north);
\coordinate (src2_tap) at ([xshift=-5mm]BypassCtrl.north);
\draw[->, purple!70!black, thick] (need_r4.south) -- ++(0,-3mm) -| (src1_tap);
\draw[->, purple!70!black, thick] (need_r5.south) -- ++(0,-5mm) -| (src2_tap);
