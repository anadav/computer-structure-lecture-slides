...
\begin{frame}{Data Hazards: When Do We Need Forwarding?}
    ...
    \begin{tikzpicture}[
        ...
        MEM/.style={stage, fill=orange!25},
        WB/.style={stage, fill=purple!25},
        inst/.style={font=\scriptsize\ttfamily, anchor=east},
        hdr/.style={font=\scriptsize\bfseries},
        case_label/.style={font=\scriptsize, anchor=west},
        solution/.style={font=\scriptsize, text=blue!70!black, anchor=west},
    ]
        % Header
        \node[hdr] at (0, 0.5) {Cycle:};
        \foreach \i in {1,...,6} {
            \node[hdr] at (\i*1.1, 0.5) {\i};
        }

        % Distance labels on left
        \node[case_label, font=\small\bfseries] at (-2.8, -0.3) {Dist 1};
        \node[case_label, font=\small\bfseries] at (-2.8, -1.3) {Dist 2};
        \node[case_label, font=\small\bfseries] at (-2.8, -2.3) {Dist 3};

        % Distance 1: Producer i, Consumer i+1
        \node[inst] at (-0.2, -0.1) {i:};
        \node[IF] at (1.1, -0.1) {IF};
        \node[ID] at (2.2, -0.1) {ID};
        \node[EX, fill=green!50] at (3.3, -0.1) {EX};
        \node[MEM] at (4.4, -0.1) {MEM};
        \node[WB] at (5.5, -0.1) {WB};

        \node[inst] at (-0.2, -0.5) {i+1:};
        \node[IF] at (2.2, -0.5) {IF};
        \node[ID, fill=yellow!60] at (3.3, -0.5) {ID};
        \node[EX] at (4.4, -0.5) {EX};
        \node[MEM] at (5.5, -0.5) {MEM};
        \node[WB] at (6.6, -0.5) {WB};

        % Arrow for dist 1
        \draw[->, thick, red!70!black] (3.3, -0.35) -- (3.3, -0.25);

        % Distance 2: Producer i, Consumer i+2
        \node[inst] at (-0.2, -1.1) {i:};
        \node[IF] at (1.1, -1.1) {IF};
        \node[ID] at (2.2, -1.1) {ID};
        \node[EX] at (3.3, -1.1) {EX};
        \node[MEM, fill=orange!50] at (4.4, -1.1) {MEM};
        \node[WB] at (5.5, -1.1) {WB};

        \node[inst] at (-0.2, -1.5) {i+2:};
        \node[IF] at (3.3, -1.5) {IF};
        \node[ID, fill=yellow!60] at (4.4, -1.5) {ID};
        \node[EX] at (5.5, -1.5) {EX};
        \node[MEM] at (6.6, -1.5) {MEM};

        % Arrow for dist 2
        \draw[->, thick, red!70!black] (4.4, -1.35) -- (4.4, -1.25);

        % Distance 3: Producer i, Consumer i+3
        \node[inst] at (-0.2, -2.1) {i:};
        \node[IF] at (1.1, -2.1) {IF};
        \node[ID] at (2.2, -2.1) {ID};
        \node[EX] at (3.3, -2.1) {EX};
        \node[MEM] at (4.4, -2.1) {MEM};
        \node[WB, fill=purple!50] at (5.5, -2.1) {WB};

        \node[inst] at (-0.2, -2.5) {i+3:};
        \node[IF] at (4.4, -2.5) {IF};
        \node[ID, fill=yellow!60] at (5.5, -2.5) {ID};
        \node[EX] at (6.6, -2.5) {EX};

        % Arrow for dist 3
        \draw[->, thick, red!70!black] (5.5, -2.35) -- (5.5, -2.25);

        % Solutions on right
        \node[solution] at (7.2, -0.3) {$\Rightarrow$ \textbf{Bypass from EX/MEM}};
        \node[solution] at (7.2, -1.3) {$\Rightarrow$ \textbf{Bypass from MEM/WB}};
        \node[solution] at (7.2, -2.3) {$\Rightarrow$ \textbf{Register file split}};
    \end{tikzpicture}
