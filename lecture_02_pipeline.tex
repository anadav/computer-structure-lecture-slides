...
\node[muxdemux, muxdemux def={Lh=4.1, NL=10, Rh=4.1, NR=10, w=2.8, square pins=1},
      external pins width=0, align=center, text depth=4ex, fill=yellow!20, anchor=lpin 5] (DMem) at ([xshift=1cm]ALU.brpin 4 -| EXMEM.east) {Data\\Cache};
...
\only<1>{
    % Instruction on IF/ID latch - prominent
    \node[latch_inst] at (IFID.center) {or};
...
\only<2>{
    % Instruction on latch
    \node[latch_inst] at (IFID.center) {jcc};
    % Latch values - predicted target and direction appear
    \node[latch_target] at ([yshift=18mm]IFID.center) {50};
    \node[latch_dir] at ([yshift=28mm]IFID.center) {taken};
    \node[latch_nextseq] at ([yshift=8mm]IFID.center) {8};
    % Stage explanation
    \node[stage_box, anchor=west] at (StageExplainPos) {
...
\only<3>{
    ...
    \node[latch_inst] at (IFID.center) {sub};
    \node[latch_inst] at (IDEX.center) {jcc};
    % Latch values propagating - now on ID/EX
    \node[latch_target] at ([yshift=18mm]IDEX.center) {50};
    \node[latch_dir] at ([yshift=28mm]IDEX.center) {taken};
    \node[latch_nextseq] at ([yshift=8mm]IDEX.center) {8};
    % Stage explanation
    \node[stage_box, anchor=west] at (StageExplainPos) {
...
\only<4>{
    % Instructions on latches - jcc highlighted as it's being verified
    \node[latch_inst] at (IFID.center) {mul};
    \node[latch_inst] at (IDEX.center) {sub};
    \node[latch_inst_highlight] at (EXMEM.center) {jcc};
    % Latch values - now at EX/MEM for verification
    \node[latch_target] at ([yshift=18mm]EXMEM.center) {50};
    \node[latch_dir] at ([yshift=28mm]EXMEM.center) {taken};
    \node[latch_nextseq] at ([yshift=8mm]EXMEM.center) {8};
    % Stage explanation
    \node[stage_box, anchor=west] at (StageExplainPos) {
    ...
    \coordinate (CalloutPointer) at ([xshift=0.5cm]DirComp);
    \node[rectangle callout, draw=red!70!black, line width=1.5pt, fill=yellow!15,
          rounded corners=4pt, font=\small, align=left, inner sep=6pt,
          callout absolute pointer=(CalloutPointer), text width=3.8cm]
          at ([xshift=1cm, yshift=-3cm]VerifyBox.south east) {
        \textbf{Verify direction}\\
        \textbf{Verify target} (if taken)
    };
}
...
\only<5>{
    % Instructions on latches
    \node[latch_inst] at (IFID.center) {add};
    \node[latch_inst] at (IDEX.center) {mul};
    \node[latch_inst] at (EXMEM.center) {sub};
    \node[latch_inst] at (MEMWB.center) {jcc};
...
\only<1>{
    ...
    \node[latch_inst] at (IDEX.center) {sub};
    \node[latch_inst_highlight] at (EXMEM.center) {jcc};
    \node[latch_inst] at (MEMWB.center) {or};
    % Latch values at EX/MEM
    \node[latch_target] at ([yshift=18mm]EXMEM.center) {50};
    \node[latch_dir] at ([yshift=28mm]EXMEM.center) {taken};
    \node[latch_nextseq] at ([yshift=8mm]EXMEM.center) {8};
...
\only<2>{
    ...
    \node[latch_inst_flush] at (IFID.center) {mul};
    \node[latch_inst_flush] at (IDEX.center) {sub};
    \node[latch_inst_highlight] at (EXMEM.center) {jcc};
    \node[latch_inst] at (MEMWB.center) {or};
    % Mismatch indicator
...
\only<3>{
    ...
    \node[latch_inst_nop] at (IFID.center) {NOP};
    \node[latch_inst_nop] at (IDEX.center) {NOP};
    \node[latch_inst] at (EXMEM.center) {jcc};
    \node[latch_inst] at (MEMWB.center) {or};
    % Stage explanation
...
\only<4>{
    ...
    \node[latch_inst_new] at (IFID.center) {and};
    \node[latch_inst_nop] at (IDEX.center) {--};
    \node[latch_inst_nop] at (EXMEM.center) {--};
    \node[latch_inst] at (MEMWB.center) {jcc};
    % Stage explanation
    \node[draw=green!50!black, line width=1.5pt, fill=green!10, rounded corners=6pt,
          font=\small, inner sep=8pt, anchor=west] at (StageExplainPos) {
