...
\draw[arrow, orange!70!black, line width=1.5pt] (zero_mem_out) -- ++(6mm,0) |- ([yshift=5mm]PCSrcMux.north) -- (PCSrcMux.tpin 1);
...
% Flushed instructions (one step forward from EXE resolution)
...
% Coordinate for explanation box at bottom
\coordinate (StageExplainPos) at ([yshift=-15mm]IMem.south -| RegFile);

% Explanation box (matching original text)
\node[draw=green!50!black, line width=1.5pt, fill=green!10, rounded corners=6pt,
      font=\small, inner sep=8pt, anchor=west, align=left] at (StageExplainPos) {
...
\only<2>{
    ...
    \node[latch_inst_flush] at (Instruction.south) {\sout{mul}};
    \node[latch_inst_flush] at (IDEX.center |- Instruction.south) {\sout{sub}};
