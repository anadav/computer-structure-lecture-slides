...
\pause
\vspace{2mm}
\normalsize Not quite -- \textbf{pipeline hazards} prevent us from reaching ideal throughput.
\end{frame}
...
%% Slide: RAW Dependency in Pipeline
\begin{frame}{RAW Dependency in Pipeline}
\begin{center}
\scalebox{0.55}{
\begin{circuitikz}[
...
\node[hazard_value] at (src1_data_pos) {[R4]};
\node[hazard_value] at (src2_data_pos) {[R5]};
\node[dst_value] at (dst_idex_pos) {R12};

% sub in EX/MEM with result
\node[data_value] at (alu_result_pos) {[R2]-[R3]};
\node[dst_value] at (dst_exmem_pos) {R5};

% lw in MEM/WB with memory result
\node[data_value] at (mem_data_pos) {M[[R1]+9]};
\node[dst_value] at (dst_memwb_pos) {R4};
