...
\pgfkeys{
    ...
    at x/.estore in = \pipeatx,
    at y/.estore in = \pipeaty,
    scale/.estore in = \pipescale
}

\newcommand{\drawpipelinearch}[1][]{%
    \pgfkeys{/pipelinearch, default, #1}%
    \begin{scope}[shift={(\pipeatx,\pipeaty)}, scale=\pipescale, transform shape]
        ...
        % Instruction Memory - positioned at origin (shift handles positioning)
        \node[imem_block] (\pipeprefix-IM) at (0,0) {IM};
...
\begin{frame}{Register File Split}
    ...
    \begin{circuitikz}[scale=0.9, transform shape]
        ...
        \node[anchor=east, font=\small\ttfamily] (and_inst) at (-0.3, 1) {and R12, R2, R5};
        \node[anchor=east, font=\small\ttfamily] (or_inst) at (-0.3, 0) {or R13, R6, R2};
        \node[anchor=east, font=\small\ttfamily] (add_inst) at (-0.3, -1) {add R14, R2, R2};
        \node[anchor=east, font=\small\ttfamily] (sw_inst) at (-0.3, -2) {sw R15, 100(R2)};

        % Draw pipeline stages using the macro in a loop
        % Each instruction i starts at cycle i+1, aligned with instruction y position
        \foreach \i in {0,...,4} {
            \pgfmathsetmacro{\cycleno}{\i + 1}
            \pgfmathsetmacro{\ypos}{2 - \i}
            \drawpipelinearch[prefix=i\i, at x=\cycleno, at y=\ypos, scale=0.4]
        }

        % Program execution order label with arrow
        \coordinate (arrow_start) at (-2, 2.2);
        \coordinate (arrow_end) at (arrow_start |- sw_inst.south);
        \node[anchor=south, align=center, font=\small] at (arrow_start) {Program\\execution\\order};
        \draw[->, thick] (arrow_start) -- (arrow_end);
    \end{circuitikz}
\end{frame}
...
then stall
    ...
    \begin{circuitikz}[scale=1, transform shape]
        ...
        \drawpipelinearch[prefix=lw, at x=1, at y=2, scale=0.5]

        % and instruction - cycle 2
        \drawpipelinearch[prefix=and1, at x=2, at y=1, scale=0.5]
        ...
        \drawpipelinearch[prefix=and2, at x=4, at y=1, scale=0.5]

        % or instruction - cycle 3
        \drawpipelinearch[prefix=or, at x=3, at y=0, scale=0.5]

        % add instruction - cycle 4
        \drawpipelinearch[prefix=add, at x=4, at y=-1, scale=0.5]

        % sw instruction - cycle 5
        \drawpipelinearch[prefix=sw, at x=5, at y=-2, scale=0.5]
    \end{circuitikz}
\end{frame}
