...
\begin{circuitikz}[
    % Component styles
    component/.style={draw, thick, minimum height=0.8cm},
    pipeline_reg/.style={draw, thick, fill=yellow!40, minimum width=6mm, minimum height=5.8cm},
...
\node[ellipse, draw, thick, fill=orange!30, minimum width=1.2cm, minimum height=0.6cm, inner sep=0pt,
      font=\scriptsize, above=3mm of RegFile.north] (Decode) {decode};
\node[control_label, right=2mm of Decode.east, anchor=west] {Control\\signals};
...
\draw[data_path, orange!80!black] (Decode.east) -- (ctrl_idex_in);
...
% ALU source (from ID/EX to ALUMux)
\coordinate (ctrl_alusrc) at ([xshift=5mm]ctrl_idex_out);
\node[data_connector, fill=orange!80!black] at (ctrl_alusrc) {};
\draw[->, >=stealth, thick, orange!80!black] (ctrl_alusrc) -- (ALUMux.bpin 1) node[control_label, midway, right] {ALU\\source};

% ALU Control (from ID/EX to ALU)
\coordinate (ctrl_aluctl) at ([xshift=15mm]ctrl_idex_out);
\node[data_connector, fill=orange!80!black] at (ctrl_aluctl) {};
\draw[->, >=stealth, thick, orange!80!black] (ctrl_aluctl) -- (ctrl_aluctl |- ALU.north) node[control_label, midway, right] {ALU\\Control};

% MemWr (from EX/MEM to DMem)
\coordinate (ctrl_memwr) at ([xshift=5mm]ctrl_exmem_out);
\node[data_connector, fill=orange!80!black] at (ctrl_memwr) {};
\draw[->, >=stealth, thick, orange!80!black] (ctrl_memwr) -- (ctrl_memwr |- DMem.north) node[control_label, midway, left, xshift=-1mm] {Mem\\Wr};

% MemRd (from EX/MEM to DMem)
\coordinate (ctrl_memrd) at ([xshift=20mm]ctrl_exmem_out);
\node[data_connector, fill=orange!80!black] at (ctrl_memrd) {};
\draw[->, >=stealth, thick, orange!80!black] (ctrl_memrd) -- (ctrl_memrd |- DMem.north) node[control_label, midway, right, xshift=1mm] {Mem\\Rd};

% WB select (from MEM/WB to WBMux)
\coordinate (ctrl_wbsel) at ([xshift=5mm]ctrl_memwb_out);
\node[data_connector, fill=orange!80!black] at (ctrl_wbsel) {};
\draw[->, >=stealth, thick, orange!80!black] (ctrl_wbsel) -- (WBMux.bpin 1) node[control_label, midway, right] {WB\\select};
...
\draw[data_path] (dst_y_level) -- (dst_idex_in);
...
\only<3>{
    ...
    \node[latch_target] at ([xshift=-2mm]calc_target_exmem) {50};
    % Explanation
    \node[explain_box] at (StageExplainPos) {
