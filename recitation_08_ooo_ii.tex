...
\newcommand{\OOODiagram}{%
    ...
    \node[draw, rectangle, minimum width=3cm, minimum height=3.5cm, label=above:{\textbf{ROB}}, right=of rat] (rob) {};
    ...
    \node[draw, rectangle, minimum width=2.5cm, minimum height=1.5cm, label=above:{\textbf{MOB}}, right=of idq] (rs) {};
    
    % Execute and Retire blocks
    \node[roundbox, below=of rs, label=right:{\textbf{Execute}}] (execute) {};
    \node[dashedbox, below=of execute, label=right:{\textbf{Retire}}] (retire) {};
}

\newcommand{\generateROBContent}[1]{%
    |[cell]| \scriptsize{} & |[cell]| \scriptsize{} \\
    |[cell]| \scriptsize{} & |[cell]| \scriptsize{} \\
    |[cell]| \scriptsize{} & |[cell]| \scriptsize{} \\
    |[cell]| \scriptsize{} & |[cell]| \scriptsize{} \\
    |[cell]| \scriptsize{} & |[cell]| \scriptsize{} \\
...
\newcommand{\generateSingleColContent}[1]{%
    |[cell]| \scriptsize{} \\
    |[cell]| \scriptsize{} \\
    |[cell]| \scriptsize{} \\
    |[cell]| \scriptsize{} \\
...
\newcommand{\generateExecContent}[1]{%
    |[nocell]| \scriptsize{} \\
    |[nocell]| \scriptsize{} \\
}

\newcommand{\generateRetireContent}[1]{%
    |[nocell]| \scriptsize{} & |[nocell]| \scriptsize{} \\
}
...
\newcommand{\OOODiagramKV}[1][]{%
    ...
    \matrix[matrix of nodes, 
            ...
            ampersand replacement=\&] (rat) at (0,0) {
        \scriptsize ARF \& \scriptsize PRF \\
    ...
    %
    % ROB (Reorder Buffer) - 2 columns, 9 rows by default
    \matrix[matrix of nodes, 
            column sep=-\pgflinewidth,
            row sep=-\pgflinewidth,
            inner sep=0pt,
            label={above:\textbf{ROB}},
            below=of rat, xshift=0.5cm,
            ampersand replacement=\&] (rob) {
        % Generate rows - fixed 9 rows
        |[cell]| \scriptsize{} \& |[cell]| \scriptsize{} \\
        |[cell]| \scriptsize{} \& |[cell]| \scriptsize{} \\
        |[cell]| \scriptsize{} \& |[cell]| \scriptsize{} \\
        |[cell]| \scriptsize{} \& |[cell]| \scriptsize{} \\
    ...
    \foreach \i in {1,...,\robRows} {
        \coordinate (robmarker\i) at ($(rob-\i-2.east)+(0.2,0)$);
        \node[anchor=west, inner sep=1pt, font=\tiny] at (robmarker\i) {rob\i};
    }
    %
    % IDQ (Instruction Decode Queue) - 6 rows by default
    \matrix[matrix of nodes,
            row sep=-\pgflinewidth,
            inner sep=0pt,
            label={above:\textbf{IDQ}},
            right=of rat, xshift=0.5cm, yshift=1cm,
            nodes={cell, minimum width=2cm},
    ...
    %
    % RS (Reservation Station) - 4 rows by default
    \matrix[matrix of nodes,
            row sep=-\pgflinewidth,
            inner sep=0pt,
            label={above:\textbf{RS}},
            below=of idq, yshift=0.3cm,
            nodes={cell, minimum width=2cm},
    ...
    \foreach \i in {1,...,\rsRows} {
        \coordinate (rsmarker\i) at ($(rs-\i-1.east)+(0.2,0)$);
        \node[anchor=west, inner sep=1pt, font=\tiny] at (rsmarker\i) {rs\i};
    }
    %
    % MOB (Memory Order Buffer) - 2 rows by default
    \matrix[matrix of nodes,
            row sep=-\pgflinewidth,
            inner sep=0pt,
            label={above:\textbf{MOB}},
            right=of rs, xshift=0.3cm,
            nodes={cell, minimum width=2cm},
    ...
    \foreach \i in {1,...,\mobRows} {
        \coordinate (mobmarker\i) at ($(mob-\i-1.east)+(0.2,0)$);
        \node[anchor=west, inner sep=1pt, font=\tiny] at (mobmarker\i) {mob\i};
    }
    %
    % Execute block with internal rows (no borders)
    \node[roundbox, below=of rs, yshift=-0.5cm] (execute) {};
    \node[anchor=west] at (execute.east) {\textbf{Execute}};
    %
    % Add internal structure to Execute (2 rows by default, no borders)
    \matrix[matrix of nodes,
            row sep=2pt,
            inner sep=0pt,
            at={(execute.center)},
    ...
    \node[dashedbox, below=of execute, yshift=-0.3cm] (retire) {};
    \node[anchor=west] at (retire.east) {\textbf{Retire}};
    %
    % Add internal structure to Retire (2 columns by default, no borders)
    \matrix[matrix of nodes,
            column sep=10pt,
            inner sep=0pt,
            at={(retire.center)},
    ...
    \foreach \i in {1,...,\retireCols} {
        \coordinate (retmarker\i) at ($(retireMatrix-1-\i.east)+(0.2,0)$);
        \node[anchor=west, inner sep=1pt, font=\tiny] at (retmarker\i) {ret\i};
    }
    %
    \end{tikzpicture}
}
...
\OOODiagramKV[
    ...
    % Configurable structure parameters
    rob rows=9,      % Number of ROB rows (default: 9)
    idq rows=6,      % Number of IDQ rows (default: 6)
    rs rows=4,       % Number of RS rows (default: 4)
    mob rows=2,      % Number of MOB rows (default: 2)
    execute rows=2,  % Number of Execute internal rows (default: 2)
    retire cols=2    % Number of Retire internal columns (default: 2)
]
\end{frame}
