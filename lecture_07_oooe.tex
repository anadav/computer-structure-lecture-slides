...
\usepackage{listings}
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, calc, tikzmark, shapes.misc, shapes.callouts, fit, matrix, backgrounds}
...
\begin{frame}{Register Renaming: Example}
  \begin{center}
    \begin{tikzpicture}[font=\small]
      \matrix[
        ...
        (1) \& \& \textcolor{blue}{r1} $\leftarrow$ 17 \& r1:pr1 \& pr1 $\leftarrow$ 17 \\
        (2) \& \& r2 $\leftarrow$ r2+\textcolor{blue}{r1} \& r2:pr2 \& pr2 $\leftarrow$ r2+pr1 \\
        (3) \& \& \textcolor{orange}{r1} $\leftarrow$ 23 \& \only<2->{r1:pr3} \& \only<2->{pr3 $\leftarrow$ 23} \\
        (4) \& \& \textcolor{red}{r3} $\leftarrow$ r3+\textcolor{orange}{r1} \& \only<2->{r3:pr4} \& \only<2->{pr4 $\leftarrow$ r3+pr3} \\
        (5) \& \& jcc L2 \& \& \\
        (6) \& L2: \& \textcolor{green}{r1} $\leftarrow$ 35 \& \only<3->{r1:pr5} \& \only<3->{pr5 $\leftarrow$ 35} \\
        (7) \& \& r4 $\leftarrow$ \textcolor{red}{r3}+\textcolor{green}{r1} \& \only<3->{r4:pr6} \& \only<3->{pr6 $\leftarrow$ pr4+pr5} \\
        (8) \& \& \textcolor{blue}{r3} $\leftarrow$ 2 \& \only<3->{r3:pr7} \& \only<3->{pr7 $\leftarrow$ 2} \\
      };
    \end{tikzpicture}
  \end{center}
  ...
  \only<1>{%
    \begin{tikzpicture}
      \node[rectangle callout, callout absolute pointer={(0,0.5)}, fill=yellow!20, draw=yellow!60!black, rounded corners=2pt, font=\small, text width=0.9\textwidth, align=left] {
  ...
  \begin{center}
    \begin{tabular}{|c|c|c|c|c|}
      ...
      \hline
      Register mapping &
        \only<1>{pr1}\only<2>{pr3}\only<3->{pr5} &
        pr2 &
        \only<1>{r3}\only<2>{pr4}\only<3->{pr7} &
  ...
  \begin{center}
    \begin{tikzpicture}[font=\small]
      \matrix[
        ...
        (1) \& \& \textcolor{blue}{r1} $\leftarrow$ 17 \& r1:pr1 \& pr1 $\leftarrow$ 17 \\
        (2) \& \& r2 $\leftarrow$ r2+\textcolor{blue}{r1} \& r2:pr2 \& pr2 $\leftarrow$ r2+pr1 \\
        (3) \& \& \textcolor{orange}{r1} $\leftarrow$ 23 \& r1:pr3 \& pr3 $\leftarrow$ 23 \\
        (4) \& \& \textcolor{red}{r3} $\leftarrow$ r3+\textcolor{orange}{r1} \& r3:pr4 \& pr4 $\leftarrow$ r3+pr3 \\
        (5) \& \& jcc L2 \& \& \\
        (6) \& L2: \& \textcolor{green}{r1} $\leftarrow$ 35 \& r1:pr5 \& pr5 $\leftarrow$ 35 \\
        (7) \& \& r4 $\leftarrow$ \textcolor{red}{r3}+\textcolor{green}{r1} \& r4:pr6 \& pr6 $\leftarrow$ pr4+pr5 \\
        (8) \& \& \textcolor{blue}{r3} $\leftarrow$ 2 \& r3:pr7 \& pr7 $\leftarrow$ 2 \\
      };
...
\begin{frame}{Execution Units Bypasses}
  \begin{center}
  \begin{tikzpicture}[font=\footnotesize,
    port/.style={draw, fill=green!20, inner sep=3mm},
    eu/.style={draw, fill=blue!15, minimum width=1cm, minimum height=0.4cm},
    ]
    ...
    % Execution units in Port 0
    \node[eu, below right=1.5cm and 0.5cm of rs.south east] (ieu0) {\tiny IEU};
    \node[eu, above=0.1cm of ieu0] (fau) {\tiny FAU};
    \node[eu, above=0.1cm of fau] (idiv) {\tiny IDIV};
    \node[eu, above=0.1cm of idiv] (fdiv) {\tiny FDIV};
    \node[eu, above=0.1cm of fdiv] (fmu) {\tiny FMU};
    ...
    % Execution units in Port 1
    \node[eu, right=0.6cm of ieu0] (ieu1) {\tiny IEU};
    \node[eu, above=0.1cm of ieu1] (jeu) {\tiny JEU};

    % Execution units in Port 2
    \node[eu, right=0.6cm of ieu1] (agu2) {\tiny AGU};

    % Execution units in Port 3,4
    \node[eu, right=0.6cm of agu2] (agu34) {\tiny AGU};

    % Port containers using fit (with transparency to show EUs)
    \node[port, fit=(ieu0)(shf), fill opacity=0.3] (port0) {};
    \node[port, fit=(ieu1)(jeu), fill opacity=0.3] (port1) {};
    \node[port, fit=(agu2), fill opacity=0.3] (port2) {};
    \node[port, fit=(agu34), fill opacity=0.3] (port34) {};

    % Port labels
    \node[above=0mm of port0.north, font=\scriptsize, fill=white, inner sep=1pt] {Port 0};
    \node[above=0mm of port1.north, font=\scriptsize, fill=white, inner sep=1pt] {Port 1};
    \node[above=0mm of port2.north, font=\scriptsize, fill=white, inner sep=1pt] {Port 2};
    \node[above=0mm of port34.north, font=\scriptsize, fill=white, inner sep=1pt] {Port 3,4};
    ...
    \draw[->, thick, red, rounded corners] ([xshift=8mm]ieu1.east) -| ([xshift=1.2cm]ieu1.east |- ieu1.north);

    % 1st-level bypass between ports (green from port 1 to port 2)
    \draw[->, thick, green!60!black] ([yshift=-5mm]port1.south) |- ([yshift=-5mm]port2.south);

    % 2nd-level bypass through RS (blue loop back to RS)
    \draw[->, thick, blue] ([xshift=-8mm]port34.north) |- ([xshift=8mm]rs.south);
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{The Bypass Network}
  \begin{itemize}
    \item \textbf{Assume:}
    \begin{itemize}
      \item Instruction Y writes PR19 (1 cycle execution)
      \item Instruction X reads PR19 + immediate
      \item 0-level bypass: same EU, back-to-back
      \item 1-level bypass: different EU, 3 cycles
    \end{itemize}

    \item \textbf{RS marks X ready when:}
    \begin{itemize}
      \item \textbf{Y finished before X allocated:} PR19 value written to X's RS entry $\Rightarrow$ X ready at allocation

      \item \textbf{Y and X sent to same EU:} X marked ready, dispatched next cycle (0-level bypass)

      \item \textbf{Y and X sent to different EUs:} X marked ready after 3 cycles (1-level bypass)

      \item \textbf{Y executing when X allocated:} X becomes ready when Y's write-back reaches RS
    \end{itemize}
  \end{itemize}
\end{frame}
