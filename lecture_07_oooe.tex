  ...
  \begin{columns}
    \begin{column}{0.65\textwidth}
      \begin{itemize}
        ...
        \begin{itemize}
          \item<1-> Let current cycle instruction retire
          \item<2-> Flush subsequent instructions
          \item<3-> Initiate interrupt service code
          \item<3-> Re-fetch subsequent instructions
        \end{itemize}
      \end{itemize}
    \end{column}
    \begin{column}{0.35\textwidth}
      \begin{center}
        \begin{tikzpicture}[font=\tiny,
          E/.style={draw, fill=white, minimum width=1.6cm, minimum height=0.3cm, inner sep=0},
          G/.style={E, fill=green!30},
          R/.style={E, fill=red!30},
          P/.style={E, fill=purple!30}
        ]
          \matrix[matrix of nodes, ampersand replacement=\&, row sep=-\pgflinewidth,
            nodes={minimum width=1.6cm, minimum height=0.3cm, inner sep=0}
          ] (m) {
            |[E]| \\
            |[E]| \\
            |[G]| \\
          ...
          % Interrupt indicator above ROB (stages 1-2)
          \only<1-2>{
            \node[inner sep=0pt, above=0.2cm of m-1-1] (boom) {\includegraphics[width=1.6cm, height=1.0cm]{figures/noun-boom-6722068.png}};
            \node[font=\scriptsize\bfseries] at (boom.center) {\textcolor{red}{interrupt}};
          }

          % Stage 2: Flushed overlay
          \only<2>{
            \fill[white, opacity=0.7] (m-4-1.north west) rectangle (m-16-1.south east);
            \node[rotate=45, font=\small, opacity=0.8] at (m-10-1.center) {flushed};
          }

          % Stage 3: ISR and re-fetched
          \only<3>{
            % Override with purple for ISR
            \node[P] at (m-3-1) {};
            \node[P] at (m-4-1) {};
            \node[P] at (m-5-1) {};
            \node[P] at (m-6-1) {};
            \node[P] at (m-7-1) {};
            \node[font=\tiny, align=center] at (m-5-1.center) {ISR};
            % Override rest with green
            \foreach \i in {8,...,16} {
              \node[G] at (m-\i-1) {};
            }
          }
          ...
          % Labels
          \node[left=0.3cm of m-3-1.west, anchor=east] (retire) {\scriptsize retire};
          \draw[->,thick] (retire) -- (m-3-1.west);
          \node[right=0.1cm of m-3-1.east, anchor=west] {\scriptsize oldest};

          \only<1-2>{
            \node[left=0.3cm of m-16-1.west, anchor=east] (alloc) {\scriptsize alloc};
            \draw[->,thick] (alloc) -- (m-16-1.west);
