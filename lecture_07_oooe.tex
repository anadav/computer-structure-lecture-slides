...
\begin{frame}{Jump Misprediction -- Flush at Retire}
    ...
    \begin{columns}[T]
        \begin{column}{0.9\textwidth}
            ...
            \begin{itemize}
                \item Flush the entire pipeline
                \begin{itemize}
                    \item All remaining instructions are younger $\Rightarrow$ all from wrong path
                \end{itemize}
                \item Reset the rename map to architectural registers
                \begin{itemize}
                    \item Safe: no in-flight consumers of physical registers
                \end{itemize}
                \item Redirect fetch to the correct path
            \end{itemize}

            \vspace{0.3cm}
            \textbf{Disadvantage}
            \begin{itemize}
                \item High misprediction penalty --- misprediction detected at execute, \\
                but recovery waits until retire
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
