...
\begin{frame}{A Superscalar OOO Machine}
  \begin{center}
    \begin{tikzpicture}[scale=0.8,
      ...
      % Stage 3: Add Execute
      \onslide<3->{
        \node[exe, below=2cm of rs] (execute) {Execute};
        \node[below=of execute] {\textbf{Out-of-order}};
      }

      % Stage 4: Add Retire
      \onslide<4->{
        \node[retire,align=center, right=of rs] (retire) {Retire\\(commit)};
        \node[above=of retire] {\textbf{In-order}};
      }

      % In-order label for initial stages
      \onslide<1-3>{
        \node[below=of retire] (inorder) {\textbf{In-order}};
      }
      ...
      \onslide<2-3>{
        \node[draw=gray, fill=gray!10, fit=(fetch) (rename) (rob) (rs) (inorder), inner sep=8pt, rounded corners] (bg2) {};
      }
      ...
      \onslide<4->{
        \node[retire,align=center, right=2cm of rs] (retire) {Retire\\(commit)};
      }
      \onslide<1-3>{
        \node[below=of retire] (inorder) {\textbf{In-order}};
      }
      ...
      \onslide<2->{
        ...
        \draw[->, thick] (rename.east) -- ++(0.3,0) -- ++(0,1.2) -- (rob.west);
      }

      % Arrows - Stage 3
      \onslide<3->{
        \draw[<->, thick] (rs.south) -- (execute.north);
        \draw[->, thick] (execute.east) -- ++(0.8,0) -- ++(0,3.7) -- (rob.south);
      }
      ...
      % Callout annotations - Stage 1
      \onslide<1>{
        \node[calloutbox, below=2cm of fetch, callout absolute pointer={(fetch.south)}]
          (fetch_call)
          {Fetch and decode instructions in parallel but in order};

        \node[calloutbox, below=2cm of rename, callout absolute pointer={(rename.south)}]
          (rename_call)
      ...
      \onslide<2>{
        \node[calloutbox, above=1cm of rob, callout absolute pointer={(rob.north)}]
          (rob_call)
          {\textbf{Reorder Buffer (ROB)}\\Pool of instructions waiting for retirement};

        \node[calloutbox, below=1cm of rs, callout absolute pointer={(rs.south)}]
          (rs_call)
          {\textbf{Reservation Stations (RS)}\\Pool of instructions waiting for execution\\Maintains per inst. sources ready/not-ready status};

        \node[calloutbox, below=1cm of rename, callout absolute pointer={(rename.south)}]
          (rename_call2)
      ...
      \onslide<3>{
        \node[calloutbox, left=1cm of rs, callout absolute pointer={(rs.west)}, font=\tiny]
          (rs_call3)
          {Track which instructions are ready: have all sources ready\\
        ...
        \node[calloutbox, below=1cm of execute, callout absolute pointer={(execute.south)}]
          (execute_call)
          {Write back value and mark more sources as ready\\
          Send "exe done" indication to ROB and reclaim RS entry};
      }

      % Callout annotations - Stage 4
      \onslide<4>{
        \node[calloutbox, below=1cm of retire, callout absolute pointer={(retire.south)}]
          (retire_call)
