...
\begin{tikzpicture}[
    ...
    % L2 cache (left)
    \node[cache, fill=green!30] (l2) at (0, 0) {L2\\cache};

    % Fill Buffers (center) as matrix with header row
    \matrix[right=0.6cm of l2, matrix of nodes,
            row sep=-0.4pt,
            column sep=-0.4pt,
            ampersand replacement=\&,
    ...
    % L1 cache (right)
    \node[cache, fill=orange!30, right=0.6cm of fb] (l1) {L1\\cache};

    % Arrows: L1 -> FB, FB <-> L2
    \draw[<-, very thick] (fb.east) -- (fb.east -| l1.west);
    \draw[<->, very thick] (fb.west) -- (fb.west -| l2.east);
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
