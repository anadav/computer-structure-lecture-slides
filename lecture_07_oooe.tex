...
\begin{frame}{OOOE: General Scheme}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.2, every node/.style={font=\small},
      fetchblock/.style={rectangle, draw, fill=cyan!20, minimum width=2.5cm, minimum height=0.7cm},
      block/.style={rectangle, draw, fill=yellow!30, minimum width=2.5cm, minimum height=0.7cm},
      pool/.style={rectangle, draw, fill=green!30, minimum width=2.5cm, minimum height=0.7cm},
      exe/.style={rectangle, draw, fill=purple!30, minimum width=2cm, minimum height=0.7cm}]

      % Blocks
      \node[fetchblock] (fetch) at (0,0) {Fetch \& Decode};
      \node[pool] (pool) at (4,0) {Instruction Pool};
      \node[exe] (exec) at (4,-1.5) {Execute};
      ...
      % Labels
      \node[below] at (fetch.south) {\textbf{In-order}};
      \node[below] at (retire.south) {\textbf{In-order}};
  ...
  \begin{itemize}
    \item \textbf{Fetch \& decode in-order} --- fill the Instruction Pool
    \item \textbf{Execute ready instructions} from the Instruction Pool
      --- source data ready + execution resources available
    \item \textbf{Once executed} --- signal dependent instructions that data is ready
    \item \textbf{Commit in-order} --- state changes (memory, register) and exception handling
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[font=\tiny, scale=0.8,
        ...
        datacell/.style={draw, fill=green!20, minimum width=0.6cm, minimum height=0.22cm, inner sep=1pt},
        statuscell/.style={draw, fill=gray!25, minimum width=0.4cm, minimum height=0.22cm, inner sep=1pt},
        ...
        \foreach \i in {0,...,7} {
          \node[datacell] (robd\i) at (0.9, 1.9-\i*0.28) {};
          \node[statuscell] (robs\i) at (1.7, 1.9-\i*0.28) {};
        }
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[font=\tiny, scale=0.8,
        ...
        \foreach \i in {0,...,7} {
          ...
          \node[statuscell] (rob\i) at (2.5, 2.2-\i*0.28) {};
        }
        ...
        \draw[->, red, thick] (rrat3.west) to[out=150,in=-150, looseness=1.3] (frat3.west)
          node[pos=0.5, left, font=\tiny, text=red, align=left, xshift=-3cm] {copy on\\recovery};
      \end{tikzpicture}
