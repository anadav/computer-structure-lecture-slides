...
\begin{frame}{Data Cache Miss}
  \begin{itemize}
    \item \textbf{Non-blocking D\$:} On load miss, continue dispatching subsequent loads
    \begin{itemize}
      \item Blocking cache would hurt a main OOO benefit: hiding cache miss latency
    \end{itemize}
    \item \textbf{On load miss:}
    \begin{itemize}
      \item Mark load's write-back data as \textit{invalid}
      \begin{itemize}
        \item RS dispatches load consumers assuming hit; re-dispatch when data arrives
      \end{itemize}
      \item Assign a \textbf{Fill Buffer (FB)} entry, issue L2 request
      \begin{itemize}
        \item FB holds request address and data (when it returns)
        \item More loads can dispatch as long as free FBs exist
      \end{itemize}
      \item Subsequent loads to same missed line share the FB entry
    \end{itemize}
    \item \textbf{When critical chunk returns:}
    \begin{itemize}
      \item Wakeup and re-dispatch the waiting load
      \item Load reads directly from FB (no need to wait for D\$ write)
    \end{itemize}
    \item \textbf{Blocked load} (unknown store addr/data): still performs D\$ lookup as \textit{prefetch}
  \end{itemize}
\end{frame}
