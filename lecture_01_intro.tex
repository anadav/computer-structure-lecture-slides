...
\begin{tikzpicture}[
    ...
    % RAM - right of CPU
    \node[icon, right=2.5cm of cpu] (ram) {\includegraphics[height=1.1cm]{figures/noun-ram-7483151.png}};
    \node[lbl, above=0.02cm of ram] {Memory};

    % NVMe SSD - below RAM (fast storage via CPU PCIe)
    \node[icon, below=1cm of ram] (nvme) {\includegraphics[height=0.9cm]{figures/noun-nvme-6849180.png}};
    \node[lbl, below=0.02cm of nvme] {SSD (NVMe)};

    % GPU - left of CPU
    \node[icon, left=2cm of cpu] (gpu) {\includegraphics[height=1cm]{figures/noun-graphics-card-1036913.png}};
    \node[lbl, above=0.02cm of gpu] {Graphics Card};

    % Display - left of GPU
    \node[icon, left=1.5cm of gpu] (display) {\includegraphics[height=1.2cm]{figures/noun-computer-screen-8056981.png}};
    ...
    \node[lbl, below=0.02cm of pch] {Chipset};

    % HDD - below right of chipset (slow storage via SATA)
    \node[icon, below right=0.8cm and 1.5cm of pch] (hdd) {\includegraphics[height=0.9cm]{figures/noun-hard-drive-7483169.png}};
    \node[lbl, below=0.02cm of hdd] {HDD};

    % USB Hub - below chipset
    \node[icon, below=1.2cm of pch] (usb) {\includegraphics[height=0.8cm]{figures/noun-usb-8182582.png}};
    \node[lbl, below=0.02cm of usb] {USB Hub};

    % Keyboard - below left of USB hub
    \node[icon, below left=0.5cm and 1.2cm of usb] (kbd) {\includegraphics[height=0.7cm]{figures/noun-keyboard-8103038.png}};
    \node[lbl, below=0.02cm of kbd] {Keyboard};

    % Connections - CPU direct
    \draw[bus] (cpu) -- node[busname, above] {Memory Bus} (ram);
    \draw[bus] (cpu) -- node[busname, above] {PCIe} (gpu);
    \draw[bus] (cpu) -- node[busname, right] {PCIe} (nvme);
    ...
    % USB hub to keyboard
    \draw[bus] (usb) -- node[busname, below, pos=0.5] {USB} (kbd);
\end{tikzpicture}
\end{frame}
