...
\begin{tikzpicture}
    ...
    % Circle the negative outliers (left end)
    \draw[red, very thick] (axis cs:5,-3) circle (8pt);
    \node[red, anchor=west] at (axis cs:12,-3.5) {\small Significant regressions};
    % Circle the positive outliers (right end)
    \draw[green!60!black, very thick] (axis cs:95,2) circle (8pt);
    \node[green!60!black, anchor=east] at (axis cs:88,3) {\small Modest gains};
    \end{axis}
\end{tikzpicture}

\vspace{0.2cm}
\begin{alertblock}{Problem}
Many applications regress significantly ($>$2\%), while gains are modest. \\
\textbf{This feature should probably be rejected.}
\end{alertblock}
\end{frame}
...
\begin{tikzpicture}
    ...
    % Circle the negative outliers (left end) - small
    \draw[red, very thick] (axis cs:5,-0.3) circle (8pt);
    \node[red, anchor=west] at (axis cs:12,-1.5) {\small Minor regressions};
    % Circle the positive outliers (right end)
    \draw[green!60!black, very thick] (axis cs:95,4.5) circle (8pt);
    \node[green!60!black, anchor=east] at (axis cs:88,5.5) {\small Strong gains};
    \end{axis}
\end{tikzpicture}

\vspace{0.2cm}
\begin{exampleblock}{Success}
Minimal regressions ($<$1\%), significant gains for many applications. \\
\textbf{This feature is worth implementing.}
\end{exampleblock}
\end{frame}
