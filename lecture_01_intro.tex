...
\author{Computer Architecture 236267}
\date{}
...
% Slide 9b: Speedup and Slowdown
\begin{frame}{Speedup and Slowdown}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{block}{Speedup}
How much \textbf{faster} is the new system?
$$\text{Speedup} = \frac{t_{\text{old}}}{t_{\text{new}}} = \frac{\text{CPI}_{\text{old}}}{\text{CPI}_{\text{new}}}$$

\vspace{0.2cm}
\small
Example: $t_{\text{old}} = 2s$, $t_{\text{new}} = 1s$

$\Rightarrow$ Speedup $= 2\times$ (``twice as fast'')
\end{block}
\end{column}

\begin{column}{0.48\textwidth}
\begin{block}{Slowdown}
How much \textbf{slower} is the new system?
$$\text{Slowdown} = \frac{t_{\text{new}}}{t_{\text{old}}} = \frac{\text{CPI}_{\text{new}}}{\text{CPI}_{\text{old}}}$$

\vspace{0.2cm}
\small
Example: $\text{CPI}_{\text{old}} = 1$, $\text{CPI}_{\text{new}} = 1.6$

$\Rightarrow$ Slowdown $= 1.6\times$ (``60\% slower'')
\end{block}
\end{column}
\end{columns}
...
Slowdown $= \dfrac{1}{\text{Speedup}}$. \quad A slowdown of $1.6\times$ means execution takes 60\% longer.
\end{alertblock}
\end{frame}
...
\begin{itemize}
    \item \textbf{Use a performance simulator to evaluate a new feature / algorithm}
    \begin{itemize}
        \item Models the $\mu$Arch in great detail
        \item Run 100's of representative applications (benchmarks)
    \end{itemize}
    \vspace{0.3cm}
    \item \textbf{Produce a sorted performance distribution (``S-curve'')}
    \begin{itemize}
        \item For each application, measure performance change vs.\ baseline (no feature)
        \item Sort applications by performance change (worst $\rightarrow$ best)
        \item Plot: X = application percentile, Y = performance change (\%)
    \end{itemize}
    \vspace{0.3cm}
    \item \textbf{Why ``S-curve''?}
    \begin{itemize}
        \item Sorted results often form an S-shape: some apps hurt, most neutral, some benefit
        \item Easy to see: how many apps improve? How many regress? By how much?
    \end{itemize}
\end{itemize}
\end{frame}

% Slide 12b: S-curve Example - Poor Feature
\begin{frame}{S-Curve Example: Poor Feature}
\centering
\begin{tikzpicture}
    \begin{axis}[
        xlabel={Applications (sorted by performance change) [\%]},
    ...
    \addplot[blue, very thick, smooth] coordinates {
        (0,-3.5) (5,-3) (10,-2.5) (20,-1.5) (30,-0.8) (40,-0.2) (50,0.1)
        (60,0.3) (70,0.5) (80,0.8) (90,1.5) (95,2) (100,2.5)
    };
    % Shaded region for negative outliers (left end)
    \fill[red, opacity=0.15] (axis cs:0,-5) rectangle (axis cs:20,6);
    \node[red!70!black, anchor=west] at (axis cs:21,-3.5) {\small Significant regressions};
    % Shaded region for positive outliers (right end)
    \fill[green, opacity=0.15] (axis cs:80,-5) rectangle (axis cs:100,6);
    \node[green!50!black, anchor=east] at (axis cs:79,2.5) {\small Modest gains};
    \end{axis}
\end{tikzpicture}

\vspace{0.2cm}
\begin{center}
\fcolorbox{red!70!black}{red!10}{\parbox{0.95\textwidth}{\centering\textbf{Problem:} Many applications regress significantly ($>$2\%), while gains are modest.\\[2pt]\textbf{This feature should probably be rejected.}}}
\end{center}
\end{frame}

% Slide 12c: S-curve Example - Good Feature
\begin{frame}{S-Curve Example: Good Feature}
\centering
\begin{tikzpicture}
    \begin{axis}[
        xlabel={Applications (sorted by performance change) [\%]},
    ...
    \addplot[blue, very thick, smooth] coordinates {
        (0,-0.5) (5,-0.3) (10,-0.1) (20,0) (30,0.1) (40,0.2) (50,0.4)
        (60,0.7) (70,1.2) (80,2) (90,3.5) (95,4.5) (100,5.5)
    };
    % Shaded region for minor regressions (left end)
    \fill[red, opacity=0.15] (axis cs:0,-5) rectangle (axis cs:20,6);
    \node[red!70!black, anchor=west] at (axis cs:21,-1.5) {\small Minor regressions};
    % Shaded region for strong gains (right end)
    \fill[green, opacity=0.15] (axis cs:70,-5) rectangle (axis cs:100,6);
    \node[green!50!black, anchor=east] at (axis cs:69,5) {\small Strong gains};
    \end{axis}
\end{tikzpicture}

\vspace{0.2cm}
\begin{center}
\fcolorbox{green!60!black}{green!10}{\parbox{0.95\textwidth}{\centering\textbf{Success:} Minimal regressions ($<$1\%), significant gains for many applications.\\[2pt]\textbf{This feature is worth implementing.}}}
\end{center}
\end{frame}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Goal: minimize all three factors
    \end{itemize}
    \vspace{0.2cm}
    \item \textbf{Amdahl's Law}
    \begin{itemize}
        \item Speedup limited by the fraction that can be improved
        \item \emph{Make The Common Case Fast}
    \end{itemize}
    \vspace{0.2cm}
    \item \textbf{CISC vs RISC}
    \begin{itemize}
        \item CISC: complex instructions, variable length, few registers
        \item RISC: simple instructions, fixed length, many registers
        \item Modern CPUs: CISC ISA with RISC-like microarchitecture
    \end{itemize}
    \vspace{0.2cm}
    \item \textbf{Architecture vs Microarchitecture}
    \begin{itemize}
        \item Architecture: what software sees (ISA)
        \item Microarchitecture: how it's implemented
    \end{itemize}
\end{itemize}
\end{frame}
