...
\centering
\begin{tikzpicture}[
    box/.style={draw, thick, minimum width=3.6cm, minimum height=0.7cm, font=\small\bfseries},
    ...
    \node[box, fill=yellow!40, minimum height=0.9cm, align=center, below=0.4cm of sw] (isa) {Instruction Set\\Architecture};

    % Hardware layer
    \node[box, fill=green!15, below=0.4cm of isa] (hw) {Hardware};

    % Icons - positioned relative to boxes
    \node[icon, left=0.25cm of sw] (progicon) {%
        \includegraphics[height=0.85cm]{figures/noun-programmer-7899885.png}};
    \node[icon, left=0.25cm of isa] (compicon) {%
        \includegraphics[height=0.85cm]{figures/noun-compiler-5057301.png}};
    \node[icon, left=0.25cm of hw] (cpuicon) {%
        \includegraphics[height=0.85cm]{figures/noun-cpu-8157304.png}};

    % Labels - right-aligned using |- relative positioning
    \node[lbl] (proglbl) at ([xshift=-0.15cm]progicon.west) {Programmer};
    \node[lbl] at (proglbl.east |- compicon) {Compiler};
    \node[lbl] at (proglbl.east |- cpuicon) {CPU};

    % Arrows icon to box
    \draw[->, thick, blue!70] (progicon) -- (sw.west);
    \draw[->, thick, orange!80!black] (compicon) -- (isa.west);
    \draw[->, thick, green!50!black] (cpuicon) -- (hw.west);
