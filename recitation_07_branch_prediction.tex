...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[
      ...
      xorbox/.style={draw, circle, minimum size=0.8cm},
      arrow/.style={-latex, thick}
    ]
      % Branch IP
      \node (ip) at (0,0) {Branch IP};
      
      % History cache
      \node[bigbox, right=2cm of ip] (hist_cache) {};
      \node[above=0.1cm of hist_cache.north] {history cache};
      \node[box, above=0.2cm of hist_cache.south, anchor=south, minimum width=0.8cm] (tag) {tag};
      \node[box, below=0.05cm of tag.south, anchor=north, minimum height=1.2cm] (history) {history};
      
      % XOR operation
      \node[xorbox, below right=1.5cm and 1cm of hist_cache] (xor) {$\oplus$};
      
      % L.s.bits of IP
      \node[below=2cm of ip] (lsbits) {l.s.bits of IP};
      
      % 2-bit counter array
      \node[bigbox, right=2.5cm of xor, minimum width=3.5cm, minimum height=2cm] (counter_array) {};
      \node[above=0.1cm of counter_array.north] {2-bit-sat counter array};
      
      % Draw counter cells
      \foreach \x in {0,1,2,3,4} {
        \draw ([xshift=\x*0.7cm]counter_array.south west) rectangle ++(0.7cm, 0.4cm);
      }
      
      % Prediction output
      \node[right=1.5cm of counter_array] (prediction) {prediction = msb of counter};
      
      % Arrows
      \draw[arrow] (ip) -- (hist_cache);
      \draw[arrow] (history.south) |- node[pos=0.3, right] {h} (xor);
      \draw[arrow] (lsbits) -- (xor);
      \draw[arrow] (xor) -- (counter_array);
      \draw[arrow] (counter_array) -- (prediction);
    \end{tikzpicture}
  \end{center}
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[
      box/.style={draw, minimum height=1cm, minimum width=2cm},
      mux/.style={draw, trapezium, trapezium left angle=70, trapezium right angle=110, minimum height=1.5cm, minimum width=1cm, rotate=270},
      arrow/.style={-latex, thick}
    ]
      % Inputs
      \node[box, align=center] (bimodal) at (0,2) {Bimodal\\or\\Local};
      \node[box] (global) at (0,0) {Global};
      
      % MUX
      \node[mux, right=2cm of bimodal, yshift=-1cm] (mux) {MUX};
      
      % Chooser array
      \node[box, below=1.5cm of mux, minimum width=3cm, minimum height=1.5cm] (chooser) {};
      \node[above=0.05cm of chooser.north] {\small Chooser array};
      \node[below=0.05cm of chooser.south] {\tiny (array of 2-bit sat. counters)};
      
      % Branch IP input
      \node[left=2cm of chooser] (branch_ip) {Branch IP};
      
      % Prediction output
      \node[right=2cm of mux] (prediction) {Prediction};
      
      % Arrows
      \draw[arrow] (bimodal.east) -| ([yshift=0.3cm]mux.north);
      \draw[arrow] (global.east) -| ([yshift=-0.3cm]mux.north);
      \draw[arrow] (mux.south) -- (prediction);
      \draw[arrow] (branch_ip) -- (chooser);
      \draw[arrow] (chooser.north) -- (mux.east);
    \end{tikzpicture}
  \end{center}
  ...
  \begin{itemize}
    \item \textbf{+1} if Bimodal/Local correct and Global wrong
    \item \textbf{-1} if Bimodal/Local wrong and Global correct
    \item The chooser may also be indexed by the GHR
  \end{itemize}
\end{frame}
...
for (i=100; i>0; i--)
  for (j=2; j<6; j++)
    ...
    \begin{tikzpicture}[
      node distance=0.5cm,
      box/.style={draw, minimum width=1cm, minimum height=0.8cm}
    ]
      % Branch indices
      \node[box] (b0) at (0,0) {0};
      \node[box, below=of b0] (b1) {1};
      \node[box, below=of b1] (b2) {2};
      \node[box, below=of b2] (b3) {3};
      \node[box, below=of b3] (b4) {4};
      \node[box, below=of b4] (b5) {5};
      \node[box, below=of b5] (b6) {6};
      
      \node[left=0.5cm of b3] {Branch indices};
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{tabular}{|l|c|}
      ...
      \hline
      2-bit counter & 76.72\% \\
      Global history \& table & 77.60\% \\
      Local history, global table & \textbf{81.12\%} \\
      Local history \& tables & 80.72\% \\
      Global history, local tables & 81.60\% \\
      \hline
    \end{tabular}
    ...
    \begin{center}
      ...
      \begin{tikzpicture}[scale=0.8]
        ...
        \foreach \branch/\simple/\local in {
          ...
          \fill[blue!60] (\branch*1.2, 0) rectangle (\branch*1.2+0.4, \simple/20);
          % Local predictor bar
          \fill[red!60] (\branch*1.2+0.4, 0) rectangle (\branch*1.2+0.8, \local/20);
          % Branch label
          \node[below] at (\branch*1.2+0.4, 0) {\tiny Br\branch};
        }
        % Y-axis
        \draw[->] (-0.2, 0) -- (-0.2, 5);
        ...
        % Legend
        \fill[blue!60] (8, 4.5) rectangle (8.3, 4.7);
        \node[right] at (8.3, 4.6) {\tiny 2-bit};
        \fill[red!60] (8, 4) rectangle (8.3, 4.2);
        \node[right] at (8.3, 4.1) {\tiny Local};
      \end{tikzpicture}
    \end{center}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{center}
    \begin{tabular}{|l|c|c|c|}
      ...
      \hline
      All wrong & 0\% & 16,000,005 & baseline \\
      Bimodal & 76.72\% & 4,492,005 & 356.2\% \\
      Local & 80.72\% & 3,892,005 & 411.2\% \\
      Global hist + local count & 81.60\% & 3,760,005 & 425.5\% \\
      Perfect & 100\% & 1,000,005 & 1599.8\% \\
      \hline
    \end{tabular}
  \end{center}
