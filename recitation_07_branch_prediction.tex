...
@\colorbox{red!25}{for (i=100; i>0; i--)}@
  @\colorbox{green!25}{for (j=2; j<5; j++)}@
    ...
    \centering
    \textbf{Translation} \hfill \textcolor{blue}{\small Assembly}
    \vspace{0.5em}
...
\begin{frame}[fragile]
  ...
  \begin{columns}[T]
    ...
    \column{0.3\textwidth}
    \centering
    \textbf{Branch indices}\\[0.5em]
    \begin{tikzpicture}[
      box/.style={draw, minimum width=0.8cm, minimum height=0.6cm, font=\footnotesize}
    ]
      % Branch indices boxes
      \node[box] (b0) at (0,0) {0};
      \node[box] (b1) at (0,-0.8) {1};
      \node[box] (b2) at (0,-1.6) {2};
      \node[box] (b3) at (0,-2.4) {3};
      \node[box] (b4) at (0,-3.2) {4};
      \node[box] (b5) at (0,-4.0) {5};
      \node[box] (b6) at (0,-4.8) {6};
    \end{tikzpicture}
  \end{columns}
\end{frame}
