...
% IF Stage Components
\node[component, fill=yellow!30, minimum width=0.8cm] (PC) at (IMem.west) [anchor=east, xshift=-0.8cm] {PC};

% Instruction Memory using muxdemux
\node[memory_block, fill=yellow!20] (IMem) at (IFID.west) [anchor=east, xshift=-0.5cm] {};
...
% Control Unit
\node[control_block, ellipse, minimum height=0.8cm, minimum width=1.2cm] (Control) at (3, 2.5) [rotate=90] {Control};
...
\node[and port, anchor=in 1, fill=red!60] (HDU_AND) at (Control.east) [xshift=1cm] {};
...
% Forwarding Unit
\node[muxdemux, muxdemux def={Lh=3, NL=2, Rh=3, NR=2, w=3.5, square pins=1}, 
...
\draw[hazard_path] (HDU.east) -| ([yshift=0.3cm]HDU.east) |- (HDU_AND.in 1);
\draw[hazard_path] (HDU_AND.out) -| (Control.west);
