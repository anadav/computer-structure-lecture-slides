...
% Parameters: #1 = show forwarding (true/false), #2 = show HDU (true/false)
\newcommand{\pipelineprocessor}[2]{%
% Set the boolean flags based on parameters
\csname showforwarding#1\endcsname
\csname showhdu#2\endcsname
\scalebox{0.7}{%
\begin{circuitikz}[
...
\else
    ...
    \draw[control_path] ([xshift=5mm]Control.east |- EX1.west) node[control_connector] {} -- (EX1.west);
...
\centering
\pipelineprocessor{true}{true}  % Show forwarding and HDU
\end{frame}

\begin{frame}{Pipeline Processor with Hazard Detection (No Forwarding)}
\centering
\pipelineprocessor{false}{true}  % Show HDU only
\end{frame}

\begin{frame}{Basic Pipeline Processor}
\centering
\pipelineprocessor{false}{false}  % Show neither forwarding nor HDU
\end{frame}
