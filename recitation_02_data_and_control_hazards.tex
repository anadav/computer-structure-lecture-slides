...
\begin{center}
\begin{tikzpicture}[
    instr_box/.style={draw, thick, minimum width=3.5cm, minimum height=0.6cm, align=center, font=\ttfamily\small},
    stage_marker/.style={font=\tiny, text=gray}
]
    % Instructions positioned to align with stages
    \node[instr_box, fill=green!20] (add) at (0, 0) {ADD \$1 $\leftarrow$ \$2, \$3};
    \node[instr_box, fill=orange!20] (sub) at (0, -0.8) {SUB \$4 $\leftarrow$ \underline{\$1}, \$5};

    % Stage markers
    \node[stage_marker, left=0.5cm of add] {EX:};
    \node[stage_marker, left=0.5cm of sub] {ID:};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tabular}{ll}
\texttt{ADD \$1 $\leftarrow$ \$2, \$3} & \textit{(produces \$1)} \\
\texttt{SUB \$4 $\leftarrow$ \$1, \$5} & \textit{(needs \$1)} \\
\end{tabular}
\end{center}
