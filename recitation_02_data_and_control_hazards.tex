...
\node[regfile, right=1.3cm of IFID] (RegFile) {Register\\File};
% Add labels to RegFile pins

% Control Unit
\node[control_block, ellipse, minimum height=0.5cm, minimum width=0.5cm,
      above=of RegFile] (Control) {\rotatebox{90}{Control}};

% Hazard Detection Unit
\node[hazard_unit, above=of Control.north west] (HDU) {HDU};
...
\node[mux4, fill=cyan!20, anchor=blpin 2] (RegDstMux) at (ForwardB.blpin 2 |- Rd_field.east) {};
\node[data_latch] (Rt_field) at (RegDstMux.lpin 1 -| Rd_field) {\tiny Rt};
...
\draw[data_path] (IFID.east) -- ++(0.3,0) |- node[pos=0.9, above, font=\tiny] {Rs} (RegFile.blpin 1);
\draw[data_path] (IFID.east) -- ++(0.3,0) |- node[pos=0.9, above, font=\tiny] {Rt} (RegFile.blpin 2);
\draw[data_path] (IFID.east) -- ++(0.3,0) |- node[pos=0.8, above, font=\tiny] {Rd} (Rd_field.west);
\draw[data_path] ([xshift=-7mm]RegFile.blpin 1) node[circle, fill, inner sep=1.2pt] {}
                    |- node[pos=0.8, above, font=\tiny] {Rs} (Rs_field.west);  
\draw[data_path] ([xshift=-8mm]RegFile.blpin 2) node[circle, fill, inner sep=1.2pt] {}
                    |- node[pos=0.8, above, font=\tiny] {Rt} (Rt_field.west);  
...
\draw[data_path] (EXMEM_RtRd.east) -- node[near end, above, font=\tiny] {RegDest} (MEMWB_RtRd.west);
\draw[data_path]
  (MEMWB_RtRd.east) -- ++(0.5,0)
  |- ([yshift=-7mm]MEMWB.south)
  -| node[midway, above right, font=\tiny]{RegDst} ([xshift=-6mm]RegFile.blpin 3)
  -- node[midway, above, font=\tiny]{RegDst} % or pos=0.9, etc.
     (RegFile.blpin 3);
...
\draw[data_path]
  (WBMux.brpin 1) -- ++(0.5,0)
  |- ([yshift=-6mm]MEMWB.south)
  -| node[midway, above right, font=\tiny]{WrData} ([xshift=-5mm]RegFile.blpin 4)
  -- node[midway, above, font=\tiny]{WrData} % or pos=0.9, etc.
     (RegFile.blpin 4);
...
\draw[control_path] (Control.east) -- ++(0.1, 0) |- (HDU_AND.bin 2);
\draw[control_path] (WB1.east) -- (WB2.west);
\draw[control_path] (M1.east) -- (M2.west);
\draw[control_path] (WB2.east) -- (WB3.west);
\draw[control_path]
    (WB3.east) -- ++(0.2, 0)
    |- node[pos=0.1, right, font=\tiny] {RegWrite} ([yshift=-3mm]MEMWB.south)
    -| node[midway, above right, font=\tiny] {RegWrite} ([xshift=-4mm]RegFile.blpin 5)
    -- (RegFile.blpin 5);
