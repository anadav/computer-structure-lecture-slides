...
% Parameters: #1 = show forwarding (true/false), #2 = show HDU (true/false), #3 = show stage labels (true/false)
...
\ifshowstagelabels
    ...
    \node[stage_label] (ID_label) at ($ (IFID.north)!0.5!(IDEX.north) + (0,3mm) $) {ID};

    % IF stage label - align with ID label Y-position
    \node[stage_label] (IF_label) at (PC.north |- ID_label) {IF};

    % EX stage label - use midpoint between IDEX and EXMEM
    \node[stage_label] (EX_label) at ($ (IDEX.north)!0.5!(EXMEM.north) + (0,3mm) $) {EX};

    % MEM stage label - use midpoint between EXMEM and MEMWB
    \node[stage_label] (MEM_label) at ($ (EXMEM.north)!0.5!(MEMWB.north) + (0,3mm) $) {MEM};

    % WB stage label - align with ID label Y-position
    \node[stage_label] (WB_label) at (WBMux.north |- ID_label) {WB};
\fi
...
\begin{frame}{RAW Data Hazard - Read After Write}
\vspace{-0.3cm}
\centering
\scalebox{0.85}{
\begin{tikzpicture}
    % Draw the pipeline processor and capture stage label positions
    \node[inner sep=0] (pipeline) {\pipelineprocessor[true]{false}{false}};

    % Define approximate X positions for stages (adjust based on your pipeline width)
    % These coordinates are relative to the pipeline node
    \coordinate (EX_pos) at ($(pipeline.south) + (0.5, -0.8)$);
    \coordinate (ID_pos) at ($(pipeline.south) + (-2.5, -0.8)$);

    % Instruction boxes positioned below pipeline, aligned with stages
    \node[draw, thick, minimum width=3.5cm, minimum height=0.6cm, align=center,
          font=\ttfamily\small, fill=green!20, anchor=west] (add_instr)
          at ($(EX_pos) + (-1.75, 0)$) {ADD \$1 $\leftarrow$ \$2, \$3};

    \node[draw, thick, minimum width=3.5cm, minimum height=0.6cm, align=center,
          font=\ttfamily\small, fill=orange!20, anchor=west] (sub_instr)
          at ($(ID_pos) + (-1.75, 0)$) {SUB \$4 $\leftarrow$ \underline{\$1}, \$5};

    % Stage labels for instructions
    \node[font=\small\bfseries, left=0.2cm of add_instr] {EX:};
    \node[font=\small\bfseries, left=0.2cm of sub_instr] {ID:};
\end{tikzpicture}
}
...
\begin{frame}{Understanding RAW Data Hazards}
...
\begin{frame}{Pipeline Processor with Hazard Detection (No Forwarding)}
\centering
\pipelineprocessor[false]{false}{true}  % No stage labels, no forwarding, show HDU
\end{frame}

\begin{frame}{Pipeline Processor with Hazard Detection and Forwarding}
\centering
