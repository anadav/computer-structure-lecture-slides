...
\begin{tikzpicture}[scale=1]
    % BTB table structure first
    \coordinate (table-top-left) at (0, 4);
    \coordinate (table-bottom-left) at (0, 0);
    \coordinate (table-top-right) at (7.5, 4);
    \coordinate (table-bottom-right) at (7.5, 0);
    
    % Draw table borders
    \draw[thick] (table-top-left) -- (table-bottom-left);
    \draw[thick] (table-top-right) -- (table-bottom-right);
    
    % Table rows and column separators
    ...
    % Top line of table
    \draw (table-top-left) -- (table-top-right);
    
    % Define column centers relative to table
    \coordinate (col1-center) at ($ (table-top-left)!0.5!(3, 4) $);
    \coordinate (col2-center) at ($ (3, 4)!0.5!(6, 4) $);
    \coordinate (col3-center) at ($ (6, 4)!0.5!(table-top-right) $);
    
    % PC input positioned relative to table
    \node[draw, rectangle] (pc) at ($ (col1-center) + (0, 1) $) {PC of instruction to fetch};
    
    % Table headers positioned relative to column centers
    \node[above=0.2cm of col1-center] {Look up};
    \node[above=0.2cm of col2-center] {Predicted PC};
    
    % BTB label positioned relative to table left
    \node[rotate=90, left=0.7cm of $ (table-top-left)!0.5!(table-bottom-left) $] {Number of entries in BTB};
    
    % Comparator positioned relative to first column bottom
    \node[circle, draw, inner sep=2pt] (comp) at ($ (col1-center) + (0, -0.5) $) {=};
    
    % Arrows using relative positioning
    \draw[->] (pc) -- ($ (col1-center) + (0, 0.5) $);
    \draw[->] ($ (col1-center) + (0, 0) $) -- (comp);
    ...
    \draw[->] ($ (col3-center) + (0, 0) $) -- ++(0, -1) node[below, align=center] {Branch prediction\\Taken or not taken};
\end{tikzpicture}
\end{center}
