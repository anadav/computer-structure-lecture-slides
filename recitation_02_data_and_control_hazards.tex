...
\begin{tikzpicture}[scale=0.8, node distance=2.5cm]
    \node[state, fill=blue!20] (nt) {Not Taken};
    \node[state, fill=green!20, right of=nt] (t) {Taken};
    
    \path[->] 
    (nt) edge[bend left, above] node{taken} (t)
    (t) edge[bend left, below] node{not taken} (nt)
    (nt) edge[loop left] node{not taken} (nt)
    (t) edge[loop right] node{taken} (t);
\end{tikzpicture}
\end{columns}
...
\begin{tikzpicture}[scale=1, node distance=3cm, auto]
    % Define states
    \node[state, fill=red!20, align=center] (00) {00\\Not\\Taken};
    \node[state, fill=orange!20, right of=00, align=center] (01) {01\\Not\\Taken};
    \node[state, fill=yellow!20, right of=01, align=center] (10) {10\\Taken};
    \node[state, fill=green!20, right of=10, align=center] (11) {11\\Taken};
    
    % Add transitions
    \path[->] 
    % From 00
    (00) edge[loop below] node{NT} (00)
    (00) edge[above] node{T} (01)
    % From 01
    (01) edge[below, bend left] node{NT} (00)
    (01) edge[above] node{T} (10)
    % From 10
    (10) edge[below] node{NT} (01)
    (10) edge[above] node{T} (11)
    % From 11
    (11) edge[below] node{NT} (10)
    (11) edge[loop below] node{T} (11);
    
    % Add direction arrows
    \draw[thick, ->, blue] (0, -1.5) -- (12, -1.5) node[right] {Taken};
    \draw[thick, ->, red] (0, -2) -- (12, -2) node[right] {Not Taken};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.7, node distance=2.5cm, auto]
    ...
    \path[->] 
    (00) edge[loop below] node{\tiny NT} (00)
    (00) edge[above] node{\tiny T} (01)
    (01) edge[below, bend left] node{\tiny NT} (00)
    (01) edge[above] node{\tiny T} (10)
    (10) edge[below] node{\tiny NT} (01)
    (10) edge[above] node{\tiny T} (11)
    (11) edge[below] node{\tiny NT} (10)
    (11) edge[loop below] node{\tiny T} (11);
\end{tikzpicture}
\end{columns}
\end{frame}
