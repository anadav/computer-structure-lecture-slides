...
\begin{frame}{Solution: Stalling to Resolve Data Hazard}
\centering
\pipelineprocessor[true]{false}{false}
...
\begin{frame}{Forwarding Unit: Inputs \& Outputs}
\textbf{Inputs:}
\begin{itemize}
...
\begin{frame}{Forwarding Unit: Logic}
\textbf{ForwardA Logic} (similar for ForwardB with Rt):

\vspace{0.3cm}
\textbf{Priority 1 - MEM $\rightarrow$ EX Forwarding:}
\begin{itemize}
    \item If (EX/MEM.RegWrite = 1) AND (EX/MEM.Rd $\neq$ 0) AND (EX/MEM.Rd = ID/EX.Rs)
    \item Then \textbf{ForwardA = 10}
\end{itemize}

\vspace{0.3cm}
\textbf{Priority 2 - WB $\rightarrow$ EX Forwarding:}
\begin{itemize}
    \item Else if (MEM/WB.RegWrite = 1) AND (MEM/WB.Rd $\neq$ 0) AND (MEM/WB.Rd = ID/EX.Rs)
    \item Then \textbf{ForwardA = 01}
\end{itemize}

\vspace{0.3cm}
\textbf{No Forwarding:}
\begin{itemize}
    \item Else \textbf{ForwardA = 00}
\end{itemize}

\vspace{0.3cm}
\textbf{Note:} MEM $\rightarrow$ EX has priority over WB $\rightarrow$ EX (newer data)
\end{frame}
