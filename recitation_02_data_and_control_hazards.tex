...
\begin{tikzpicture}[remember picture, overlay,
    instr_box/.style={draw, thick, minimum height=0.5cm, align=center, font=\ttfamily\footnotesize}
]
    % Instruction boxes aligned with stage labels from the pipeline
    \node[instr_box, fill=orange!20, minimum width=2.8cm]
          at ($(ID_label) + (0, -6.5)$) {SUB \$4$\leftarrow$\textcolor{red}{\$1},\$5};

    \node[instr_box, fill=green!20, minimum width=2.8cm]
...
\begin{frame}{Stalling to Resolve Data Hazard}
\centering
\pipelineprocessor[true]{false}{false}
...
\begin{tikzpicture}[remember picture, overlay,
    ...
    % Instruction boxes aligned with stage labels from the pipeline
    \node[instr_box, fill=orange!20, minimum width=2.8cm] (sub_instr)
          at ($(ID_label) + (0, -6.5)$) {SUB \$4$\leftarrow$\textcolor{red}{\$1},\$5};
    ...
    \node[instr_box, fill=green!20, minimum width=2.8cm, anchor=east] (add_instr)
...
\begin{frame}{Forwarding: MEM to EX}
\centering
...
\begin{tikzpicture}[remember picture, overlay,
    ...
    % Blue forwarding line from ADD to SUB (MEM to EX)
    \draw[->, very thick, blue] ($(MEM_label) + (0, -6.2)$) -- ($(EX_label) + (0, -6.2)$);
...
\begin{frame}{Forwarding: WB to EX}
\centering
\pipelineprocessor[true]{true}{false}
...
\begin{tikzpicture}[remember picture, overlay,
    ...
    \node[instr_box, fill=green!20, minimum width=2.8cm, anchor=east]
          at ($(current page.east |- EX_label) + (-0.3, -6.5)$) {ADD \textcolor{red}{\$1}$\leftarrow$\$2,\$3};

    \node[instr_box, fill=orange!20, minimum width=2.8cm]
          at ($(EX_label) + (0, -6.5)$) {SUB \$4$\leftarrow$\textcolor{red}{\$1},\$5};

    % Green forwarding line from ADD to SUB (WB to EX)
    \draw[->, very thick, green] ($(WB_label) + (0, -6.2)$) -- ($(EX_label) + (0, -6.2)$);
...
\begin{center}
  ...
  \begin{tikzpicture}[
    ...
    \draw[arrow] ([xshift=2mm]L3.east |- dmem.lpin 2) |- (L4.west |- wb_mux.lpin 2);
