...
\begin{tikzpicture}[
    ...
    pipeline_reg/.style={draw, thick, fill=gray!20, minimum width=0.3cm, minimum height=3.5cm},
    stage_label/.style={draw, thick, fill=blue!60, text=white, font=\bfseries, minimum width=0.8cm, minimum height=0.5cm},
    control_block/.style={draw, thick, fill=orange!30, rounded corners=3pt},
    hazard_unit/.style={muxdemux, muxdemux def={Lh=2, NL=2, Rh=3, NR=3, NB=2, w=2, square pins=1}, 
                       external pins width=0, scale=0.5, fill=red!60, text=white, font=\bfseries},
    memory_block/.style={muxdemux, muxdemux def={Lh=4, NL=2, Rh=4, NR=1, w=3.2, square pins=1}, 
                        external pins width=0, scale=0.5},
    regfile/.style={muxdemux, muxdemux def={Lh=5, NL=3, Rh=5, NR=2, NB=1, w=3.2, square pins=1}, 
                    external pins width=0, scale=0.5, fill=cyan!20},
    alu_style/.style={muxdemux, muxdemux def={Lh=4, NL=2, Rh=2, NR=1, w=2, inset w=1, inset Lh=2, inset Rh=0, square pins=1},
                     external pins width=0, scale=0.5, fill=green!20},
    mux2/.style={muxdemux, muxdemux def={Lh=3, Rh=1.5, NL=2, NR=1, NB=1, w=0.8}, 
                 external pins width=0, scale=0.5},
    mux3/.style={muxdemux, muxdemux def={Lh=4, Rh=2, NL=3, NR=1, NB=1, w=1}, 
                 external pins width=0, scale=0.5},
    arrow/.style={->, >=stealth, thick},
    data_path/.style={arrow, blue!70},
...
\node[stage_label] (ID_label) at (3, 4) {ID};
\node[stage_label] (EX_label) at (6.5, 4) {EX};
\node[stage_label] (MEM_label) at (10, 4) {MEM};
\node[stage_label] (WB_label) at (13, 4) {WB};

% Pipeline registers
\node[pipeline_reg] (IFID) at (1.5, 0) {};
\node[pipeline_reg] (IDEX) at (4.5, 0) {};
\node[pipeline_reg] (EXMEM) at (8, 0) {};
\node[pipeline_reg] (MEMWB) at (11.5, 0) {};
...
% IF Stage Components
\node[component, fill=yellow!30, minimum width=0.8cm] (PC) at (-0.5, 0) {PC};

% Instruction Memory using muxdemux
\node[memory_block, fill=yellow!20] (IMem) at (0.5, 1.5) {};
\node[font=\small, align=center] at (IMem.center) {Instruction\\Memory};

% ID Stage Components
% Register File using muxdemux style
\node[regfile] (RegFile) at (3, 0.5) {};
\node[font=\small, align=center] at (RegFile.center) {Register\\File};

% Control Unit
\node[control_block, ellipse, minimum width=1.2cm, minimum height=0.8cm] (Control) at (3, 2.5) {Control};

% Hazard Detection Unit
\node[hazard_unit] (HDU) at (Control.east) [anchor=south east, yshift=0.5cm] {};
\node[font=\small, text=white] at (HDU.center) {HDU};

% EX Stage Components
% ALU using muxdemux style
\node[alu_style] (ALU) at (6.5, 0.5) {};
\node[rotate=90, font=\small] at (ALU.center) {ALU};

% MUXes in EX stage
\node[mux2, fill=orange!20] (ALUSrcMux) at (5.5, -0.5) {};
\node[mux3, fill=orange!20] (ForwardA) at (5.5, 1.2) {};
\node[mux3, fill=orange!20] (ForwardB) at (5.5, 0.2) {};

% Forwarding Unit
\node[control_block, minimum width=1.8cm, minimum height=0.8cm, align=center] (FwdUnit) at (6.5, -2) {Forwarding\\Unit};

% MEM Stage Components
% Data Memory using muxdemux
\node[memory_block, fill=yellow!20] (DMem) at (10, 0.5) {};
\node[font=\small, align=center] at (DMem.center) {Data\\Memory};

% WB Stage Components
\node[mux2, fill=blue!20] (WBMux) at (12.5, 0) {};

% RegDst MUX (between ID and EX)
\node[mux2, fill=blue!20] (RegDstMux) at (4, -1) {};

% Control signal blocks in pipeline registers
\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (WB1) at ([yshift=1.2cm]IDEX.center) {\tiny WB};
\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (M1) at ([yshift=0.6cm]IDEX.center) {\tiny M};
\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (EX1) at (IDEX.center) {\tiny EX};

\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (WB2) at ([yshift=1.2cm]EXMEM.center) {\tiny WB};
\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (M2) at ([yshift=0.6cm]EXMEM.center) {\tiny M};

\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (WB3) at ([yshift=1.2cm]MEMWB.center) {\tiny WB};

% Main data paths
% PC to Instruction Memory
\draw[data_path] (PC.east) -- (IMem.west |- PC.east);
\draw[data_path] (IMem.east |- PC.east) -- (IFID.west |- PC.east);

% IF/ID to Register File
\draw[data_path] ([yshift=0.3cm]IFID.east) -- ++(0.3,0) |- ([yshift=0.6cm]RegFile.west);
\draw[data_path] ([yshift=0cm]IFID.east) -- ++(0.3,0) |- ([yshift=0cm]RegFile.west);

% Register File outputs to ID/EX
\draw[data_path] ([yshift=0.6cm]RegFile.east) -- ([yshift=0.6cm]IDEX.west);
\draw[data_path] ([yshift=0cm]RegFile.east) -- ([yshift=0cm]IDEX.west);
...
\draw[data_path] (ForwardA.east) -- ([yshift=0.6cm]ALU.west);
\draw[data_path] (ForwardB.east) -- (ALUSrcMux.north);
\draw[data_path] (ALUSrcMux.east) -- ([yshift=-0.3cm]ALU.west);
...
% MEM/WB to WB Mux
\draw[data_path] ([yshift=0.3cm]MEMWB.east) -- ([yshift=0.3cm]WBMux.west);
\draw[data_path] ([yshift=-0.3cm]MEMWB.east) -- ([yshift=-0.3cm]WBMux.west);
...
% Control paths
\draw[control_path] (Control.south) -- (IDEX.north |- Control.south);
...
\draw[hazard_path] ([yshift=0.3cm]HDU.south) -| (IFID.south);
\draw[hazard_path] (HDU.east) -- (Control.east);

% Forwarding paths
\draw[forward_path] (EXMEM.south) -- ++(0,-0.3) -| (FwdUnit.east);
\draw[forward_path] (MEMWB.south) -- ++(0,-0.5) -| ([xshift=0.3cm]FwdUnit.east);
\draw[forward_path] (FwdUnit.north) -- (ForwardA.south);
\draw[forward_path] (FwdUnit.north) -- (ForwardB.south);
