...
\begin{tikzpicture}[
    ...
    alu_style/.style={muxdemux, muxdemux def={Lh=8, NL=2, Rh=4, NR=1, w=2.5, inset w=1, inset Lh=4, inset Rh=0, square pins=1},
...
\node[memory_block, fill=yellow!20, align=center] (IMem) at (IFID.west) [anchor=east, xshift=-0.5cm] {Instruction\\Memory};
...
\node[regfile, align=center] (RegFile) at (3, 0.5) {Register\\File};

% Control Unit
\node[control_block, ellipse, minimum height=0.8cm, minimum width=1.2cm] (Control) at (3, 2.5) [rotate=90] {Control};

% Hazard Detection Unit
\node[hazard_unit] (HDU) at (Control.east) [anchor=south east, yshift=0.5cm] {HDU};

% AND gate for HDU control
\node[and port, anchor=in 1, fill=red!60, scale=0.5, external pins width=0] (HDU_AND) at (Control.east) [xshift=1cm] {};

% EX Stage Components
% ALU using muxdemux style
\node[alu_style] (ALU) at (6.5, 0.5) {\rotatebox{90}{ALU}};

% MUXes in EX stage
\node[mux2, fill=orange!20] (ForwardA) at (ALU.blpin 1) [anchor=brpin 1, xshift=-1cm] {};
\node[mux2, fill=orange!20] (ForwardB) at (ALU.blpin 2) [anchor=brpin 1, xshift=-1cm] {};
\node[mux2, fill=blue!20] (RegDstMux) at (5.5, -0.8) {};

% Forwarding Unit
\node[muxdemux, muxdemux def={Lh=3, NL=2, Rh=3, NR=2, w=3.5, square pins=1}, 
      external pins width=0, scale=0.5, fill=orange!30, align=center] (FwdUnit) at (6.5, -2) {Forwarding\\Unit};

% MEM Stage Components
% Data Memory using muxdemux
\node[memory_block, fill=yellow!20, align=center] (DMem) at (10, 0.5) {Data\\Memory};
...
\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (WB1) at ([yshift=1.8cm]IDEX.center) {\tiny WB};
\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (M1) at ([yshift=1.2cm]IDEX.center) {\tiny M};
\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (EX1) at ([yshift=0.6cm]IDEX.center) {\tiny EX};

\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (WB2) at ([yshift=1.8cm]EXMEM.center) {\tiny WB};
\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (M2) at ([yshift=1.2cm]EXMEM.center) {\tiny M};

\node[control_block, minimum width=0.5cm, minimum height=0.3cm] (WB3) at ([yshift=1.8cm]MEMWB.center) {\tiny WB};
...
\draw[hazard_path, line width=0pt] (HDU.brpin 3) |- (HDU_AND.in 1);
