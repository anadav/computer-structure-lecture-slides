...
\begin{tikzpicture}[scale=1]
    ...
    \matrix[matrix of nodes,
            ...
            column 1/.append style={nodes={text width=2.4cm, minimum height=0.3cm}},
            column 2/.append style={nodes={text width=2.4cm, minimum height=0.3cm}},
            column 3/.append style={nodes={text width=1.5cm, minimum height=0.3cm}},
    ...
    \node[draw, rectangle, above=1cm of btb-1-1.north, xshift=-10mm, font=\scriptsize] (pc) {PC of instruction to fetch};
    ...
    \node[rotate=90, left=6mm of btb.north west, font=\tiny, anchor=north east] {$\leftarrow$ \# of BTB entries $\rightarrow$};
    
    % Comparator positioned relative to first column bottom
    \node[circle, draw, inner sep=2pt] (comp) at ($ (btb-8-1.south) + (0, -0.5) $) {=};
    
    % Arrows using relative positioning
    \draw[->] (pc) -- (btb-1-1.north -| pc);
    \draw[->] ([xshift=3mm]pc.south west) |- (comp.west);
    \draw[->] (btb-8-1.south) -- (comp);

    \node[right=1cm of comp, font=\scriptsize, align=left, text=red] (no) {instruction is not\\predicted to be a branch.\\ $\Rightarrow$ Proceed normally.};

    % Output logic positioned relative to comparator
    \draw[->] (comp.east) -- node[above, font=\scriptsize]{no} (no);

    \node[below=5mm of comp, font=\scriptsize, align=left, text=green] (yes) {Instruction is predicted to be a branch.\\ $\Rightarrow$ Should be used as next PC.};
    \draw[->] (comp.south) -- node[left, font=\scriptsize]{yes} (yes);
    
    % Prediction output from third column
    \draw[->] ([xshift=4mm]btb-8-3.south) -- ++(0, -1) node[below, align=center, font=\scriptsize] {Branch prediction\\Taken or not taken};
\end{tikzpicture}
\end{center}
