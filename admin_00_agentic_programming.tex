...
Install Claude Code (macOS/Linux/WSL --- see \url{https://code.claude.com/docs/en/setup}):
...
\begin{minted}[fontsize=\scriptsize,bgcolor=lightgray,escapeinside=||]{bash}
export ANTHROPIC_AUTH_TOKEN="sk-or-v1-..."|\tikzmark{apikey}|
...
\node[rectangle callout, draw=red, fill=calloutbg, text width=2.8cm, font=\small, align=center,
      callout absolute pointer={(pic cs:apikey)}] at (0,0) {
...
\begin{itemize}
    \item Open terminal: \colorbox{keybg}{\texttt{Ctrl+Shift+`}} (or \colorbox{keybg}{\texttt{Cmd+Shift+`}} on Mac)
...
\begin{enumerate}
    ...
    \item Use \colorbox{keybg}{\texttt{Cmd+Esc}} (Mac) or \colorbox{keybg}{\texttt{Ctrl+Esc}} to open Claude Code
\end{enumerate}
