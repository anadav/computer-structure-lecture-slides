    ...
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \begin{tikzpicture}
                ...
                % Callouts (shown on overlay 2)
                \visible<2->{
                    \node[draw, fill=blue!10, font=\tiny, align=left, rounded corners,
                          above=0.5cm of m-1-4] (gnote) {g = guest};

                    \node[draw, fill=orange!10, font=\tiny, align=left, rounded corners,
                          left=0.1cm of m-3-2.west] (enote) {e = EPT};

                    \node[draw, fill=green!10, font=\tiny, align=center, rounded corners,
                          below=0.2cm of m-6-2] (hpanote) {reads gPML4\\content};
                    \draw[->, thin] (hpanote.north) -- (m-6-2.south);
                }
            \end{tikzpicture}
        \end{column}
        \begin{column}{0.4\textwidth}
            ...
            \begin{tcolorbox}[colback=blue!10, fontupper=\small, width=\textwidth-0.5cm]
                \textbf{Native:} 4 + 1 accesses\\
                {\scriptsize 4 page walk + data}
            \end{tcolorbox}

            \vspace{0.1cm}
            \begin{tcolorbox}[colback=orange!15, fontupper=\small, width=\textwidth-0.5cm]
                \textbf{With EPT:} \textbf{20} + 1\\
                {\scriptsize 5 $\times$ 4 EPT + access}
            \end{tcolorbox}
