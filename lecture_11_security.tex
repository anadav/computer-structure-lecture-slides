...
\begin{frame}{SMEP - Supervisor Mode Execution Prevention}
    \begin{columns}
        ...
        \begin{column}{0.45\textwidth}
            \begin{tikzpicture}[
                space/.style={draw, thick, minimum width=3.2cm, minimum height=2.2cm,
                            align=center, font=\small, rounded corners=2pt},
                ...
                \node[space, fill=cyan!10, below=1cm of kernel.south, minimum height=2.5cm] (user)
                    {\textbf{User Space}};
                \node[label, below=2mm of user.north] (cpl3) {CPL = 3};
                \node[label, below=1mm of cpl3] (u1) {U = 1};
                \node[font=\small, below=0mm of u1, fill=red!20, rounded corners=2pt,
...
\begin{frame}{ARM MTE - How It Works}
    \begin{columns}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}[scale=0.75,
                ...
                % Root
                \node[draw,fill=red!30,circle] (root) {Root};
                \node[above=0mm of root] {\tiny On-chip};
                \node[right=1mm of root, font=\tiny] {= hash(H1,H2)};

                % Level 1 - below root
                \node[draw,fill=yellow!30,circle, below left=\leveldist and \siblingdist of root] (n1) {H1};
                \node[right=1mm of n1, font=\tiny] {= hash(H3,H4)};
                \node[draw,fill=yellow!30,circle, below right=\leveldist and \siblingdist of root] (n2) {H2};

                % Level 2 - below level 1
                \node[draw,fill=green!30,circle, below left=\leveldist and 0.5cm of n1] (n3) {H3};
                \node[draw,fill=green!30,circle, below right=\leveldist and 0.5cm of n1] (n4) {H4};
                \node[draw,fill=green!30,circle, below left=\leveldist and 0.5cm of n2] (n5) {H5};
                \node[draw,fill=green!30,circle, below right=\leveldist and 0.5cm of n2] (n6) {H6};

                % Data blocks - below level 2 (one per H node for compactness)
                \node[draw,fill=blue!20, below=0.7cm of n3] (d1) {\tiny Data};
                \node[draw,fill=blue!20, below=0.7cm of n4] (d3) {\tiny Data};
                \node[draw,fill=blue!20, below=0.7cm of n5] (d4) {\tiny Data};
                \node[draw,fill=blue!20, below=0.7cm of n6] (d6) {\tiny Data};
                ...
                % Attack
                \node[red] at (d3) {X};
                \draw[->,thick,red] ([yshift=-0.5cm]d3.south) -- (d3.south);
                \node[red, below=0.6cm of d3] {\tiny Replay};

                % Detection path
                \draw[ultra thick,red] (d3) -- (n4);
                \draw[ultra thick,red] (n4) -- (n1);
                \draw[ultra thick,red] (n1) -- (root);
                \node[red, left=0.5cm of n1] {\tiny Detected!};
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[
                ...
                \draw[<-, thick, blue!70] (enclave.east) -- ++(1cm, 0)
                    node[right, font=\scriptsize] {EPC Memory};
            \end{tikzpicture}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.55\textwidth}
            \begin{tikzpicture}[
                ...
                \node[chainlink, fill=purple!20, below=0.3cm of kernel] (ima) {IMA};
                \node[font=\tiny, below=-1mm of ima] {(Runtime)};

                % Keys - right side (using key icon) - signs the next stage
                \node[right=1cm of hw] (pk) {\includegraphics[height=0.4cm]{figures/noun-key-8095348.png}};
                \node[font=\tiny, right=-1mm of pk] {PK};
                \node[right=1cm of uefi] (kek) {\includegraphics[height=0.4cm]{figures/noun-key-8095348.png}};
                \node[font=\tiny, right=-1mm of kek] {KEK};
                \node[right=1cm of bootloader] (db) {\includegraphics[height=0.4cm]{figures/noun-key-8095348.png}};
                \node[font=\tiny, right=-1mm of db] {db};
                ...
                % Key usage (dashed to chain) - keys sign the next stage
                \draw[dashed, <-, gray] (pk.west) -- ++(-0.6cm,0) |- (uefi.east);
                \draw[dashed, <-, gray] (kek.west) -- ++(-0.6cm,0) |- (bootloader.east);
                \draw[dashed, <-, gray] (db.west) -- ++(-0.6cm,0) |- (kernel.east);

                % Legend for key icon
                \node[font=\tiny, below=0.5cm of db, xshift=0.3cm] (keylegend) {\includegraphics[height=0.25cm]{figures/noun-key-8095348.png} = Signed by};

                % TPM measurement arrows (blue dashed) to specific PCRs - staircase routing
                \draw[measure] (uefi.east) -| ([xshift=-23mm]pcrs-1-1.west) -- (pcrs-1-1.west);
                \draw[measure] (bootloader.east) -| ([xshift=-21mm]pcrs-2-1.west) -- (pcrs-2-1.west);
                \draw[measure] (kernel.east) -| ([xshift=-19mm]pcrs-3-1.west) -- (pcrs-3-1.west);
                \draw[measure] (ima.east) -| ([xshift=-17mm]pcrs-5-1.west) -- (pcrs-5-1.west);
