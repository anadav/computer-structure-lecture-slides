    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[
                ...
                vm/.style={box, minimum height=1.8cm, minimum width=1.5cm, rounded corners=3pt},
                connection/.style={<->, thick, blue!70!black},
                label/.style={font=\scriptsize, text=blue!70!black}]

                % Hardware (bottom layer)
                \node[box, fill=black!20, minimum width=5.2cm] (hw) {\textbf{Hardware}};

                % Hypervisor (middle layer) - privileged mode
                \node[box, fill=orange!30, minimum width=5.2cm, above=0.6cm of hw] (vmm)
                    {\textbf{Hypervisor (VMM)}};

                % VMs (top layer) - isolated guests
                \node[vm, fill=blue!30, above left=0.8cm and 0.3cm of vmm.north] (vm1)
                    {\textbf{VM1}\\[-2pt]\small Guest OS};
                \node[vm, fill=green!30, above right=0.8cm and 0.3cm of vmm.north] (vm2)
                    {\textbf{VM2}\\[-2pt]\small Guest OS};

                % Privilege boundary with labels
                ...
                % Mode labels on the boundary
                \node[font=\scriptsize, text=blue!70, fill=white, inner sep=2pt,
                      above=2pt of boundary-left, anchor=south west] {Guest};
                \node[font=\scriptsize, text=red!70, fill=white, inner sep=2pt,
                      below=2pt of boundary-right, anchor=north east] {VMM};

                % EPT connections
                \draw[connection] (vm1.south) -- (vm1.south |- vmm.north)
                    node[midway, label, left, xshift=-1pt] {EPT};
                \draw[connection] (vm2.south) -- (vm2.south |- vmm.north)
                    node[midway, label, right, xshift=1pt] {EPT};
