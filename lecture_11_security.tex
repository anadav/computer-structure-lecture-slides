    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}[font=\small,
                ...
                % CPU enforcement (top)
                \node[cpu] (cpu) at (0,0) {CPU (TDX Module)};

                % Protected VMs - explicit positioning
                \node[vm, fill=green!30, draw=green!70!black] (vm1) at (-1,-1.4) {
                    \textbf{VM1}\\[-2pt]{\tiny Customer A}};
                \node[vm, fill=blue!30, draw=blue!70!black] (vm2) at (1,-1.4) {
                    \textbf{VM2}\\[-2pt]{\tiny Customer B}};

                % Protection barrier
                \draw[barrier] (-2.2,-2.2) -- (2.2,-2.2)
                    node[right, font=\small] {TDX};

                % Threat Actor (bottom)
                \node[inner sep=0pt] (provider) at (0,-4.2)
                    {\includegraphics[width=1.5cm]{figures/noun-cloud-5095064.png}};
                \node[font=\scriptsize] at (0,-5.2) {Cloud Provider (Untrusted)};

                % Attack attempts - arrows from cloud toward barrier
                \draw[->, thick, red] (0.3,-3.5) -- (0.3,-2.6)
                    node[pos=0.3, right, font=\tiny] {Read};
                \draw[->, thick, red] (-0.5,-3.5) -- (-0.5,-2.6)
                    node[pos=0.3, left, font=\tiny] {Modify};

                % X marks for blocked attacks - clearly below barrier
                \node[inner sep=0pt] at (0.3,-2.85)
                    {\includegraphics[width=0.4cm]{figures/noun-x-2222229.png}};
                \node[inner sep=0pt] at (-0.5,-2.85)
                    {\includegraphics[width=0.4cm]{figures/noun-x-2222229.png}};
