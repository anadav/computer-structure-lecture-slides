...
\begin{frame}{2D Page Walk with EPT/NPT}
    \begin{center}
        \begin{tikzpicture}[scale=0.85,
            gbox/.style={draw, fill=blue!20, minimum width=0.9cm, minimum height=0.6cm, font=\tiny, align=center},
            ebox/.style={draw, fill=orange!20, minimum width=0.9cm, minimum height=0.5cm, font=\tiny, align=center},
            arrow/.style={->, thick, >=stealth}]

            % Guest walk labels (horizontal) - top row
            \node[font=\scriptsize\bfseries] at (0, 2.5) {GVA};
            \node[font=\scriptsize\bfseries] at (1.5, 2.5) {gPML4};
            \node[font=\scriptsize\bfseries] at (3, 2.5) {gPDPT};
            \node[font=\scriptsize\bfseries] at (4.5, 2.5) {gPD};
            \node[font=\scriptsize\bfseries] at (6, 2.5) {gPT};
            \node[font=\scriptsize\bfseries] at (7.5, 2.5) {GPA};

            % Horizontal arrows for guest walk
            \draw[arrow, blue!70] (0.4, 2.5) -- (1.1, 2.5);
            \draw[arrow, blue!70] (1.9, 2.5) -- (2.6, 2.5);
            \draw[arrow, blue!70] (3.4, 2.5) -- (4.1, 2.5);
            \draw[arrow, blue!70] (4.9, 2.5) -- (5.6, 2.5);
            \draw[arrow, blue!70] (6.4, 2.5) -- (7.1, 2.5);

            % EPT walks (vertical columns under each guest PT access)
            \foreach \x/\label in {1.5/gPML4, 3/gPDPT, 4.5/gPD, 6/gPT, 7.5/Data} {
                \node[ebox] at (\x, 1.5) {ePML4};
                \node[ebox] at (\x, 0.8) {ePDPT};
                \node[ebox] at (\x, 0.1) {ePD};
                \node[ebox] at (\x, -0.6) {ePT};
                \node[ebox, fill=green!20] at (\x, -1.3) {HPA};

                % Vertical arrows
                \draw[arrow, orange!70, thin] (\x, 2.2) -- (\x, 1.8);
                \draw[arrow, orange!70, thin] (\x, 1.2) -- (\x, 1.1);
                \draw[arrow, orange!70, thin] (\x, 0.5) -- (\x, 0.4);
                \draw[arrow, orange!70, thin] (\x, -0.2) -- (\x, -0.3);
                \draw[arrow, orange!70, thin] (\x, -0.9) -- (\x, -1.0);
            }

            % Labels
            \node[font=\scriptsize, rotate=90] at (-0.8, 0.5) {EPT walk};
            \node[font=\scriptsize] at (3.5, 3.1) {Guest page walk $\longrightarrow$};

            % Access count annotation
            \node[draw, fill=yellow!20, font=\scriptsize, align=left] at (9.5, 0.5) {
                5 guest accesses\\
                $\times$ 5 EPT each\\
                = \textbf{25 total}\\[2pt]
                {\tiny (TLB helps!)}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \small
            \textbf{Guest page walk} (blue): GVA $\rightarrow$ GPA\\
            Each step accesses guest page table in memory
        \end{column}
        \begin{column}{0.5\textwidth}
            \small
            \textbf{EPT walk} (orange): GPA $\rightarrow$ HPA\\
            Every guest memory access needs EPT translation
        \end{column}
    \end{columns}
\end{frame}
