...
\begin{frame}[fragile]{Shadow Stack - Hardware Return Address Protection}
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            ...
            \vspace{-2mm}
            \begin{tcolorbox}[colback=yellow!10, left=1mm, right=1mm, top=1mm, bottom=1mm]
\begin{minted}[fontsize=\scriptsize]{text}
CALL func:
...
\begin{frame}{TEE Memory Protection Approaches}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            {\small \textbf{Ex:} SGX (EPCM), TrustZone (NS bit)}

            {\small \textcolor{red}{\textbf{Limitation:}} Physical attacks (cold boot, bus snooping)}
            ...
            {\small \textbf{Ex:} AMD SEV, Intel MKTME}
        \end{column}
        \begin{column}{0.5\textwidth}
            ...
            {\small \textbf{Ex:} SEV-SNP (RMP), TDX (SEPT)}
...
\begin{frame}{Reverse Map Table (RMP) - AMD SEV-SNP}
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            ...
            \begin{itemize}
                ...
                \item VM must validate pages before use
            \end{itemize}
        \end{column}
        \begin{column}{0.45\textwidth}
            \centering
            \begin{tikzpicture}[
                mempage/.style={draw, minimum width=1.2cm, minimum height=0.45cm, font=\tiny},
                arrow/.style={->, thick}]

                % Physical Memory
                \node[font=\small\bfseries] at (0, 2.4) {Physical Memory};
                \node[mempage, fill=green!20] (p0) at (0, 1.85) {Page 0};
                \node[mempage, fill=green!20] (p1) at (0, 1.3) {Page 1};
                \node[mempage, fill=orange!20] (p2) at (0, 0.75) {Page 2};
                \node[mempage, fill=green!20] (p3) at (0, 0.2) {Page 3};

                % RMP Table using matrix of nodes
                \node[font=\small\bfseries] at (3.2, 2.4) {RMP Table};
                \matrix[matrix of nodes,
                    ampersand replacement=\&,
                    nodes={draw, text width=0.55cm, text height=0.25cm, text depth=0.08cm,
                           align=center, inner sep=1pt, font=\tiny, fill=blue!10},
                    row sep=-\pgflinewidth,
                    column sep=-\pgflinewidth,
                    row 1/.style={nodes={fill=gray!30, font=\tiny\bfseries}}] (rmp) at (3.2, 1.0) {
                    Pg \& Owner \& V \\
                    0 \& VM1 \& \checkmark \\
                    1 \& VM1 \& \checkmark \\
                ...
                % Arrows to matrix rows
                \draw[arrow, green!60!black] (p0.east) -- (rmp-2-1.west);
                \draw[arrow, green!60!black] (p1.east) -- (rmp-3-1.west);
                \draw[arrow, orange!60!black] (p2.east) -- (rmp-4-1.west);
                \draw[arrow, green!60!black] (p3.east) -- (rmp-5-1.west);

                % Legend
                \node[font=\tiny] at (1.6, -0.35) {V = Validated, HV = Hypervisor};
            \end{tikzpicture}

            \vspace{0.2cm}
            \begin{tcolorbox}[colback=green!10, fontupper=\scriptsize]
                \textbf{Access Check:} CPU verifies RMP entry matches expected owner before allowing memory access
            \end{tcolorbox}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}[font=\small,
                vm/.style={draw, line width=1pt, minimum width=1.4cm, minimum height=1.2cm, align=center},
                cpu/.style={draw, fill=yellow!30, minimum width=4cm, minimum height=0.7cm, font=\small\bfseries},
                barrier/.style={ultra thick, blue}]

                % CPU enforcement (top) - anchor for everything
                \node[cpu] (cpu) {CPU (TDX Module)};

                % Protected VMs - relative to CPU
                \node[vm, fill=green!30, draw=green!70!black, below left=0.6cm and 0.2cm of cpu.south] (vm1) {
                    \textbf{VM1}\\[-2pt]{\tiny Customer A}};
                \node[vm, fill=blue!30, draw=blue!70!black, below right=0.6cm and 0.2cm of cpu.south] (vm2) {
                    \textbf{VM2}\\[-2pt]{\tiny Customer B}};

                % Protection barrier - relative to VMs
                \coordinate (barrier-left) at ([xshift=-0.5cm]vm1.south west);
                \coordinate (barrier-right) at ([xshift=0.5cm]vm2.south east);
                \coordinate (barrier-center) at ($(barrier-left)!0.5!(barrier-right)$);
                \draw[barrier] (barrier-left) -- (barrier-right)
                    node[right, font=\small] {TDX};

                % Threat Actor - below barrier center
                \node[inner sep=0pt, below=1.5cm of barrier-center] (provider)
                    {\includegraphics[width=1.5cm]{figures/noun-cloud-5095064.png}};
                \node[font=\scriptsize, below=0.1cm of provider] (provider-label) {Cloud Provider (Untrusted)};

                % Attack arrows from provider toward barrier
                \draw[->, thick, red] ([xshift=0.3cm]provider.north) -- ([xshift=0.3cm]provider.north |- barrier-center)
                    node[pos=0.4, right, font=\tiny] {Read};
                \draw[->, thick, red] ([xshift=-0.3cm]provider.north) -- ([xshift=-0.3cm]provider.north |- barrier-center)
                    node[pos=0.4, left, font=\tiny] {Modify};

                % X marks for blocked attacks - on the arrow paths
                \node[inner sep=0pt] at ([xshift=0.3cm, yshift=-0.5cm]barrier-center)
                    {\includegraphics[width=0.4cm]{figures/noun-x-2222229.png}};
                \node[inner sep=0pt] at ([xshift=-0.3cm, yshift=-0.5cm]barrier-center)
                    {\includegraphics[width=0.4cm]{figures/noun-x-2222229.png}};

                % Protection arrows from CPU to VMs
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[
                ...
                \node[vm, fill=green!30, below left=1cm and 0.3cm of amdsp.south] (vm1)
                    {\textbf{VM1}\\[-2pt]\small Encrypted\\[-2pt]\includegraphics[height=0.3cm]{figures/noun-key-8095348.png}{\tiny 1}};
                \node[vm, fill=blue!30, below right=1cm and 0.3cm of amdsp.south] (vm2)
                    {\textbf{VM2}\\[-2pt]\small Encrypted\\[-2pt]\includegraphics[height=0.3cm]{figures/noun-key-8095348.png}{\tiny 2}};
...
\begin{frame}{Intel SGX Enclaves}
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[
                app/.style={draw, thick, minimum width=4cm, minimum height=4cm,
                           fill=gray!15, rounded corners=3pt},
                enclave/.style={draw, very thick, minimum width=3cm, minimum height=2.2cm,
                ...
                \node[enclave] (enclave) at (app.center)
                    {\textbf{SGX Enclave}\\[-2pt]
                     \scriptsize Trusted Code\\[-2pt]
                     \scriptsize Encrypted Data\\[2pt]
                ...
                \draw[<-, thick, blue!70] (enclave.east) -- ++(1cm, 0)
                    node[right, font=\scriptsize, align=left] {Enclave Page\\Cache (EPC)};
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
