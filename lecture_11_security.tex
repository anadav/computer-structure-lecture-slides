    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.55\textwidth}
            \begin{tikzpicture}[
                ...
                \node at ([xshift=1cm,yshift=-4mm]hw.east) (pk) {\includegraphics[height=0.4cm]{figures/noun-key-8095348.png}};
                \node[font=\tiny, right=-1mm of pk] {PK};
                \node at ([xshift=1cm,yshift=-4mm]uefi.east) (kek) {\includegraphics[height=0.4cm]{figures/noun-key-8095348.png}};
                \node[font=\tiny, right=-1mm of kek] {KEK};
                \node at ([xshift=1cm,yshift=-4mm]bootloader.east) (db) {\includegraphics[height=0.4cm]{figures/noun-key-8095348.png}};
                ...
                % Key usage (dashed to chain) - keys sign the stage
                \draw[dashed, ->, gray] (pk.west) -- (uefi.east);
                \draw[dashed, ->, gray] (kek.west) -- (bootloader.east);
                \draw[dashed, ->, gray] (db.west) -- (kernel.east);

                % TPM measurement arrows (blue dashed) to specific PCRs - go right first to avoid keys
                \draw[measure] (uefi.east) -- ++(2.3cm,0) |- (pcrs-1-1.west);
                \draw[measure] (bootloader.east) -- ++(2.4cm,0) |- (pcrs-2-1.west);
                \draw[measure] (kernel.east) -- ++(2.5cm,0) |- (pcrs-3-1.west);
                \draw[measure] (ima.east) -- ++(2.6cm,0) |- (pcrs-5-1.west);

                % Legend for key icon
                \node[font=\tiny, below=1cm of tpm] (keylegend) {\includegraphics[height=0.25cm]{figures/noun-key-8095348.png} = Signed by};
