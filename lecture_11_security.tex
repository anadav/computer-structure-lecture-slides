    ...
    \begin{columns}
        ...
        \begin{column}{0.42\textwidth}
            ...
            \begin{tikzpicture}[scale=0.6]
                ...
                \node at (2,-1) {\textbf{CFG\textsuperscript{*} Enforcement}};
            \end{tikzpicture}
        ...
        \end{column}
    \end{columns}
    \footnotetext{*~Control Flow Graph}
\end{frame}
...
\begin{frame}[fragile]{Shadow Stack (Backward-Edge CFI)}
    \begin{columns}
        \begin{column}{0.5\textwidth}
...
\begin{frame}[fragile]{Pointer Authentication (PAC)}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item Cryptographic signature in upper pointer bits
                \item Uses QARMA block cipher
                \item Sign on store, verify before use
            \end{itemize}

            \vspace{0.3cm}
            \textbf{What It Protects:}
            \begin{itemize}
                \item Return addresses - \textit{backward} (IA/IB)
                \item Call/jump targets - \textit{forward} (IA/IB)
                \item Vtables, data pointers (DA/DB)
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
...
\begin{frame}{Secure VMs vs Secure Enclaves}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                \item Small TCB, app explicitly uses enclave
            \end{itemize}

            \vspace{0.15cm}
            \textbf{Threat Model:} Untrusted OS/kernel

            \vspace{0.15cm}
            \textbf{Examples:} Intel SGX, ARM TrustZone
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                \item Large TCB, transparent to guest OS
            \end{itemize}

            \vspace{0.15cm}
            \textbf{Threat Model:} Untrusted hypervisor

            \vspace{0.15cm}
            \textbf{Examples:} Intel TDX, AMD SEV-SNP

            \vspace{0.15cm}
            \textbf{Use Cases:}
            \begin{itemize}
                \item Confidential cloud computing
                \item Lift-and-shift workloads
    ...
    \vspace{0.1cm}
    \begin{tcolorbox}[colback=green!10, boxsep=1mm]
        \centering
        {\small \textbf{Key Difference:} Enclaves require app changes; Secure VMs work with unmodified OS}
    \end{tcolorbox}
\end{frame}
...
\begin{frame}{MKTME (Multi-Key Total Memory Encryption)}
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            ...
            \vspace{0.3cm}
            \textbf{Encryption Model:}
            \begin{itemize}
                \item Encrypt/decrypt on memory bus
                \item Plaintext in CPU cache
