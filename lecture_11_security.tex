    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[font=\footnotesize,
                ...
                perm/.style={font=\scriptsize, anchor=west}]

                % Memory layout using matrix
                \matrix[matrix of nodes,
                    nodes={mem section},
                    row sep=0.1cm] (mem) {
                    |[fill=orange!30]| Heap \\
                    |[fill=yellow!30]| Stack \\
                    |[fill=green!30]| Data Section \\
                    |[fill=blue!30]| Code Section \\
                };

                % Permissions - relative to each row
                \node[perm, right=0.2cm of mem-1-1] {W=1, \textcolor{red}{X=0}};
                \node[perm, right=0.2cm of mem-2-1] {W=1, \textcolor{red}{X=0}};
                \node[perm, right=0.2cm of mem-3-1] {W=1, \textcolor{red}{X=0}};
                \node[perm, right=0.2cm of mem-4-1] {W=0, \textcolor{green!50!black}{X=1}};

                % CPU node
                \node[draw, fill=gray!20, minimum width=1.2cm, minimum height=0.6cm,
                      right=2.5cm of mem-3-1] (cpu) {CPU};

                % Execution arrow - only from Code Section (with checkmark)
                \draw[->, thick, green!50!black] (mem-4-1.east) -- (cpu.south west)
                    node[pos=0.85, above, font=\small, sloped] {\checkmark};

                % Blocked execution from Stack
                \draw[->, thick, red, dashed] (mem-2-1.east) -- (cpu.north west)
...
\begin{frame}{ROP Attack Visualization}
    \begin{center}
        \begin{tikzpicture}[scale=0.8, font=\footnotesize,
            gadget/.style={draw, fill=yellow!40, minimum width=2cm, minimum height=0.9cm, align=center},
            ...
            \node[gadget] (g1) at (1.2,1) {Gadget 1\\[-2pt]{\scriptsize pop rdi}};
            \node[gadget] (g2) at (3.7,1.8) {Gadget 2\\[-2pt]{\scriptsize pop rsi}};
            \node[gadget] (g3) at (6.2,0.8) {Gadget 3\\[-2pt]{\scriptsize mov rax,59}};
...
\begin{frame}{ARM TrustZone - Trusted Execution Environment (TEE)}
    \begin{center}
        \begin{tikzpicture}[font=\footnotesize,
            world/.style={rounded corners, line width=2pt, minimum width=4cm, minimum height=3.8cm},
            ...
            \node[secure world] (secure) at (-3.2,2) {};
            \node[anchor=north, font=\footnotesize\bfseries] at (secure.north) {Secure World (TEE)};

            % Secure levels
            \node[secure level, fill=blue!40] (sel3) at ([yshift=-0.5cm]secure.north) {S-EL3: Monitor};
            \node[level, fill=green!50, below=0.15cm of sel3] (sel2) {S-EL2: S-Hypervisor};
            \node[level, fill=yellow!40, below=0.15cm of sel2] (sel1) {S-EL1: Trusted OS};
            \node[secure level, fill=red!30, below=0.15cm of sel1] (sel0) {S-EL0: Trusted Apps};

            % Normal World container
            \node[normal world] (normal) at (3.2,2) {};
            \node[anchor=north, font=\footnotesize\bfseries] at (normal.north) {Normal World (REE)};

            % Normal levels - aligned with secure levels
            \node[normal level, fill=green!30] (el2) at (normal.center |- sel2) {EL2: Hypervisor};
            \node[normal level, fill=yellow!40, below=0.15cm of el2] (el1) {EL1: Rich OS};
            \node[normal level, fill=red!50, below=0.15cm of el1] (el0) {EL0: Applications};

            % SMC transition
            \draw[<->, thick, blue] (secure.east) -- (normal.west) node[midway, above] {SMC};
        \end{tikzpicture}
    \end{center}
