...
\begin{frame}[fragile, label=rs-timeline-frame, shrink=20]
...
\hline
    ...
    5 & \texttt{blt (R1<100)} & 20 & 40 & 110 & - & - & 2 & P4 & - & 100 & 3 & - & 4 & - & - & 23 & \onslide<6->{+1=4} \\
    6 & \texttt{ld R2=[R1+30]} & - & 110 & - & 50 & 110 & 2 & P4 & - & 30 & 3 & - & 4 & {\tiny \makecell{addr blk\\data blk}} & 24 & \tikzmarknode{tcommit6}{25} & \onslide<7->{+1=5} \\
    7 & \texttt{st [R2+20]=R1} & - & - & - & 130 & 20 & \only<1-7>{\textcolor{orange}{?}}\only<8->{\textcolor{orange}{3}} & P6 & P4 & 20 & 24 & 3 & \makecell{\tiny Std:4\\\tiny Sta:25} & - & 25 & \tikzmarknode{tcommit7}{26} & \onslide<8->{5\tikzmarknode{minus1}{-1}+2=\textcolor{red}{6}} \\
    8 & \texttt{ld R3=[R1+100]} & - & - & 120 & 120 & 120 & 4 & P4 & - & 100 & 4 & - & 5 & {\tiny addr blk} & 27 & 28 & \onslide<9->{6-3+1=4} \\
    9 & \texttt{st [R1+40]=R3} & - & - & - & 60 & 120 & 4 & P8 & P4 & 40 & 27 & 4 & \makecell{\tiny Sta:5\\\tiny Std:28} & - & 28 & 29 & \onslide<10->{+2=\textcolor{red}{6}} \\
    10 & \texttt{add R1=R1+10} & 30 & - & - & - & - & 5 & P4 & - & 10 & 3 & - & 4 & - & - & 23 & \onslide<11->{2+1=3} \\
    11 & \texttt{blt (R1<100)} & 30 & 110 & 120 & - & - & 5 & P10 & - & 100 & 4 & - & 5 & - & - & 23 & \onslide<12->{+1=4} \\
    \hline
\end{tabular}
...
\onslide<5>{
\begin{tikzpicture}[remember picture, overlay]
    % Draw circles around the four "2"s in T exe
    ...
    % Draw arrows from circles to the "-4"
...
% Explanation box for slide 5
\onslide<5>{
\begin{tikzpicture}[remember picture, overlay]
    \node[fill=yellow!30, fill opacity=1, draw=yellow!50, line width=1pt, rounded corners=3mm, font=\small, text width=9.5cm, align=left, anchor=south west]
        at ([yshift=1.8cm, xshift=0.3cm]current page.south west)
        {\textbf{Key:} Inst 4 allocated at cycle 2 (T alloc=2), same cycle when 4 uops execute (T exe=2)\\
        Those 4 executing uops free their RS entries, making room for inst 4:\\
        \hspace{3mm}6 (occupied) - 4 (freed by circled uops) + 1 (inst 4) = 3};
\end{tikzpicture}
}

% X mark on T commit for inst 6 (from slide 7 onwards) - won't commit due to branch misprediction
\onslide<7->{
\begin{tikzpicture}[remember picture, overlay]
...
% Explanation box for slide 7
\onslide<7>{
\begin{tikzpicture}[remember picture, overlay]
    \node[fill=yellow!30, fill opacity=1, draw=yellow!50, line width=1pt, rounded corners=3mm, font=\small, text width=9.5cm, align=left, anchor=south west]
        at ([yshift=1.8cm, xshift=0.3cm]current page.south west)
        {\textbf{Note:} Instruction 6 will never commit. The first branch (inst 5, \texttt{blt}) mispredicts at T=4, causing a flush at T=5. All instructions after the branch (inst 6+) are on the wrong path and will be squashed.};
\end{tikzpicture}
}

% X mark on T commit for inst 7 (from slide 8 onwards) - won't commit due to branch misprediction
\onslide<8->{
\begin{tikzpicture}[remember picture, overlay]
...
% Circle and arrow for slide 8: T exe=3 of Pdst 4 frees RS entry
\onslide<8>{
\begin{tikzpicture}[remember picture, overlay]
...
% Explanation box for slide 8
\onslide<8>{
\begin{tikzpicture}[remember picture, overlay]
    \node[fill=yellow!30, fill opacity=1, draw=yellow!50, line width=1pt, rounded corners=3mm, font=\small, text width=3.5cm, align=left, anchor=south west]
        at ([yshift=6.5cm, xshift=0.3cm]current page.south west)
        {\textbf{Note:} Instruction 7 (store) requires 2 RS entries (STA+STD), but RS was full at cycle 2. It could only be allocated at cycle 3 when inst 4 freed its RS entry (circled T exe=3 $\rightarrow$ -1).\\
