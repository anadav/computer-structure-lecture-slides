...
\onslide<23->{\draw[green circle] ([xshift=\circlexshift,yshift=\circleyshift] pic cs:green1addr) circle (9pt);}
\onslide<24->{\draw[green circle] ([xshift=\circlexshift,yshift=\circleyshift] pic cs:green2) circle (9pt);}
\onslide<25->{\draw[green circle] ([xshift=\circlexshift,yshift=\circleyshift] pic cs:green3) circle (9pt);}
\onslide<32->{\draw[green circle] ([xshift=3mm,yshift=\circleyshift] pic cs:red4) circle (12pt);}
% Red circle on address
\onslide<26->{\draw[red circle] ([xshift=\circlexshift,yshift=\circleyshift] pic cs:red1) circle (9pt);}
% Red circles around Std/Sta pairs
\onslide<27->{\draw[red circle] ([xshift=3mm,yshift=\circleyshift] pic cs:red2) circle (12pt);}
\onslide<28->{\draw[red circle] ([xshift=3mm,yshift=\circleyshift] pic cs:red3) circle (12pt);}
\onslide<29->{\draw[red circle] ([xshift=3mm,yshift=\circleyshift] pic cs:red4) circle (12pt);}
% Blue circle around T exe
\onslide<30->{\draw[blue circle] ([xshift=1mm,yshift=\circleyshift] pic cs:blue1) circle (9pt);}
% Arrow from 22 to 24 in T data ready column
\onslide<31->{\draw[arrow style, very thick] ([yshift=-2mm, xshift=2mm] pic cs:arrow1start) -- ([yshift=3mm, xshift=2mm] pic cs:arrow1end);}
\end{tikzpicture}
...
\hline
    ...
    \hline
    \onslide<1->{6 & \texttt{ld R2=[R1+30]} & - & \underline{110} & - & \underline{50} & 110} & \onslide<17->{2} & \onslide<17->{P4} & \onslide<17->{-} & \onslide<17->{30} & \onslide<17->{3} & \onslide<17->{-} & \onslide<17->{4} & \onslide<17->{\makecell{\tiny addr blk\\[-4pt]\tiny data blk}} & \onslide<17->{\tikzmark{arrow1end}24} & \onslide<17->{25} \\
...
\begin{tikzpicture}[remember picture, overlay,
    ...
    % Yellow background - lower and taller
    \node[fill=yellow!30, fill opacity=1, draw=yellow!50, line width=1pt, minimum width=13cm, minimum height=4.8cm, rounded corners=5mm] at ([yshift=-1.8cm]current page.center) {};

    % Main cycle nodes - positioned lower
    \node[bluecircle] at ([xshift=-3.5cm, yshift=-1.8cm] current page.center) (n11) {11};
    ...
    % Instruction boxes on the left - closer to cycle nodes
    \node[bluerect, left=8mm of n11] (load1) {Load};
    ...
    \node[label, below=2mm of n11] (l11) {R2 (pb0)\\is known};
    \node[label] at (n12.south |- store) [yshift=-2mm] (l12) {Addr. Calc:\\PB0+20};
    \node[label] at (n13.south |- load2) [yshift=-2mm] (l13) {Memory\\checks};
    \node[label] at (n14.south |- load2) [yshift=-2mm] (l14) {L1 miss};
    \node[label, below=2mm of n21] (l21) {L2 Hit};
\end{tikzpicture}
}
