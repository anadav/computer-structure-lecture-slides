...
\newcommand{\LoadLifecycleDiagram}[9]{%
% #1-4: RAT R0-R3 phys regs, #5-8: ROB first row data, #9: arrow flags
\begin{tikzpicture}[
    scale=0.75,
    every node/.style={transform shape},
    cell/.style={draw, rectangle, minimum height=4.5mm, font=\tiny},
    headerbox/.style={draw, rectangle, fill=gray!20, font=\scriptsize\bfseries, minimum height=5mm, align=center},
    execbox/.style={draw, rectangle, fill=lightyellow, minimum width=1.2cm, minimum height=0.9cm, font=\scriptsize, align=center},
    smallbox/.style={draw, rectangle, minimum width=1.2cm, minimum height=0.7cm, font=\scriptsize, align=center},
...
\matrix[matrix of nodes,
        ...
        inner sep=0pt,
        label={[font=\scriptsize\bfseries]above:RAT},
        nodes={cell},
        column 1/.append style={nodes={text width=0.9cm, align=center, fill=gray!10}},
        column 2/.append style={nodes={text width=0.9cm, align=center}},
        ampersand replacement=\&] (rat) {
    \scriptsize Arch \& \scriptsize Phys \\
    \scriptsize Reg. \& \scriptsize Reg. \\
    R0 \& #1 \\
    R1 \& #2 \\
    R2 \& #3 \\
    R3 \& #4 \\
...
\matrix[matrix of nodes,
        ...
        column sep=-\pgflinewidth,
        inner sep=0pt,
        label={[font=\scriptsize\bfseries]above:ROB},
    ...
    |[fill=gray!10]| \# \& |[fill=gray!10]| Val \& |[fill=gray!10]| Rdy \& |[fill=gray!10]| Data \& |[fill=gray!10]| DST \\
    {} \& |[fill=gray!10]| id \& {} \& {} \& {} \\
    #5 \& #6 \& #7 \& #8 \& R3 \\
    0 \& \& \& \& \\
    0 \& \& \& \& \\
...
% Instruction Queue
\node[headerbox, above=0.25cm of rob.north, text width=3.2cm] (instq) {Instruction Q};
\node[cell, below=0pt of instq, text width=3.2cm, minimum height=6mm, align=center, font=\scriptsize] {R3$\leftarrow$MEM(R2+50)};

% RS (Reservation Station)
\node[headerbox, below right=0.25cm and 0.4cm of rob.north east, anchor=north west, text width=2.8cm] (rshdr) {RS};
\node[cell, below=0pt of rshdr, text width=2.8cm, minimum height=6mm, align=center, font=\scriptsize] (rs) {};

% MOB (Memory Order Buffer)
\node[headerbox, right=0.4cm of rshdr.north east, anchor=north west, text width=2.2cm, fill=lightcyan!20] (mobhdr) {Store Buffer};
\node[headerbox, below=0pt of mobhdr, text width=2.2cm, fill=lightcyan!20] (mobhdr2) {Load Buffer};
\matrix[matrix of nodes,
        row sep=-\pgflinewidth,
        column sep=-\pgflinewidth,
        inner sep=0pt,
        below=0pt of mobhdr2,
        anchor=north,
        nodes={cell, align=center},
        column 1/.append style={nodes={text width=0.35cm, fill=lightcyan!40}},
        column 2/.append style={nodes={text width=1.3cm, fill=lightcyan!40}},
        column 3/.append style={nodes={text width=0.35cm, fill=lightcyan!40}},
        ampersand replacement=\&] (mob) {
    V \& Addr. \& BC \\
};

% Data Cache
\node[smallbox, right=0.4cm of mob.north east, anchor=north west] (dcache) {Data\\Cache};

% EXE block
\node[execbox, below=0.7cm of rs.south west, anchor=north west] (alu1) {ALU1};
\node[font=\large, right=0.25cm of alu1] (dots) {$\bullet\bullet\bullet$};
\node[execbox, right=0.25cm of dots] (agu) {AGU};

% dTLB
\node[smallbox, right=0.4cm of agu] (dtlb) {dTLB};

% Retire
\node[headerbox, below=0.4cm of alu1.south, text width=1.8cm] (retire) {Retire};

% Draw green arrows based on flags
\ifnum#9>1 \draw[greenarrow] (rat.east) -- ++(0.25,0); \fi
\ifnum#9>2 \draw[greenarrow] (agu.north) -- (rs.south); \fi
\ifnum#9>3 \draw[greenarrow] (agu.east) -- (dtlb.west); \fi
\ifnum#9>4 \draw[greenarrow] (mob.east) -- (dcache.west); \fi
...
\begin{center}
\LoadLifecycleDiagram{R0}{R1}{R2}{R3}%RAT R0-R3
    {}{}{}{}%ROB entry (empty)
    {0}%arrow flags
\end{center}
\end{frame}
...
\frametitle{The Life of a Load\ldots}
\begin{center}
\LoadLifecycleDiagram{R0}{R1}{R2}{R3 \textcolor{green!60!black}{RF0}}%RAT R0-R3
    {Ld}{1}{0}{X}%ROB entry
    {2}%arrow flags
\end{center}
\vspace{-3mm}
