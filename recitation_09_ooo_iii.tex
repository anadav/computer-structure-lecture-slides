...
\pgfkeys{
    ...
    default/.style = {
        ...
        % Instruction Queue (4 rows)
        iq1 = {R3$\leftarrow$MEM(R2+50)}, iq2 = { }, iq3 = { }, iq4 = { },
        % RS content
        rs = { },
    ...
    iq1/.estore in = \iqOne,
    iq2/.estore in = \iqTwo,
    iq3/.estore in = \iqThree,
    iq4/.estore in = \iqFour,
    rs/.estore in = \rsContent,
    sb1/.estore in = \sbOne,
    sb2/.estore in = \sbTwo,
    lbV/.estore in = \lbV,
...
%   iq1, iq2, iq3, iq4 - Instruction Queue entries
%   rs - Reservation Station content
%   sb1, sb2 - Store Buffer entries
%   lbV, lbAddr, lbBC - Load Buffer fields (Valid, Address, Blocking Conditions)
%   alu, agu - EXE unit contents
...
% MOB (Memory Order Buffer) - Load Buffer with title above
\node[headerbox, right=0.5cm of rs.north east, anchor=north west, text width=2.1cm] (loadbuftitle) {Load Buffer};

\matrix[matrix of nodes,
        row sep=-\pgflinewidth,
        column sep=-\pgflinewidth,
        inner sep=0pt,
        below=0pt of loadbuftitle.south,
        anchor=north,
        nodes={cell, align=center},
        row 1/.style={nodes={headerbox}},
...
% Store Buffer title
\node[headerbox, above=0pt of loadbuftitle.north, anchor=south, text width=2.1cm] (storebuftitle) {Store Buffer};

% Store Buffer (above Load Buffer title, matching its width)
\matrix[matrix of nodes,
        row sep=-\pgflinewidth,
        column sep=-\pgflinewidth,
        inner sep=0pt,
        below=0pt of storebuftitle.south,
        anchor=north,
        nodes={cell, align=center},
        column 1/.append style={nodes={text width=0.6cm}},
        column 2/.append style={nodes={text width=1.0cm}},
...
\node[labelstyle, above=0.1cm of storebuftitle.north] (moblabel) {MOB};
\begin{scope}[on background layer]
\node[draw, fill=lightcyan, fit=(moblabel)(storebuftitle)(storebuf)(loadbuftitle)(loadbuf), inner sep=2pt] {};
\end{scope}
...
% Retire (below EXE block, anchored to south east)
\matrix[matrix of nodes,
        row sep=-\pgflinewidth,
        column sep=-\pgflinewidth,
        inner sep=0pt,
        label={[labelstyle]left:Retire},
        below=0.4cm of exeblock.south east,
        anchor=north east,
        nodes={cell, align=center},
...
\matrix[matrix of nodes,
        ...
        column sep=-\pgflinewidth,
        inner sep=0pt,
        right=0.3cm of retire.north east,
        anchor=north west,
        nodes={cell, align=center},
        column 1/.append style={nodes={text width=1.8cm}},
        ampersand replacement=\&] (retirebox) {
    \retireBoxContent \\
};
