...
\begin{tikzpicture}[
    ...
    cell/.style={draw, minimum height=0.4cm, inner sep=0pt, font=\tiny, text depth=0pt, text height=0.25cm, anchor=center},
    % Virtual pages
...
\matrix[matrix of nodes, row sep=-\pgflinewidth, ampersand replacement=\&,
    label={[mytitle]above:Virtual Pages}] (vp) {
    |[colheader]| VPN \\
    |[vpage]| \phantom{x} \\
    |[vpage]| \phantom{x} \\
    |[vpage]| \phantom{x} \\
...
\matrix[matrix of nodes, row sep=-\pgflinewidth, column sep=-\pgflinewidth, ampersand replacement=\&,
    ...
    label={[mytitle]above:Page Table}] (pt) at (vp.north east) {
...
\matrix[matrix of nodes, row sep=-\pgflinewidth, ampersand replacement=\&,
    ...
    label={[mytitle]above:Physical Pages}] (pp) at (pt.north east) {
    |[colheader]| PFN \\
    |[ppage]| \phantom{x} \\
    |[ppage]| \phantom{x} \\
    |[ppage]| \phantom{x} \\
...
% Disk - positioned relative to pp
\node[mytitle, below=2cm of pp] (disklabel) {Disk};
\begin{scope}[shift={(disklabel.south)}, yshift=-0.3cm]
    ...
    % Disk slots
    \fill[green!30] (-0.95, -0.3) rectangle (0.95, -0.6);
...
\draw[arrow] (pt-2-2) -- (pp-2-1.west);
\draw[arrow] (pt-3-2.east) -- (pp-4-1.west);
\draw[arrow] (pt-4-2.east) -- (pp-3-1.west);
\draw[arrow] (pt-6-2.east) -- (pp-6-1.west);
\draw[arrow] (pt-7-2.east) -- (pp-5-1.west);
\draw[arrow] (pt-9-2.east) -- (pp-7-1.west);
\draw[arrow] (pt-10-2.east) -- (pp-6-1.west);
\draw[arrow] (pt-12-2.east) -- (pp-7-1.west);

% Arrows from Page Table to Disk (for present=0)
...
\draw[arrow] (pt-5-2.east) -- ++(0.5,0) |- (diskslot1);
\draw[arrow] (pt-8-2.east) -- ++(0.7,0) |- (diskslot2);
\draw[arrow] (pt-11-2.east) -- ++(0.9,0) |- (diskslot3);
...
\begin{tikzpicture}[
    ...
    \node[addrlabel, blue!70!black] at (4, 3.8) (vatitle) {Virtual Address};
    ...
    \matrix[matrix of nodes,
            ...
            column 2/.style={nodes={minimum width=0.4cm}},
            column 3/.style={nodes={minimum width=0.45cm}},
            column 4/.style={nodes={minimum width=1.6cm}},
            row 1/.style={nodes={draw=none, font=\tiny\bfseries, minimum height=0.3cm}},
            ] (pt) at (2.2, 0.5) {
        |[name=hdrP]| P \& D \& AC \& Phy.\ page \# \\
        |[name=r1c1]| \phantom{0} \& \phantom{0} \& \phantom{0} \& |[name=r1c4]| \phantom{0} \\
        \phantom{0} \& \phantom{0} \& \phantom{0} \& \phantom{0} \\
        \phantom{0} \& \phantom{0} \& \phantom{0} \& \phantom{0} \\
        |[fill=green!30, name=selP]| 1 \& |[fill=yellow!30, name=selD]| 0 \& |[fill=gray!20, name=selAC]| \phantom{0} \& |[fill=orange!40, name=selPPN]| \phantom{0} \\
        \phantom{0} \& \phantom{0} \& \phantom{0} \& \phantom{0} \\
        \phantom{0} \& \phantom{0} \& \phantom{0} \& \phantom{0} \\
        \phantom{0} \& \phantom{0} \& \phantom{0} \& \phantom{0} \\
        |[name=rlastc1]| \phantom{0} \& \phantom{0} \& \phantom{0} \& |[name=rlastc4]| \phantom{0} \\
    };

    % Outer box around page table (excluding header)
    \node[draw, thick, inner sep=0pt, fit=(r1c1) (rlastc4)] (ptbox) {};
    ...
    \node[draw, fill=red!15, minimum width=1.1cm, minimum height=0.9cm, align=center, font=\tiny,
          anchor=east] (ptbr) at ([xshift=-0.3cm]ptbox.north west) {Page\\table\\base\\reg};

    % Arrow from PTBR to page table
    \draw[arrowstyle] (ptbr.east) -- (ptbox.north west);

    % Arrow from VPN to page table (index into table)
    \draw[arrowstyle] (vpn.south) -- ++(0, -0.4) -| (hdrP.north);

    % Present bit label
    \node[callout, anchor=east] (presentlbl) at ([xshift=-0.5cm]selP.west) {Present bit};
    \draw[arrowstyle] (presentlbl.east) -- (selP.west);

    % Dirty bit label
    \node[callout, anchor=east] (dirtylbl) at ([xshift=-0.5cm, yshift=1.5cm]selD.west) {Dirty bit};
    \draw[arrowstyle] (dirtylbl.east) -- (selD.north west);

    % Access Control explanation
    \node[callout, anchor=west, align=left, text width=3.5cm] (actext) at ([xshift=0.8cm]selPPN.east) {%
    ...
    \draw[arrowstyle] (selAC.east) -- ++(0.2, 0) |- (actext.west);
    ...
    \node[addrlabel, green!50!black, anchor=north] at (vatitle |- ppn.south) {Physical Address};
