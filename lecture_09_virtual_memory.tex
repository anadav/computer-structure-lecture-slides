...
\begin{tikzpicture}[
    ...
    \node[addrlabel, blue!70!black] at (4, 3.8) {Virtual Address};
    ...
    \node[bitnumber, anchor=south west] at (vpn.north east) {12};
    \node[bitnumber, anchor=south east] at (voffset.north west) {11};
    ...
    \node[draw, fill=red!15, minimum width=1.1cm, minimum height=0.9cm, align=center, font=\tiny,
          anchor=east] (ptbr) at (\ptx - 0.4, \pty) {Page\\table\\base\\reg};
    ...
    \coordinate (ptnw) at (\ptx - 0.05, \pty + 0.02);  % table north west
    ...
    \draw[arrowstyle] (ptbr.east) -- (ptnw);
    ...
    \node[offsetblock, minimum width=2.2cm, anchor=west] (poffset) at (ppn.east) {Page offset};

    % Bit numbers for physical address
    \node[bitnumber, anchor=south west] at (ppn.north west) {38};
    \node[bitnumber, anchor=south west] at (ppn.north east) {12};
    \node[bitnumber, anchor=south east] at (poffset.north west) {11};
