...
\begin{tikzpicture}[scale=0.8]
    % CPU cores with TLB inside
    \foreach \i in {0,1,2,3} {
        \node[rectangle, draw, thick, minimum width=1.4cm, minimum height=1.2cm, fill=blue!20]
            (cpu\i) at ({mod(\i,2)*2.5}, {-floor(\i/2)*2}) {};
        \node[font=\small, anchor=north] at (cpu\i.north) {CPU \i};
        \node[rectangle, draw, minimum width=1.1cm, minimum height=0.35cm, fill=yellow!30, anchor=south]
            (tlb\i) at ([yshift=1mm]cpu\i.south) {\tiny TLB};
    }

    % IPI arrows from CPU 0
    \draw[->, thick, red] (cpu0.east) -- node[above] {\tiny IPI} (cpu1.west);
    \draw[->, thick, red] (cpu0.south) -- ++(0,-0.2) -| node[near end, left] {\tiny IPI} (cpu2.north);
    \draw[->, thick, red] (cpu0.south east) -- ++(0.2,-0.2) |- node[pos=0.3, above] {\tiny IPI} (cpu3.west);

    % ACK arrows back
    \draw[->, dashed, green!60!black] ([yshift=-1mm]cpu1.west) -- node[below] {\tiny ACK} ([yshift=-1mm]cpu0.east);
    \draw[->, dashed, green!60!black] (cpu2.north west) -- ++(0,0.3) -| node[near start, right] {\tiny ACK} ([xshift=2mm]cpu0.south);
    \draw[->, dashed, green!60!black] (cpu3.north west) -- ++(-0.2,0.2) |- node[pos=0.7, below] {\tiny ACK} (cpu0.south east);

    \node[above] at (1.25, 0.8) {\small TLB Shootdown};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \node[rectangle, draw, thick, minimum width=3.5cm, minimum height=0.6cm, fill=gray!30]
        (interconnect) at (1.25, 0.5) {Interconnect};

    % CPU cores with TLB inside
    \foreach \i in {0,1,2,3} {
        \node[rectangle, draw, thick, minimum width=1.4cm, minimum height=1.2cm, fill=blue!20]
            (cpu\i) at ({mod(\i,2)*2.5}, {-floor(\i/2)*2-1.2}) {};
        \node[font=\small, anchor=north] at (cpu\i.north) {CPU \i};
        \node[rectangle, draw, minimum width=1.1cm, minimum height=0.35cm, fill=yellow!30, anchor=south]
            (tlb\i) at ([yshift=1mm]cpu\i.south) {\tiny TLB};

        % Hardware broadcast lines
        \draw[<->, thick, orange] (cpu\i.north) -- (interconnect.south -| cpu\i);
    }

    % Broadcast indication
    \node[above] at (1.25, 1) {\small Hardware Broadcast};
    \node[text=orange, below] at (1.25, -0.2) {\tiny INVLPGB};
\end{tikzpicture}
\end{center}
