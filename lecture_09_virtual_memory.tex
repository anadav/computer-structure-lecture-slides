...
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, calc, tikzmark, patterns, fit, matrix, backgrounds, shapes.callouts}
...
\begin{tikzpicture}[scale=0.6, transform shape,
    ...
    pml4table/.style={rectangle, draw=black, thick, minimum width=1.2cm, minimum height=2.8cm, fill=yellow!40},
    pdptable/.style={rectangle, draw=black, thick, minimum width=1.2cm, minimum height=2.8cm, fill=red!30},
    dirtable/.style={rectangle, draw=black, thick, minimum width=1.2cm, minimum height=2.8cm, fill=blue!20},
    pagetable/.style={rectangle, draw=black, thick, minimum width=1.2cm, minimum height=2.8cm, fill=blue!40},
    page/.style={rectangle, draw=black, thick, minimum width=1.2cm, minimum height=2.8cm, fill=orange!40},
    % Entry styles for each level (smaller boxes with thick border)
    pml4entry/.style={rectangle, draw=black, line width=1.5pt, minimum width=1.2cm, minimum height=0.5cm, fill=yellow!40},
    pdpentry/.style={rectangle, draw=black, line width=1.5pt, minimum width=1.2cm, minimum height=0.5cm, fill=red!30},
    direntry/.style={rectangle, draw=black, line width=1.5pt, minimum width=1.2cm, minimum height=0.5cm, fill=blue!20},
    pageentry/.style={rectangle, draw=black, line width=1.5pt, minimum width=1.2cm, minimum height=0.5cm, fill=blue!40},
    pageoffset/.style={rectangle, draw=black, line width=1.5pt, minimum width=1.2cm, minimum height=0.5cm, fill=orange!40},
    arrow/.style={->, black}
]
...
% Note about 8 bits
\node[font=\tiny, text width=2.2cm, align=left, red] at (-1.8,10.5) {8 bits instead of\\ 9 for example\\ purposes only};

% Vertical alignment of tables (from left to right)
% PML4 Table
\node[pml4table] (pml4base) at (1,5) {};
\node[font=\scriptsize, anchor=south] at (pml4base.north) {PML4 Table};
\node[pml4entry] (pml4d3) at ([yshift=-0.7cm]pml4base) {};
\node[pml4entry] (pml471) at ([yshift=-2.3cm]pml4base) {};

% CR3 register pointing to PML4
\node[rectangle, draw=black, thick, fill=gray!20, minimum width=1.5cm, minimum height=0.4cm, font=\scriptsize, anchor=east] (cr3) at ([xshift=-3mm]pml4base.south west) {CR3};
...
\node[font=\scriptsize, draw=black, thick, fill=yellow!20, align=center,
      rectangle callout, callout absolute pointer={(cr3.east)}] at (-1.5,1.5) {Address=\\ CR3+D3$\times$8};

% PDP Table (2 instances: one from D3=82,46)
\node[pdptable] (pdp82base) at (4,6) {};
\node[font=\scriptsize, anchor=south] at (pdp82base.north) {PDP Table};
\node[pdpentry] (pdp82) at ([yshift=-0.4cm]pdp82base) {};
\node[pdpentry] (pdp46) at ([yshift=-3.5cm]pdp82base) {};

% Page Dir instances
\node[dirtable] (dirffbase) at (7,7) {};
\node[font=\scriptsize, anchor=south] at (dirffbase.north) {Page Dir};
\node[direntry] (dirff) at ([yshift=-0.2cm]dirffbase) {};
\node[direntry] (dir28) at ([yshift=-2cm]dirffbase) {};

\node[dirtable] (dirb5base) at (7,2.5) {};
\node[direntry] (dirb5) at ([yshift=-0.3cm]dirb5base) {};

% Page Table instances
\node[pagetable] (pte2base) at (10,8) {};
\node[font=\scriptsize, anchor=south] at (pte2base.north) {Page Table};
\node[pageentry] (ptee2) at ([yshift=-0.3cm]pte2base) {};
\node[pageentry] (pte68) at ([yshift=-2cm]pte2base) {};

\node[pagetable] (pte49base) at (10,3.5) {};
\node[pageentry] (pte49) at ([yshift=-0.2cm]pte49base) {};
\node[pageentry] (pte54) at ([yshift=-2cm]pte49base) {};

% 4KB Pages with offsets
\node[page] (pageb25base) at (13,8.8) {};
\node[font=\scriptsize, anchor=south] at (pageb25base.north) {4KB Page};
\node[pageoffset] (pageb25) at ([yshift=-0.3cm]pageb25base) {};
\node[pageoffset] (page349) at ([yshift=-1.1cm]pageb25base) {};

\node[page] (page937base) at (13,5.8) {};
\node[pageoffset] (page937) at ([yshift=-0.2cm]page937base) {};

\node[page] (pagec00base) at (13,3.5) {};
\node[pageoffset] (pagec00) at ([yshift=-0.2cm]pagec00base) {};

\node[page] (page622base) at (13,1.2) {};
\node[pageoffset] (page622) at ([yshift=-0.2cm]page622base) {};

% Paths from address fields to table entries
% PML4 entries
\draw[arrow] (pml4label) -- ++(0, -0.8) -| ([xshift=-10mm]pml4d3.west) -- (pml4d3.west) node[near end, above, font=\tiny] {D3};
\draw[arrow] (pml4label) -- ++(0, -0.8) -| ([xshift=-10mm]pml471.west) -- (pml471.west) node[near end, above, font=\tiny] {71};

% PDP entries
\draw[arrow] (pdplabel) -- ++(0, -0.8) -| ([xshift=-10mm]pdp82.west) -- (pdp82.west) node[near end, above, font=\tiny] {82};
\draw[arrow] (pdplabel) -- ++(0, -0.8) -| ([xshift=-10mm]pdp46.west) -- (pdp46.west) node[near end, above, font=\tiny] {46};

% DIR entries
\draw[arrow] (dirlabel) -- ++(0, -0.8) -| ([xshift=-10mm]dirff.west) -- (dirff.west) node[near end, above, font=\tiny] {FF};
\draw[arrow] (dirlabel) -- ++(0, -0.8) -| ([xshift=-10mm]dir28.west) -- (dir28.west) node[near end, above, font=\tiny] {28};
\draw[arrow] (dirlabel) -- ++(0, -0.8) -| ([xshift=-10mm]dirb5.west) -- (dirb5.west) node[near end, above, font=\tiny] {B5};

% TABLE entries
\draw[arrow] (tablelabel) -- ++(0, -0.8) -| ([xshift=-10mm]ptee2.west) -- (ptee2.west) node[near end, above, font=\tiny] {E2};
\draw[arrow] (tablelabel) -- ++(0, -0.8) -| ([xshift=-10mm]pte68.west) -- (pte68.west) node[near end, above, font=\tiny] {68};
\draw[arrow] (tablelabel) -- ++(0, -0.8) -| ([xshift=-10mm]pte49.west) -- (pte49.west) node[near end, above, font=\tiny] {49};
\draw[arrow] (tablelabel) -- ++(0, -0.8) -| ([xshift=-10mm]pte54.west) -- (pte54.west) node[near end, above, font=\tiny] {54};

% OFFSET entries
\draw[arrow] (offsetlabel) -- ++(0, -0.8) -| ([xshift=-10mm]pageb25.west) -- (pageb25.west) node[near end, above, font=\tiny] {B25};
\draw[arrow] (offsetlabel) -- ++(0, -0.8) -| ([xshift=-10mm]page349.west) -- (page349.west) node[near end, above, font=\tiny] {349};
\draw[arrow] (offsetlabel) -- ++(0, -0.8) -| ([xshift=-10mm]page937.west) -- (page937.west) node[near end, above, font=\tiny] {937};
\draw[arrow] (offsetlabel) -- ++(0, -0.8) -| ([xshift=-10mm]pagec00.west) -- (pagec00.west) node[near end, above, font=\tiny] {C00};
\draw[arrow] (offsetlabel) -- ++(0, -0.8) -| ([xshift=-10mm]page622.west) -- (page622.west) node[near end, above, font=\tiny] {622};

% Arrows connecting the hierarchy
% PML4[D3] -> PDP
\draw[arrow] (pml4d3.east) -- ++(0.3, 0) |- (pdp82base.south west);
% PML4[71] -> PDP[46] (dashed to show it's the same PDP table, different entry)
\draw[arrow] (pml471.east) -- ++(0.3, 0) |- (pdp82base.south west);
...
% Dir[B5] -> same PT[49,54]
\draw[arrow] (dirb5.east) -- ++(0.3, 0) |- (pte49base.south west);

% PT[E2] -> Page[B25,349]
\draw[arrow] (ptee2.east) -- ++(0.3, 0) |- (pageb25base.south west);
% PT[68] -> Page[937]
...
% Right side: Complete virtual addresses (color-coded)
\node[font=\small\ttfamily, anchor=west] at (16,8.5) {
\textcolor{green!40!black}{00000} \textcolor{yellow!40!black}{D3} \textcolor{red!40!black}{82} \textcolor{blue!20!black}{FF} \textcolor{blue!40!black}{E2} \textcolor{orange!60!black}{B25}
};
\node[font=\small\ttfamily, anchor=west] at (16,7.7) {
\textcolor{green!40!black}{00000} \textcolor{yellow!40!black}{D3} \textcolor{red!40!black}{82} \textcolor{blue!20!black}{FF} \textcolor{blue!40!black}{E2} \textcolor{orange!60!black}{349}
};
\node[font=\small\ttfamily, anchor=west] at (16,5.6) {
\textcolor{green!40!black}{00000} \textcolor{yellow!40!black}{D3} \textcolor{red!40!black}{82} \textcolor{blue!20!black}{FF} \textcolor{blue!40!black}{68} \textcolor{orange!60!black}{937}
};
\node[font=\small\ttfamily, anchor=west] at (16,3.3) {
\textcolor{green!40!black}{00000} \textcolor{yellow!40!black}{D3} \textcolor{red!40!black}{82} \textcolor{blue!20!black}{28} \textcolor{blue!40!black}{49} \textcolor{orange!60!black}{C00}
};
\node[font=\small\ttfamily, anchor=west] at (16,1) {
\textcolor{green!40!black}{00000} \textcolor{yellow!40!black}{71} \textcolor{red!40!black}{46} \textcolor{blue!20!black}{B5} \textcolor{blue!40!black}{54} \textcolor{orange!60!black}{622}
};

% Labels at top right
\node[font=\tiny, anchor=west] at (15.5,10) {PML4 PDP DIR PTE offset};

% Bottom left: Note about OS dynamic allocation
\node[font=\scriptsize, text width=4cm, align=left] at (-1.5,5) {
This example shows\\
...
\begin{center}
\begin{tikzpicture}[
    pte/.style={draw, thick, minimum width=40mm, minimum height=2.5mm, font=\tiny, inner sep=0pt},
    pte_regular/.style={pte, fill=gray!20},
    pte_contig/.style={pte, fill=yellow!30},
    ...
    \node[draw, thick, minimum width=50mm, minimum height=4mm] (va) at (0,0) {};
    \node[above=1mm of va, font=\tiny] {Virtual Address (48-bit)};
    
    % VA sections - showing the ContPTE-relevant bits
    \fill[blue!20] ([xshift=0mm]va.west) rectangle ([xshift=15mm]va.west |- va.north);
    \fill[orange!20] ([xshift=15mm]va.west) rectangle ([xshift=32mm]va.west |- va.north);
    \fill[yellow!20] ([xshift=32mm]va.west) rectangle ([xshift=38mm]va.west |- va.north);
    \fill[gray!20] ([xshift=38mm]va.west) rectangle ([xshift=50mm]va.west |- va.north);
    
    \node[font=\tiny] at ([xshift=7.5mm]va.west) {[47:16]};
    \node[font=\tiny] at ([xshift=23.5mm]va.west) {VPN[15:4]};
    \node[font=\tiny] at ([xshift=35mm]va.west) {[3:0]};
    \node[font=\tiny] at ([xshift=44mm]va.west) {Offset};
    ...
    % Regular PTEs (0-15)
    \foreach \i in {0,...,15} {
        \node[pte_regular, anchor=north] (pte\i) at ([yshift=-\i*2.7mm]pt_title.south) {};
        \node[font=\tiny, anchor=west] at ([xshift=1mm]pte\i.west) {\i};
        \draw ([xshift=5mm]pte\i.west) -- ([xshift=5mm]pte\i.west |- pte\i.south);
        \node[font=\tiny] at (pte\i.center) {PFN};
        \draw ([xshift=-4mm]pte\i.east) -- ([xshift=-4mm]pte\i.east |- pte\i.south);
    }
    
    % Gap before aligned block
    \coordinate (gap_start) at ([yshift=-1mm]pte15.south);
    
    % Contiguous block (PTEs 16-31) - aligned
    \foreach \i in {0,...,15} {
        \pgfmathsetmacro\idx{int(\i + 16)}
        \node[pte_contig, anchor=north] (pte_c\i) at ([yshift=-\i*2.7mm]gap_start) {};
        \node[font=\tiny, anchor=west] at ([xshift=1mm]pte_c\i.west) {\idx};
        \draw ([xshift=5mm]pte_c\i.west) -- ([xshift=5mm]pte_c\i.west |- pte_c\i.south);
        \node[font=\tiny] at (pte_c\i.center) {PFN+\i};
        \draw ([xshift=-4mm]pte_c\i.east) -- ([xshift=-4mm]pte_c\i.east |- pte_c\i.south);
        \fill[red!40] ([xshift=-4mm]pte_c\i.east) rectangle (pte_c\i.south east);
        \node[font=\tiny] at ([xshift=-2mm]pte_c\i.east) {C};
    }
    
    % Red border around contiguous block
    \draw[thick, red, line width=1.2pt] 
        ([shift={(-0.5mm,0.5mm)}]pte_c0.north west) rectangle 
        ([shift={(0.5mm,-0.5mm)}]pte_c15.south east);
    
    % More regular PTEs (32+)
    \foreach \i in {0,...,3} {
        \pgfmathsetmacro\idx{int(\i + 32)}
        \node[pte_regular, anchor=north] (pte_r\i) at ([yshift=-1mm-\i*2.7mm]pte_c15.south) {};
        \node[font=\tiny, anchor=west] at ([xshift=1mm]pte_r\i.west) {\idx};
    }
    
    % Arrow with label for contiguous block
    \draw[->, thick, orange] (pte_c7.east) -- ++(8mm,0) 
        node[midway, above, font=\tiny, align=center] {16 aligned\\4KB pages\\= 64KB};
    \node[font=\tiny, red] at ([xshift=6mm]pte_c0.east) {Aligned at 16};
    ...
    % Lookup arrows from VA
    \draw[->, thick, blue] ([xshift=-10mm, yshift=-2mm]va.south) -- ++(0,-4mm) -| (pt_title.north)
        node[pos=0.5, left, font=\tiny] {Lookup};
    \draw[->, thick, blue] ([xshift=10mm, yshift=-2mm]va.south) -- ++(0,-4mm) -| (tlb_title.north)
        node[pos=0.5, right, font=\tiny] {TLB check};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
