...
\begin{tikzpicture}[
    ...
    \draw[dashed, thick, blue!50] (ptAbox.east) -- (ptAphys.west)
        node[midway, above, font=\tiny, text=blue!50] {resides in};
    \draw[dashed, thick, green!50] (ptBbox.east) -- (ptBphys.west)
        node[midway, below, font=\tiny, text=green!50] {resides in};
...
\begin{minted}[fontsize=\scriptsize, frame=single, bgcolor=gray!10]{python}
# OS: Present bit = 0
def handle_non_present_page(vaddr):
    vpn = vaddr >> PAGE_BITS
...
def handle_protection_violation(vaddr, access):
    vpn = vaddr >> PAGE_BITS

    # Copy-on-Write: shared page,
    # write triggers private copy
    if is_cow_page(vpn) and access == WRITE:
    ...
    if tracking_dirty(vpn) and access == WRITE:
        pte[vpn].dirty = 1
        pte[vpn].writable = 1
        return_to_user()
...
\begin{tikzpicture}[scale=0.75, every node/.style={scale=0.75},
    % Base style for all table cells
    cell/.style={draw, minimum height=0.32cm, inner sep=0pt, font=\tiny, text depth=0pt, text height=0.22cm, anchor=center},
    % TLB base column styles
    tlbvalidbase/.style={cell, minimum width=0.8cm, inner sep=0pt},
    tlbtagbase/.style={cell, minimum width=0.6cm},
    tlbacbase/.style={cell, minimum width=0.7cm},
    tlbpfnbase/.style={cell, minimum width=1.5cm, inner sep=0pt},
    % TLB valid/invalid variants
    tlbvalid/.style={tlbvalidbase, fill=green!30},
    tlbinvalid/.style={tlbvalidbase, fill=gray!20},
    tlbtag/.style={tlbtagbase, fill=green!30},
    tlbtaginvalid/.style={tlbtagbase, fill=gray!20},
    tlbac/.style={tlbacbase, fill=green!30},
    tlbacinvalid/.style={tlbacbase, fill=gray!20},
    tlbpfn/.style={tlbpfnbase, fill=green!30},
    tlbpfninvalid/.style={tlbpfnbase, fill=gray!20},
    % Page table base column styles
    ptpresentbase/.style={cell, minimum width=1cm, inner sep=0},
    ptacbase/.style={cell, minimum width=0.7cm},
    ptdatabase/.style={cell, minimum width=1.6cm},
    % Page table valid/invalid variants
    ptgreen/.style={ptpresentbase, fill=green!30},
    ptred/.style={ptpresentbase, fill=red!30},
    ptacgreen/.style={ptacbase, fill=green!30},
    ptacred/.style={ptacbase, fill=red!30},
    ptdatagreen/.style={ptdatabase, fill=green!30},
    ptdatared/.style={ptdatabase, fill=red!30},
    % Physical memory
    ppage/.style={cell, fill=green!30, minimum width=1.7cm},
    % Labels
    mytitle/.style={font=\small\bfseries, text=gray!70!black},
    colheader/.style={font=\tiny\bfseries, text=gray!70!black, minimum height=0.28cm},
    arrow/.style={-Stealth, thick},
    dotarrow/.style={{Circle[length=2pt]}-Stealth, thick},
    orangearrow/.style={-Stealth, thick, orange}
]

% TLB - positioned first as reference
\matrix[matrix of nodes, row sep=-\pgflinewidth, column sep=-\pgflinewidth, ampersand replacement=\&,
    label={[mytitle, label distance=-1mm]above:TLB}] (tlb) {
    |[colheader]| Valid \& |[colheader]| Tag \& |[colheader]| AC \& |[colheader]| PFN \\
    |[tlbinvalid]| 0 \& |[tlbtaginvalid]| \& |[tlbacinvalid]| \& |[tlbpfninvalid]| \\
    |[tlbvalid]| 1 \& |[tlbtag]| \& |[tlbac]| \& |[tlbpfn]| \\
    |[tlbvalid]| 1 \& |[tlbtag]| \& |[tlbac]| \& |[tlbpfn]| \\
    |[tlbvalid]| 1 \& |[tlbtag]| \& |[tlbac]| \& |[tlbpfn]| \\
    |[tlbvalid]| 1 \& |[tlbtag]| \& |[tlbac]| \& |[tlbpfn]| \\
    |[tlbinvalid]| 0 \& |[tlbtaginvalid]| \& |[tlbacinvalid]| \& |[tlbpfninvalid]| \\
...
% Virtual Page Number box - positioned above-left, so connector goes down then splits
\node[draw, fill=blue!15, minimum width=2cm, minimum height=0.5cm, font=\scriptsize,
    left=2mm of tlb.north west, anchor=south east] (vpn)
    {Virtual page number};

% Page Table - positioned below TLB
\matrix[matrix of nodes, row sep=-\pgflinewidth, column sep=-\pgflinewidth, ampersand replacement=\&,
    below=0.6cm of tlb,
    label={[mytitle, label distance=-2mm]above:Page Table}] (pt) {
    |[colheader]| Present \& |[colheader]| AC \& |[colheader]| PFN/Disk \\
    |[ptgreen]| 1 \& |[ptacgreen]| \& |[ptdatagreen]| \\
    |[ptgreen]| 1 \& |[ptacgreen]| \& |[ptdatagreen]| \\
    |[ptgreen]| 1 \& |[ptacgreen]| \& |[ptdatagreen]| \\
    |[ptgreen]| 1 \& |[ptacgreen]| \& |[ptdatagreen]| \\
    |[ptred]| 0 \& |[ptacred]| \& |[ptdatared]| \\
    |[ptgreen]| 1 \& |[ptacgreen]| \& |[ptdatagreen]| \\
    |[ptgreen]| 1 \& |[ptacgreen]| \& |[ptdatagreen]| \\
    |[ptred]| 0 \& |[ptacred]| \& |[ptdatared]| \\
    |[ptgreen]| 1 \& |[ptacgreen]| \& |[ptdatagreen]| \\
...
% Physical Memory - positioned to the right, lower (centered between TLB and PT)
\matrix[matrix of nodes, row sep=-\pgflinewidth, ampersand replacement=\&,
    right=1.8cm of tlb.south east, anchor=north west, yshift=6mm,
    label={[mytitle, label distance=-1mm, align=center]above:Physical\\Memory}] (pp) {
    |[ppage]| \\
    |[ppage]| \\
    |[ppage]| \\
    |[ppage]| \\
...
% Disk - positioned below Physical Memory
\node[mytitle, below=0.3cm of pp] (disklabel) {Disk};
\begin{scope}[shift={(disklabel.south)}, yshift=-0.2cm]
    \draw[thick, fill=blue!10] (0, 0) ellipse (0.9cm and 0.22cm);
    \draw[thick, fill=blue!10] (-0.9, 0) -- (-0.9, -1.2);
    \draw[thick, fill=blue!10] (0.9, 0) -- (0.9, -1.2);
    \draw[thick, fill=blue!10] (0, -1.2) ellipse (0.9cm and 0.22cm);
    % Disk slots (red for swapped pages)
    \fill[red!30] (-0.85, -0.25) rectangle (0.85, -0.5);
    \draw (-0.85, -0.25) rectangle (0.85, -0.5);
    \fill[red!30] (-0.85, -0.55) rectangle (0.85, -0.8);
    \draw (-0.85, -0.55) rectangle (0.85, -0.8);
    \fill[red!30] (-0.85, -0.85) rectangle (0.85, -1.1);
    \draw (-0.85, -0.85) rectangle (0.85, -1.1);
\end{scope}

% Connection from VPN - line goes down, then splits right to TLB and down to PT
\coordinate (splitpoint) at (vpn |- tlb);
\draw[thick] (vpn.south) -- (splitpoint);
\fill (splitpoint) circle (2pt);

% VPN to TLB - go right to TLB
\draw[arrow] (splitpoint) -- (tlb.west);

% VPN to Page Table - go down then right to PT
\draw[arrow] (splitpoint) |- (pt.west);

% Orange arrows from TLB (valid entries) to Physical Memory (PFN is now column 4)
\draw[orangearrow] (tlb-3-4.east) -- (pp-2-1.west);
\draw[orangearrow] (tlb-4-4.east) -- (pp-4-1.west);
\draw[orangearrow] (tlb-5-4.east) -- (pp-3-1.west);
\draw[orangearrow] (tlb-6-4.east) -- (pp-5-1.west);
\draw[orangearrow] (tlb-8-4.east) -- (pp-6-1.west);

% Black arrows from Page Table (present=1) to Physical Memory (data is now column 3)
\draw[dotarrow] (pt-2-3.east) -- (pp-1-1.west);
\draw[dotarrow] (pt-3-3.east) -- (pp-3-1.west);
\draw[dotarrow] (pt-4-3.east) -- (pp-2-1.west);
\draw[dotarrow] (pt-5-3.east) -- (pp-5-1.west);
\draw[dotarrow] (pt-7-3.east) -- (pp-7-1.west);
\draw[dotarrow] (pt-8-3.east) -- (pp-4-1.west);
\draw[dotarrow] (pt-10-3.east) -- (pp-6-1.west);
\draw[dotarrow] (pt-12-3.east) -- (pp-7-1.west);

% Dashed arrows from Page Table (present=0) to Disk - straight lines (data is now column 3)
\coordinate (diskslot1) at ($(disklabel.south) + (-0.85, -0.6)$);
\coordinate (diskslot2) at ($(disklabel.south) + (-0.85, -0.95)$);
\coordinate (diskslot3) at ($(disklabel.south) + (-0.85, -1.3)$);
\draw[dotarrow, dashed] (pt-6-3.center) -- (diskslot1);
\draw[dotarrow, dashed] (pt-9-3.center) -- (diskslot2);
\draw[dotarrow, dashed] (pt-11-3.center) -- (diskslot3);
...
\begin{itemize}
\item L1 caches have \alert{separate TLBs}: ITLB (for I\$) and DTLB (for D\$)
    \begin{itemize}
    \item Accessed in different pipeline stages
    \end{itemize}
\item TLB miss $\rightarrow$ check \alert{STLB} (Second-Level TLB)
\item STLB miss $\rightarrow$ \alert{Page Walk}:
    \begin{itemize}
    \item PMH (Page Miss Handler) fetches PTE from page table
    \item Uses regular memory loads: L1 D\$ $\rightarrow$ L2 $\rightarrow$ \ldots
    \end{itemize}
\end{itemize}
\begin{center}
...
\node[cache, fill=orange!30, right=0.7cm of l1inst, minimum width=2.5cm] (l1data) {L1 data cache};

% TLBs
\node[tlb, fill=cyan!30, below=0.4cm of l1inst] (insttlb) {Inst. TLB};
\node[tlb, fill=violet!30, below=0.4cm of l1data.south west, anchor=north west] (datatlb) {Data TLB};

% STLB in PMH box
\node[stlbcache, below=0.5cm of insttlb, xshift=1cm] (stlb) {STLB};

% PMH and Page Walk Logic
\node[rectangle, draw, thick, fill=yellow!20, minimum height=0.8cm, minimum width=1.5cm,
      right=2.5cm of stlb, inner sep=0pt, align=center] (pmh) {Page\\Walker};
\node[boxlabel, anchor=north west] (pmhlabel) at (pmh.north east) {PMH};

% Fit box around STLB and PMH
\begin{scope}[on background layer]
\node[bigbox, fill=gray!10, fit=(stlb)(pmh)(pmhlabel), inner sep=0.5pt] (transbox) {};
\end{scope}

% L2, L3, and Memory
\node[cache, fill=gray!30, right=1.7cm of l1data, minimum width=1cm] (l2) {L2};
\node[cache, fill=green!20, right=of l2, minimum width=1cm] (l3) {L3};
\node[cache, fill=green!30, right=of l3, minimum width=1.5cm] (memory) {Memory};

% Labels above components
\node[boxlabel, anchor=south west] at ([yshift=0mm]l1inst.north west) (corelabel) {Core};
\node[boxlabel, above=0mm of l3] (ondielabel) {On-die};
\node[boxlabel, above=0mm of memory] (platformlabel) {Platform};

% Core box
\node[bigbox, fit=(corelabel)(l1inst)(l1data)(insttlb)(datatlb)(transbox)(pmhlabel)(l2),
      inner ysep=1mm, inner xsep=3mm] (corebox) {};

% On-die box
\node[bigbox, fit=(ondielabel)(l3), inner ysep=1mm, inner xsep=3mm] (ondiebox) {};

% Platform box
\node[bigbox, fit=(platformlabel)(memory), inner ysep=1mm, inner xsep=3mm] (platformbox) {};

% Arrows and connections
\draw[arrow, blue] ([yshift=-1mm]l2.north west) to[bend right=13] node[near start, right=4mm, label] {Instruction bytes} ([yshift=-1mm]l1inst.north east);
...
% VA from STLB to PMH
\draw[arrow] (stlb.rpin 1) -- node[arrowlabel] {VA} (pmh.west |- stlb.rpin 1);

% PTE from PMH to STLB
\draw[arrow, red, thick] (pmh.west |- stlb.rpin 2) -- node[arrowlabel] {PTE} (stlb.rpin 2);

% Load arrow from L1 data cache to PMH (Page Walk Logic)
\draw[arrow, red, thick] ([xshift=-3mm]l1data.south east) -- ++(0,-8mm) -| node[near end, right, label] {Load} (pmh.north);

% L1 data cache to L2
\draw[arrow, blue, thick] (l2.west) -- (l1data.east) node[midway, below, blue, label] {Data \& PTEs};
...
\draw[arrow] (tlb.east) -- ++(0.3,0) |- node[right] {Yes} (pfn);
\draw[arrow] (stlb.west) -- ++(-0.5, 0) |- node[above] {No} (pmh.east);
...
\begin{tikzpicture}[
    node distance=5mm,
    addr/.style={draw, rectangle, minimum height=0.35cm, inner sep=1pt, font=\tiny, text depth=0pt},
    tlbcell/.style={draw, minimum width=4mm, minimum height=2mm, inner sep=0pt, font=\tiny, text depth=0pt},
    cachecell/.style={draw, minimum width=4mm, minimum height=2mm, inner sep=0pt, font=\tiny, text depth=0pt},
    comp/.style={circle, draw, inner sep=0pt, minimum size=3mm, font=\tiny, text depth=0pt},
...
% TLB Tag and TLB Set below VPN
\node[addr, fill=green!20, minimum width=2cm, below=3mm of vpn.west, anchor=west] (vtag) {TLB Tag};
\node[addr, fill=cyan!20, minimum width=1cm, right=0pt of vtag] (vset) {TLB Set};
...
\foreach \way in {0,...,1} {
    \coordinate (tlbtagjunc-\way) at ([xshift={-1mm + 0.3*\way}, yshift=3mm]tlbcomp-\way.north);
    \draw[arrow, blue] (vtag.south)
        |- ([xshift=5mm, yshift=-2mm] vtag.south west)
        |- (tlbtagjunc-\way)
        -- ([xshift={-1mm + 0.3*\way}]tlbcomp-\way.north);
    \fill[blue] (tlbtagjunc-\way) circle (1pt);
}
...
% Comparator outputs to OR gate for hit detection (swapped pins)
\foreach \way in {0,...,1} {
    \pgfmathtruncatemacro{\inputnum}{2 - \way}
    \pgfmathtruncatemacro{\muxpin}{1 + \way}
    % Junction at the corner where wire turns into MUX
    \coordinate (tlbjunc-\way) at (tlbcomp-\way.south |- tlbmux.btpin \muxpin);
    \draw[arrow] (tlbcomp-\way.south) -- (tlbjunc-\way) -- (tlbmux.btpin \muxpin);
    % Staggered route to OR gate
    \draw[arrow] (tlbjunc-\way) -- ++(0,-1mm - \way*1mm) -| (tlbor.bin \inputnum);
    \fill (tlbjunc-\way) circle (1pt);
}
...
\foreach \way in {0,...,3} {
    \coordinate (ppnturn-\way) at ([xshift={-1mm}]ccomp-\way.north |- ppn.east);
    \draw[arrow, blue, thick] (ppn.east) -| ([xshift={-1mm}]ccomp-\way.north);
    \fill[blue] (ppnturn-\way) circle (1pt);
}
...
% Cache comparator outputs to OR gate and MUX (with junction dots at turn points)
\foreach \way in {0,...,3} {
    \pgfmathtruncatemacro{\orpin}{4 - \way}
    \pgfmathtruncatemacro{\muxpin}{1 + \way}
    % Junction at the corner where wire turns into MUX
    \coordinate (cachejunc-\way) at (ccomp-\way.south |- cachemux.btpin \muxpin);
    \draw[arrow] (ccomp-\way.south) -- (cachejunc-\way) -- (cachemux.btpin \muxpin);
    % Staggered route to OR gate
    \draw[arrow] (cachejunc-\way) -- ++(0,-1mm - \way*1mm) -| (cacheor.bin \orpin);
    \fill (cachejunc-\way) circle (1pt);
}
...
def evict_page_to_disk(victim_vpn):
    # 1. Update page table
    pte[victim_vpn].present = 0

    # 2. TLB invalidate
    tlb_invalidate(victim_vpn)
...
def context_switch(proc_A, proc_B):
...
\section{Modern Paging}

\begin{frame}{}
\begin{center}
\Huge Modern Paging\\[3mm]
\large (x86-64 as an example)
\end{center}
\end{frame}
...
\begin{column}{0.44\textwidth}
\begin{itemize}
\item \textbf{A page directory entry} (2\textsuperscript{nd} level table) has a ``page size'' bit
\item \textbf{If page size = 1}
    \begin{itemize}
    \item Entry points to a 2MB page
    \item VA bits[20:0] used as offset within page
    \end{itemize}
\item \textcolor{gray}{\textbf{If page size = 0}}
    \begin{itemize}
    \item \textcolor{gray}{Entry points to page table base}
    \item \textcolor{gray}{Page table maps 512 pages of 4KB}
    \item \textcolor{gray}{Total: $512 \times 4\text{KB} = 2\text{MB}$}
    \item \textcolor{gray}{VA bits[20:12] select entry in PT}
    \item \textcolor{gray}{VA bits[11:0] offset in 4KB page}
    \end{itemize}
\end{itemize}
\end{column}
...
\begin{tikzpicture}[scale=0.72,
    table/.style={rectangle, draw, thick, minimum width=0.9cm, fill=blue!15},
    entry/.style={rectangle, draw, thick, minimum width=0.9cm, minimum height=0.3cm, fill=blue!30},
    data/.style={rectangle, draw, thick, minimum width=0.9cm, minimum height=1.6cm, fill=orange!15},
    arrow/.style={->, thick},
    dashedarrow/.style={->, thick, dashed}]
    ...
    % Address fields with colors - 2MB version (no TABLE field)
    \fill[green!20] (0,7.5) rectangle (2,8.3);
    \fill[blue!20] (2,7.5) rectangle (3.4,8.3);
    \fill[blue!20] (3.4,7.5) rectangle (4.8,8.3);
    \fill[blue!20] (4.8,7.5) rectangle (6.2,8.3);
    \fill[orange!20] (6.2,7.5) rectangle (11,8.3);
    ...
    \node[font=\tiny] at (4.1,7.9) {PDP};
    \node[font=\tiny] at (5.5,7.9) {DIR};
    \node[font=\tiny] at (8.6,7.9) {OFFSET};

    % Page table hierarchy - 3 levels for 2MB pages (same style as 4KB version)
    % PML4 Table (lowest)
    \node[table, minimum height=1.6cm] (pml4) at (2,3.2) {};
    \node[font=\tiny] at (2,3.4) {PML4};
    \node[font=\tiny] at (2,3.1) {Table};
    ...
    % PDP Table (higher)
    \node[table, minimum height=1.6cm] (pdp) at (4.5,3.9) {};
    \node[font=\tiny] at (4.5,4.2) {Page};
    \node[font=\tiny] at (4.5,3.9) {Directory};
    \node[font=\tiny] at (4.5,3.6) {Pointer};
    \node[entry] (pdpen) at (4.5,3.0) {};
    \node[font=\tiny] at (pdpen) {PDP en};

    % Page Directory (higher) - directly points to 2MB page
    \node[table, minimum height=1.6cm] (dir) at (7,4.6) {};
    \node[font=\tiny] at (7,4.9) {Page};
    \node[font=\tiny] at (7,4.6) {Directory};
    \node[entry] (pde) at (7,3.8) {};
    \node[font=\tiny] at (pde) {PDE};

    % 2MB Data Page (highest) - replaces Page Table level
    \node[data, minimum height=1.6cm] (datapage) at (9.5,5.3) {};
    \node[font=\tiny] at (9.5,5.8) {2MByte};
    \node[font=\tiny] at (9.5,5.4) {Page};
    \node[entry, fill=orange!30] (databox) at (9.5,4.7) {};
    \node[font=\tiny] at (databox) {data};

    % CR3
    \node[rectangle, draw, thick, minimum width=1.2cm, minimum height=0.4cm] (cr3) at (0.5,1.2) {\tiny CR3 (PDPTR)};

    % Arrows from address fields to table entries (same style as 4KB)
    % PML4 arrow with "9"
    \draw[arrow] (2.7,7.5) -- (2.7,6.5) -- (0.8,6.5) -- (0.8,2.5) -- (pml4en.west);
    \node[font=\tiny] at (3,7.3) {9};
    \draw[thick] (2.65,7.2) -- (2.75,7.15);
    ...
    % PDP arrow with "9"
    \draw[arrow] (4.1,7.5) -- (4.1,6.2) -- (3.3,6.2) -- (3.3,3.0) -- (pdpen.west);
    \node[font=\tiny] at (4.4,7.3) {9};
    \draw[thick] (4.05,7.2) -- (4.15,7.15);

    % DIR arrow with "9"
    \draw[arrow] (5.5,7.5) -- (5.5,5.9) -- (5.8,5.9) -- (5.8,3.8) -- (pde.west);
    \node[font=\tiny] at (5.8,7.3) {9};
    \draw[thick] (5.45,7.2) -- (5.55,7.15);

    % OFFSET arrow to data box with "21"
    \draw[arrow] (8.6,7.5) |- (databox.west);
    \node[font=\tiny] at (8.9,7.3) {21};
    \draw[thick] (8.55,7.2) -- (8.65,7.15);

    % Arrows connecting the hierarchy
    % CR3 to PML4 Table
    \draw[dashedarrow] (cr3.east) -| (pml4.south);
    \draw[thick] ($(cr3.east) + (0.15,0.05)$) -- ($(cr3.east) + (0.25,-0.05)$);
    \node[font=\tiny] at ($(cr3.east) + (1.2,-0.3)$) {M-12 (4KB aligned)};

    % PML4 en to PDP Table
    \draw[dashedarrow] (pml4en.east) -| (pdp.south);
    \draw[thick] ($(pml4en.east) + (0.35,0.05)$) -- ($(pml4en.east) + (0.45,-0.05)$);
    \node[font=\tiny] at ($(pml4en.east) + (0.5,-0.2)$) {M-12};

    % PDP en to Page Directory
    \draw[dashedarrow] (pdpen.east) -| (dir.south);
    \draw[thick] ($(pdpen.east) + (0.35,0.05)$) -- ($(pdpen.east) + (0.45,-0.05)$);
    \node[font=\tiny] at ($(pdpen.east) + (0.5,-0.2)$) {M-12};

    % PDE to 2MB Page - M-21 bits (2MB aligned)
    \draw[dashedarrow] (pde.east) -| (datapage.south);
    \draw[thick] ($(pde.east) + (0.35,0.05)$) -- ($(pde.east) + (0.45,-0.05)$);
    \node[font=\tiny] at ($(pde.east) + (0.8,-0.2)$) {M-21};
...
\begin{column}{0.44\textwidth}
\begin{itemize}
\item \textbf{A PDP entry} (3\textsuperscript{rd} level table) has a ``page size'' bit
\item \textbf{If page size = 1}
    \begin{itemize}
    \item Entry points to a 1GB page
    \item VA bits[29:0] used as offset within page
    \end{itemize}
\item \textcolor{gray}{\textbf{If page size = 0}}
    \begin{itemize}
    \item \textcolor{gray}{Entry points to page directory base}
    \item \textcolor{gray}{Page directory maps a total of 1GB}
    \end{itemize}
\end{itemize}
\end{column}
...
\begin{tikzpicture}[scale=0.72,
    ...
    % Address fields with colors - 1GB version (no DIR or TABLE fields)
    \fill[green!20] (0,7.5) rectangle (2,8.3);
    \fill[blue!20] (2,7.5) rectangle (3.4,8.3);
    \fill[blue!20] (3.4,7.5) rectangle (4.8,8.3);
    \fill[orange!20] (4.8,7.5) rectangle (11,8.3);
    ...
    \node[font=\tiny] at (2.7,7.9) {PML4};
    \node[font=\tiny] at (4.1,7.9) {PDP};
    \node[font=\tiny] at (7.9,7.9) {OFFSET};

    % Page table hierarchy - 2 levels for 1GB pages
    % PML4 Table (lowest)
    \node[table, minimum height=1.6cm] (pml4) at (2,3.2) {};
    \node[font=\tiny] at (2,3.4) {PML4};
    \node[font=\tiny] at (2,3.1) {Table};
    ...
    % PDP Table (higher) - directly points to 1GB page
    \node[table, minimum height=1.6cm] (pdp) at (5,3.9) {};
    \node[font=\tiny] at (5,4.2) {Page};
    \node[font=\tiny] at (5,3.9) {Directory};
    \node[font=\tiny] at (5,3.6) {Pointer};
    \node[entry] (pdpen) at (5,3.0) {};
    \node[font=\tiny] at (pdpen) {PDP en};

    % 1GB Data Page (highest) - replaces Page Directory level
    \node[data, minimum height=1.6cm] (datapage) at (9.2,4.6) {};
    \node[font=\tiny] at (9.2,5.1) {1GByte};
    \node[font=\tiny] at (9.2,4.7) {Page};
    \node[entry, fill=orange!30] (databox) at (9.2,4.0) {};
    \node[font=\tiny] at (databox) {data};

    % CR3
    \node[rectangle, draw, thick, minimum width=1.2cm, minimum height=0.4cm] (cr3) at (0.5,1.2) {\tiny CR3 (PDPTR)};

    % Arrows from address fields to table entries
    % PML4 arrow with "9"
    \draw[arrow] (2.7,7.5) -- (2.7,6.5) -- (0.8,6.5) -- (0.8,2.5) -- (pml4en.west);
    \node[font=\tiny] at (3,7.3) {9};
    \draw[thick] (2.65,7.2) -- (2.75,7.15);
    ...
    % PDP arrow with "9"
    \draw[arrow] (4.1,7.5) -- (4.1,6.2) -- (3.8,6.2) -- (3.8,3.0) -- (pdpen.west);
    \node[font=\tiny] at (4.4,7.3) {9};
    \draw[thick] (4.05,7.2) -- (4.15,7.15);

    % OFFSET arrow to data box with "30"
    \draw[arrow] (7.9,7.5) |- (databox.west);
    \node[font=\tiny] at (8.2,7.3) {30};
    \draw[thick] (7.85,7.2) -- (7.95,7.15);
    ...
    % PDP en to 1GB Page - M-30 bits (1GB aligned)
    \draw[dashedarrow] (pdpen.east) -| (datapage.south);
    \draw[thick] ($(pdpen.east) + (0.35,0.05)$) -- ($(pdpen.east) + (0.45,-0.05)$);
    \node[font=\tiny] at ($(pdpen.east) + (0.8,-0.2)$) {M-30};
...
\hline
\rowcolor{gray!20} & \textbf{4 KB Pages} & \textbf{2/4 MB Pages} & \textbf{1 GB Pages} \\
\hline
\textbf{Instruction TLBs} & 128 entries, 8 ways & 8 entries/thread & None \\
\hline
\textbf{Data TLBs} & 64 entries, 4 ways & 32 entries, 4 ways & 4 entries, 4 ways \\
\hline
\textbf{2nd Level TLB} & \multicolumn{2}{c|}{1536 entries, 12 ways (shared by 4 KB and 2/4 MB pages)} & 16 entries, 4 ways \\
\hline
\end{tabular}
\end{center}
...
\hline
\rowcolor{gray!20}\textbf{Cache} & \textbf{VA Bits} & \textbf{Returns} & \textbf{Saves Accesses To} \\
\hline
\textbf{PDE cache} & [47:21] & PDE & PTE \\
\hline
\textbf{PDP cache} & [47:30] & PDP entry & PDE and PTE \\
\hline
\textbf{PML4 cache} & [47:39] & PML4 entry & PDP, PDE, and PTE \\
\hline
\end{tabular}
\end{center}
...
\end{tikzpicture}
\end{frame}
\begin{frame}{Real-World Example: Intel Xeon Gold 5420+ (Sapphire Rapids)}
\scriptsize
\textbf{Cache Hierarchy}\tikzmark{cachetableleft}

\begin{center}
\begin{tabular}{llrrll}
\toprule
\rowcolor{gray!20}\textbf{Level} & \textbf{Type} & \textbf{Size} & \textbf{Line} & \textbf{Assoc.} & \textbf{Comments} \\
\midrule
L1 & Instruction & 32 KB & 64 B & 8-way & Per-core \\
L1 & Data & 48 KB & 64 B & 12-way & Per-core \\
L2 & \tikzmark{unified}Unified & 2 MB & 64 B & 16-way & Per-core \\
L3 & Unified & 52.5 MB \tikzmark{l3size} & 64 B & 15-way & Shared, complex indexing \\
\bottomrule
\end{tabular}\tikzmark{cachetablebelow}
\end{center}
...
\begin{tabular}{llrl}
\toprule
\rowcolor{gray!20}\textbf{Type} & \textbf{Page Size} & \textbf{Entries} & \textbf{Assoc.} \\
\midrule
Instruction & 4 KB & 256 & 8-way \\
Instruction & 2/4 MB\tikzmark{largepage} & 32 & 8-way \\
\tikzmark{loadtlb}Load & 4 KB & 64 & 4-way \\
Load & 2/4 MB & 32 & 4-way \\
Load & 1 GB & 8 & Full \\
\tikzmark{storetlb}Store & All sizes & 16 & Full\tikzmark{l1tlbse} \\
\bottomrule
\end{tabular}
\end{minipage}
\end{column}
\begin{column}{0.45\textwidth}
\centering
\tikzmark{stlbabove}\textbf{L2 STLB (Unified)}\tikzmark{stlb}

\begin{tabular}{lrl}
\toprule
\rowcolor{gray!20}\textbf{Page Sizes} & \textbf{Entries} & \textbf{Assoc.} \\
\midrule
4 KB / 2 MB / 4 MB & 1024 & 8-way \\
4 KB / 1 GB & 1024 & 8-way \\
\bottomrule
\end{tabular}
\end{column}
\end{columns}
...
\begin{tikzpicture}[remember picture, overlay,
    mycallout/.style={rectangle callout, draw, fill=yellow!20, font=\scriptsize}]
\node[mycallout, callout absolute pointer={(pic cs:loadtlb)}, anchor=south east, align=left]
    at ([shift={(-1mm,1mm)}]pic cs:storetlb) {Split L1 TLBs for\\load/store};
\node[mycallout, anchor=south west, callout absolute pointer={(pic cs:largepage)}]
    at ([shift={(3mm,-2mm)}]pic cs:l1tlbse) {Large pages reduce TLB pressure};
\node[mycallout, callout absolute pointer={(pic cs:stlb)}]
    at ([shift={(3cm,5mm)}]pic cs:stlbabove) {Backs up all L1 TLB misses};
\node[mycallout, anchor=east, callout absolute pointer={(pic cs:unified)}]
    at ([shift={(-1.5cm,2mm)}]pic cs:unified) {Instruction + Data};
\node[mycallout, callout absolute pointer={(pic cs:l3size)}]
    at ([shift={(2cm,-8mm)}]pic cs:l3size) {Memory is $\sim$1000$\times$ larger!};
\end{tikzpicture}
\end{frame}
...
\begin{itemize}
\item Cached translations lost $\rightarrow$ cold TLB
\end{itemize}

\textbf{Solution: Address Space ID (ASID)}
\begin{itemize}
\item Tags TLB entries with process ID
\item Lookup: VA + ASID must match
...
\begin{column}{0.5\textwidth}
\textbf{x86: PCID in CR3 (CR4.PCIDE=1)}\vspace{4mm}

\begin{bytefield}[bitwidth=\dimexpr\linewidth/64\relax, endianness=big, boxformatting={\centering\tiny}]{64}
\bitheader{63,12,0} \\
\bitbox{52}[bgcolor=green!20]{PML4 Base (4KB aligned)}
\bitbox{12}[bgcolor=blue!20]{PCID}
\end{bytefield}

\vspace{4mm}
\textbf{ARM: ASID in TTBR0\_EL1}\vspace{4mm}

\begin{bytefield}[bitwidth=\dimexpr\linewidth/64\relax, endianness=big, boxformatting={\centering\tiny}]{64}
\bitheader{63,48,0} \\
\bitbox{16}[bgcolor=blue!20]{ASID}
\bitbox{48}[bgcolor=green!20]{Translation Table Base}
\end{bytefield}
\end{column}
\end{columns}
\end{frame}
...
\item Hardware coalesces into a single TLB entry
\end{itemize}
...
\begin{tikzpicture}[
    ...
    pfn cell/.style={pte cell, minimum width=16mm},
    flag cell/.style={pte cell, minimum width=5mm},
    tlb_entry/.style={draw, thick, minimum width=20mm, minimum height=3.5mm, font=\tiny},
    section_title/.style={font=\tiny, anchor=south west}
]
    ...
    % VA fields as nodes for referencing
    \node[fill=blue!20, minimum height=6mm, minimum width=15mm, inner sep=0pt, anchor=west]
        (va_upper) at (va.west) {};
    \node[fill=orange!20, minimum height=6mm, minimum width=17.5mm, inner sep=0pt, anchor=west]
        (va_vpn) at (va_upper.east) {};
    \node[fill=yellow!20, minimum height=6mm, minimum width=6.5mm, inner sep=0pt, anchor=west]
        (va_idx) at (va_vpn.east) {};
    \node[fill=gray!20, minimum height=6mm, minimum width=11mm, inner sep=0pt, anchor=west]
        (va_offset) at (va_idx.east) {};

    \node[font=\tiny] at (va_upper.center) {Upper bits};
    \node[font=\tiny] at (va_vpn.center) {VPN[15:4]};
    \node[font=\tiny] at (va_idx.center) {[3:0]};
    \node[font=\tiny] at (va_offset.center) {Offset};

    % Outline around VPN[15:4] and [3:0]
    \node[draw, thick, red, fit=(va_vpn)(va_idx), inner sep=0pt] (va_contpte) {};

    % Callout for [3:0] - offset within 64KB
    \node[rectangle callout, draw, fill=yellow!20, font=\tiny, align=center,
        callout absolute pointer={(va_idx.north)}]
        at ([yshift=8mm, xshift=8mm]va_idx.north) {Offset within\\64KB entry};

    % Bit positions
    \node[below, font=\tiny] at (va.south west) {47};
    \node[below, font=\tiny] at (va_idx.south west) {16};
    \node[below, font=\tiny] at (va_offset.south west) {12};
    \node[below, font=\tiny] at (va.south east) {0};

    % Combined page table matrix with vdots
    \matrix[matrix of nodes,
        anchor=north,
        row sep=-\pgflinewidth,
        column sep=-\pgflinewidth,
    ...
    ] (pte_matrix) at (-1.5,-1.4) {
        |[fill=gray!20]| 0 \& |[fill=gray!20]| PFN \& |[fill=gray!20]| {} \\
        |[fill=gray!20]| 1 \& |[fill=gray!20]| PFN \& |[fill=gray!20]| {} \\
        |[fill=gray!20]| 2 \& |[fill=gray!20]| PFN \& |[fill=gray!20]| {} \\
        |[draw=none, fill=none]| \vdots \& |[draw=none, fill=none]| \vdots \& |[draw=none, fill=none]| {} \\
        |[fill=gray!20]| 15 \& |[fill=gray!20]| PFN \& |[fill=gray!20]| {} \\
        |[fill=yellow!30]| 16 \& |[fill=yellow!30]| PFN$_{64K}$+0 \& |[fill=red!40]| C \\
        |[fill=yellow!30]| 17 \& |[fill=yellow!30]| PFN$_{64K}$+1 \& |[fill=red!40]| C \\
        |[draw=none, fill=none]| \vdots \& |[draw=none, fill=none]| \vdots \& |[draw=none, fill=none]| {} \\
        |[fill=yellow!30]| 30 \& |[fill=yellow!30]| PFN$_{64K}$+14 \& |[fill=red!40]| C \\
        |[fill=yellow!30]| 31 \& |[fill=yellow!30]| PFN$_{64K}$+15 \& |[fill=red!40]| C \\
        |[fill=gray!20]| 32 \& |[fill=gray!20]| PFN \& |[fill=gray!20]| {} \\
        |[draw=none, fill=none]| \vdots \& |[draw=none, fill=none]| \vdots \& |[draw=none, fill=none]| {} \\
    };

    % Page table title (positioned relative to matrix)
    \node[section_title] (pt_title) at ([yshift=-2mm]pte_matrix.north west) {Page Table};

    % Red border around contiguous block (rows 6-10)
    \draw[thick, red, line width=1.2pt]
        ([shift={(-0.5mm,0.5mm)}]pte_matrix-6-1.north west) rectangle
        ([shift={(0.5mm,-0.5mm)}]pte_matrix-10-3.south east);

%    \node[font=\tiny, red, align=center] at ([xshift=10mm]pte_matrix-6-3.east) {64KB\\aligned};

    % TLB entries using matrix
    \matrix[matrix of nodes,
        anchor=north west,
        row sep=1mm,
        column sep=-\pgflinewidth,
        ampersand replacement=\&,
        nodes={tlb_entry},
    ] (tlb_matrix) at ([xshift=1.5cm]pte_matrix.north east) {
        |[fill=gray!20]| 4KB page \\
        |[fill=gray!20]| 4KB page \\
        |[fill=gray!20]| 4KB page \\
        |[fill=yellow!30, line width=1.2pt, minimum height=5mm]| 64KB coalesced \\
    };

   % Arrow with label for contiguous block
    \draw[->, thick, orange, dashed] (pte_matrix-8-3.east) -- 
        node[midway, above, font=\tiny, align=center] {16 aligned\\4KB pages\\= 64KB} (tlb_matrix-4-1.west);


    % TLB title (positioned relative to matrix)
    \node[section_title, anchor=south east] (tlb_title) at (tlb_matrix.north east) {TLB};

    % TLB box around matrix (tighter fit)
    \node[draw, thick, fit=(tlb_matrix), inner sep=1pt] (tlb_box) {};

    % Callout for coalesced entry (above, pointing to south)
    \node[rectangle callout, draw, fill=yellow!20, font=\tiny, align=center,
        callout absolute pointer={(tlb_matrix-4-1.south)}]
        at ([yshift=-8mm]tlb_matrix-4-1.south) {1 entry for\\PTEs 16-31};

    % Lookup arrows from VA to Page Table and TLB
    \draw[->, thick, blue] ([xshift=-2mm]va_contpte.south) -- ++(0,-8mm) -| (pte_matrix.north)
        node[pos=0.25, above=-2pt, font=\tiny] {Page walk};
    \draw[->, thick, blue] ([xshift=2mm]va_contpte.south) -- ++(0,-8mm) -| (tlb_box.north)
        node[pos=0.25, above=-2pt, font=\tiny] {TLB lookup};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\end{column}
\end{columns}
\fcolorbox{red!50}{red!10}{\parbox{0.95\textwidth}{\small\textbf{Performance impact:} Synchronous (initiator waits), IPIs interrupt all CPUs, scales poorly with core count}}
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{INVLPGB (AMD) / TLBSYNC:}\vspace{-2mm}
...
\textbf{ARM: TLBI with broadcast}\vspace{-2mm}
\begin{itemize}\setlength\itemsep{0pt}
\item Inner Shareable scope $\rightarrow$ auto-broadcast
\item No software IPIs needed
\end{itemize}
\end{column}

\begin{column}{0.52\textwidth}
\textbf{Benefits:}\vspace{-2mm}
\begin{itemize}\setlength\itemsep{0pt}
...
\textbf{Limitations:}\vspace{-2mm}
\begin{itemize}\setlength\itemsep{0pt}
\item \textbf{All-or-nothing}: cannot skip idle CPUs
\item Software IPIs allow selective targeting
\item ASID reuse requires global flush
\end{itemize}

\textbf{When broadcast wins:}\vspace{-2mm}
\begin{itemize}\setlength\itemsep{0pt}
\item JVM GC (unmapping large heaps)
...
\begin{frame}{Accessed and Dirty Bits}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{Motivation:}
\begin{itemize}\setlength\itemsep{0pt}
\item OS needs to know: which pages in use? modified?
\item Page replacement: evict ``cold'' pages
\item Write-back: only write dirty pages to disk
\end{itemize}

\textbf{HW Mechanism:}
\begin{itemize}\setlength\itemsep{0pt}
\item \textbf{Accessed (A)}: Set on any access
\item \textbf{Dirty (D)}: Set on write
\item Both ``sticky'' -- only SW can clear
\item Set via \textbf{atomic} RMW to PTE
\end{itemize}

\textbf{In Page Table Hierarchy:}
\begin{itemize}\setlength\itemsep{0pt}
\item A-bit: Set at \textbf{all levels}
\item D-bit: Set only at \textbf{leaf level}
\end{itemize}
\end{column}
...
\textbf{TLB Behavior:}
\begin{itemize}\setlength\itemsep{0pt}
\item A-bit: Often \textbf{not cached} (assumed accessed once in TLB)
\item D-bit: \textbf{Cached} in TLB (avoids memory write on every store)
\end{itemize}

\textbf{TLB Flush After Clearing A/D:}
\begin{itemize}\setlength\itemsep{0pt}
\item After clearing bits in PTE, must flush TLB
\item \textbf{A-bit}: OS may skip flush (approx. OK for LRU)
\item \textbf{D-bit}: Must \textbf{always} flush (correctness!)
\end{itemize}

\vspace{2mm}
\fcolorbox{red!50}{red!10}{\parbox{0.95\linewidth}{\scriptsize A-bit flush = optimization; D-bit flush = correctness}}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Memory Protection Keys (MPK)}
\definecolor{pkcolor}{HTML}{FF9999}
\definecolor{addrcolor}{HTML}{E6E6FA}
\definecolor{flagcolor}{HTML}{90EE90}
\definecolor{adcolor}{HTML}{FFB366}
\definecolor{wdcolor}{HTML}{66B3FF}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Problem:} Changing page permissions is expensive
\begin{itemize}\setlength\itemsep{0pt}
\item modify PTE $\rightarrow$ TLB flush $\rightarrow$ IPI
\item System call overhead for mprotect()
\end{itemize}

\textbf{MPK Solution:}
\begin{itemize}\setlength\itemsep{0pt}
\item 4-bit \textbf{protection key} in each PTE
\item 16 keys (0--15) per address space
\item Per-key permissions in user-space register (PKRU)
\item Change via register write -- no syscall!
\item TLB caches the 4-bit key from PTE
\end{itemize}
\end{column}

\begin{column}{0.52\textwidth}
\textbf{PTE with Protection Key (x86-64):}\vspace{3mm}

\begin{bytefield}[bitwidth=0.30em, endianness=big, boxformatting={\centering\tiny}]{64}
\bitheader{62,59,51,12,0} \\
\bitbox{1}[bgcolor=gray!30]{}
\bitbox{4}[bgcolor=pkcolor]{PK}
\bitbox{7}[bgcolor=gray!15]{}
\bitbox{40}[bgcolor=addrcolor]{Physical Address}
\bitbox{12}[bgcolor=flagcolor]{Flags}
\end{bytefield}

\vspace{5mm}
\textbf{PKRU Register (user-space):}\vspace{3mm}

\begin{bytefield}[bitwidth=0.58em, endianness=big, boxformatting={\centering\tiny}]{32}
\bitheader{31,0} \\
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{15}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{15}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{14}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{14}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{13}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{13}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{12}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{12}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{11}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{11}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{10}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{10}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{9}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{9}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{8}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{8}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{7}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{7}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{6}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{6}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{5}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{5}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{4}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{4}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{3}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{3}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{2}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{2}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{1}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{1}$}}
\bitbox{1}[bgcolor=adcolor]{\rotatebox{90}{\tiny AD$_{0}$}}
\bitbox{1}[bgcolor=wdcolor]{\rotatebox{90}{\tiny WD$_{0}$}}
\end{bytefield}

\scriptsize AD=Access Disable, WD=Write Disable

\vspace{2mm}
\textbf{ISA:} \texttt{WRPKRU} (write, $\sim$20 cycles), \texttt{RDPKRU} (read)
\end{column}
\end{columns}
\end{frame}
...
\textbf{Big Picture:}
\begin{itemize}
\item Virtual memory is the \textbf{primary mechanism for protection} in modern systems
\item \textbf{Performance-critical}: every memory access involves translation
\item Hardware support (TLB, page walker) essential for acceptable overhead
\end{itemize}

\vspace{3mm}
\textbf{Key Mechanisms:}
\begin{itemize}
\item Hierarchical page tables with TLB caching
\item Per-page permissions (R/W/X, User/Supervisor)
\item Hardware-managed A/D bits for OS memory management
\item Multi-core coherency via TLB shootdown
\end{itemize}
\end{frame}
