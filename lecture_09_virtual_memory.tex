...
\textbf{Motivation:}
\begin{itemize}\setlength\itemsep{0pt}
\item OS needs to know: which pages are in use? which were modified?
\item \textbf{Page replacement}: evict ``cold'' pages (LRU approximation)
\item \textbf{Write-back}: only write dirty pages to disk
\end{itemize}

\textbf{Mechanism:}
\begin{itemize}\setlength\itemsep{0pt}
\item \textbf{Accessed (A)}: Set by HW on any access
\item \textbf{Dirty (D)}: Set by HW on write
\item Both are ``sticky'' -- only SW can clear
\item HW sets bits via \textbf{atomic} read-modify-write to PTE
\end{itemize}

\textbf{In Page Table Hierarchy:}
\begin{itemize}\setlength\itemsep{0pt}
\item A-bit: Set at \textbf{all levels} (PML4, PDP, PDE, PTE)
...
\textbf{TLB Behavior:}
\begin{itemize}\setlength\itemsep{0pt}
\item A-bit: Often \textbf{not cached} in TLB
  \begin{itemize}\scriptsize
  \item Once in TLB, assumed accessed
  \end{itemize}
\item D-bit: \textbf{Cached} in TLB entry
  \begin{itemize}\scriptsize
  \item Avoids memory write on every store
  \end{itemize}
\end{itemize}
...
\item \textbf{A-bit}: OS may skip flush
  \begin{itemize}\scriptsize
  \item Missing some accesses is acceptable for LRU
  \end{itemize}
\item \textbf{D-bit}: Must \textbf{always} flush
  \begin{itemize}\scriptsize
  \item Correctness! Missing dirty $\rightarrow$ data loss
  \end{itemize}
\end{itemize}
...
\begin{frame}{MPK: Use Cases}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{Isolation within Process:}
\begin{itemize}\setlength\itemsep{0pt}
\item \textbf{Sensitive data protection}:
  \begin{itemize}\scriptsize
  \item Cryptographic keys in key=1 region
  \item Enable access only during crypto ops
  \item Prevents Spectre/buffer overflow leaks
  \end{itemize}
\item \textbf{JIT compiler isolation}:
  \begin{itemize}\scriptsize
  \item JIT code region: normally W=0, X=1
  \item During compilation: W=1, X=0
  \item Switch via WRPKRU, not mprotect()
  \end{itemize}
\item \textbf{Library sandboxing}:
  \begin{itemize}\scriptsize
  \item Untrusted library code uses key=2
  \item Main app disables key=2 when not calling lib
  \end{itemize}
\end{itemize}
\end{column}
...
\begin{itemize}\setlength\itemsep{0pt}
\item mprotect(): ~1000s of cycles (syscall + TLB)
\item WRPKRU: ~20 cycles (user-mode register write)
\item $\sim$50x faster permission changes
\end{itemize}

\textbf{Limitations:}
\begin{itemize}\setlength\itemsep{0pt}
\item Only 16 keys per process
\item Protection is per-thread (PKRU not shared)
\item Does not prevent code within same key
\item Requires careful key assignment design
\end{itemize}

\textbf{OS/Language Support:}
\begin{itemize}\setlength\itemsep{0pt}\scriptsize
\item Linux: pkey\_alloc(), pkey\_mprotect()
\item OpenJDK: Uses MPK for heap isolation
\item Rust: crates for safe MPK wrappers
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\item Virtual $\rightarrow$ Physical translation via page tables
\item Page: fixed-size unit (4KB, 2MB, 1GB)
\item Multi-level hierarchy reduces table size
\item Page faults for on-demand allocation
\end{itemize}

\textbf{Translation Performance:}
\begin{itemize}\setlength\itemsep{1pt}
\item TLB caches recent translations
\item TLB miss $\rightarrow$ page walk (4 memory accesses for 4-level)
\item PMH (Page Miss Handler) in hardware
\item ASID avoids flush on context switch
\end{itemize}

\textbf{Page Table Entry (PTE) Bits:}
\begin{itemize}\setlength\itemsep{1pt}
\item Present, R/W, U/S, Accessed, Dirty
\item Global, XD (execute disable), Protection Key
\end{itemize}
\end{column}
...
\begin{itemize}\setlength\itemsep{1pt}
\item Large pages: fewer TLB entries, less fragmentation
\item ContPTE: TLB coalescing for contiguous pages
\item Global entries: survive CR3 reload
\item ASID: tag TLB entries by address space
\end{itemize}

\textbf{Multi-core Considerations:}
\begin{itemize}\setlength\itemsep{1pt}
\item TLB shootdown: maintain coherency across cores
\item IPI-based vs. broadcast (INVLPGB/TLBI)
\item A/D bits: tracked in TLB, must flush on clear
\end{itemize}

\textbf{Protection Features:}
\begin{itemize}\setlength\itemsep{1pt}
\item Per-page R/W/X permissions
\item User/Supervisor separation
\item MPK: fast intra-process isolation (16 keys)
\end{itemize}
\end{column}
\end{columns}
\end{frame}
