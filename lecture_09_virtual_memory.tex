...
\begin{tikzpicture}[
    ...
    cachehit/.style={diamond, draw, fill=green!20, font=\scriptsize, inner sep=0pt, aspect=2.5, align=center, minimum width=12mm, minimum height=6mm},
    tlbhit/.style={diamond, draw, fill=blue!20, font=\scriptsize, inner sep=0pt, aspect=2.5, align=center, minimum width=12mm, minimum height=6mm},
    stlbhit/.style={diamond, draw, fill=orange!20, font=\scriptsize, inner sep=0pt, aspect=2.5, align=center, minimum width=12mm, minimum height=6mm},
    entrybox/.style={ellipse, draw, fill=blue!15, font=\scriptsize, inner sep=1pt, align=center, minimum width=14mm, minimum height=8mm},
    loadtext/.style={font=\tiny, text width=1.8cm, align=center, inner sep=1pt},
    basebox/.style={rectangle, draw, fill=yellow!30, font=\scriptsize, inner sep=2pt, rounded corners=2pt, minimum width=10mm, minimum height=5mm},
    ...
    ptebox/.style={ellipse, draw, fill=blue!30, font=\scriptsize, inner sep=2pt, minimum width=12mm, minimum height=5mm},
...
\node[loadtext, below=5mm of pml4e] (pml4load) {inject load to\\PML4 base +\\VA[47:39]$\times$8\\to get PML4\\entry};
% PML4 cache chain
...
% Column 2: PDP
\node[vabox, right=15mm of va1] (va2) {VA[47:30]};
\node[cachehit, below=4mm of va2] (pdphit) {PDP\$\\hit};
\node[entrybox, below=4mm of pdphit] (pdpe) {PDP\\entry};
\node[loadtext, below=5mm of pdpe] (pdpload) {inject load to\\PDP base +\\VA[38:30]$\times$8\\to get PDP\\entry};
% PDP cache chain
...
% Column 3: PDE
\node[vabox, right=15mm of va2] (va3) {VA[47:21]};
\node[cachehit, below=4mm of va3] (pdehit) {PDE\$\\hit};
\node[entrybox, below=4mm of pdehit] (pdee) {PDE};
\node[loadtext, below=5mm of pdee] (pdeload) {inject load to\\PDE base +\\VA[29:21]$\times$8\\to get PDE};
% PDE cache chain
...
% Column 4: PT/TLB
\node[vabox, right=15mm of va3] (va4) {VA[47:12]};
\node[tlbhit, below=4mm of va4] (tlbhit) {TLB hit};
\node[stlbhit, below=4mm of tlbhit] (stlbhit) {STLB\\hit};
\node[loadtext, below=5mm of stlbhit] (ptload) {inject load to\\PT base +\\VA[20:12]$\times$8\\to get PTE};
% PT cache chain
...
\node[ptebox, right=10mm of stlbhit] (pte) {PTE};
...
% Midpoint coordinates for cache chain "Yes" arrows
...
% Entry to fit box arrows
\draw[arrow] (pml4e) -- (pml4fit.north);
\draw[arrow] (pdpe) -- (pdpfit.north);
\draw[arrow] (pdee) -- (pdefit.north);
\draw[arrow] (stlbhit) -- node[right, nmark] {No} (ptfit.north);
...
% CR3 to fit box
\draw[arrow] (cr3.south) -- ++(0,-0.3) -| (pml4fit.west);

% Cache hits go to entry above (via midpoint)
\draw[arrow] (pml4l1.east) -- (pml4l1.east -| pml4mid) -- ++(0.5,0) |- (pml4e.west);
\draw[arrow] (pml4l2.east) -- (pml4l2.east -| pml4mid) -- ++(0.5,0) |- (pml4e.west);
\draw[arrow] (pml4l3.east) -- (pml4l3.east -| pml4mid) -- ++(0.5,0) |- (pml4e.west);
\draw[arrow] (pml4mem.east) -- (pml4mem.east -| pml4mid) -- ++(0.5,0) |- (pml4e.west);

\draw[arrow] (pdpl1.east) -- (pdpl1.east -| pdpmid) -- ++(0.5,0) |- (pdpe.west);
\draw[arrow] (pdpl2.east) -- (pdpl2.east -| pdpmid) -- ++(0.5,0) |- (pdpe.west);
\draw[arrow] (pdpl3.east) -- (pdpl3.east -| pdpmid) -- ++(0.5,0) |- (pdpe.west);
\draw[arrow] (pdpmem.east) -- (pdpmem.east -| pdpmid) -- ++(0.5,0) |- (pdpe.west);

\draw[arrow] (pdel1.east) -- (pdel1.east -| pdemid) -- ++(0.5,0) |- (pdee.west);
\draw[arrow] (pdel2.east) -- (pdel2.east -| pdemid) -- ++(0.5,0) |- (pdee.west);
\draw[arrow] (pdel3.east) -- (pdel3.east -| pdemid) -- ++(0.5,0) |- (pdee.west);
\draw[arrow] (pdemem.east) -- (pdemem.east -| pdemid) -- ++(0.5,0) |- (pdee.west);

% PT cache chain goes to PTE (via midpoint)
\draw[arrow] (ptl1.east) -- (ptl1.east -| ptmid) -- ++(0.5,0) |- (pte.south);
\draw[arrow] (ptl2.east) -- (ptl2.east -| ptmid) -- ++(0.5,0) |- (pte.south);
\draw[arrow] (ptl3.east) -- (ptl3.east -| ptmid) -- ++(0.5,0) |- (pte.south);
\draw[arrow] (ptmem.east) -- (ptmem.east -| ptmid) -- ++(0.5,0) |- (pte.south);
