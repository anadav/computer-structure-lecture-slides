...
\begin{frame}{Page Table Size -- The Solution}
...
\begin{tikzpicture}[
    ...
    node distance=2mm,
    vabox/.style={ellipse, draw, fill=yellow!20, font=\scriptsize, inner sep=1pt, minimum height=4mm},
    cachehit/.style={diamond, draw, fill=green!20, font=\scriptsize, inner sep=0pt, aspect=2.5, align=center},
    tlbhit/.style={diamond, draw, fill=blue!20, font=\scriptsize, inner sep=0pt, aspect=2.5, align=center},
    stlbhit/.style={diamond, draw, fill=orange!20, font=\scriptsize, inner sep=0pt, aspect=2.5, align=center},
    entrybox/.style={rectangle, draw, fill=blue!15, font=\scriptsize, inner sep=2pt, align=center, rounded corners=2pt},
    loadbox/.style={rectangle, draw, fill=gray!15, font=\tiny, text width=1.8cm, align=center, inner sep=2pt},
    basebox/.style={rectangle, draw, fill=yellow!30, font=\scriptsize, inner sep=2pt, rounded corners=2pt},
    l1hit/.style={diamond, draw, fill=yellow!40, font=\tiny, inner sep=0pt, aspect=2.5},
    l2hit/.style={diamond, draw, fill=orange!40, font=\tiny, inner sep=0pt, aspect=2.5},
    l3hit/.style={diamond, draw, fill=red!40, font=\tiny, inner sep=0pt, aspect=2.5},
    membox/.style={rectangle, draw, fill=red!50, font=\tiny, inner sep=2pt, rounded corners=2pt},
    ptebox/.style={ellipse, draw, fill=blue!30, font=\scriptsize, inner sep=2pt},
    arrow/.style={->, >=stealth, thick},
...
% Column 1: PML4
\node[vabox] (va1) {VA[47:39]};
\node[cachehit, below=4mm of va1] (pml4hit) {PML4\$\\hit};
\node[entrybox, below=4mm of pml4hit] (pml4e) {PML4\\entry};
\node[basebox, left=8mm of pml4e] (cr3) {CR3};
\node[font=\tiny, below=0pt of cr3] {PML4 base};
\node[loadbox, below=5mm of pml4e] (pml4load) {inject load to\\PML4 base +\\VA[47:39]$\times$8\\to get PML4\\entry};
% PML4 cache chain
...
% Column 2: PDP
\node[vabox, right=22mm of va1] (va2) {VA[47:30]};
\node[cachehit, below=4mm of va2] (pdphit) {PDP\$\\hit};
\node[entrybox, below=4mm of pdphit] (pdpe) {PDP\\entry};
\node[loadbox, below=5mm of pdpe] (pdpload) {inject load to\\PDP base +\\VA[38:30]$\times$8\\to get PDP\\entry};
\node[font=\tiny, above=0pt of pdpload.north west, anchor=south west] {PDP base};
% PDP cache chain
...
% Column 3: PDE
\node[vabox, right=22mm of va2] (va3) {VA[47:21]};
\node[cachehit, below=4mm of va3] (pdehit) {PDE\$\\hit};
\node[entrybox, below=4mm of pdehit] (pdee) {PDE};
\node[loadbox, below=5mm of pdee] (pdeload) {inject load to\\PDE base +\\VA[29:21]$\times$8\\to get PDE};
\node[font=\tiny, above=0pt of pdeload.north west, anchor=south west] {PDE base};
% PDE cache chain
...
% Column 4: PT/TLB
\node[vabox, right=22mm of va3] (va4) {VA[47:12]};
\node[tlbhit, below=4mm of va4] (tlbhit) {TLB hit};
\node[stlbhit, below=4mm of tlbhit] (stlbhit) {STLB\\hit};
\node[loadbox, below=5mm of stlbhit] (ptload) {inject load to\\PT base +\\VA[20:12]$\times$8\\to get PTE};
\node[font=\tiny, above=0pt of ptload.north west, anchor=south west] {PT base};
% PT cache chain
...
% PTE output (to the right of column 4)
\node[ptebox, right=12mm of stlbhit] (pte) {PTE};
...
\draw[arrow] (tlbhit.east) -- ++(0.3,0) |- node[near start, above, ymark] {Yes} (pte);
\draw[arrow] (stlbhit.east) -- node[above, ymark] {Yes} (pte);

% No arrows (go left to previous level)
\draw[arrow] (pml4hit.west) -- node[above, nmark] {No} ++(-0.5,0) |- (cr3.east);
\draw[arrow] (pdphit.west) -- node[above, nmark] {No} (pml4hit.east);
\draw[arrow] (pdehit.west) -- node[above, nmark] {No} (pdphit.east);
\draw[arrow] (stlbhit.west) -- node[above, nmark] {No} ++(-0.5,0) |- (pdehit.east);

% Entry to load box arrows
\draw[arrow] (pml4e) -- (pml4load);
\draw[arrow] (pdpe) -- (pdpload);
\draw[arrow] (pdee) -- (pdeload);

% Load box to cache chain
\draw[arrow] (pml4load) -- (pml4l1);
\draw[arrow] (pdpload) -- (pdpl1);
\draw[arrow] (pdeload) -- (pdel1);
\draw[arrow] (stlbhit) -- node[right, nmark] {No} (ptload);
...
% CR3 to load box
\draw[arrow] (cr3.south) -- ++(0,-0.3) -| (pml4load.west);

% Cache hits go to entry above
\draw[arrow] (pml4l1.west) -- ++(-0.2,0) |- node[near start, left, ymark] {Yes} (pml4e.west);
\draw[arrow] (pml4l2.west) -- ++(-0.25,0) |- (pml4e.west);
\draw[arrow] (pml4l3.west) -- ++(-0.3,0) |- (pml4e.west);
\draw[arrow] (pml4mem.west) -- ++(-0.35,0) |- (pml4e.west);

\draw[arrow] (pdpl1.west) -- ++(-0.2,0) |- node[near start, left, ymark] {Yes} (pdpe.west);
\draw[arrow] (pdpl2.west) -- ++(-0.25,0) |- (pdpe.west);
\draw[arrow] (pdpl3.west) -- ++(-0.3,0) |- (pdpe.west);
\draw[arrow] (pdpmem.west) -- ++(-0.35,0) |- (pdpe.west);

\draw[arrow] (pdel1.west) -- ++(-0.2,0) |- node[near start, left, ymark] {Yes} (pdee.west);
\draw[arrow] (pdel2.west) -- ++(-0.25,0) |- (pdee.west);
\draw[arrow] (pdel3.west) -- ++(-0.3,0) |- (pdee.west);
\draw[arrow] (pdemem.west) -- ++(-0.35,0) |- (pdee.west);

% PT cache chain goes to PTE
\draw[arrow] (ptl1.east) -- ++(0.2,0) |- node[near start, right, ymark] {Yes} (pte.south);
\draw[arrow] (ptl2.east) -- ++(0.25,0) |- (pte.south);
\draw[arrow] (ptl3.east) -- ++(0.3,0) |- (pte.south);
\draw[arrow] (ptmem.east) -- ++(0.35,0) |- (pte.south);
