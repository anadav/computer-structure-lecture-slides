...
% Column 1: PML4
\node[vabox, right=9mm of cr3] (va1) {VA[47:39]};
...
\node[loadtext, below=5mm of pml4e] (pdpload) {inject load to\\PDP base +\\VA[38:30]$\times$8\\to get PDP\\entry};
...
% Column 2: PDP
\node[vabox, right=9mm of va1] (va2) {VA[47:30]};
...
\node[loadtext, below=5mm of pdpe] (pdeload) {inject load to\\PDE base +\\VA[29:21]$\times$8\\to get PDE};
...
% Column 3: PDE
\node[vabox, right=9mm of va2] (va3) {VA[47:21]};
\node[cachehit, below=4mm of va3] (pdehit) {PDE\$\\hit};
\node[entrybox, below=4mm of pdehit] (pdee) {PDE};
% PT inject/cache chain (below PDE)
\node[loadtext, below=5mm of pdee] (ptload) {inject load to\\PT base +\\VA[20:12]$\times$8\\to get PTE};
...
% Column 4: TLB
\node[vabox, right=9mm of va3] (va4) {VA[47:12]};
...
\draw[arrow] (pml4hit.west) -- node[above, nmark] {No} (cr3.north);
\draw[arrow] (pdphit.west) -- node[above, nmark] {No} (pml4hit.east);
\draw[arrow] (pdehit.west) -- node[above, nmark] {No} (pdphit.east);
\draw[arrow] (stlbhit.west) -- node[above, nmark] {No} (pdehit.east);

% CR3 to fit box
\draw[arrow] (cr3label.south) -- (pml4fit.north);

% Entry to fit box arrows
\draw[arrow] (pml4e) -- (pdpfit.north);
\draw[arrow] (pdpe) -- (pdefit.north);
\draw[arrow] (pdee) -- (ptfit.north);
...
% Cache hits go to next entry (via midpoint)
