...
\begin{tikzpicture}[scale=0.75, every node/.style={scale=0.75},
    ...
    % TLB cells
    tlbvalid/.style={cell, fill=green!30, minimum width=0.45cm},
    tlbinvalid/.style={cell, fill=gray!20, minimum width=0.45cm},
    tlbtag/.style={cell, fill=green!30, minimum width=0.8cm},
    tlbtaginvalid/.style={cell, fill=gray!20, minimum width=0.8cm},
    tlbpfn/.style={cell, fill=green!30, minimum width=1.1cm},
    tlbpfninvalid/.style={cell, fill=gray!20, minimum width=1.1cm},
    % Page table cells
...
% Virtual Page Number box - positioned on the left
\node[draw, fill=blue!15, minimum width=2cm, minimum height=0.5cm, font=\scriptsize] (vpn)
    {Virtual page number};

% TLB - positioned to the right of VPN
\matrix[matrix of nodes, row sep=-\pgflinewidth, column sep=-\pgflinewidth, ampersand replacement=\&,
    right=1.2cm of vpn,
...
\matrix[matrix of nodes, row sep=-\pgflinewidth, column sep=-\pgflinewidth, ampersand replacement=\&,
    below=0.6cm of tlb,
    label={[mytitle, label distance=-1mm]above:Page Table}] (pt) {
...
% Physical Memory - positioned to the right
\matrix[matrix of nodes, row sep=-\pgflinewidth, ampersand replacement=\&,
    right=1.8cm of tlb.north east, anchor=north west,
...
% "Physical Page Or Disk Address" label for Page Table
\node[font=\tiny, text=gray!70!black, align=center, rotate=90, anchor=south]
    at ($(pt.west) + (-0.4, -0.5)$) {Physical Page\\Or Disk Address};

% Connection from VPN - dot at the split point
\coordinate (splitpoint) at ($(vpn.east) + (0.4, 0)$);
\draw[thick] (vpn.east) -- (splitpoint);
\fill (splitpoint) circle (2pt);

% VPN to TLB
\draw[arrow] (splitpoint) -- (splitpoint |- tlb.west) -- (tlb.west);
...
\draw[dotarrow] (pt-11-2.east) -- (pp-8-1.west);
\draw[dotarrow] (pt-12-2.east) -- (pp-7-1.west);

% Dashed arrows from Page Table (present=0) to Disk
\coordinate (diskslot1) at ($(disklabel.south) + (-0.85, -0.6)$);
\coordinate (diskslot2) at ($(disklabel.south) + (-0.85, -0.95)$);
\draw[dotarrow, dashed] (pt-6-2.east) -- ++(0.3, 0) |- (diskslot1);
\draw[dotarrow, dashed] (pt-9-2.east) -- ++(0.5, 0) |- (diskslot2);
