...
\node[rectangle callout, draw, fill=yellow!20, font=\scriptsize,
    callout absolute pointer={(pic cs:loadtlb)}, anchor=south east, align=left]
    at ([shift={(-1mm,1mm)}]pic cs:storetlb) {Split L1 TLBs for\\load/store};
\end{tikzpicture}

\pause
\begin{tikzpicture}[remember picture, overlay]
\node[rectangle callout, draw, fill=green!20, font=\scriptsize, anchor=south west,
    callout absolute pointer={(pic cs:largepage)}]
    at ([shift={(3mm,-2mm)}]pic cs:l1tlbse) {Large pages reduce TLB pressure};
\end{tikzpicture}

\pause
\begin{tikzpicture}[remember picture, overlay]
\node[rectangle callout, draw, fill=cyan!20, font=\scriptsize,
    callout absolute pointer={(pic cs:stlb)}]
    at ([shift={(3cm,5mm)}]pic cs:stlbabove) {Backs up all L1 TLB misses};
\end{tikzpicture}

\pause
\begin{tikzpicture}[remember picture, overlay]
\node[rectangle callout, draw, fill=orange!20, font=\scriptsize, anchor=east,
    callout absolute pointer={(pic cs:unified)}]
    at ([shift={(-1.5cm,2mm)}]pic cs:unified) {Instruction + Data};
\end{tikzpicture}

\pause
\begin{tikzpicture}[remember picture, overlay]
\node[rectangle callout, draw, fill=red!20, font=\scriptsize,
    callout absolute pointer={(pic cs:l3size)}]
